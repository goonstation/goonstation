...
then
  ...
  mkdir -p "$HOME/BYOND"
  cd "$HOME/BYOND"
  if ! curl --fail --connect-timeout 2 --max-time 10 "https://byond-builds.dm-lang.org/${BYOND_MAJOR_VERSION}/${BYOND_MAJOR_VERSION}.${BYOND_MINOR_VERSION}_byond_linux.zip" -o byond.zip -A "GoonstationCI/2.0"; then
