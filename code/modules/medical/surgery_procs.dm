...
/obj/item/proc/surgeryConfusion(var/mob/living/carbon/human/patient as mob, var/mob/surgeon as mob, var/damage as num)
	...
	if (prob(33)) // if they REALLY fuck up
		...
		patient.visible_message("<span class='alert'><font size=+1><b>Blood gushes from the incision!</b></font> That can't have been the correct thing to do!</span>")
		return
		...
		surgeon, "<span class='alert'>You [fluff][fluff2] at [patient]'s [target_area] with [src].[pick("", " <b>That doesn't seem right...</b>")]</span>")
		return
...
/proc/insertChestItem(var/mob/living/carbon/human/patient as mob, var/mob/surgeon as mob)
	...
	if (surgeon.zone_sel.selecting == "chest" && patient.chest_cavity_open == 1)
		...
		if (patient.chest_item == null)
			...
			patient.tri_message("<span class='alert'><b>[surgeon]</b> shoves [chest_item] into [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] chest.</span>",\
			surgeon, "<span class='alert'>You shove [chest_item] into [surgeon == patient ? "your" : "[patient]'s"] chest.</span>",\
			patient, "<span class='alert'>[patient == surgeon ? "You shove" : "<b>[surgeon]</b> shoves"] [chest_item] into your chest.</span>")
			...
			if (patient.organHolder.head.op_stage == 0.0)
				...
				if (prob(screw_up_prob[1]))
					...
					if (length(screw_up_prob[2]) > 0)
						surgeon.show_text("<span class='alert'><font size=+1><b>Step needs reattempting</b></font> - [english_list(screw_up_prob[2])]!</span>")
					else
						surgeon.show_text("<span class='alert'><font size=+1><b>Step needs reattempting!</b></font></span>")
...
/* ---------- SCALPEL - CAVITY ---------- */
	// Surgeon targeting chest with grab intent
		...
		if(patient.chest_cavity_open == 0)
			playsound(patient, "sound/impact_sounds/Slimy_Cut_1.ogg", 50, 1)
			patient.tri_message("<span class='alert'><b>[surgeon]</b> carefully cuts down [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] chest with [src] and opens it.</span>",\
			surgeon, "<span class='alert'>You carefully cut down [surgeon == patient ? "your" : "[patient]'s"] chest with [src] and open it up.</span>",\
			patient, "<span class='alert'>[patient == surgeon ? "You carefully cut" : "<b>[surgeon]</b> carefully cuts"] down your chest with [src] and [patient == surgeon ? "open" : "opens"] it.</span>")
		...
		// Chest cavity is open and an item exists
			...
			playsound(patient, "sound/impact_sounds/Slimy_Cut_1.ogg", 50, 1)
			patient.tri_message("<span class='alert'><b>[surgeon]</b> cuts [patient.chest_item] out of [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] chest with [src].</span>",\
			surgeon, "<span class='alert'>You cut [patient.chest_item] out of [surgeon == patient ? "your" : "[patient]'s"] chest with [src].</span>",\
			patient, "<span class='alert'>[patient == surgeon ? "You cut" : "<b>[surgeon]</b> cuts"] [patient.chest_item] out of your chest with [src].</span>")
		...
		if (patient.organHolder.chest)
			switch (patient.organHolder.chest.op_stage)
				...
				if(10.0)
					...
					if(istype(patient.organHolder.tail, /obj/item/organ/tail/bone)) // Has tail, is tailbone
						patient.tri_message("<span class='alert'><b>[surgeon]</b> cuts through the tissue covering [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] tailbone with [src]!</span>",\
						surgeon, "<span class='alert'>You cut through the tissue covering [surgeon == patient ? "your" : "[patient]'s"] tailbone with [src]!</span>",\
						patient, "<span class='alert'>[patient == surgeon ? "You cut" : "<b>[surgeon]</b> cuts"] through the tissue covering your tailbone with [src]!</span>")
					else if (!patient.organHolder.tail)	// Doesnt have tail at all, likely removed earlier
...
/* ---------- SUTURE - CHEST ---------- */
		...
		// Sew chest item securely into chest cavity
			...
			patient.tri_message("<span class='alert'><b>[surgeon]</b> sews the [patient.chest_item] into [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] chest cavity with [src].</span>",\
			surgeon, "<span class='alert'>You sew the [patient.chest_item] securely into [surgeon == patient ? "your" : "[patient]'s"] chest cavity with [src].</span>",\
			patient, "<span class='alert'>[patient == surgeon ? "You sew" : "<b>[surgeon]</b> sews"] the [patient.chest_item] into your chest cavity with [src].</span>")
	...
	if (surgeon.zone_sel.selecting == "chest")
		if (patient.mob_flags & IS_BONER)
			...
			if(istype(patient.organHolder.tail, /obj/item/organ/tail/bone))
				patient.tri_message("<span class='alert'><b>[surgeon]</b> jams one end of the [src] between [patient == surgeon ? "[his_or_her(patient)]" : "[patient]'s"] sacrum and coccyx and pries [his_or_her(patient)] tailbone free!</span>",\
				surgeon, "<span class='alert'>You jam one end of the [src] between [surgeon == patient ? "your" : "[patient]'s"] sacrum and coccyx and pries [his_or_her(patient)] tailbone free!</span>",\
				patient, "<span class='alert'>[patient == surgeon ? "You jam" : "<b>[surgeon]</b> jams"] one end of the [src] between your sacrum and coccyx and pry your tailbone free!</span>")
			else
