...
/proc/calc_screw_up_prob(var/mob/living/carbon/human/patient as mob, var/mob/surgeon as mob, var/screw_up_prob = 25)
	...
	if (patient.stat) // is the patient dead?
		screw_up_prob -= 30
	if (patient.getStatusDuration("unconscious") || patient.getStatusDuration("paralyzed") || patient.getStatusDuration("knockdown")) // unable to move?
		screw_up_prob -= 15
	if (patient.sleeping) // asleep?
		screw_up_prob -= 10
	if (patient.getStatusDuration("stunned")) // stunned?
		screw_up_prob -= 5
	if (patient.getStatusDuration("drowsy") || patient.getStatusDuration("resting")) // sleepy?
		screw_up_prob -= 5
...
/datum/action/bar/icon/remove_organ
	...
	onInterrupt()
		...
		if (src.rip_out_organ)
			slipup_message = " loses [his_or_her(src.surgeon)] grip on the [src.organ_name]"
