/datum/ailment/disease/dnaspread
	...
	cure_flags = (CURE_MEDICINE|CURE_HIGH_TEMPERATURE)
	reagentcure = list("spaceacillin"=100)
...
/datum/ailment/disease/dnaspread/on_infection(mob/living/affected_mob, datum/ailment_data/disease/D)
	...
	if (!D.strain_data["pzero_bioholder"]) // Oh-hoh-hoo, patient zero~
		...
		D.state = AILMENT_STATE_ASYMPTOMATIC // We can't turn into ourselves
...
/datum/ailment/disease/dnaspread/stage_act(var/mob/living/affected_mob, var/datum/ailment_data/disease/D, mult)
	...
	switch(D.stage)
		...
		if(4)
			if (probmult(20) && !D.strain_data["orig_bioholder"] && D.state != AILMENT_STATE_REMISSIVE)
				...
				D.state = AILMENT_STATE_DORMANT //Just chill out at stage 4
	return
