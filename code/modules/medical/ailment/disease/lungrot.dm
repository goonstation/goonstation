/datum/ailment/disease/lungrot
	...
	reagentcure = list("cold_medicine"=1) // changes based on salbutamol content in stage_act
	affected_species = list("Human")

/datum/ailment/disease/lungrot/stage_act(mob/living/affected_mob, datum/ailment_data/affecting_ailment, mult)
	...
	if (affected_mob.reagents?.get_reagent_amount("salbutamol") > 0)
		affecting_ailment.reagentcure = list("cold_medicine"=1)
	else
		affecting_ailment.reagentcure = list("cold_medicine"=20)
	...
	if (affecting_ailment.state == AILMENT_STATE_REMISSIVE)
