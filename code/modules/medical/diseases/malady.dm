	...
	switch (D.stage)
		if (1)
			if (prob(1) && prob(10))
				...
				return
			if (prob(20))
				affected_mob.emote(pick("groan", "blink_r"))
			if (prob(8))
				boutput(affected_mob, "<span class='alert'>You feel a dull ache inside your head.</span>")

		if (2)
			if (prob(20))
				affected_mob.emote(pick("groan", "cry", "weep", "moan"))
			if (prob(8))
				boutput(affected_mob, "<span class='alert'>You feel a shooting pain inside your head.</span>")
				affected_mob.emote(pick("groan", "cry", "weep", "moan"))
			if (prob(8))
				boutput(affected_mob, "<span class='alert'>The pain in your head is making it hard to concentrate.</span>")
		...
		if (3)
			affected_mob.take_oxygen_deprivation(1)
			if (prob(20))
				affected_mob.emote(pick("groan", "cry", "weep", "moan", "blink_r"))
			if (prob(8))
				boutput(affected_mob, "<span class='alert'>Your head really hurts!</span>")
			if (prob(8))
			...
			if (prob(8))
				boutput(affected_mob, "<span class='alert'>The pain in your head is very distracting.</span>")
	...
	switch (D.stage)
		if (1)
			if (prob(1) && prob(10))
				...
				return
			if (prob(5))
				affected_mob.changeStatus("slowed", rand(80,120))
				affected_mob.change_misstep_chance(7)
				boutput(affected_mob, "<span class='alert'>You struggle to keep balance.</span>")
			if (prob(8))
				affected_mob.emote(pick("pale", "shudder", "faint"))
			if (prob(5))
				affected_mob.visible_message("<span class='alert'>A clear, thin liquid seeps from <b>[affected_mob.name]'s</b> nose.</span>")
				affected_mob.take_brain_damage(1)
			if (prob(5))
				affected_mob.visible_message("<span class='alert'><b>[affected_mob.name]'s</b> nose drips blood.</span>")				
				take_bleeding_damage(affected_mob, null, 0, DAMAGE_STAB, 1)
			if(prob(6))
				affected_mob.stuttering = max(10, affected_mob.stuttering)
				boutput(affected_mob, "<span class='alert'>You feel extremely [pick("numb", "confused", "dizzy", "dazed")].</span>")
				affected_mob.emote("collapse")
		...
		if (2)
			if (prob(8))
				...
				affected_mob.change_misstep_chance(3)
			if (prob(5))
				affected_mob.changeStatus("slowed", rand(120,180))
				affected_mob.change_misstep_chance(8)
			...
			if (prob(5))
				...
				take_bleeding_damage(affected_mob, null, 0, DAMAGE_STAB, 1)
			if (prob(5))
				affected_mob.visible_message("<span class='alert'><b>[affected_mob.name]'s</b> sneezes, spraying blood everywhere!</span>")
		...
		if (3)
			...
			if(prob(12))
				...
				affected_mob.emote("collapse")
			if (prob(5))
				affected_mob.changeStatus("slowed", rand(180,240))
...
/* -------------------- Coma -------------------- */
/datum/ailment/malady/coma
	name = "Coma"
	scantype = "Medical Emergency"
	max_stages = 1
	spread = "The patient's brain is severely damaged."
	cure = "Neurological Stimulants"
	affected_species = list("Human","Monkey")
	reagentcure = list( "coffee" = list(1,1),		// 
						"espresso" = list(1,10),	// 
						"mannitol" = list(1), 		//
						"synaptizine" = list(3)) 	// i like synaptizine :D

/datum/ailment/malady/coma/stage_act(var/mob/living/affected_mob,var/datum/ailment/D)
	if (..())
		return
	if (ishuman(affected_mob))
		var/mob/living/carbon/human/H = affected_mob
