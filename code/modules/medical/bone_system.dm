	...
	proc/take_damage(var/damage_type, var/amt = 1)
		...
		switch (src.damage_status)
			...
			if (BONE_CRACKED)
				...
				src.donor.visible_message(SPAN_ALERT("[src.donor]'s [src.parent_organ] emits a [pick("", "disturbing ", "unsettling ", "worrying ")][pick("crack", "crunch", "snap")]!"),\
				SPAN_ALERT("<b>You feel something in your [src.parent_organ] break!</b>"))

			if (BONE_FRACTURED)
				BONE_DEBUG("[donor]'s [parent_organ]'s bones shattered")
				src.damage_status = BONE_SHATTERED
				src.donor.visible_message(SPAN_ALERT("[src.donor]'s [src.parent_organ] emits a [pick("", "disturbing ", "unsettling ", "worrying ")][pick("crack", "crunch", "snap")]!"),\
				SPAN_ALERT("<b>You feel something in your [src.parent_organ] shatter!</b>"))
