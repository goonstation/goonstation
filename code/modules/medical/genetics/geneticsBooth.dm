			...
			if (started == 2)
				if (!try_billing(occupant))
					for (var/mob/O in hearers(src, null))
						O.show_message(SPAN_SUBTLE("<span class='say'>[SPAN_NAME("[src]")] beeps, \"<b>[occupant.name]<b>! You can't afford [selected_product.name] with a bank account like that.\"</span>"), 2)
					occupant.show_message(SPAN_SUBTLE("<span class='say'>[SPAN_NAME("[src]")] beeps, \"<b>[occupant.name]<b>! You can't afford [selected_product.name] with a bank account like that.\"</span>"), 2)
					...
					if (account)
						if (account["current_money"] >= selected_product.cost)
							...
							for (var/mob/O in hearers(src, null))
								...
								O.show_message(SPAN_SUBTLE("<span class='say'>[SPAN_NAME("[src]")] beeps, \"Thank you for your patronage, <b>[M.name]<b>.\"</span>"), 2)
