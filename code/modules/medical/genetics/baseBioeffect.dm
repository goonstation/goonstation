...
/datum/targetable/geneticsAbility
	...
	cast(atom/target)
		...
		if (!linked_power)
			return 1
		if (ismob(target))
			logTheThing("combat", owner, "used the [linked_power.name] power on [constructTarget(target,"combat")].")
		else if (target)
			logTheThing("combat", owner, "used the [linked_power.name] power on [target].")
		else
			logTheThing("combat", owner, "used the [linked_power.name] power.")
		return 0
	...
	proc/cast_misfire(atom/target)
		...
		if (!linked_power)
			return 1
		if (ismob(target))
			logTheThing("combat", owner, "misfired the [linked_power.name] power on [constructTarget(target,"combat")].")
		else if (target)
			logTheThing("combat", owner, "misfired the [linked_power.name] power on [target].")
		else
			logTheThing("combat", owner, "misfired the [linked_power.name] power.")
		return 0
