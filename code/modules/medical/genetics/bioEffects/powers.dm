...
/datum/targetable/geneticsAbility/telepathy
	...
	cast(atom/target)
		...
		logTheThing("telepathy", owner, "TELEPATHY to [constructTarget(recipient,"telepathy")]: [msg]")
	...
	cast_misfire(atom/target)
		...
		logTheThing("telepathy", owner, "TELEPATHY misfire to [constructTarget(recipient,"telepathy")]: [msg]")
	...
	cast()
		...
		if (can_act(owner))
			if(owner.reagents.has_reagent("anti_fart"))
				...
				logTheThing("combat", owner, "was gibbed by superfarting while containing anti_fart at [log_loc(owner)].")
				indigestion_gib()
				return 1
...
/datum/targetable/geneticsAbility/bigpuke
	...
	cast(atom/target)
		...
		logTheThing("combat", owner, "power-pukes [log_reagents(owner)] at [log_loc(owner)].")
		playsound(owner.loc, "sound/misc/meat_plop.ogg", 50, 0)
		...
		if (ishuman(owner))
			...
			SPAWN(1 DECI SECOND)
				if (istype(thrown_limb))
					...
					logTheThing("combat", owner, "shoot_limb [!linked_power.safety ? "Accidently" : ""] at [ismob(target)].")
					SPAWN(1 SECOND)
						if (thrown_limb)
