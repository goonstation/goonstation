...
/datum/bioEffect/thermalres
	...
	OnAdd()
		...
		APPLY_ATOM_PROPERTY(owner, PROP_HEATPROT, src.type, 50)
		APPLY_ATOM_PROPERTY(owner, PROP_COLDPROT, src.type, 50)
		owner.temp_tolerance *= 10

	OnRemove()
		..()
		REMOVE_ATOM_PROPERTY(owner, PROP_HEATPROT, src.type)
		REMOVE_ATOM_PROPERTY(owner, PROP_COLDPROT, src.type)
		owner.temp_tolerance /= 10
		if(overlay_image_two)
			if(isliving(owner))
				var/mob/living/L = owner
...
/datum/bioEffect/elecres
	...
	OnAdd()
		...
		if(ismob(owner))
			if(src.power > 1)
				APPLY_ATOM_PROPERTY(owner, PROP_DISORIENT_RESIST_BODY, src, 40)
				APPLY_ATOM_PROPERTY(owner, PROP_DISORIENT_RESIST_BODY_MAX, src, 40)

	onPowerChange(oldval, newval)
		. = ..()
		if(ismob(owner))
			if(oldval > 1)
				REMOVE_ATOM_PROPERTY(owner, PROP_DISORIENT_RESIST_BODY, src)
				REMOVE_ATOM_PROPERTY(owner, PROP_DISORIENT_RESIST_BODY_MAX, src)
			if(newval > 1)
				APPLY_ATOM_PROPERTY(owner, PROP_DISORIENT_RESIST_BODY, src, 40)
				APPLY_ATOM_PROPERTY(owner, PROP_DISORIENT_RESIST_BODY_MAX, src, 40)
...
/datum/bioEffect/rad_resist
	...
	OnAdd()
		...
		if(ismob(owner))
			var/mob/M = owner
			APPLY_ATOM_PROPERTY(M, PROP_RADPROT, src, min(75 * power, 100))

	onPowerChange(oldval, newval)
		. = ..()
		if(ismob(owner))
			var/mob/M = owner
			APPLY_ATOM_PROPERTY(M, PROP_RADPROT, src, min(75 * newval, 100))
...
/datum/bioEffect/breathless
	...
	OnAdd()
		...
		if(ismob(owner))
			if(src.power == 1)
				APPLY_ATOM_PROPERTY(H, PROP_REBREATHING, src.type)
			else
	...
	onPowerChange(oldval, newval)
		...
		if(ismob(owner))
			if(oldval == 1)
				REMOVE_ATOM_PROPERTY(owner, PROP_REBREATHING, src.type)
			else
				REMOVE_ATOM_PROPERTY(owner, PROP_BREATHLESS, src.type)
			if(newval == 1)
				APPLY_ATOM_PROPERTY(owner, PROP_REBREATHING, src.type)
			else
				APPLY_ATOM_PROPERTY(owner, PROP_BREATHLESS, src.type)
...
/datum/bioEffect/detox
	...
	OnAdd()
		. = ..()
		if(ismob(owner))
			APPLY_ATOM_PROPERTY(owner, PROP_CHEM_PURGE, src.type, remove_per_tick * power)
...
/datum/bioEffect/xray
	...
	OnAdd()
		...
		if(ismob(owner))
			if(power == 1)
				APPLY_ATOM_PROPERTY(owner, PROP_XRAYVISION_WEAK, src)
			else
				APPLY_ATOM_PROPERTY(owner, PROP_XRAYVISION, src)

	onPowerChange(oldval, newval)
		. = ..()
		if(ismob(owner))
			if(oldval == 1)
				REMOVE_ATOM_PROPERTY(owner, PROP_XRAYVISION_WEAK, src)
			else
				REMOVE_ATOM_PROPERTY(owner, PROP_XRAYVISION, src)
...
/datum/bioEffect/nightvision
	...
	OnAdd()
		...
		if(ismob(owner))
			if(power == 1)
				APPLY_ATOM_PROPERTY(owner, PROP_NIGHTVISION_WEAK, src)
			else
	...
	onPowerChange(oldval, newval)
		...
		if(ismob(owner))
			if(oldval == 1)
				REMOVE_ATOM_PROPERTY(owner, PROP_NIGHTVISION_WEAK, src)
			else
...
/datum/bioEffect/fitness_buff
	...
	OnAdd()
		. = ..()
		if(ismob(owner))
			APPLY_ATOM_PROPERTY(src.owner, PROP_STAMINA_REGEN_BONUS, "g-fitness-buff", 1.33 * power)
			src.owner.add_stam_mod_max("g-fitness-buff", 20 * power)

	onPowerChange(oldval, newval)
		. = ..()
		if(ismob(owner))
			APPLY_ATOM_PROPERTY(src.owner, PROP_STAMINA_REGEN_BONUS, "g-fitness-buff", 1.33 * newval)
