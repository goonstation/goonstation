/obj/item/genetics_injector
	...
	attack(mob/M, mob/user)
		...
		if(M == user)
			...
			logTheThing("combat", user, "tries to inject [constructTarget(M,"combat")] with [src.name] at [log_loc(user)]")
	...
	proc/injected(var/mob/living/carbon/user,var/mob/living/carbon/target)
		...
		if(!istype(target.bioHolder))
			return 1
		logTheThing("combat", user, "injects [constructTarget(target,"combat")] with [src.name] at [log_loc(user)]")
		return 0
...
/obj/item/speed_injector
	...
	attack(mob/M, mob/user)
		...
		if (payload)
			...
			logTheThing("combat", user, "stabs [constructTarget(M,"combat")] with the speed injector (<b>Payload:</b> [payload.name]).")
...
/obj/item/dna_scrambler
	...
	attack(mob/M, mob/user)
		...
		if(M == user)
			...
			logTheThing("combat", user, "injects [constructTarget(M,"combat")] with [src.name] at [log_loc(user)]")
