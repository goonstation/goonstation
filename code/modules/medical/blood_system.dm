...
/client/proc/haine_blood_debug()
	...
	logTheThing("admin", usr, "toggled blood debug messages [haine_blood_debug ? "on" : "off"].")
	logTheThing("diary", usr, "toggled blood debug messages [haine_blood_debug ? "on" : "off"].", "admin")
...
/proc/internal_bleeding_damage(var/mob/some_idiot as mob, var/mob/some_jerk as mob, var/damage as num)
	...
	if (haine_blood_debug) logTheThing("debug", some_idiot, "<b>HAINE BLOOD DEBUG: [some_idiot] begins internal bleed damage proc</b>")

	if (!isliving(some_idiot))
		if (haine_blood_debug) logTheThing("debug", some_idiot, "<b>HAINE BLOOD DEBUG: [some_idiot] is not living so internal bleed damage was canceled</b>")
		return
	...
	if (H.stat ==  2 || H.nodamage || !H.can_bleed || isvampire(H))
		if (H.bleeding)
			H.bleeding = 0
			H.bleeding_internal = 0
		if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG: [H] is dead/immortal/a vampire/otherwise not supposed to bleed, so their bleeding has been set to 0 and internal bleed damage was canceled</b>")
		return

	if (!(H.blood_volume > 0)) // make sure we have blood to bleed
		H.bleeding = 0 // if we don't have any blood to bleed, just stop okay, just stop.
		H.bleeding_internal = 0
		if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG: [H] has no blood and their bleeding has been set to 0 and internal bleed damage was canceled</b>")
		return

	if (H.bleeding_internal >= 10) // don't bleed more if you're already at bleeding 10 tia
		if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG: [H]'s internal bleeding was [H.bleeding_internal] and has been set to 10 and internal bleed damage was canceled</b>")
		H.bleeding_internal = 10
		return

	var/increase_chance = rand(30, 50)
	if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG:</b> [H]'s initial increase chance is [increase_chance]")

	if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG:</b> [H]'s initial internal bleeding is [H.bleeding_internal]")
	switch (H.bleeding_internal)
		if (-INFINITY to 1)
			increase_chance += rand(30, 50)
			if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG:</b> [H]'s increase chance increased to [increase_chance]")
		if (2)
			increase_chance += rand(20, 30)
			if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG:</b> [H]'s increase chance increased to [increase_chance]")
		if (3)
		...
		if (4)
			...
			if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG:</b> [H]'s increase chance increased to [increase_chance]")
		if (5 to INFINITY)
			if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG:</b> [H]'s internal bleeding was already high and chance was not increased")

	if (some_jerk?.zone_sel?.selecting)
		if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG:</b> [some_jerk]'s target zone is [some_jerk.zone_sel.selecting]")
		switch (some_jerk.zone_sel.selecting)
			if ("head")
				increase_chance += rand(0, 10)
				if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG:</b> [H]'s increase chance increased to [increase_chance]")
	...
	if (ischangeling(H))
		increase_chance -= rand(10, 20)
		if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG:</b> [H] is a changeling - [H]'s increase chance decreased to [increase_chance]")

	var/final_increase_chance = min(increase_chance, 100)
	if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG:</b> [H]'s final increase chance is [final_increase_chance]")
	if (prob(final_increase_chance))
		H.bleeding_internal ++
		if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG: [H] rolls internal bleeding increase, internal bleeding is now [H.bleeding_internal]</b>")
	else
		if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG: [H]'s internal bleeding does not increase</b>")
	H.bleeding_internal = clamp(H.bleeding_internal, 0, 5)
	if (haine_blood_debug) logTheThing("debug", H, "<b>HAINE BLOOD DEBUG: [H]'s internal bleeding is [H.bleeding_internal] after clamp</b>")
