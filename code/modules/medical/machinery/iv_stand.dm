...
/obj/machinery/medical/iv_stand/disposing()
	if (src.iv_drip)
		...
		src.iv_drip.stand = null
		src.iv_drip = null
	..()

/obj/machinery/medical/iv_stand/get_desc()
	if (src.iv_drip)
		var/list/examine_list = src.iv_drip.examine()
		. = examine_list.Join("\n")
...
/obj/machinery/medical/iv_stand/attackby(obj/item/W, mob/user)
	...
	if (isrobot(user)) // are they a borg? it's probably a mediborg's IV then, don't take that!
		return
	if (src.iv_drip)
		W.AfterAttack(src.iv_drip, user)
		return
	if (istype(W, /obj/item/reagent_containers/iv_drip))
		var/obj/item/reagent_containers/iv_drip/newIV = W
		user.visible_message(SPAN_NOTICE("[user] hangs [newIV] on [src]."), SPAN_NOTICE("You hang [newIV] on [src]."))
		user.u_equip(newIV)
		newIV.set_loc(src)
		src.iv_drip = newIV
		newIV.stand = src
...
/obj/machinery/medical/iv_stand/attack_hand(mob/user)
	if (src.iv_drip && !isrobot(user))
		var/obj/item/reagent_containers/iv_drip/oldIV = src.iv_drip
