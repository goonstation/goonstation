...
/datum/listen_module/input/outloud/ears/process(datum/say_message/message)
	...
	if ((!isturf(src.parent_tree.parent.loc) || !isturf(message.speaker.loc)) && (src.parent_tree.parent != message.speaker.loc))
		// If the message is a whisper, it may only be heard if the listener is in the same loc as the speaker.
		if (message.flags & SAYFLAG_WHISPER)
			return

		if (!(message.flags & SAYFLAG_IGNORE_POSITION))
...
/datum/listen_module/input/outloud/ears/can_hear(datum/say_message/message)
	return TRUE
