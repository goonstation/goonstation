...
/datum/phrase_log/proc/random_phrase(category, include_old=TRUE, include_new=TRUE)
	var/lower = 1
...
/datum/phrase_log/proc/log_phrase(category, phrase, no_duplicates=FALSE, mob/user = null, strip_html=FALSE)
	if (!user)
		user = usr
	if(strip_html)
...
/datum/phrase_log/proc/random_api_phrase(category)
...
/datum/phrase_log/proc/random_station_name_replacement_proc(old_name)
	if(!length(data_core.general.records))
		return old_name
	var/datum/db_record/record = pick(data_core.general.records)
	return record["name"]

/datum/phrase_log/proc/random_custom_ai_law(max_tries=20, replace_names=FALSE)
	while(max_tries-- > 0)
