...
/proc/swapTheThing()
	...
	if(!source)
		logTheThing("debug", null, "Swap failed, no source")
		return 0
	var/turf/corner = locate(source.x+1,source.y+1,source.z)
	if(!corner)
		logTheThing("debug", null, "Swap failed, no corner")
		return 0
	var/loaded = file2text("assets/maps/prefabs/prefab_ksol.dmm")
	if(corner && loaded)
		logTheThing("debug", null, "Starting SwapLoad (["assets/maps/prefabs/prefab_ksol.dmm"]) at [time2text(world.timeofday)]")
		...
		logTheThing("debug", null, "Finished SwapLoad (["assets/maps/prefabs/prefab_ksol.dmm"]) at [time2text(world.timeofday)]")
		logTheThing("debug", null, "Starting SwapDelete (size : [props.maxX - props.sourceX]x - [props.maxY - props.sourceY]y) at [time2text(world.timeofday)]")
		var/count = 0
		...
		for(var/turf/T as anything in block)
			for(var/Y in T)
				if(isobj(Y) && !istype(Y, /obj/overlay/tile_effect))
					qdel(Y)
					count++
		logTheThing("debug", null, "Finished SwapDelete (count: [count]) at [time2text(world.timeofday)]")
...
/datum/swapMaster
	...
	proc/placeSwapPrefab(var/prefabPath=null)
		...
		if(corner && loaded)
			logTheThing("debug", null, "Starting SwapLoad ([prefabPath]) at [time2text(world.timeofday)]")
			var/dmm_suite/D = new/dmm_suite()
			D.read_map(loaded,corner.x,corner.y,corner.z,prefabPath)
			logTheThing("debug", null, "Finished SwapLoad ([prefabPath]) at [time2text(world.timeofday)]")
			busy = 0
			return 1
		else
			busy = 0
