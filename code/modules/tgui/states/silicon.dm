...
/datum/ui_state/tgui_silicon_state/can_use_topic(src_object, mob/user)
	return user.can_use_topic(src_object) // Call the individual mob-overridden procs.

/mob/proc/can_use_topic(src_object)
	return UI_CLOSE // Don't allow interaction by default.

/mob/living/can_use_topic(src_object)
...
/mob/living/silicon/robot/can_use_topic(src_object)
	. = shared_ui_interaction(src_object)
	if(. <= UI_DISABLED)
		return
...
/mob/dead/aieye/can_use_topic(src_object)
	. = shared_ui_interaction(src_object)
	if(. < UI_INTERACTIVE)
		return
