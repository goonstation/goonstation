 ...
 */
/proc/tgui_input_list(mob/user, message, title, list/buttons, timeout = 0, autofocus = TRUE, allowIllegal = FALSE)
	if (!user)
		user = usr
	if(!length(buttons))
		return
	...
	if (!user)
		return
	var/datum/tgui_modal/list_input/input = new(user, message, title, buttons, timeout, autofocus, allowIllegal)
 ...
 * * allowIllegal - Whether to allow illegal characters in buttons.
 */
/proc/tgui_input_list_async(mob/user, message, title, list/buttons, datum/callback/callback, timeout = 60 SECONDS, autofocus = TRUE, allowIllegal = FALSE)
	if (!user)
		user = usr
	if(!length(buttons))
		return
	if (!istype(user))
		if (istype(user, /client))
			...
			return
	var/datum/tgui_modal/list_input/async/input = new(user, message, title, buttons, callback, timeout, autofocus, allowIllegal)
	input.ui_interact(user)
...
/datum/tgui_modal/list_input/New(mob/user, message, title, list/buttons, timeout, autofocus = TRUE, allowIllegal = FALSE)
	src.buttons = list()
...
/datum/tgui_modal/list_input/async/New(mob/user, message, title, list/buttons, callback, timeout, allowIllegal = FALSE)
	..(user, title, message, buttons, timeout, allowIllegal)
