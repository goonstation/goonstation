...
proc/calculateHeatTransferCoefficient(var/datum/material/matA, var/datum/material/matB)
	...
	if(matA)
		...
		else if(matA.hasProperty("conductivity"))
			hTC1 = max(matA.getProperty("conductivity"),1)/100
		else
			hTC1 = 0.5 //default value
	if(matB)
		if(matB.hasProperty("thermal"))
			hTC2 = max(matB.getProperty("thermal"),1)/100
		else if(matB.hasProperty("conductivity"))
			hTC2 = max(matB.getProperty("conductivity"),1)/100
		else
