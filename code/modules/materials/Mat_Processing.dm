		...
		if(GET_DIST(src, target) <= world.view)
			...
			if(!W.material)
				...
				if(length(W.material.getMaterialProperties()))
					...
					switch(W.material.getAlpha())
						if(-INFINITY to 80)
							boutput(user, SPAN_NOTICE("• extremely transparent ([W.material.getAlpha()])"))
						if(80 to 130)
							boutput(user, SPAN_NOTICE("• very transparent ([W.material.getAlpha()])"))
						if(130 to 180)
							boutput(user, SPAN_NOTICE("• transparent ([W.material.getAlpha()])"))
						if(180 to INFINITY)
							boutput(user, SPAN_NOTICE("• opaque ([W.material.getAlpha()])"))
				else
