...
/image/maptext/aside/init(datum/say_message/message)
	var/maptext_css = ""
	for (var/key in message.maptext_css_values)
		maptext_css += "[key]: [message.maptext_css_values[key]]; "

	src.maptext = "<span class='pixel c ol' style=\"font-size: 5px; [maptext_css]\">[message.maptext_prefix][message.format_content_style_prefix][message.content][message.format_content_style_suffix][message.maptext_suffix]</span>"
	src.pixel_x += prob(50) ? 28 : -28
	src.pixel_y -= rand(24,32)
