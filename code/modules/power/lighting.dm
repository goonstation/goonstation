		...
		if(current_lamp.light_status == LIGHT_OK)
			if(on && current_lamp.rigged)
				if (current_lamp.rigger)
					...
					logTheThing("combat", current_lamp.rigger, "'s rigged bulb exploded in [current_lamp.rigger.loc.loc] ([log_loc(src)])")
				explode()
			if(on && prob(current_lamp.breakprob))
				...
				logTheThing("station", null, "Light '[name]' burnt out (breakprob: [current_lamp.breakprob]) at ([log_loc(src)])")
...
/obj/machinery/light/attackby(obj/item/W, mob/user)
	...
	// attempt to break the light
		...
		if(prob(1+W.force * 5))
			...
			logTheThing("station", user, "smashes a light at [log_loc(src)]")
...
/obj/machinery/light/proc/broken(var/nospark = 0)
	...
	if(!nospark)
		if(on)
			logTheThing("station", null, "Light '[name]' was on and has been broken, spewing sparks everywhere ([log_loc(src)])")
			elecflash(src,radius = 1, power = 2, exclude_center = 0)
	current_lamp.light_status = LIGHT_BROKEN
	current_lamp.update()
	SPAWN(0)
		...
		if(rigged)
			if(prob(1))
				if (rigger)
					...
					logTheThing("combat", rigger, "'s rigged bulb exploded in [rigger.loc.loc] ([log_loc(src)])")
				explode()
				rigged = 0
				rigger = null
			else if(prob(2))
				if (rigger)
					...
					logTheThing("combat", rigger, "'s rigged bulb tried to explode but failed in [rigger.loc.loc] ([log_loc(src)])")
				rigged = 0
				rigger = null
*/
...
/obj/item/light/attackby(var/obj/item/I, var/mob/user)
	...
	if(istype(I, /obj/item/reagent_containers/syringe))
		...
		if(S.reagents.has_reagent("plasma", 1))
			logTheThing("combat", user, "rigged [src] to explode in [user.loc.loc] ([log_loc(user)])")
			rigged = 1
			rigger = user
