	...
	onMaterialChanged()
		if(can_transform)
			...
			if(src.material.hasProperty(MATERIAL_PROPERTY_ELECTRICAL))
				electrical_conductivity = src.material.getProperty(MATERIAL_PROPERTY_ELECTRICAL)/9 * 10

			var/thermal_conductivity = 50
			if(src.material.hasProperty(MATERIAL_PROPERTY_THERMAL))
				thermal_conductivity =  src.material.getProperty(MATERIAL_PROPERTY_THERMAL)/9 * 10
