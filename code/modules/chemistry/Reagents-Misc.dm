		...
		omegazine
			...
			on_mob_life(var/mob/living/M, var/mult = 1)
				...
				if (src.volume > 5)
					...
					if (probmult(10))
						M.setStatusMax("stunned", 4 SECONDS)
				..()
				return
			...
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume_passed)
				...
				if(method == TOUCH)
					...
					if(L.getStatusDuration("slowed")>=10 SECONDS)
						L.setStatusMax("staggered", (0.3 SECONDS)*volume_passed)
		...
		capulettium
			...
			on_mob_life(var/mob/M, var/mult = 1)
				...
				if (counter >= 19 && !fakedeathed)
					...
					M.setStatusMax("muted", 3 SECONDS * mult)
			...
			reaction_mob(var/mob/living/M, var/method=TOUCH, var/volume_passed)
				...
				if(method == INGEST)
					...
					if (H.bioHolder && H.bioHolder.HasEffect("bee"))
						...
						M.setStatusMax("weakened", 3 SECONDS)
	...
	proc/process()
		while (!disposed)
			if (get_dist(src, src.deathtarget) <= 1)
				...
				deathtarget.setStatusMax("stunned", 5 SECONDS)
				deathtarget.setStatusMax("weakened", 5 SECONDS)
