	...
	reagent
		nitroglycerin // Yes, this is a bad idea.
			...
			proc/explode(var/list/covered_turf, expl_reason)
				...
				logTheThing("combat", usr, "is associated with a nitroglycerin explosion (volume = [volume]) due to [expl_reason] at [log_loc(T)]. Context: [context].")
			...
			on_transfer(var/datum/reagents/source, var/datum/reagents/target, var/trans_volume)
				...
				logTheThing("combat", usr, "caused physical shock to nitroglycerin by transferring [trans_volume]u from [source.my_atom] to [target.my_atom].")
			...
			on_mob_life(var/mob/M, var/mult = 1)
				...
				if (our_amt < 5)
					...
					SPAWN(rand(20, 100))
						...
						if (ishuman(M))
							logTheThing("combat", M, "was transformed into a dog by reagent [name] at [log_loc(M)].")
						M.gib()
		...
		hootagen_unstable
			...
			on_mob_life(var/mob/M, var/mult = 1)
				...
				if (our_amt < 5)
					...
					SPAWN(rand(20, 100))
						if (ishuman(M))
							logTheThing("combat", M, "was owlgibbed by reagent [name] at [log_loc(M)].")
				...
				if (ishuman(M))
					...
					if(holder.get_reagent_amount(src.id) > gib_threshold)
						...
						if(probmult(1)) // i hate you all, players
							H.flockbit_gib()
							logTheThing("combat", H, "was gibbed by reagent [name] at [log_loc(H)].")
					else
			...
			on_mob_life(var/mob/M, var/mult = 1)
				...
				if (!pathogens_processed) //Only process pathogens once
					...
					for (var/uid in src.pathogens)
						...
						logTheThing("pathology", M, "metabolizing [src] containing pathogen [P].")
						M.infected(P)
				..()
...
*/
		...
		big_bang
			...
			on_mob_life(var/mob/M, var/mult = 1)
				...
				M.ex_act(1)
				if (ishuman(M))
					logTheThing("combat", M, "was gibbed by reagent [name] at [log_loc(M)].")
				M.gib()
				#endif
