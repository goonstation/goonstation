	...
	afterattack(obj/target, mob/user , flag)
		...
		if (ismob(target))
			...
			if (G && user.a_intent == "help" && T.a_intent == "help" && user != T)
				...
				logTheThing("combat", user, "splashes [src] onto [constructTarget(MOB,"combat")] [log_reagents(src)] at [log_loc(MOB)].") // Added location (Convair880).
				can_mousedrop = 0
				if (src.splash_all_contents)
			...
			logTheThing("combat", user, "splashes [src] onto [constructTarget(target,"combat")] [log_reagents(src)] at [log_loc(user)].") // Added location (Convair880).
			if (reagents)
				reagents.physical_shock(14)
	...
	attackby(obj/item/I, mob/user)
		/*if (istype(I, /obj/item/reagent_containers/pill))
			...
			if (src.reagents && I.reagents)
				...
				logTheThing("combat", user, "poisoned [I] [log_reagents(I)] with reagents from [src] [log_reagents(src)] at [log_loc(user)].") // Added location (Convair880).
