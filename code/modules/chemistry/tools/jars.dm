...
proc/save_intraround_jars()
	...
	for_by_tcl(jar, /obj/item/reagent_containers/glass/jar)
		...
		logTheThing("debug", null, "<b>Pickle Jar:</b> Jar saved at [log_loc(jar)] ([zname]) containing [json_encode(jar_contents)]")
...
proc/generate_backup_jars()
	...
	while(length(by_type[/obj/item/reagent_containers/glass/jar]) < DEFAULT_JAR_COUNT && tries_left > 0)
		...
		if(length(turfs))
			...
			logTheThing("debug", null, "<b>Pickle Jar:</b> New empty jar created at [log_loc(T)]")
		else
			tries_left--
	tries_left = 50
	while(length(by_type[/obj/item/reagent_containers/glass/jar]) < DEFAULT_JAR_COUNT && tries_left > 0)
	...
	for(var/datum/zlevel/zlevel in global.zlevels)
		...
		try
			...
			if(!emitted_full_savefile)
				logTheThing("debug", null, "<b>Pickle Jar:</b> full savefile<br>[jar_save.ExportText()]")
		...
		for(var/list/jar_data in jars_data)
			...
			logTheThing("debug", null, "<b>Pickle Jar:</b> Jar created at [log_loc(jar)] ([zname]) containing [json_encode(jar_contents)]")
			...
			if(length(length(by_type[/obj/item/reagent_containers/glass/jar])) >= MAX_JAR_COUNT)
				logTheThing("debug", null, "<b>Pickle Jar:</b> Jar creation process hit maximum limit of [MAX_JAR_COUNT], further jars are lost to time.")
				return
