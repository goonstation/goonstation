...
/obj/item/reagent_containers/patch
	...
	attack_self(mob/user as mob)
		...
		if (can_operate_on(user))
			...
			logTheThing("combat", user, "applies a patch to themself [log_reagents(src)] at [log_loc(user)].")
	...
	throw_impact(atom/M, datum/thrown_thing/thr)
		...
		if (src.medical && !borg && !src.in_use && (can_operate_on(M)))
			if (prob(30) || good_throw && prob(70))
				...
				logTheThing("combat", M, "is stuck by a patch [log_reagents(src)] thrown by [constructTarget(usr,"combat")] at [log_loc(M)].")
				apply_to(M,usr)
		...
		if (can_operate_on(M))
			...
			if (M == user)
				...
				if (medical == 0)
					...
					logTheThing("combat", user, "tries to apply a patch [log_reagents(src)] to [constructTarget(M,"combat")] at [log_loc(user)].")
			...
			logTheThing("combat", user, "applies a patch to [constructTarget(M,"combat")] [log_reagents(src)] at [log_loc(user)].")
...
/obj/item/reagent_containers/mender
	...
	attack(mob/M, mob/user)
		...
		if (can_operate_on(M) && !actions.hasAction(user,"automender_apply"))
			...
			logTheThing("combat", user, "begins automending [constructTarget(M,"combat")] [log_reagents(src)] at [log_loc(user)].")
	...
	proc/apply_to(mob/M as mob, mob/user as mob, var/mult = 1, var/silent = 0)
		...
		if (reagents?.total_volume)
			...
			logTheThing("combat", user, " automends [constructTarget(M,"combat")] [log_reagents(src)] at [log_loc(user)].")
