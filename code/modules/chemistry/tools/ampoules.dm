...
/obj/item/reagent_containers/ampoule/attack(mob/M, mob/user)
	if(expended || reagents.total_volume <= 0)
		...
		logTheThing("combat", user, "tries to make [constructTarget(M,"combat")] inhale [src] [log_reagents(src)] at [log_loc(user)].")
		if(!do_mob(user, M))
			if(user && ismob(user))
	...
	logTheThing("combat", user, "[user == M ? "inhales" : "makes [constructTarget(M,"combat")] inhale"] an ampoule [log_reagents(src)] at [log_loc(user)].")
