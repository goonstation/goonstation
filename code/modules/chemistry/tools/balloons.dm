...
//					user.update_inhands()
	...
	afterattack(obj/target, mob/user)
		if ((is_reagent_dispenser(target) || (target.is_open_container() == -1 && target.reagents)) && (istype(target, /obj/machinery/portable_atmospherics)) == FALSE) //A dispenser. Transfer FROM it TO us.
