...
/obj/item/reagent_containers/pill
	...
	attack_self(mob/user as mob)
		...
		if (iscarbon(user) || ismobcritter(user))
			...
			logTheThing("combat", user, "swallows a [src.name] [log_reagents(src)] at [log_loc(user)].")
	...
	attack(mob/M, mob/user, def_zone)
		...
		if (iscarbon(M) || ismobcritter(M))
			if (M == user)
				...
				logTheThing("combat", user, "tries to force-feed a [src.name] [log_reagents(src)] to [constructTarget(M,"combat")] at [log_loc(user)].")
			...
			logTheThing("combat", user, "[user == M ? "swallows" : "makes [constructTarget(M,"combat")] swallow"] a [src.name] [log_reagents(src)] at [log_loc(user)].")
			if (reagents.total_volume)
	...
	afterattack(var/atom/target, mob/user, flag)
		...
		if (target.is_open_container() && target.reagents)
			...
			logTheThing("combat", user, "dissolves a [src.name] [log_reagents(src)] in [target] at [log_loc(user)].")
