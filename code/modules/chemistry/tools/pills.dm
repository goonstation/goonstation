...
/obj/item/reagent_containers/pill
	...
	attack_self(mob/user as mob)
		...
		if (iscarbon(user) || ismobcritter(user))
			...
			logTheThing("chemistry", user, null, "swallows a [src.name] [log_reagents(src)] at [log_loc(user)].")
	...
	afterattack(var/atom/target, mob/user, flag)
		...
		if (target.is_open_container() && target.reagents)
			...
			logTheThing("chemistry", user, null, "dissolves a [src.name] [log_reagents(src)] in [target] at [log_loc(user)].")
