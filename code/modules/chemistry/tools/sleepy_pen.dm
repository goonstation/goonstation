/obj/item/pen/sleepypen
	...
	attack(mob/M, mob/user)
		...
		if (src.reagents.total_volume)
			...
			logTheThing("combat", user, "stabs [constructTarget(M,"combat")] with the sleepy pen [log_reagents(src)] at [log_loc(user)].")
	...
	attack(mob/M, mob/user)
		...
		if (src.reagents.total_volume)
			...
			if(luck==1)
				...
				logTheThing("combat", user, "stabs [constructTarget(M,"combat")] with the discount sleepy pen [log_reagents(src)] at [log_loc(user)].")
			...
			if(luck==2)
				if(src.reagents.total_volume)
					...
					logTheThing("combat", user, "tries to stab [constructTarget(M,"combat")] with the discount sleepy pen with [log_reagents(src)] but fails at [log_loc(user)].")
			...
			if(luck==3)
				...
				logTheThing("combat", user, "tries to stab [constructTarget(M,"combat")] with the discount sleepy pen [log_reagents(src)] but uses it on themselves at [log_loc(user)].")
