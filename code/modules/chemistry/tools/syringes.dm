	...
	afterattack(var/atom/target, mob/user, flag)
		...
		switch(mode)
			if (S_DRAW)
				...
				if (!target.is_open_container(FALSE) && !istype(target,/obj/reagent_dispensers))
			...
			if (S_INJECT)
				...
				if (target.is_open_container(TRUE) && !ismob(target) && !istype(target,/obj/item/reagent_containers/food) && !istype(target,/obj/item/reagent_containers/patch))
