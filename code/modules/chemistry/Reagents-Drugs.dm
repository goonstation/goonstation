			...
			do_overdose(var/severity, var/mob/M, var/mult = 1)
				...
				if (severity == 1)
					if (effect <= 2)
						...
						M.setStatusMax("stunned", 4 SECONDS * mult)
					...
					if (effect <= 2)
						...
						M.take_toxin_damage(2 * mult)
						M.take_brain_damage(1 * mult)
						M.setStatusMax("weakened", 9 SECONDS * mult)
						M.emote("gasp")
						M.reagents.add_reagent("salts1", 5 * mult)
					else if (effect <= 7)
						M.emote("scream")
			...
			do_overdose(var/severity, var/mob/M, var/mult = 1)
				...
				if (severity == 1)
					...
					if (effect <= 2)
						...
						M.setStatusMax("weakened", 4 SECONDS * mult)
						M.change_misstep_chance(25 * mult)
						M.emote("scream")
						M.reagents.add_reagent("salts1", 5 * mult)
					else if (effect <= 7)
			...
			do_overdose(var/severity, var/mob/M, var/mult = 1)
				...
				if (severity == 1)
					...
					if (effect <= 2)
						...
						M.take_toxin_damage(3 * mult)
						M.setStatusMax("stunned", 1 SECOND * mult)
					else if (effect <= 4)
						boutput(M, "<span class='alert'><b>You feel terrible!</b></span>")
						M.emote("drool")
						M.make_jittery(10)
						M.take_toxin_damage(5 * mult)
						M.setStatusMax("weakened", 1 SECOND * mult)
						...
						M.setStatusMax("paralysis", 5 SECONDS * mult)
						M.make_jittery(30)
						...
						if(3)
							switch (rand(1,4))
								...
								if(4)
									...
									M.setStatusMax("paralysis", 3 SECONDS * mult)
				..()
		...
		drug/krokodil
			...
			on_mob_life(var/mob/M, var/mult = 1)
				...
				if(prob(5))
					...
					M.setStatusMax("stunned", 2 SECONDS * mult)
					M.take_toxin_damage(1 * mult)
					M.take_brain_damage(1 * mult)
			...
			do_overdose(var/severity, var/mob/M, var/mult = 1)
				...
				if (severity == 1)
					if (effect <= 2)
						...
						M.setStatusMax("weakened", 5 SECONDS * mult)
					else if (effect <= 4)
					...
					if (effect <= 2)
						...
						M.make_jittery(10)
						M.setStatusMax("weakened", 10 SECONDS * mult)
					else if (effect <= 7)
						M.emote("laugh")
			...
			do_overdose(var/severity, var/mob/M, var/mult = 1)
				...
				if (severity == 1)
					...
					if (effect <= 2)
						...
						M.visible_message("<span class='alert'><b>[M.name]</b> falls to the floor and flails uncontrollably!</span>")
						M.make_jittery(10)
						M.setStatusMax("weakened", 2 SECONDS * mult)
					else if (effect <= 7)
						M.emote("laugh")
