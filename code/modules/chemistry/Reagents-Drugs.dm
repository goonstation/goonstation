			...
			on_mob_life(var/mob/M, var/mult = 1) // commence bad times
				...
				if (istype(H))
					if (M.reagents.has_reagent("milk"))
						boutput(M, "<span style=\"color:blue\">The milk stops the burning. Ahhh.</span>")
					...
					if(check < 5)
						...
						boutput(M, "<span style=\"color:red\"><b>A hellbat begins to chase you</b>!</span>")
						M.emote("scream")
					if(check < 20)
						boutput(M, "<span style=\"color:red\"><b>Oh god! Oh GODD!!</b></span>")
					if(check < 20)
						boutput(M, "<span style=\"color:red\"><b>You feel like you are melting from the inside!</b></span>")
					if(check < 20)
						boutput(M, "<span style=\"color:red\">Your throat feels like it's on fire!</span>")
						M.emote(pick("scream","cry","twitch_s","choke","gasp","grumble"))
						M.changeStatus("paralysis", 2 SECONDS)
					if(check < 20)
						boutput(M, "<span style=\"color:blue\"><b>You feel A LOT warmer.</b></span>")
						M.bodytemperature += rand(30,60)
				..()
				return
...
datum/reagent/drug/hellshroom_extract/proc/breathefire(var/mob/M)
	...
	M.visible_message("<span style=\"color:red\"><b>[M] burps a stream of fire!</b></span>")
