		...
		drug/bathsalts
			...
			on_mob_life(var/mob/M, var/mult = 1) // commence bad times
				...
				if (ishuman(M))
					...
					if (check < 8 && H.bioHolder.mobAppearance.customizations[2].style.id != "tramp") // M.is_hobo = very yes
						H.bioHolder.mobAppearance.customizations[2].style =  new /datum/customization_style/beard/tramp
