				...
				switch(counter += (1 * mult))
					...
					if (175 to INFINITY)
						...
						if (probmult(8))
							...
							M.setStatusMax("stunned", 6 SECONDS * mult)
		...
		harmful/itching
			...
			on_mob_life(var/mob/M, var/mult = 1) // commence the tickling
				...
				if (prob(2))
					...
					M.setStatusMax("weakened", 6 SECONDS * mult)
					M.make_jittery(6)
				...
				switch(counter += (1 * mult))
					...
					if (6 to 10)
						if (prob(8))
							...
							M.emote(pick("drool", "tremble"))
					if (11 to INFINITY)
						M.setStatusMax("weakened", 20 SECONDS * mult)
						if (prob(10))
			...
			on_mob_life(var/mob/M, var/mult = 1) // sped this up a bit due to mob loop changes
				...
				switch(counter += 1)
					...
					if (10)
						...
						M.setStatusMax("weakened", 5 SECONDS * mult)
					if (11 to INFINITY)
						M.setStatusMax("paralysis", 25 SECONDS * mult)
			...
			on_mob_life(var/mob/M, var/mult = 1)
				...
				switch(counter+= (1 * mult))
					...
					if (22 to INFINITY)
						if (prob(20))
							M.emote("faint")
							M.setStatusMax("paralysis", 8 SECONDS * mult)
						M.setStatus("drowsy", 40 SECONDS)
				M.take_toxin_damage(1 * mult)
				..()
				return
		...
		harmful/spider_venom
			...
			on_mob_life(var/mob/M, var/mult = 1)
				...
				if (our_amt > 40 && probmult(4))
					...
					M.setStatusMax("weakened", 15 SECONDS * mult)
					M.make_jittery(1000)
					SPAWN(rand(20, 100))
				...
				switch(counter += (1 * mult))
					...
					if (18 to INFINITY)
						if (!fainted)
							M.emote("faint")
							fainted = 1
						M.setStatusMax("paralysis", 10 SECONDS * mult)
						M.setStatus("drowsy", 40 SECONDS)
