...
datum
	reagents
		...
		proc/get_description(mob/user, rc_desc_flag=0)
			if(rc_desc_flag == 0)	// Report nothing about the reagents in this case
				return null

			if(reagent_list.len)
				. += get_inexact_description(rc_desc_flag)
				if(rc_desc_flag & RC_SPECTRO)
		...
		proc/get_inexact_description(var/rc_desc_flag=0)
			if(rc_desc_flag == 0)
				return null
			...
			if(full_text == "empty")
				if(rc_desc_flag & (RC_SCALE | RC_VISIBLE | RC_FULLNESS) )
			...
			if(rc_desc_flag & RC_VISIBLE)
				if(rc_desc_flag & RC_SCALE)
