...
/datum/reagents
	...
	proc/smoke_start(var/volume, var/classic = 0)
		...
		if (our_user && ismob(our_user))
			...
			logTheThing(LOG_CHEMISTRY, our_user, "Smoke reaction ([my_atom ? log_reagents(my_atom) : log_reagents(src)]) at [T ? "[log_loc(T)]" : "null"]. Container last touched by: [last_ckey]")
