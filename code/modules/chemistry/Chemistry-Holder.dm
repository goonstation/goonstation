	...
	reagents
				...
				for(var/datum/chemical_reaction/C in src.possible_reactions)
					...
					for(var/B in C.required_reagents)
						...
						try
							...
							logTheThing("debug", usr, "CRASH: reagent holder / handle_reactions: [C.name] reaction, reagent checked: '[B]', required = '[B_required_volume]', reagents in thing: [log_reagents(my_atom)].")
					...
					if(total_matching_reagents == C.required_reagents.len)
						...
						if (!old_reactions.Find(C))
							...
							if (C.special_log_handling)
								logTheThing("combat", usr, "[C.name] chemical reaction [log_reagents(my_atom)] at [T ? "[log_loc(T)]" : "null"].")
								...
								logTheThing("combat", usr, "[C.name] chemical reaction at [T ? "[log_loc(T)]" : "null"].")
		...
		proc/smoke_start(var/volume, var/classic = 0)
			...
			if (our_user && ismob(our_user))
				logTheThing("combat", our_user, "Smoke reaction ([my_atom ? log_reagents(my_atom) : log_reagents(src)]) at [T ? "[log_loc(T)]" : "null"].")
				if(istype(src.my_atom, /obj/item/reagent_containers) && !locate(/obj/machinery/chem_dispenser) in get_turf(src.my_atom))
				...
				logTheThing("combat", our_user, "Smoke reaction ([my_atom ? log_reagents(my_atom) : log_reagents(src)]) at [T ? "[log_loc(T)]" : "null"].[our_fingerprints ? " Container last touched by: [our_fingerprints]." : ""]")
