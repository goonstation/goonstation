	...
	reagents
					...
					if(total_matching_reagents == C.required_reagents.len)
						...
						if (!old_reactions.Find(C))
							...
							if (C.special_log_handling)
								logTheThing("chemistry", usr, null, "[C.name] chemical reaction [log_reagents(my_atom)] at [T ? "[log_loc(T)]" : "null"].")
								...
								logTheThing("chemistry", usr, null, "[C.name] chemical reaction at [T ? "[log_loc(T)]" : "null"].")
		...
		proc/smoke_start(var/volume, var/classic = 0)
			...
			if (our_user && ismob(our_user))
				logTheThing("chemistry", our_user, null, "Smoke reaction ([my_atom ? log_reagents(my_atom) : log_reagents(src)]) at [T ? "[log_loc(T)]" : "null"].")
				if(istype(src.my_atom, /obj/item/reagent_containers) && !locate(/obj/machinery/chem_dispenser) in get_turf(src.my_atom))
				...
				logTheThing("chemistry", our_user, null, "Smoke reaction ([my_atom ? log_reagents(my_atom) : log_reagents(src)]) at [T ? "[log_loc(T)]" : "null"].[our_fingerprints ? " Container last touched by: [our_fingerprints]." : ""]")
