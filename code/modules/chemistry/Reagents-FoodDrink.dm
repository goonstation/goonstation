			...
			on_mob_life(var/mob/M, var/mult = 1)
				...
				if (holder.has_reagent(src.id,75))
					...
					if (prob(5))
						...
						M.setStatusMax("stunned", 10 SECONDS * mult)
						M.setStatusMax("paralysis", 6 SECONDS * mult)
				else
