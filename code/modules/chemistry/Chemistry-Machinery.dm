...
/obj/machinery/chem_master
	...
	Topic(href, href_list)
		...
		if (href_list["analyze"])
			...
			color_icon(P)
			src.updateUsrDialog()
			logTheThing("combat", usr, "used the [src.name] to create a [pillname] pill containing [log_reagents(P)] at [log_loc(src)].")
			return
			...
			logTheThing("combat", usr, "used the [src.name] to create [pillcount] [pillname] pills containing [log_reagents(R)] at [log_loc(src)].")
			...
			src.updateUsrDialog()
			logTheThing("combat", usr, "used the [src.name] to create [bottlename] bottle containing [log_reagents(B)] at log_loc[src].")
			return
			...
			C.name = "[bottlename]"
			src.updateUsrDialog()
			logTheThing("combat", usr, "used the [src.name] to create a can named [bottlename] containing [log_reagents(C)] at log_loc[src].")
			return
			...
			P.on_reagent_change()
			src.updateUsrDialog()
			logTheThing("combat", usr, "used the [src.name] to create a [patchname] patch containing [log_reagents(P)] at [log_loc(src)].")
			return
			...
			R.trans_to(A, 5)
			logTheThing("combat", usr, "used the [src.name] to create a [ampoulename] ampoule containing [log_reagents(A)] at [log_loc(src)].")
			updateUsrDialog()
			return
			...
			logTheThing("combat", usr, "used the [src.name] to create [patchcount] [patchname] patches from [log_reagents(R)] at [log_loc(src)].")
			...
			if(mode_type == "CheMaster")
				...
				switch(action)
					...
					if("Create Patch")
						...
						P.medical = all_safe
						P.on_reagent_change()
						logTheThing("combat", user, "used the [src.name] to create a [patchname] patch containing [log_reagents(P)] at [log_loc(src)].")
					if("Create Ampoule")
						var/datum/reagents/R = B.reagents
						...
						A.name = "ampoule ([ampoulename])"
						R.trans_to(A, 5)
						logTheThing("combat", user, "used the [src.name] to create a [ampoulename] ampoule containing [log_reagents(A)] at [log_loc(src)].")
