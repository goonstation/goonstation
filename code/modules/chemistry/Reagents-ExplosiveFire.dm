						...
						for(var/turf/turf in covered)
							...
							if(holder && volume/length(covered) >= explosion_threshold)
								if(holder.my_atom)
									...
									logTheThing(LOG_BOMBING, last_ckey_atom, "Welding Fuel explosion (inside [holder.my_atom], reagent type: [id]) at [log_loc(holder.my_atom)]. Last touched by: [key_name(last_ckey_atom)] (usr: [ismob(usr) ? key_name(usr) : usr]).")
								else
