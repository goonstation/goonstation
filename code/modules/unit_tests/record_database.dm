...
/datum/unit_test/record_database/Run()
	...
	var/datum/db_record/r1 = db.add_record(list())
	...
	var/datum/db_record/r2 = db.add_record(list("id" = 1, "test_index" = "test", "name" = "test2"))
	...
	r2.delete()
	TEST_ASSERT(isnull(r2.db), "A record got deleted but it still holds a reference to its database.")
