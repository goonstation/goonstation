/datum/unit_test/galaxy_rand
	var/datum/galaxy/G

/datum/unit_test/galaxy_rand/Run()
	G = new

	distribution_check(/datum/galaxy/proc/xor_rand, list(0,100),1000)
	distribution_check(/datum/galaxy/proc/xor_rand, list(1,99),1000)

	distribution_check(/datum/galaxy/proc/xor_randf, list(1, 99),1000)
	distribution_check(/datum/galaxy/proc/xor_randf, list(0, 100),1000)

	distribution_check(/datum/galaxy/proc/xor_rand, list(),1000)
	distribution_check(/datum/galaxy/proc/xor_rand, list(),1000)

/datum/unit_test/galaxy_rand/proc/distribution_check(delegate, args, iterations)
	var/result
	var/sum
	...
	for(var/i in 1 to iterations)
		if(delegate)
			result = call(G, delegate)(arglist(args))
		else
			if(length(args))
	...
	TEST_ASSERT(cv >= 0.55, "Test Coefficient of variation is within tolerance. [delegate] [cv] >= 0.55")
	TEST_ASSERT(cv <= 0.65, "Test Coefficient of variation is within tolerance. [delegate] [cv] <= 0.59")
