...
/datum/unit_test/regression/say_message_gc_without_listener/Run()
...
/datum/unit_test/regression/say_message_gc_without_listener/proc/say_and_release_message(obj/speaker)
	RETURN_TYPE(/datum/weakref)
	var/datum/say_message/message = speaker.say("say_message GC test")
	return get_weakref(message)

/datum/unit_test/regression/say_message_flush_registration_cleanup/Run()
	var/obj/speaker = src.allocate(/obj/say_message_gc)
...
/datum/unit_test/regression/say_message_flush_registration_cleanup/proc/say_to_listener_and_release_message(obj/speaker, obj/listener)
	RETURN_TYPE(/datum/weakref)
