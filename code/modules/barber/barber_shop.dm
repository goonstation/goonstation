...
/obj/item/clothing/head/wig/spawnable/random
	...
	New()
		...
		if (prob(50))
			possible_hairstyles = pick(get_available_custom_style_types(gender=CUSTOMIZATION::GENDER::FEMININE))
		else
			possible_hairstyles = pick(get_available_custom_style_types(gender=CUSTOMIZATION::GENDER::MASCULINE))
