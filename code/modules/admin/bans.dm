...
/proc/checkBan(ckey, compID, ip, record = 0)
	...
	if (!expired && (row["ckey"] != ckey || row["ip"] != ip || row["compID"] != compID)) //Insert a new ban for this JERK
		...
		var/addData[] = new()
		addData["ckey"] = ckey
		addData["compID"] = row["compID"] && compID // don't record CID if the original ban doesn't have one down
		addData["ip"] = row["ip"] && ip // don't record ip if the original ban doesn't have one down
