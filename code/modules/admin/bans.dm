...
/proc/checkBan(ckey, compID, ip, record = 0)
	set background = 1
	if (!ckey && !compID && !ip)
		logTheThing("debug", null, "<b>Bans Error</b>: No details passed to <b>checkBan</b>")
		logTheThing("diary", null, "Bans Error: No details passed to checkBan", "debug")
		return 0
	...
	if (data["error"]) //Error returned from the API welp
		logTheThing("debug", null, "<b>Bans Error</b>: Error returned in <b>checkBan</b> for <b>[ckey]</b>: [data["error"]]")
		logTheThing("diary", null, "Bans Error: Error returned in checkBan for [ckey]: [data["error"]]", "debug")
		return 0
	...
	if (!expired && (row["ckey"] != ckey || row["ip"] != ip || row["compID"] != compID)) //Insert a new ban for this JERK
		...
		if (rVal)
			logTheThing("debug", null, "<b>Bans Error</b>: Add ban in checkBan failed with message <b>[rVal]</b>")
			logTheThing("diary", null, "Bans Error: Add ban in checkBan failed with message [rVal]", "debug")
	...
	if (timestamp > 0) //Temp ban found, determine if it should expire or not
		if (expired) //It expired! Go you!
			...
			if (rVal)
				logTheThing("debug", null, "<b>Bans Error</b>: Delete temp ban in checkBan failed with message <b>[rVal]</b>")
				logTheThing("diary", null, "Bans Error: Delete temp ban in checkBan failed with message [rVal]", "debug")
			return 0
...
/proc/addBan(data)
	...
	if (data["data_hub_callback"])
		...
		if (text2num(row["timestamp"]) <= 0)
			...
			logTheThing("admin", adminC, "has banned [targetC ? "[constructTarget(targetC,"admin")]" : replacement_text] [serverLogSnippet]. duration: [duration] Reason: [row["reason"]].")
			logTheThing("diary", adminC, "has banned [targetC ? "[constructTarget(targetC,"diary")]" : replacement_text] [serverLogSnippet]. duration: [duration] Reason: [row["reason"]].", "admin")
			...
			logTheThing("admin", adminC, "has banned [targetC ? "[constructTarget(targetC,"admin")]" : replacement_text] [serverLogSnippet]. Reason: [row["reason"]]. This will be removed in [expiry].")
			logTheThing("diary", adminC, "has banned [targetC ? "[constructTarget(targetC,"diary")]" : replacement_text] [serverLogSnippet]. Reason: [row["reason"]]. This will be removed in [expiry].", "admin")
			var/adminMsg = "<span class='notice'>"
			adminMsg += (isclient(adminC) ? key_name(adminC) : adminC)
		...
		if (targetC)
			...
			logTheThing("debug", null, "<b>Bans:</b> Unable to find client with ckey '[row["ckey"]]' during banning.")

		targetC = find_player(row["ckey"])?.client
		if (targetC)
			logTheThing("debug", null, "<b>Bans:</b> Client with ckey '[row["ckey"]]' somehow survived banning, retrying kick.")
			del(targetC)
...
/proc/editBan(data)
	...
	if (data["data_hub_callback"])
		...
		logTheThing("admin", adminC, "edited [constructTarget(target,"admin")]'s ban. Reason: [row["reason"]] Duration: [duration] [serverLogSnippet]")
		logTheThing("diary", adminC, "edited [constructTarget(target,"diary")]'s ban. Reason: [row["reason"]] Duration: [duration] [serverLogSnippet]", "admin")
...
/proc/deleteBan(data)
	...
	if (data["data_hub_callback"])
		...
		if (expired)
			logTheThing("admin", null, "[row["ckey"]]'s ban expired.")
			logTheThing("diary", null, "[row["ckey"]]'s ban expired.", "admin")
			message_admins("<span class='internal'>Ban expired for [target]</span>")
		else
			logTheThing("admin", adminC, "unbanned [row["ckey"]]")
			logTheThing("diary", adminC, "unbanned [row["ckey"]]", "admin")
...
/client/proc/openBanPanel()
	...
	if (src.holder && !src.holder.tempmin)
		...
		logTheThing("admin", src, "tried to access the ban panel")
		logTheThing("diary", src, "tried to access the ban panel", "admin")
	return
...
/proc/forceUpdateLocalBans(latestLocalID, latestRemoteID)
	...
	if (data["error"]) //Error returned from the API welp
		logTheThing("debug", null, "<b>Bans Error</b>: Error returned in <b>forceUpdateLocalBans</b>: [data["error"]]")
		logTheThing("diary", null, "Bans Error: Error returned in forceUpdateLocalBans: [data["error"]]", "debug")
		return 0

	logTheThing("debug", null, "UPDATE LOCAL DEBUG: data: [list2params(data)]")

	//Loop through the bans we were given
	for (var/row in data)
		var/logID = row
		if (!row) break
		logTheThing("debug", null, "UPDATE LOCAL DEBUG: logID: [logID]")
		var/list/details = json_decode(row[logID])
		logTheThing("debug", null, "UPDATE LOCAL DEBUG: details: [list2params(details)]")
		...
		if (returnData["error"]) //Error returned from the local db jeeeeez aint nothing going our way
			logTheThing("debug", null, "<b>Local API Error</b> - Callback failed in <b>[type]BanApiFallback</b> with message: <b>[returnData["error"]]</b>")
			logTheThing("diary", null, "<b>Local API Error</b> - Callback failed in [type]BanApiFallback with message: [returnData["error"]]", "debug")
...
/proc/forceUpdateRemoteBans(latestLocalID, latestRemoteID)
	logTheThing("debug", null, "UPDATE REMOTE DEBUG: latestLocalID: [latestLocalID]")
	logTheThing("debug", null, "UPDATE REMOTE DEBUG: latestRemoteID: [latestRemoteID]")
	//Construct the log IDs the remote is missing
	var/sendBans[] = new()
	for (var/id = latestRemoteID + 1, id <= latestLocalID, id++)
		sendBans.Add(id)

	logTheThing("debug", null, "UPDATE REMOTE DEBUG: sendBans: [list2params(sendBans)]")
	...
	for (var/i = 1, i <= log.len, i++)
		var/row = log[i]
		logTheThing("debug", null, "UPDATE REMOTE DEBUG: row: [row]")
		var/list/rowDetails = splittext(row, ":")
		logTheThing("debug", null, "UPDATE REMOTE DEBUG: rowDetails: [list2params(rowDetails)]")
		var/logID = rowDetails[1]
		logID = text2num(logID)
		logTheThing("debug", null, "UPDATE REMOTE DEBUG: logID: [logID]")
		if (logID in sendBans)
			...
			logTheThing("debug", null, "UPDATE REMOTE DEBUG: details: [details]")
			parsedLog["[logID]"] = details

	logTheThing("debug", null, "UPDATE REMOTE DEBUG: parsedLog: [json_encode(parsedLog)]")
