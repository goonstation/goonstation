...
/datum/bansHandler
	...
	proc/add(admin_ckey, server_id, ckey, comp_id, ip, reason, duration = FALSE, requires_appeal = FALSE, added_externally = FALSE)
		...
		if (ban)
			adminMsg += " <a href='https://tomato.goonhub.com/admin/bans/[ban.id]' target='_blank'>View Ban</a>"
		adminMsg += "</span>"
		...
		if (!added_externally)
			...
			ircmsg["msg"] = reason + "\n\n\[View Ban\](<https://tomato.goonhub.com/admin/bans/[ban.id]>)"
	...
	proc/update(banId, admin_ckey, server_id, ckey, comp_id, ip, reason, duration, requires_appeal)
		...
		message_admins("<span class='internal'>[key_name(adminClient ? adminClient : admin_ckey)] edited [target]'s ban. Reason: [ban.reason] Duration: [durationHuman] [serverLogSnippet]. <a href='https://tomato.goonhub.com/admin/bans/[ban.id]' target='_blank'>View Ban</a></span>")
	...
	proc/addDetails(datum/apiModel/Tracked/BanResource/ban, evasion = FALSE, admin_ckey, ckey, comp_id, ip)
		...
		// Tell admins
		var/msg = "added ban [evasion ? "evasion" : ""] details [target] to Ban ID <a href='https://tomato.goonhub.com/admin/bans/[ban.id]' target='_blank'>[ban.id]</a>, Original Ckey: [original_ckey]"
		...
		// Tell discord
		msg = "added ban [evasion ? "evasion" : ""] details [target] to Ban ID \[[ban.id]\](<https://tomato.goonhub.com/admin/bans/[ban.id]>), Original Ckey: [original_ckey]"
		var/ircmsg[] = new()
		ircmsg["key"] = adminClient ? adminClient.key : admin_ckey
		ircmsg["name"] = adminClient && adminClient.mob && adminClient.mob.name ? stripTextMacros(adminClient.mob.name) : "N/A"
		ircmsg["msg"] = msg
