...
/datum/admins/Topic(href, href_list)
	...
	if (usr.client != src.owner)
		...
		logTheThing("admin", usr, "tried to use the admin panel without authorization.")
		logTheThing("diary", usr, "tried to use the admin panel without authorization.", "admin")
		return
	...
	switch(href_list["action"])
					...
					if("2")
						...
						switch(emergency_shuttle.direction)
							...
							if(1)
								...
								logTheThing("admin", usr, "sent the Emergency Shuttle back")
								logTheThing("diary", usr, "sent the Emergency Shuttle back", "admin")
		...
		if("edit_shuttle_time")
			if (src.level >= LEVEL_PA)
				...
				logTheThing("admin", usr, "edited the Emergency Shuttle's timeleft to [timeleft]")
				logTheThing("diary", usr, "edited the Emergency Shuttle's timeleft to [timeleft]", "admin")
		...
		if("toggle_shuttle_calling")
			if (src.level >= LEVEL_PA)
				...
				logTheThing("admin", usr, "[emergency_shuttle.disabled ? "dis" : "en"]abled calling the Emergency Shuttle")
				logTheThing("diary", usr, "[emergency_shuttle.disabled ? "dis" : "en"]abled calling the Emergency Shuttle", "admin")
		...
		if("toggle_shuttle_recalling")
			if (src.level >= LEVEL_PA)
				...
				logTheThing("admin", usr, "[emergency_shuttle.can_recall ? "en" : "dis"]abled recalling the Emergency Shuttle")
				logTheThing("diary", usr, "[emergency_shuttle.can_recall ? "en" : "dis"]abled recalling the Emergency Shuttle", "admin")
		...
		if("notes2")
			...
			switch(href_list["type"])
				if("del")
					...
					if(href_list["id"])
						if(tgui_alert(usr,"Delete This Note?","Confirmation",list("Yes","No")) != "Yes")
							...
							logTheThing("admin", usr, "deleted note [noteId] belonging to [player].")
							logTheThing("diary", usr, "deleted note [noteId] belonging to [player].", "admin")
				...
				if("add")
					...
					logTheThing("admin", usr, "added a note for [player]: [the_note]")
					logTheThing("diary", usr, "added a note for [player]: [the_note]", "admin")
		...
		if("viewcompids")
			...
			if(src.tempmin)
				logTheThing("admin", usr, "tried to access the compIDs of [constructTarget(player,"admin")]")
				logTheThing("diary", usr, "tried to access the compIDs of [constructTarget(player,"diary")]", "admin")
				...
				if (jobban_isbanned(M, job))
					...
					if(job in list("Captain","Head of Personnel","Head of Security","Chief Engineer","Research Director"))
						if(player.cached_jobbans.Find("Heads of Staff"))
							...
							return
					logTheThing("admin", usr, "unbanned [constructTarget(M,"admin")] from [job]")
					logTheThing("diary", usr, "unbanned [constructTarget(M,"diary")] from [job]", "admin")
					...
					logTheThing("admin", usr, "banned [constructTarget(M,"admin")] from [job]")
					logTheThing("diary", usr, "banned [constructTarget(M,"diary")] from [job]", "admin")
				...
				if (jobban_isbanned(cache, job))
					...
					if(job in list("Captain","Head of Personnel","Head of Security","Chief Engineer","Research Director"))
						if(cache.Find("Heads of Staff"))
							tgui_alert(usr,"This person is banned from Heads of Staff. You must lift that ban first.")
							return
					logTheThing("admin", usr, "unbanned [M](Offline) from [job]")
					logTheThing("diary", usr, "unbanned [M](Offline) from [job]", "admin")
					...
					logTheThing("admin", usr, "banned [M](Offline) from [job]")
					logTheThing("diary", usr, "banned [M](Offline) from [job]", "admin")
			...
			if (src.level >= LEVEL_MOD)
				...
				if (ismob(M) && M.client)
					...
					if (M.client.ismuted())
						...
						M.client.mute(-1)
						muted = 1
					logTheThing("admin", usr, "has [(muted ? "permanently muted" : "unmuted")] [constructTarget(M,"admin")]")
					logTheThing("diary", usr, "has [(muted ? "permanently muted" : "unmuted")] [constructTarget(M,"diary")].", "admin")
			...
			if (src.level >= LEVEL_MOD)
				...
				if (ismob(M))
					...
					if (M.client.ismuted())
						...
						M.client.mute(60)
						muted = 1
					logTheThing("admin", usr, "has [(muted ? "temporarily muted" : "unmuted")] [constructTarget(M,"admin")]")
					logTheThing("diary", usr, "has [(muted ? "temporarily muted" : "unmuted")] [constructTarget(M,"diary")].", "admin")
			...
			if (src.level >= LEVEL_MOD)
				...
				if (ismob(M) && M.client)
					...
					if (!oocban_isbanned(M))
						...
						oocban_unban(M)
					logTheThing("admin", usr, "has [(oocbanned ? "OOC Banned" : "OOC Unbanned")] [constructTarget(M,"admin")]")
					logTheThing("diary", usr, "has [(oocbanned ? "OOC Banned" : "OOC Unbanned")] [constructTarget(M,"diary")].", "admin")
		...
		if ("c_mode_current")
			if (src.level >= LEVEL_SA)
				...
				if (requestedMode in global.valid_modes)
					logTheThing("admin", usr, "set the mode as [requestedMode].")
					logTheThing("diary", usr, "set the mode as [requestedMode].", "admin")
		...
		if ("c_mode_next")
			if (src.level >= LEVEL_SA)
				...
				logTheThing("admin", usr, "set the next round's mode as [newmode].")
				logTheThing("diary", usr, "set the next round's mode as [newmode].", "admin")
		...
		if ("monkeyone")
			if (src.level >= LEVEL_SA)
				...
				if(ishuman(M))
					...
					logTheThing("admin", usr, "attempting to monkeyize [constructTarget(M,"admin")]")
					logTheThing("diary", usr, "attempting to monkeyize [constructTarget(M,"diary")]", "admin")
		...
		if ("forcespeech")
			...
			if (src.level >= LEVEL_PA || isnull(M.client) && src.level >= LEVEL_SA)
				if (ismob(M))
					...
					logTheThing("admin", usr, "forced [constructTarget(M,"admin")] to say: [speech]")
					logTheThing("diary", usr, "forced [constructTarget(M,"diary")] to say: [speech]", "admin")
					if(M.client)
		...
		if ("tdome")
			if (src.level >= LEVEL_SA)
				...
				logTheThing("admin", usr, "sent [constructTarget(M,"admin")] to the thunderdome. ([team])")
				logTheThing("diary", usr, "sent [constructTarget(M,"diary")] to the thunderdome. ([team])", "admin")
			...
			if (src.level >= LEVEL_SA)
				...
				if (ismob(M))
					...
					if(config.allow_admin_rev)
						...
						logTheThing("admin", usr, "healed / revived [constructTarget(M,"admin")]")
						logTheThing("diary", usr, "healed / revived [constructTarget(M,"diary")]", "admin")
					else
		...
		if ("kill")
			if (src.level >= LEVEL_SA)
				...
				if(M)
					...
					logTheThing("admin", usr, "killed [constructTarget(M,"admin")]")
					logTheThing("diary", usr, "killed [constructTarget(M,"diary")]", "admin")
				return
		...
		if ("addreagent")
			if(( src.level >= LEVEL_PA ) || ((src.level >= LEVEL_SA) ))
				...
				logTheThing("admin", usr, "added [amount] units of [reagent.id] to [M] at [log_loc(M)].")
				logTheThing("diary", usr, "added [amount] units of [reagent.id] to [M] at [log_loc(M)].", "admin")
		...
		if ("addabil")
			if (src.level >= LEVEL_PA)
				...
				logTheThing("admin", usr, "added ability [ab_to_add] to [constructTarget(M,"admin")].")
		...
		if ("removeabil")
			if (src.level >= LEVEL_PA)
				...
				logTheThing("admin", usr, "removed ability [ab_to_rem] from [constructTarget(M,"admin")].")
		...
		if ("abilholder")
			if (src.level >= LEVEL_PA)
				...
				logTheThing("admin", usr, "created abilityHolder [ab_to_add] for [constructTarget(M,"admin")].")
			else
				tgui_alert(usr,"You must be at least a Primary Administrator to do this!")
		...
		if ("manageabils_remove")
			if (src.level >= LEVEL_PA)
				...
				logTheThing("admin", usr, "removed ability [A] from [constructTarget(M,"admin")].")
		...
		if ("managetraits_remove")
			if (src.level >= LEVEL_PA)
				...
				logTheThing("admin", usr, "removed trait [trait.name] from [constructTarget(M,"admin")].")
		...
		if ("addtrait")
			if (src.level >= LEVEL_PA)
				...
				logTheThing("admin", usr, "added the trait [trait_to_add_name] to [constructTarget(M,"admin")].")
			...
			if (src.level >= LEVEL_PA)
				...
				for(var/trait in M.traitHolder.traits)
					...
					if(trait_obj.name == trait_to_remove_name)
						...
						logTheThing("admin", usr, "removed the trait [trait_to_remove_name] from [constructTarget(M,"admin")].")
						break
			else
				tgui_alert(usr,"You must be at least a Primary Administrator to do this!")
		...
		if ("chgadlvl")
			if (src.level >= LEVEL_PA)
				...
				if (rank == "Remove")
					...
					logTheThing("admin", usr, "has removed [constructTarget(C,"admin")]'s adminship")
					logTheThing("diary", usr, "has removed [C]'s adminship", "admin")
					...
					logTheThing("admin", usr, "has made [constructTarget(C,"admin")] a [rank]")
					logTheThing("diary", usr, "has made [C] a [rank]", "admin")
		...
		if ("object_list")
			if (src.level >= LEVEL_SA)
				if (config.allow_admin_spawning && (src.state == 2 || src.level >= LEVEL_SA))
					...
					if (number == 1)
						logTheThing("admin", usr, "created a [english_list(paths)]")
						logTheThing("diary", usr, "created a [english_list(paths)]", "admin")
						for(var/path in paths)
							if(ispath(path, /mob))
						...
						logTheThing("admin", usr, "created [number]ea [english_list(paths)]")
						logTheThing("diary", usr, "created [number]ea [english_list(paths)]", "admin")
						for(var/path in paths)
							if(ispath(path, /mob))
		...
		if("giveantagtoken") //Gives player a token they can redeem to guarantee an antagonist role
			if (src.level >= LEVEL_SA)
				...
				if (tokens <= 0)
					logTheThing("admin", usr, "Removed all antag tokens from [constructTarget(M,"admin")]")
					logTheThing("diary", usr, "Removed all antag tokens from [constructTarget(M,"diary")]", "admin")
					message_admins("<span class='internal'>[key_name(usr)] removed all antag tokens from [key_name(M)]</span>")
				else
					logTheThing("admin", usr, "Set [constructTarget(M,"admin")]'s Antag tokens  to [tokens].")
					logTheThing("diary", usr, "Set [constructTarget(M,"diary")]'s Antag tokens  to [tokens].")
		...
		if("setspacebux")
			if (src.level >= LEVEL_SA)
				...
				logTheThing("admin", usr, "Set [constructTarget(M,"admin")]'s Persistent Bank (Spacebux) to [spacebux].")
				logTheThing("diary", usr, "Set [constructTarget(M,"diary")]'s Persistent Bank (Spacebux) to [spacebux].")
		...
		if ("grantcontributor")
			if (src.level >= LEVEL_CODER)
				...
				logTheThing("admin", usr, "gave [constructTarget(M,"admin")] contributor status.")
				logTheThing("diary", usr, "gave [constructTarget(M,"diary")] contributor status.")
		...
		if ("revokecontributor")
			if (src.level >= LEVEL_CODER)
				...
				if(!suc)
					...
					logTheThing("admin", usr, "revoked [constructTarget(M,"admin")]'s contributor status.")
					logTheThing("diary", usr, "revoked [constructTarget(M,"diary")]'s contributor status.")
		...
		if ("grantclown")
			if (src.level >= LEVEL_SA)
				...
				logTheThing("admin", usr, "gave [constructTarget(M,"admin")] their clown college diploma.")
				logTheThing("diary", usr, "gave [constructTarget(M,"diary")] their clown college diploma.")
		...
		if ("revokeclown")
			if (src.level >= LEVEL_SA)
				...
				if(!suc)
					...
					logTheThing("admin", usr, "revoked [constructTarget(M,"admin")]'s clown college diploma.")
					logTheThing("diary", usr, "revoked [constructTarget(M,"diary")]'s clown college diploma.")
			...
			if (src.level >= LEVEL_SA)
				switch(href_list["type"])
					...
					if("transform_one")
						...
						switch(which)
							...
							if ("Cow")
								H.set_mutantrace(/datum/mutantrace/cow)
						message_admins("<span class='internal'>[key_name(usr)] transformed [H.real_name] into a [which].</span>")
						logTheThing("admin", usr, "transformed [H.real_name] into a [which].")
						logTheThing("diary", usr, "transformed [H.real_name] into a [which].", "admin")
					...
					if("transform_all")
						...
						logTheThing("admin", usr, "transformed everyone into a [which].")
						logTheThing("diary", usr, "transformed everyone into a [which].", "admin")
					if("prisonwarp")
						if(!ticker)
							tgui_alert(usr,"The game hasn't started yet!")
							return
						message_admins("<span class='internal'>[key_name(usr)] teleported all players to the prison zone.</span>")
						logTheThing("admin", usr, "teleported all players to the prison zone.")
						logTheThing("diary", usr, "teleported all players to the prison zone.", "admin")
						for(var/mob/living/carbon/human/H in mobs)
					...
					if("critterize_all")
						if (src.level >= LEVEL_PA)
							...
							logTheThing("admin", usr, "critterized everyone into [CT]")
							logTheThing("diary", usr, "critterized everyone into a critter [CT]", "admin")
						else
					...
					if("traitor_all")
						if (src.level >= LEVEL_SA)
							...
							logTheThing("admin", usr, "made everyone a[hardmode ? " hard-mode" : ""] [which_traitor]. Objective is [custom_objective]")
							logTheThing("diary", usr, "made everyone a[hardmode ? " hard-mode" : ""] [which_traitor]. Objective is [custom_objective]", "admin")
						else
							tgui_alert(usr,"You're not of a high enough rank to do this")
					if("flicklights")
						while(!usr.stat)
					...
					if("stupify")
						if (src.level >= LEVEL_ADMIN)
							...
							logTheThing("admin", usr, "gave everybody severe brain damage.")
							logTheThing("diary", usr, "gave everybody severe brain damage.", "admin")
						else
							tgui_alert(usr,"You cannot perform this action. You must be of a higher administrative rank!")
							return
					// FUN SECRETS CODE
					if ("randomguns")
						if (src.level >= LEVEL_PA)
							...
							logTheThing("admin", usr, "gave everyone a random firearm.")
							logTheThing("diary", usr, "gave everyone a random firearm.", "admin")
						else
					...
					if	("swaprooms")
						if (src.level >= LEVEL_PA)
							...
							logTheThing("admin", usr, "swapped the stations rooms.")
							logTheThing("diary", usr, "swapped the stations rooms.", "admin")
						else
							tgui_alert(usr,"You must be at least a Primary Administrator")
							return
					...
					if	("timewarp")
						if (src.level >= LEVEL_PA)
							...
							logTheThing("admin", usr, "triggered a time warp.")
							logTheThing("diary", usr, "triggered a time warp.", "admin")
						else
							tgui_alert(usr,"You must be at least a Primary Administrator")
							return
					...
					if ("add_bioeffect_one","remove_bioeffect_one")
						if (src.level >= LEVEL_PA)
							...
							if (length(picklist))
								...
								logTheThing("admin", usr, "[adding ? "added" : "removed"] the [string_version] bio-effect[picklist.len > 1 ? "s" : ""] [adding ? "to" : "from"] [key_name(M)].")
								logTheThing("diary", usr, "[adding ? "added" : "removed"] the [string_version] bio-effect[picklist.len > 1 ? "s" : ""] [adding ? "to" : "from"] [key_name(M)].", "admin")
						else
					...
					if ("add_ability_one","remove_ability_one")
						if (src.level >= LEVEL_PA)
							...
							logTheThing("admin", usr, "[adding ? "added" : "removed"] the [ab_to_do] ability [adding ? "to" : "from"] [key_name(M)].")
							logTheThing("diary", usr, "[adding ? "added" : "removed"] the [ab_to_do] ability [adding ? "to" : "from"] [key_name(M)].", "admin")
						else
					...
					if ("add_reagent_one","remove_reagent_one")
						if (src.level >= LEVEL_PA)
							...
							if (length(picklist))
								...
								logTheThing("admin", usr, "[adding ? "added" : "removed"] [string_version] [adding ? "to" : "from"] [key_name(M)].")
								logTheThing("diary", usr, "[adding ? "added" : "removed"] [string_version] [adding ? "to" : "from"] [key_name(M)].", "admin")
						else
					...
					if ("add_bioeffect_all","remove_bioeffect_all")
						if (src.level >= LEVEL_PA)
							...
							if (length(picklist))
								...
								logTheThing("admin", usr, "[adding ? "added" : "removed"] the [string_version] bio-effect[picklist.len > 1 ? "s" : ""] [adding ? "to" : "from"] everyone.")
								logTheThing("diary", usr, "[adding ? "added" : "removed"] the [string_version] bio-effect[picklist.len > 1 ? "s" : ""] [adding ? "to" : "from"] everyone.", "admin")
						else
							tgui_alert(usr,"You must be at least a Primary Administrator to bioeffect players.")
							return
					...
					if ("add_ability_all","remove_ability_all")
						if (src.level >= LEVEL_PA)
							...
							logTheThing("admin", usr, "[adding ? "added" : "removed"] the [ab_to_do] ability [adding ? "to" : "from"] everyone.")
							logTheThing("diary", usr, "[adding ? "added" : "removed"] the [ab_to_do] ability [adding ? "to" : "from"] everyone.", "admin")
						else
							tgui_alert(usr,"You must be at least a Primary Administrator to change player abilities.")
							return
					...
					if ("add_reagent_all","remove_reagent_all")
						if (src.level >= LEVEL_PA)
							...
							if (length(picklist))
								...
								logTheThing("admin", usr, "[adding ? "added" : "removed"] [string_version] [adding ? "to" : "from"] everyone.")
								logTheThing("diary", usr, "[adding ? "added" : "removed"] [string_version] [adding ? "to" : "from"] everyone.", "admin")
					...
					if ("ballpit")
						if (src.level >= LEVEL_SA)
							...
							logTheThing("admin", usr, "replaced z1 pools with ballpits.")
							logTheThing("diary", usr, "replaced z1 pools with ballpits.", "admin")
						else
							tgui_alert(usr,"You cannot perform this action. You must be of a higher administrative rank!")
							return
					...
					if ("woodstation")
						if (src.level >= LEVEL_PA)
							...
							logTheThing("admin", usr, "replaced z1 floors and walls with wooden doors.")
							logTheThing("diary", usr, "replaced z1 floors and walls with wooden doors.", "admin")
						else
							tgui_alert(usr,"You cannot perform this action. You must be of a higher administrative rank!")
							return
					...
					if ("yeolde")
						if (src.level >= LEVEL_PA)
							...
							logTheThing("admin", usr, "replaced z1 airlocks with wooden doors.")
							logTheThing("diary", usr, "replaced z1 airlocks with wooden doors.", "admin")
						else
							tgui_alert(usr,"You cannot perform this action. You must be of a higher administrative rank!")
							return
					...
					if("fakeguns")
						if (src.level >= LEVEL_ADMIN)
							...
							logTheThing("admin", usr, "used Fake Gun secret.")
							logTheThing("diary", usr, "used Fake Gun secret.", "admin")
						else
							tgui_alert(usr,"You cannot perform this action. You must be of a higher administrative rank!")
							return
					...
					if("flipstation")
						...
						if (src.level >= LEVEL_ADMIN)
							...
							logTheThing("admin", src, "set station direction to [direction].")
							logTheThing("diary", src, "set station direction to [direction]", "admin")
						else
							tgui_alert(usr,"You cannot perform this action. You must be of a higher administrative rank!")
							return
					...
					if("raiseundead")
						if (src.level >= LEVEL_ADMIN)
							...
							logTheThing("admin", usr, "brought back all dead humans as zombies.")
							logTheThing("diary", usr, "brought back all dead humans as zombies", "admin")
						else
							tgui_alert(usr,"You cannot perform this action. You must be of a higher administrative rank!")
							return
					...
					if("forcerandomnames")
						if (src.level >= LEVEL_PA)
							...
							logTheThing("admin", usr, "set all players to use random names.")
							logTheThing("diary", usr, "set all players to use random names.", "admin")
						else
							tgui_alert(usr,"You cannot perform this action. You must be of a higher administrative rank!")
							return
					...
					if("forcerandomlooks")
						if (src.level >= LEVEL_PA)
							...
							logTheThing("admin", usr, "set all players to use random appearances.")
							logTheThing("diary", usr, "set all players to use random appearances.", "admin")
						else
							tgui_alert(usr,"You cannot perform this action. You must be of a higher administrative rank!")
							return
					...
					if("sawarms")
						if (src.level >= LEVEL_ADMIN)
							...
							logTheThing("admin", usr, "used the Saw Arms secret.")
							logTheThing("diary", usr, "used the Saw Arms secret.", "admin")
					...
					if("emag_all_things")
						if (src.level >= LEVEL_ADMIN)
							if (tgui_alert(usr,"Do you really want to emag everything?","Bad Idea", list("Yes", "No")) == "Yes")
								...
								logTheThing("admin", usr, "used the Emag Everything secret.")
								logTheThing("diary", usr, "used the Emag Everything secret.", "admin")
								//DO IT!
					...
					if("shakecamera")
						if (src.level >= LEVEL_ADMIN)
							...
							logTheThing("admin", src, "created a shake effect (intensity [intensity], length [time])")
							logTheThing("diary", src, "created a shake effect (intensity [intensity], length [time])", "admin")
					...
					if("creepifystation")
						if (src.level >= LEVEL_ADMIN)
							if (tgui_alert(usr,"Are you sure you should creepify the station? There's no going back.", "PARENTAL CONTROL", list("Yes", "No")) == "Yes")
								...
								logTheThing("admin", usr, "used the Creepify Station button")
								logTheThing("diary", usr, "used the Creepify Station button", "admin")
								creepify_station()
						else
					...
					if ("command_report_zalgo")
						if (src.level >= LEVEL_ADMIN)
							...
							if (alert(src, "Origin: [input3 ? "\"[input3]\"" : "None"]\nHeadline: [input2 ? "\"[input2]\"" : "None"]\nBody: \"[input]\"", "Confirmation", "Send Report", "Cancel") == "Send Report")
								...
								logTheThing("admin", usr, "has created a command report (zalgo): [input]")
								logTheThing("diary", usr, "has created a command report (zalgo): [input]", "admin")
					...
					if ("command_report_void")
						if (src.level >= LEVEL_ADMIN)
							...
							if (alert(src, "Origin: [input3 ? "\"[input3]\"" : "None"]\nHeadline: [input2 ? "\"[input2]\"" : "None"]\nBody: \"[input]\"", "Confirmation", "Send Report", "Cancel") == "Send Report")
								...
								logTheThing("admin", usr, "has created a command report (void): [input]")
								logTheThing("diary", usr, "has created a command report (void): [input]", "admin")
						...
						if(src.level >= LEVEL_ADMIN)
							if (noir)
								if (tgui_alert(usr,"Had enough of noir?", "Good decisions", list("Yes", "No")) == "Yes")
									...
									logTheThing("admin", usr, "used the Noir secret to remove noir")
									logTheThing("diary", usr, "used the Noir secret to remove noir", "admin")
							if (tgui_alert(usr,"Are you sure you should noir?", "PARENTAL CONTROL", list("Yes", "No")) == "Yes")
								...
								logTheThing("admin", usr, "used the Noir secret")
								logTheThing("diary", usr, "used the Noir secret", "admin")
						...
						if(src.level >= LEVEL_ADMIN) //Will be SG when tested
							if (tgui_alert(usr,"Do you really wanna do the great switcharoo?", "Awoo, awoo", list("Yes", "No")) == "Yes")
								...
								if(people_to_swap.len > 1) //Jenny Antonsson switches bodies with herself! #wow #whoa
									...
									logTheThing("admin", usr, "used The Great Switcharoo secret")
									logTheThing("diary", usr, "used The Great Switcharoo secret", "admin")
					...
					if("fartyparty")
						...
						logTheThing("admin", usr, "used Farty Party secret")
						logTheThing("diary", usr, "used Farty Party secret", "admin")

					else
				if (usr) logTheThing("admin", usr, "used secret [href_list["secretsfun"]]")
				logTheThing("diary", usr, "used secret [href_list["secretsfun"]]", "admin")
			else
			...
			if (src.level >= LEVEL_MOD)
				...
				switch(href_list["type"])
					...
					if("unelectrify_all")
						...
						logTheThing("admin", usr, "de-electrified all airlocks.")
						logTheThing("diary", usr, "de-electrified all airlocks.", "admin")
					if("DNA")
...
#endif
				if (usr)
					logTheThing("admin", usr, "used secret [href_list["secretsadmin"]]")
					logTheThing("diary", usr, "used secret [href_list["secretsadmin"]]", "admin")
					if (ok)
			...
			if (src.level >= LEVEL_PA)
				switch(href_list["type"])
					if("spawn_syndies")
						...
						logTheThing("admin", src, "has spawned [amount] syndicate operatives.")
						logTheThing("diary", src, "has spawned [amount] syndicate operatives.", "admin")
					...
					if("spawn_normal")
						...
						logTheThing("admin", src, "has spawned [amount] normal players.")
						logTheThing("diary", src, "has spawned [amount] normal players.", "admin")
					...
					if("spawn_player") //includes antag players
						...
						logTheThing("admin", src, "has spawned [amount] players.")
						logTheThing("diary", src, "has spawned [amount] players.", "admin")
					...
					if("spawn_player_job") //includes antag players
						...
						logTheThing("admin", src, "has spawned [amount] players, and kept any antag statuses.")
						logTheThing("diary", src, "has spawned [amount] players, and kept any antag statuses.", "admin")
					...
					if("spawn_player_job_strip_antag") //includes antag players but strips antag status
						...
						logTheThing("admin", src, "has spawned [amount] players, and stripped any antag statuses.")
						logTheThing("diary", src, "has spawned [amount] players, and stripped any antag statuses.", "admin")
	...
	/*				if("spawn_commandos")
					...
					if("spawn_turds")
						...
						logTheThing("admin", src, "has spawned [amount] TURDS.")
						logTheThing("diary", src, "has spawned [amount] TURDS.", "admin")

					if("spawn_smilingman")
						var/datum/special_respawn/SR = new /datum/special_respawn/
						SR.spawn_smilingman(1)
						logTheThing("admin", src, "has spawned a Smiling Man.")
						logTheThing("diary", src, "has spawned a Smiling Man.", "admin")
	*/
					...
					if("spawn_custom")
						...
						logTheThing("admin", src, "has spawned [amount] mobs of type [blType].")
						logTheThing("diary", src, "has spawned [amount] mobs of type [blType].", "admin")
...
/datum/admins/proc/restart()
	...
	if(confirm == "Yes")
		...
		logTheThing("admin", usr, "initiated a reboot.")
		logTheThing("diary", usr, "initiated a reboot.", "admin")
...
/datum/admins/proc/announce()
	...
	if (message)
		...
		logTheThing("admin", usr, ": [message]")
		logTheThing("diary", usr, ": [message]", "admin")
...
/datum/admins/proc/startnow()
	...
	if(current_state <= GAME_STATE_PREGAME)
		...
		logTheThing("admin", usr, "has started the game.")
		logTheThing("diary", usr, "has started the game.", "admin")
...
/datum/admins/proc/delay_start()
	...
	if (game_start_delayed)
		...
		logTheThing("admin", usr, "delayed the game start.")
		logTheThing("diary", usr, "delayed the game start.", "admin")
		message_admins("<span class='internal'>[usr.key] has delayed the game start.</span>")
	else
		boutput(world, "<b>The game will start soon.</b>")
		logTheThing("admin", usr, "removed the game start delay.")
		logTheThing("diary", usr, "removed the game start delay.", "admin")
...
/datum/admins/proc/delay_end()
	...
	if (game_end_delayed == 2)
		logTheThing("admin", usr, "removed the restart delay and triggered an immediate restart.")
		logTheThing("diary", usr, "removed the restart delay and triggered an immediate restart.", "admin")
		...
		logTheThing("admin", usr, "delayed the server restart.")
		logTheThing("diary", usr, "delayed the server restart.", "admin")
		...
		logTheThing("admin", usr, "removed the restart delay.")
		logTheThing("diary", usr, "removed the restart delay.", "admin")
...
/datum/admins/proc/evilize(mob/M as mob, var/traitor_type, var/special = null, var/mass_traitor_obj = null, var/mass_traitor_esc = null)
	...
	if(!mass_traitor_obj)
		logTheThing("admin", usr, "made [constructTarget(M,"admin")] a[special ? " [special]" : ""] [traitor_type].")
		logTheThing("diary", usr, "made [constructTarget(M,"diary")] a[special ? " [special]" : ""] [traitor_type].", "admin")
	...
	if (client.holder.level >= LEVEL_PA)
		...
		if (chosen)
			if (ispath(chosen, /turf))
				...
				if (client.flourish)
					spawn_animation1(A)
			logTheThing("admin", usr, "spawned [chosen] at ([log_loc(usr)])")
			logTheThing("diary", usr, "spawned [chosen] at ([showCoords(usr.x, usr.y, usr.z, 1)])", "admin")
...
/datum/admins/proc/spawn_figurine(var/figurine as text)
	...
	if (client.holder.level >= LEVEL_PA)
		...
		if (chosen)
			...
			if (client.flourish)
				spawn_animation1(A)
			logTheThing("admin", usr, "spawned figurine [chosen] at ([log_loc(usr)])")
			logTheThing("diary", usr, "spawned figurine [chosen] at ([showCoords(usr.x, usr.y, usr.z, 1)])", "admin")
...
/datum/admins/proc/heavenly_spawn_obj(var/obj/object as text)
	...
	if (usr.client.holder.level >= LEVEL_PA)
		...
		if (chosen)
			...
			A.set_loc(T)
			heavenly_spawn(A)
			logTheThing("admin", usr, "spawned [chosen] at ([log_loc(T)])")
			logTheThing("diary", usr, "spawned [chosen] at ([showCoords(T.x, T.y, T.z, 1)])", "admin")
...
/datum/admins/proc/show_chatbans(var/client/C)//do not use this as an example of how to write DM please.
	...
	if(C.cloud_get( "adminhelp_banner" ))
		...
		logTheThing("admin", src, "unmuted [constructTarget(C,"admin")] from adminhelping.")
	else
		built += "<a href='?src=\ref[src];target=\ref[C];action=ah_mute'>Adminhelp Mute</a><br/>"
		logTheThing("admin", src, "muted [constructTarget(C,"admin")] from adminhelping.")
	...
	if(C.cloud_get( "mentorhelp_banner" ))
		...
		logTheThing("admin", src, "unmuted [constructTarget(C,"admin")] from mentorhelping.")
	else
		built += "<a href='?src=\ref[src];target=\ref[C];action=mh_mute'>Mentorhelp Mute</a><br/>"
		logTheThing("admin", src, "muted [constructTarget(C,"admin")] from mentorhelping.")
	...
	if(C.cloud_get( "prayer_banner" ))
		...
		logTheThing("admin", src, "unmuted [constructTarget(C,"admin")] from praying.")
	else
		built += "<a href='?src=\ref[src];target=\ref[C];action=pr_mute'>Prayer Mute</a><br/>"
		logTheThing("admin", src, "muted [constructTarget(C,"admin")] from praying.")
...
/client/proc/respawn_target(mob/M as mob in world, var/forced = 0)
	...
	logTheThing("admin", src, "respawned [constructTarget(M,"admin")]")
	logTheThing("diary", src, "respawned [constructTarget(M,"diary")].", "admin")
...
/client/proc/respawn_self()
	...
	logTheThing("admin", src, "respawned themselves.")
	logTheThing("diary", src, "respawned themselves.", "admin")
