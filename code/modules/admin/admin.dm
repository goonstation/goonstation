			...
			if (src.level >= LEVEL_MOD)
				...
				switch(href_list["type"])
					...
					if("manifest")
						...
						for(var/mob/living/carbon/human/H in mobs)
							if(H.ckey)
								dat += "<tr><td>[H.name]</td><td>[(H.mind ? H.mind.assigned_role : "Unknown Position")]</td><td>[(istype(H.wear_id, /obj/item/card/id) || (istype(H.wear_id, /obj/item/device/pda2)) || istype(H.wear_id, /obj/item/clothing/lanyard)) ? "[H.wear_id:assignment]" : "Unknown Position"]</td></tr>"
							LAGCHECK(LAG_LOW)
						dat += "</table>"
