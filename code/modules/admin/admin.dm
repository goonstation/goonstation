			...
			if(ishuman(M) || isAI(M) || isrobot(M) || ismobcritter(M))
				...
				if(antagonize == "Yes")
					if (issilicon(M))
						...
						var/list/traitor_types = list("Traitor", "Wizard", "Changeling", "Vampire", "Werewolf", "Hunter", "Wrestler", "Grinch", "Omnitraitor", "Spy_Thief", "Energy_Vampire")
				...
				switch(href_list["type"])
					...
					if("traitor_all")
						if (src.level >= LEVEL_SA)
							...
							var/which_traitor = input("What kind of traitor?","Everyone's a Traitor") as null|anything in list("Traitor","Wizard","Changeling","Werewolf","Vampire","Energy_Vampire","Hunter","Wrestler","Grinch","Omnitraitor")
							if(!which_traitor)
								return
							var/hardmode = null
