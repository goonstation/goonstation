...
/proc/message_admins(var/text, var/asay = 0, var/irc = 0)
	var/rendered = "<span class='adminLog'><span class='prefix'>[irc ? "DISCORD" : "ADMIN <wbr>LOG"]</span>: <span class='message'>[text]</span></span>"
...
/proc/message_coders(var/text) //Shamelessly adapted from message_admins
	var/rendered = "<span class='coderLog'><span class='prefix coderPrefix'>CODER <wbr>LOG</span>: <span class='message'>[text]</span></span>"
	for (var/client/C)
...
/proc/message_coders_vardbg(var/text, var/datum/d)
	...
	for (var/client/C)
		if (C.mob && C.holder && rank_to_level(C.holder.rank) >= LEVEL_CODER)
			...
			rendered = "<span class='coderLog'><span class='prefix coderPrefix'>CODER <wbr>LOG</span>: <span class='message'>[text]</span>[dbg_html]</span>"
			boutput(C.mob, replacetext(rendered, "%admin_ref%", "\ref[C.holder]"))

/proc/message_attack(var/text) //Sends a message to folks when an attack goes down
	var/rendered = "<span class='attackLog'><span class='prefix attackPrefix'>ATTACK <wbr>LOG</span>: <span class='message'>[text]</span></span>"
	for (var/client/C)
