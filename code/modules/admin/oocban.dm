...
/proc/oocban_loadbanfile()
	...
	logTheThing("admin", null, "Loading oocban_rank")
	logTheThing("diary", null, "Loading oocban_rank", "admin")
	S["runonce"] >> oocban_runonce
	if (!length(oocban_keylist))
		oocban_keylist=list()
		logTheThing("admin", null, "oocban_keylist was empty")
		logTheThing("diary", null, "oocban_keylist was empty", "admin")
...
/proc/oocban_updatelegacybans()
	set background = 1
	if(!oocban_runonce)
		logTheThing("admin", null, "Updating jobbanfile!")
		logTheThing("diary", null, "Updating jobbanfile!", "admin")
