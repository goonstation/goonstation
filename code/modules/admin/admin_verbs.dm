...
/client/proc/cmd_add_to_screen(var/atom/A as obj|mob|turf in world)
	...
	if (chosen == "everyone")
		...
		logTheThing("admin", usr, "added [A] to everyone's screen.")
	else
		var/client/C = who[chosen]
		C.screen += A
		boutput(usr, "<span class='notice'>Successful.</span>")
		logTheThing("admin", usr, "added [A] to [constructTarget(C.mob,"admin")]'s screen.")
*/
...
/datum/admins/proc/set_stealth_mode(var/new_key = null, var/force_on = 0)
	...
	logTheThing("admin", src.owner, "has turned stealth mode [src.owner:stealth ? "ON using key \"[src.owner:fakekey]\"" : "OFF"]")
	logTheThing("diary", src.owner, "has turned stealth mode [src.owner:stealth ? "ON using key \"[src.owner:fakekey]\"" : "OFF"]", "admin")
...
/datum/admins/proc/set_alt_key(var/new_key = null, var/force_on = 0)
	...
	logTheThing("admin", src.owner, "has changed their displayed key to [src.owner:alt_key ? "\"[src.owner:fakekey]\"" : "\"[src.owner:key]\""]")
	logTheThing("diary", src.owner, "has changed their displayed key to [src.owner:alt_key ? "\"[src.owner:fakekey]\"" : "\"[src.owner:key]\""]", "admin")
...
/client/proc/banooc()
	...
	if (!oocban_isbanned(target))
		...
		logTheThing("admin", usr, "Banned [constructTarget(target,"admin")] from OOC")
		logTheThing("diary", usr, "Banned [constructTarget(target,"diary")] from OOC", "admin")
	else
		oocban_unban(selection)
		message_admins("[key_name(src)] has unbanned [key_name(target)] from OOC")
		logTheThing("admin", usr, "Unbanned [constructTarget(target,"admin")] from OOC")
		logTheThing("diary", usr, "Unbanned [constructTarget(target,"diary")] from OOC", "admin")
...
/client/proc/show_image_to_all()
	...
	if (I)
		...
		logTheThing("admin", src, "has uploaded icon [I] to all players")
		logTheThing("diary", src, "has uploaded icon [I] to all players", "admin")
...
/client/proc/show_rules_to_player(mob/M as mob in world)
	...
	if(!M.client)
		...
		return
	logTheThing("admin", src, "forced [constructTarget(M,"admin")] to view the rules")
	logTheThing("diary", src, "forced [constructTarget(M,"diary")] to view the rules", "admin")
...
/client/proc/cmd_admin_humanize(var/mob/M in world)
	...
	logTheThing("admin", src, "has made [constructTarget(M,"admin")] a human.")
	logTheThing("diary", src, "has made [constructTarget(M,"diary")] a human.", "admin")
...
/client/proc/cmd_admin_pop_off_all_the_limbs_oh_god()
	...
	if(alert("Really pop off everyone's limbs?", "JESUS CHRIST", "Yes, I'm a crazy bastard", "No") == "Yes, I'm a crazy bastard")
		logTheThing("admin", src, "popped off all limbs.")
		logTheThing("diary", src, "has popped off all limbs", "admin")
...
/client/proc/blobsay(msg as text)
	...
	logTheThing("admin", src, "BLOBSAY: [msg]")
	logTheThing("diary", src, "BLOBSAY: [msg]", "admin")
...
/client/proc/hivesay(msg as text)
	...
	if (src.holder.level < LEVEL_ADMIN)
		msg = copytext(sanitize(html_encode(msg)), 1, MAX_MESSAGE_LEN)
	logTheThing("admin", src, "HIVESAY: [msg]")
	logTheThing("diary", src, "HIVESAY: [msg]", "admin")
...
/client/proc/silisay(msg as text)
	...
	if (src.holder.level < LEVEL_ADMIN)
		msg = copytext(sanitize(html_encode(msg)), 1, MAX_MESSAGE_LEN)
	logTheThing("admin", src, "SILISAY: [msg]")
	logTheThing("diary", src, "SILISAY: [msg]", "admin")
...
/client/proc/dronesay(msg as text)
	...
	if (src.holder.level < LEVEL_ADMIN)
		msg = copytext(sanitize(html_encode(msg)), 1, MAX_MESSAGE_LEN)
	logTheThing("admin", src, "DRONESAY: [msg]")
	logTheThing("diary", src, "DRONESAY: [msg]", "admin")
...
/client/proc/marsay(msg as text)
	...
	if (src.holder.level < LEVEL_ADMIN)
		msg = copytext(sanitize(html_encode(msg)), 1, MAX_MESSAGE_LEN)
	logTheThing("admin", src, "MARSAY: [msg]")
	logTheThing("diary", src, "MARSAY: [msg]", "admin")
...
/client/proc/flocksay(msg as text)
	...
	if (src.holder.level < LEVEL_ADMIN)
		msg = copytext(sanitize(html_encode(msg)), 1, MAX_MESSAGE_LEN)
	logTheThing("admin", src, "FLOCKSAY: [msg]")
	logTheThing("diary", src, "FLOCKSAY: [msg]", "admin")
...
/client/proc/cmd_dectalk()
	...
	if (audio && audio["audio"])
		...
		logTheThing("admin", src, "has used the dectalk verb with message: [audio["message"]]")
		logTheThing("diary", src, "has used the dectalk verb with message: [audio["message"]]", "admin")
...
/client/proc/cmd_customgrenade()
	...
	if(new_grenade)
		...
		nade.name = "mysterious grenade"
		nade.desc = "There could be anything inside this."
	logTheThing("admin", src, "spawned a custom grenade at [usr.loc]")
	logTheThing("diary", src, "spawned a custom grenade at [usr.loc]", "admin")
...
/client/proc/removeOther(var/mob/M as mob in world)
	...
	var/Target = C ? C : M
	logTheThing("admin", src, "removed [constructTarget(Target,"admin")] from existence!")
	logTheThing("diary", src, "removed [constructTarget(Target,"diary")] from existence!", "admin")
...
/client/proc/cmd_change_map()
	...
	try
		...
		logTheThing("debug", null, "<b>Map Switcher:</b> [e.name]")
...
/client/proc/showPregameHTML()
	...
	if(pregameHTML)
		if(alert("There's already some HTML shown. Do you want to remove or replace it?", "HTML clear?", "Remove", "Replace") == "Remove")
			...
			logTheThing("admin", src, "cleared the pre-game HTML.")
			for(var/client/C)
				try
	...
	if(newHTML)
		...
		logTheThing("admin", src, "changed the pre-game HTML.")
		for(var/client/C)
...
/proc/microbombs_4_everyone(var/bomb_power = null)
	...
	logTheThing("admin", usr, "[bomb_power > 0 ? "implanted everyone with a microbomb. Power: [bomb_power]" : "turned off microbombs for everyone."]")
...
/client/proc/vpn_whitelist_add(vpnckey as text)
	...
	logTheThing("admin", src, "Ckey [vpnckey] added to the VPN whitelist.")
...
/client/proc/vpn_whitelist_remove(vpnckey as text)
	...
	logTheThing("admin", src, "Ckey [vpnckey] removed from the VPN whitelist.")
	return 1
...
/client/proc/flock_cheat()
	...
	logTheThing("admin", src, "has toggled [cheat] [toggle ? "on" : "off"] for flock [flockname]")
