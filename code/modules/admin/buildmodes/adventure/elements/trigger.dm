/datum/puzzlewizard/trigger
	...
	initialize()
		...
		boutput(usr, SPAN_NOTICE("Left click to place triggers, right click triggerables to (de)select them for automatic assignment to the triggers. Ctrl+click anywhere to finish."))
		boutput(usr, SPAN_NOTICE("NOTE: Select stuff first, then make triggers for extra comfort!"))
