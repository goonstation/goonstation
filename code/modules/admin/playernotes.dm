...
/datum/admins/proc/viewPlayerNotes(var/player)
	...
	if(src.tempmin)
		logTheThing("admin", usr, "tried to access the notes of [constructTarget(player,"admin")]")
		logTheThing("diary", usr, "tried to access the notes of [constructTarget(player,"diary")]", "admin")
	...
	if (response.errored || !response.body)
		logTheThing("debug", null, "viewPlayerNotes: Failed to fetch notes of player: [player].")
		return
