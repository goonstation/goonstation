...
/datum/spacebee_extension_command/state_based/confirmation/mob_targeting/prepare(user, ckey)
	...
	if(!M)
		...
		return null
	src.ckey = M.get_ckey() // make sure we can do exact match in do_it(), partial matches could get fucked up by newjoiners etc
	return "You are about to [src.action_name] [M] ([M.get_ckey()])[isdead(M) ? " DEAD" : ""][M.mind?.is_antagonist() ? " \[T\]" : ""]."
...
/datum/spacebee_extension_command/state_based/confirmation/mob_targeting/do_it(user)
	...
	if(!M)
		...
		return
	var/success_msg = "Done: [src.action_name] [M] ([M.get_ckey()])[isdead(M) ? " DEAD" : ""][M.mind?.is_antagonist() ? " \[T\]" : ""]."
