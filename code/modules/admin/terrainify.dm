...
#if TRUE
	src.open_terrainify_editor(src.mob)
#else
...
/datum/terrainify/void
	name = "Void Station"
	desc = "Turn space into the unknowable void?"
...
/datum/terrainify/ice_moon
	...
	desc = "Turns space into the Outpost Theta... CO2 + Ice"
...
/datum/terrainify/mars
	...
	convert_station_level(params, datum/tgui/ui)
		if(..())
			...
			logTheThing("admin", ui.user, null, "turned space into a Mars.")
			logTheThing("diary", ui.user, null, "turned space into a Mars.", "admin")
			message_admins("[key_name(ui.user)] turned space into a Mars.")
...
/client/proc/open_terrainify_editor(atom/in_atom)
	set name = "Terrainify-Panel"
	set desc = "Turns space into a terrain type"
	if(holder)
		var/datum/terrainify_editor/E = new /datum/terrainify_editor(in_atom)
		E.ui_interact(mob)
...
/datum/terrainify_editor/ui_data()
	...
	data["cars"] = cars
	var/datum/terrainify/T = locate() in terrains
	data["locked"] = !isnull(T.terrainify_lock)
	return data
