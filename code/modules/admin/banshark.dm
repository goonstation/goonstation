/client/proc/sharkban(mob/sharktarget as mob in world)
	...
	if(!isadmin(src))
		...
		if(data)
			...
			logTheThing("diary", usr, "has set the Banshark on [constructTarget(sharktarget,"diary")]!", "admin")
...
//				logTheThing("admin", usr, "has sharked [constructTarget(sharktarget,"admin")].<br>Reason: [reason]<br>This will be removed in [sharkmins] minutes.")
//				logTheThing("diary", usr, "has sharked [constructTarget(sharktarget,"diary")].<br>Reason: [reason]<br>This will be removed in [sharkmins] minutes.", "admin")
...
/obj/banshark/
	...
	proc/banproc()
		...
		if(sharktarget2)
			...
			logTheThing("admin", caller:client, "has been eaten by the banshark!")
			...
			logTheThing("admin", caller:client, "has evaded the shark by ceasing to exist!  Banning them anyway.")
	...
	proc/gibproc()
		...
		if ((BOUNDS_DIST(src, src.sharktarget2) == 0))
			...
			if(sharktarget2?.client)
				logTheThing("admin", caller:client, "sharkgibbed [constructTarget(sharktarget2,"admin")]")
				logTheThing("diary", caller:client, "sharkgibbed [constructTarget(sharktarget2,"diary")]", "admin")
