...
/client/proc/debug_messages()
	...
	logTheThing("admin", usr, "toggled debug messages [debug_messages ? "on" : "off"].")
	logTheThing("diary", usr, "toggled debug messages [debug_messages ? "on" : "off"].", "admin")
...
/client/proc/call_proc_all(var/typename as null|text)
	...
	if (thetype)
		...
		logTheThing("admin", src, "called [procname] on all instances of [thetype]")
		logTheThing("diary", src, "called [procname] on all instances of [thetype]")
	else
...
/client/proc/cmd_admin_mobileAIize(var/mob/M in world)
	...
	if(!ticker)
		...
		return
	if(ishuman(M))
		logTheThing("admin", src, "has mobile-AIized [constructTarget(M,"admin")]")
		logTheThing("diary", src, "has mobile-AIized [constructTarget(M,"diary")]", "admin")
		SPAWN(1 SECOND)
			M:AIize(1)
...
/client/proc/cmd_admin_makeai(var/mob/M in world)
	...
	if(ishuman(M))
		...
		logTheThing("admin", src, "AIized [constructTarget(M,"admin")]")
		logTheThing("diary", src, "AIized [constructTarget(M,"diary")]", "admin")
		return H.AIize()
...
/client/proc/cmd_admin_changelinginize(var/mob/M in world)
	...
	if(ishuman(M) && M.mind != null)
		logTheThing("admin", src, "has made [constructTarget(M,"admin")] a changeling.")
		logTheThing("diary", src, "has made [constructTarget(M,"diary")] a changeling.", "admin")
		SPAWN(1 SECOND)
...
/client/proc/cmd_debug_del_all(var/typename as text)
	...
	if(hsbitem)
		...
		logTheThing("admin", src, "has deleted [numdeleted] instances of [hsbitem].")
		logTheThing("diary", src, "has deleted [numdeleted] instances of [hsbitem].", "admin")
...
/client/proc/cmd_debug_del_half(var/typename as text)
	...
	if(hsbitem)
		...
		logTheThing("admin", src, "has deleted [numdeleted]/[numtotal] instances of [hsbitem].")
		logTheThing("diary", src, "has deleted [numdeleted]/[numtotal] instances of [hsbitem].", "admin")
...
/client/proc/cmd_explosion(var/turf/T in world)
	...
	logTheThing("admin", src, "created an explosion (power [esize], brisance [bris]) at [log_loc(T)].")
	logTheThing("diary", src, "created an explosion (power [esize], brisance [bris]) at [log_loc(T)].", "admin")
...
/client/proc/show_camera_paths()
	...
	logTheThing("admin", usr, "[debug_camera_paths ? "displayed" : "hid"] all camera connections!")
	logTheThing("diary", usr, "[debug_camera_paths ? "displayed" : "hid"] all camera connections!", "admin")
...
/client/proc/toggle_camera_network_reciprocity()
	...
	logTheThing("admin", usr, "toggled camera network reciprocity [camera_network_reciprocity ? "on" : "off"]")
	logTheThing("diary", usr, "toggled camera network reciprocity [camera_network_reciprocity ? "on" : "off"]", "admin")
...
/client/proc/cmd_randomize_look()
	...
	logTheThing("admin", usr, "randomized their appearance")
	logTheThing("diary", usr, "randomized their appearance", "admin")
...
/client/proc/cmd_randomize_handwriting()
	...
	if (src.mob && src.mob.mind)
		...
		logTheThing("admin", usr, "randomized their handwriting style: [src.mob.mind.handwriting]")
		logTheThing("diary", usr, "randomized their handwriting style: [src.mob.mind.handwriting]", "admin")
...
/client/proc/upload_custom_hud()
	...
	if (alert("Create: \"[new_style_name]\" with icon [new_style]?", "Confirmation", "Yes", "No") == "Yes")
		...
		logTheThing("admin", usr, "added a new HUD style with the name \"[new_style_name]\"")
		logTheThing("diary", usr, "added a new HUD style with the name \"[new_style_name]\"", "admin")
...
/client/proc/test_mass_flock_convert()
	...
	if(alert("This will IRREVERSIBLY FUCK UP THE STATION and might be laggy, do not use this live. Are you sure?","Misclick Prevention","Yes","No") == "Yes")
		logTheThing("admin", usr, "started a mass flocktile conversion at [log_loc(usr)]")
		logTheThing("diary", usr, "started a mass flocktile conversion at [log_loc(usr)]", "admin")
...
/client/proc/clear_string_cache()
	...
	if(alert("Really clear the string cache?","Invalidate String Cache","OK","Cancel") == "OK")
		...
		string_cache = new
		logTheThing("admin", usr, "cleared the string cache, clearing [length] existing list(s).")
		logTheThing("diary", usr, "cleared the string cache, clearing [length] existing list(s).", "admin")
...
/client/proc/delete_profiling_logs()
	...
	logTheThing("admin", usr, "deleted profiling logs.")
	logTheThing("diary", usr, "deleted profiling logs.")
...
/client/proc/cause_lag(a as num, b as num)
	...
	logTheThing("admin", usr, "decided to cause lag with parameters of [a] and [b]")
...
/client/proc/persistent_lag(cpu_usage as num)
	...
	logTheThing("admin", usr, "decided to set persistent lag to [cpu_usage]%.")
