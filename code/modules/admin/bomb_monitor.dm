	...
	proc/display_ui(var/mob/user, var/force_update = 0)
		...
		for(var/obj/item/device/transfer_valve/TV in TVs)
			if(!filter_active_only || (TV.tank_one || TV.tank_two))
				...
				temp += {"<tr>
							...
							<td>
								Last ckey: [replace_if_null(TV.get_last_ckey(), "None")]
							</td>
							<td>
		...
		for (var/obj/item/assembly/checked_bomb in bomb_assemblies)
			...
			temp += {"<tr>
						...
						<td>
							Last ckey: [replace_if_null(checked_bomb.get_last_ckey(), "None")]
						</td>
						<td>
		...
		for(var/obj/item/canbomb_detonator/det in dets)
			if(!filter_active_only || det.attachedTo)
				...
				temp += {"<tr>
							...
							<td>
								Last ckey: [replace_if_null(det.get_last_ckey(), "None")]
							</td>
							<td>
								Last ckey: [replace_if_null(det.attachedTo.get_last_ckey(), "None")]
							</td>
							<td>
