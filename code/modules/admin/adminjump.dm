	...
	if(config.allow_admin_jump)
		...
		if (length(turfs))
			...
			if (length(turfs))
				...
				return
		logTheThing("admin", usr, "jumped to [A] ([log_loc(usr)])")
		logTheThing("diary", usr, "jumped to [A] ([log_loc(usr)])", "admin")
...
/client/proc/jumptoturf(var/turf/T in world)
	...
	if(config.allow_admin_jump)
		...
		logTheThing("admin", usr, "jumped to [log_loc(T)]")
		logTheThing("diary", usr, "jumped to [log_loc(T)]", "admin")
...
/client/proc/jumptomob(var/mob/M in world)
	...
	if(config.allow_admin_jump)
		logTheThing("admin", usr, "jumped to [constructTarget(M,"admin")] ([log_loc(M)])")
		logTheThing("diary", usr, "jumped to [constructTarget(M,"diary")] ([log_loc(M)])", "admin")
...
/client/proc/jumptokey(var/client/ckey in clients)
	...
	if(config.allow_admin_jump)
		...
		if (!ckey)
			...
			target = ckey.mob
		logTheThing("admin", usr, "jumped to [constructTarget(target,"admin")] ([log_loc(target)])")
		logTheThing("diary", usr, "jumped to [constructTarget(target,"diary")] ([log_loc(target)])", "admin")
...
/client/proc/jumptocoord(var/x = 1 as num, var/y = 1 as num, var/z = 1 as num)
	...
	if(config.allow_admin_jump)
		...
		if(flourish)
			shrink_teleport(src.mob)
		usr.set_loc(turf)
		logTheThing("admin", usr, "jumped to [log_loc(usr)]")
		logTheThing("diary", usr, "jumped to [log_loc(usr)]", "admin")
...
/client/proc/Getmob(var/mob/M in world)
	...
	ADMIN_ONLY
	if(config.allow_admin_jump)
		logTheThing("admin", usr, "teleported [constructTarget(M,"admin")] ([log_loc(usr)])")
		logTheThing("diary", usr, "teleported [constructTarget(M,"diary")] ([log_loc(usr)])", "admin")
...
/client/proc/sendmob(var/mob/M in world, var/area/A in world)
	...
	if(config.allow_admin_jump)
		...
		M.set_loc(T)
		logTheThing("admin", usr, "sent [constructTarget(M,"admin")] to [A] ([log_loc(T)] in [get_area(A)])")
		logTheThing("diary", usr, "sent [constructTarget(M,"diary")] to [A] ([log_loc(T)] in [get_area(A)])", "admin")
...
/client/proc/sendhmobs(var/area/A in world)
	...
	if(config.allow_admin_jump)
		...
		logTheThing("admin", usr, "teleported all humans to [A] ([log_loc(A)])")
		logTheThing("diary", usr, "teleported all humans to [A] ([log_loc(A)])", "admin")
...
/client/proc/sendmobs(var/area/A in world)
	...
	if(config.allow_admin_jump)
		...
		logTheThing("admin", usr, "teleported all mobs to [A] ([log_loc(A)])")
		logTheThing("diary", usr, "teleported all mobs to [A] ([log_loc(A)])", "admin")
	...
	if(config.allow_admin_jump)
		switch(alert("Are you sure?",,"Yes","No"))
			if("Yes")
				...
				logTheThing("admin", usr, "teleported all humans to themselves ([log_loc(usr)])")
				logTheThing("diary", usr, "teleported all humans to themselves ([log_loc(usr)])", "admin")
	...
	if(config.allow_admin_jump)
		switch(alert("Are you sure?",,"Yes","No"))
			if("Yes")
				...
				logTheThing("admin", usr, "teleported all clients to themselves ([log_loc(usr)])")
				logTheThing("diary", usr, "teleported all clients to themselves ([log_loc(usr)])", "admin")
	...
	if(config.allow_admin_jump)
		switch(alert("Are you sure?",,"Yes","No"))
			if("Yes")
				...
				logTheThing("admin", usr, "brought all traitors to themselves ([log_loc(usr)])")
				logTheThing("diary", usr, "brought all traitors to themselves ([log_loc(usr)])", "admin")
	...
	if(config.allow_admin_jump)
		switch(alert("Are you sure?",,"Yes","No"))
			if("Yes")
				...
				logTheThing("admin", usr, "brought all non-traitors to themselves ([log_loc(usr)])")
				logTheThing("diary", usr, "brought all non-traitors to themselves ([log_loc(usr)])", "admin")
...
/client/proc/cmd_admin_get_mobject(var/atom/target as mob|obj in world)
	...
	if (config.allow_admin_jump)
		logTheThing("admin", usr, "teleported [target] to their turf ([log_loc(usr)] from [log_loc(target)])")
		logTheThing("diary", usr, "teleported [target] to their turf ([log_loc(usr)] from [log_loc(target)])", "admin")
...
/client/proc/cmd_admin_get_mobject_loc(var/atom/target as mob|obj in world)
	...
	if (config.allow_admin_jump)
		logTheThing("admin", usr, "teleported [target] to their loc ([log_loc(usr)] from [log_loc(target)])")
		logTheThing("diary", usr, "teleported [target] to their loc ([log_loc(usr)] from [log_loc(target)])", "admin")
