...
// replace all limbs
			if ("replace_all_limbs")
				...
				if (!limbs || !limbs.holder)
					...
					return
				limbs.mend()
				logTheThing("admin", usr, "replaced all of [constructTarget(limbs.holder,"admin")]'s missing limbs")
...
// drop organs
			if ("drop_organ")
				...
				if (organs.drop_organ(target_organ, null)) // returns with the organ when at least one organ is dropped
					logTheThing("admin", usr, "made [constructTarget(organs.donor,"admin")] drop their organ(s): [uppertext(target_organ)]")
...
// replace organs
			if ("replace_organ")
				...
				if (organs.receive_organ(I, target_organ, 0.0, 1)) // returns 1 if replace was successful
					logTheThing("admin", usr, "replaced [constructTarget(organs.donor,"admin")]'s [uppertext(target_organ)] with [new_type]")
...
// replace all organs
			if ("replace_all_organs")
				...
				logTheThing("admin", usr, "replaced all of [constructTarget(organs.donor,"admin")]'s missing organs")
