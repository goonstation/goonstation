	...
	proc/do_step()
		...
		if (!isnull(internal_speed))
			...
			if (src.proj_data.affected_by_gravity)
				src.travelled += src.internal_speed * curr_turf ? curr_turf.get_gforce_current()/100 : 1
			else
			...
			curr_t++
			if (src.proj_data.affected_by_gravity)
				src.travelled += src.proj_data.projectile_speed * curr_turf ? curr_turf.get_gforce_current()/100 : 1
			else
