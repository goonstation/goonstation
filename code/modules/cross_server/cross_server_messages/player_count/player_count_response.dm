/datum/cross_server_message/player_count_response
	...
	receive(list/data, datum/game_server/server)
		...
		logTheThing(LOG_DEBUG, null, "<b>Sov:</b> Received player count from [server.name]: [player_count]")
	...
	send(datum/game_server/server, player_count)
		logTheThing(LOG_DEBUG, null, "<b>Sov:</b> Sending player count response to [server.name] with count [player_count]")
