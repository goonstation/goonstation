...
/mob/living/carbon/wall/horror
	say_quote(var/text)
		if(src.emote_allowed)
			...
			if(src.gender == MALE) playsound(get_turf(src), "sound/voice/screams/male_scream.ogg", 100, 0, 0, 0.91, channel=VOLUME_CHANNEL_SCREAMFART)
			else playsound(get_turf(src), "sound/voice/screams/female_scream.ogg", 100, 0, 0, 0.9, channel=VOLUME_CHANNEL_SCREAMFART)
			SPAWN_DBG(5 SECONDS)
				src.emote_allowed = 1
			return "screams!"
		else
