...
/proc/decideSolid(var/turf/current, var/turf/center, var/sizemod = 0)
	if(!current || !center || (current.loc.type != /area/concrete && !istype(current.loc , /area/allowGenerate)) || !istype(current, /turf/space))
		return 0
	...
	generate(var/list/miningZ)
		...
		for(var/x=1,x<=world.maxx,x++)
			for(var/y=1,y<=world.maxy,y++)
				...
				if(map[x][y] && !ISDISTEDGE(T, 3) && T.loc && ((T.loc.type == /area/concrete) || istype(T.loc , /area/allowGenerate)) )
					var/turf/simulated/wall/asteroid/N = new/turf/simulated/wall/asteroid(T)
					N.quality = rand(-101,101)
					generated.Add(N)
				if(T.loc.type == /area/concrete || istype(T.loc, /area/allowGenerate))
...
/datum/mapGenerator/asteroidsDistance //Generates a bunch of asteroids based on distance to seed/center. Super simple.
	generate(var/list/miningZ)
		...
		for(var/i=0, i<numAsteroidSeed, i++)
			...
			while(!istype(X, /turf/space) || ISDISTEDGE(X, AST_MAPSEEDBORDER) || (X.loc.type != /area/concrete && !istype(X.loc , /area/allowGenerate)))
				X = pick(miningZ)
				LAGCHECK(LAG_REALTIME)
			...
			for(var/turf/T in solidTiles)
				if(!isnull(T) && T.loc && ((T.loc.type == /area/concrete) || istype(T.loc , /area/allowGenerate)))
	...
	proc/applyTo(var/turf/target)
		...
		for(var/x=0, x<prefabSizeX; x++)
			for(var/y=0, y<prefabSizeX; y++)
				...
				if(L && L.loc && ((L.loc.type != /area/concrete) && !istype(L.loc , /area/allowGenerate))) // istype(L.loc, /area/noGenerate)
					return 0
