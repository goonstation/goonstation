	...
	for (var/n = 1, n <= num_to_place, n++)
		...
		if (M)
			...
			while (!stop && count < maxTries) //Kinda brute forcing it. Dumb but whatever.
				...
				if (ret == 0)
					logTheThing("debug", null, "Prefab placement #[n] [M.type] failed due to blocked area. [target] @ [log_loc(target)]")
				else
					logTheThing("debug", null, "Prefab placement #[n] [M.type][M.required?" (REQUIRED)":""] succeeded. [target] @ [log_loc(target)]")
					stop = 1
				count++
				if (count >= 33)
					logTheThing("debug", null, "Prefab placement #[n] [M.type] failed due to maximum tries [maxTries][M.required?" WARNING: REQUIRED FAILED":""]. [target] @ [log_loc(target)]")
		else break
