...
/datum/statusEffect/patches_applied
	...
	onUpdate(timePassed)
		...
		var/waste_volume = src.use_volume * max(length(L.applied_patches) * 0.75, 1) //amount that gets removed from the patch. Half of this gets transferred into the body

		for (var/atom/movable/A as anything in L.applied_patches)
			if (A.reagents?.total_volume)
				A.reagents.reaction(L, TOUCH, react_volume = src.use_volume, paramslist = \
