...
/**
	...
	simplehot/stimulants
		...
		onAdd(optional)
			...
			if(ismob(owner))
				var/mob/M = owner
				APPLY_ATOM_PROPERTY(M, PROP_STAMINA_REGEN_BONUS, "stims", 500)
	...
	simpledot/radiation
		...
		modify_change(change)
			...
			if (owner && ismob(owner) && change > 0)
				...
				var/percent_protection = clamp(GET_ATOM_PROPERTY(M, PROP_RADPROT), 0, 100)
	...
	simpledot/stimulant_withdrawl
		...
		onAdd(optional)
			...
			if(ismob(owner))
				var/mob/M = owner
				APPLY_ATOM_PROPERTY(M, PROP_STAMINA_REGEN_BONUS, "stim_withdrawl", -5)
				M.jitteriness = 0
		...
		onRemove()
			...
			if(ismob(owner))
				var/mob/M = owner
				REMOVE_ATOM_PROPERTY(M, PROP_STAMINA_REGEN_BONUS, "stim_withdrawl")
