...
/obj/machinery/portable_atmospherics/canister/proc/healthcheck(mob/user)
	...
	if (src.health <= 10)
		...
		logTheThing("station", null, "[src] [log_atmos(src)] was destructively opened[user ? " by [key_name(user)]" : ""], emptying contents at [log_loc(src)].")
	...
	if (istype(W, /obj/item/assembly/detonator)) //Wire: canister bomb stuff
		if (holding)
			...
			if (Det.det_state != 4)
				...
				logTheThing("bombing", user, "builds a canister bomb [log_atmos(src)] at [log_loc(src)].")
				if(src.air_contents.check_if_dangerous())
		...
		logTheThing("combat", user, "attacked [src] [log_atmos(src)] with [W] at [log_loc(src)].")
		src.health -= W.force
		healthcheck(user)
	..()
...
/obj/machinery/portable_atmospherics/canister/proc/toggle_valve()
	...
	if (!src.holding && !src.connected_port)
		logTheThing("station", usr, "[valve_open ? "opened [src] into" : "closed [src] from"] the air [log_atmos(src)] at [log_loc(src)].")
