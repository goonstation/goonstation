/obj/machinery/portable_atmospherics
	...
	proc
		connect(obj/machinery/atmospherics/portables_connector/new_port)
			...
			//logTheThing("combat", usr, "attaches [src] to [new_port] at [log_loc(new_port)].")
	...
	if(istype(W, /obj/item/tank))
		...
		if ((istype(src, /obj/machinery/portable_atmospherics/canister))) //No messing with anchored canbombs. -ZeWaka
			...
			if (!isnull(C.det) && C.anchored)
				...
				return
		if(connected_port)
			logTheThing("station", user, "has disconnected \the [src] [log_atmos(src)] from the port at [log_loc(src)].")
			disconnect()
			...
			if(possible_port)
				if(connect(possible_port))
					logTheThing("station", user, "has connected \the [src] [log_atmos(src)] to the port at [log_loc(src)].")
