...
/obj/item/putt/frame_box
	...
	attack_self(mob/user as mob)
		...
		logTheThing("station", user, "builds [O] in [get_area(user)] ([log_loc(user)])")
...
/obj/structure/vehicleframe/verb/deconstruct()
	...
	logTheThing("station", usr, "deconstructs [src] in [get_area(usr)] ([log_loc(usr)])")
	O.fingerprints = src.fingerprints
	O.fingerprints_full = src.fingerprints_full
	qdel(src)
	...
	switch(stage)
		...
		if(10)
			if(istype(W, /obj/item/sheet))
				...
				logTheThing("station", user, "finishes building a [V] in [get_area(user)] ([log_loc(user)])")
				qdel(src)
