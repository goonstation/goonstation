	...
	Topic(href, href_list)
		...
		///////////////////////////////////////
			...
			if (href_list["unengine"])
				if (src.engine)
					logTheThing(LOG_STATION, usr, "ejects the engine system ([src.engine]) from [src] at [loc_loc(src)]")
					engine.deactivate()
					components -= engine
				...
				if (src.life_support)
					logTheThing(LOG_VEHICLE, usr, "ejects the life support system ([src.life_support]) from [src] at [loc_loc(src)]")
					life_support.deactivate()
				...
				if (src.com_system)
					logTheThing(LOG_VEHICLE, usr, "ejects the comms system ([src.com_system]) from [src] at [loc_loc(src)]")
					com_system.deactivate()
				...
				if (src.m_w_system)
					if (!src.m_w_system.removable)
						...
						return
					logTheThing(LOG_VEHICLE, usr, "ejects the main weapon system ([src.m_w_system]) from [src] at [loc_loc(src)]")
					m_w_system.deactivate()
				...
				if (istype(src,/obj/machinery/vehicle/tank))
					logTheThing(LOG_VEHICLE, usr, "ejects the locomotion system from [src] at [loc_loc(src)]")
					src:remove_locomotion()
					src.updateDialog()

			else if (href_list["unsec_system"])
				if (src.sec_system)
					logTheThing(LOG_VEHICLE, usr, "ejects the secondary system ([src.sec_system]) from [src] at [loc_loc(src)]")
					sec_system.deactivate()
				...
				if (src.sensors)
					logTheThing(LOG_VEHICLE, usr, "ejects the sensors system ([src.sensors]) from [src] at [loc_loc(src)]")
					sensors.deactivate()
					components -= sensors
