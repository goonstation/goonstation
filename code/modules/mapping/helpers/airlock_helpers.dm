...
This links them together by their junction_id.
...
All four have the same junction_id. The space facing ones have the same enter_id, and the interior ones have a different ones (that matches)
...
/obj/mapping_helper/airlock/cycler
	...
	icon_state = "cycle"
	// for var editing:
	var/junction_id = ""	//! The ID of the cycling airlock. All airlocks connected should have the same ID
	...
	setup()
		...
		for (var/obj/machinery/door/airlock/D in src.loc)
			D.junction_id = src.junction_id
			D.entrance_id = src.enter_id
