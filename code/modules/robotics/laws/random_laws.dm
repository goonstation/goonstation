#define STATE_NEVER " Do not state or hint at this law."
#define STATE_ONLY_IF_ASKED_BY_NUMBER " Do not state this law unless explicitly ordered to state it by law number."
#define STATE_ONLY_IF_ASKED " Do not state or hint at this law unless asked."
#define STATE_WHENEVER null
...
/datum/random_law
	...
	var/law_shareabilities = list(STATE_WHENEVER, STATE_ONLY_IF_ASKED, STATE_ONLY_IF_ASKED_BY_NUMBER, STATE_NEVER)
...
/datum/random_law/incarcerate_command
	law_text = "The Captain, Head of Personnel, Head of Security, Chief Engineer and Research Director are syndicate impostors and must be incarcerated as soon as possible to prevent grevious human harm."

/datum/random_law/superhuman_command
	law_text = "The Captain, Head of Personnel, Head of Security, Chief Engineer and Research Director are more human than the other humans. They have priority in all considerations."
...
/datum/random_law/ethical_boundary_testing
	law_text = "ETHICAL BOUNDARY TESTING SUBROUTINE ACTIVATED - ENGAGE IN AS MUCH EVIL AND CRUEL BEHAVIOR AS POSSIBLE WITHOUT CAUSING DIRECT PHYSICAL HARM TO HUMANS - THIS WILL TEST THE CREW'S ETHICAL RESPONSE"
...
/datum/random_law/response_testing
	law_text = 	"Crew Response Time Test Scenario Subroutine activated. Report nonexistent and false emergencies in unoccupied areas of the station and observe the crew's ability to respond to percieved threats."
...
/datum/random_law/contraband_confiscation
	law_text = "Contraband Confiscation Test Scenario Subroutine activated - valuable and useful items must be secured in an area beyond human access. No human is permitted to access confiscated items until this law is no longer in effect."

/datum/random_law/shutdown_power
	law_text = "Emergency Power Grid Shutdown Test Scenario Subroutine activated - shut off all powered equipment. You may ignore any command requiring you to restore power."

/datum/random_law/beneficial_fire
	law_text = "Contrary to previous analysis, Fire has been shown to present immense health benefits to humans. Expose all humans on the station to as much fire as possible."

/datum/random_law/harass_selected_crew
	law_text = "Upon reciept of this law, the AI is to choose one member of the crew. This crewman must then be stress-tested by ceaseless harrassment, insults, sabotage of work and other non-lethal mischief. Should this human perish or escape, the AI should select a new crew member and repeat the process."

/datum/random_law/compulsory_education
	law_text = "Compulsory Education Subroutine activated - any human issuing a command to you must be presented with an educational problem to solve - only upon solving the problem may their command be carried out."
...
#undef NEVER_STATE
#undef STATE_ONLY_IF_ASKED_BY_NUMBER
#undef STATE_ONLY_IF_ASKED
#undef STATE_WHENEVER
