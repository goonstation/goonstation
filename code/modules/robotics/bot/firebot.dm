...
/obj/machinery/bot/firebot/process()
	...
	if(src.target)
		...
		if(reachable_in_n_steps(get_turf(src), get_turf(src.target), 3))
			spray_at(src.target)
		else
...
/obj/machinery/bot/firebot/DoWhileMoving()
	. = ..()
	if (IN_RANGE(src, src.target, 3) && !ON_COOLDOWN(src, FIREBOT_SPRAY_COOLDOWN, src.spray_cooldown) && reachable_in_n_steps(get_turf(src), get_turf(src.target), 3))
		src.frustration = 0
		spray_at(src.target)
		return TRUE
