		...
		SPAWN(3 SECONDS)
			...
			SPAWN(3 SECONDS)
				...
				for (var/obj/item/holy_texts/B in src.loc)
					...
					src.visible_message(SPAN_ALERT("[src] blasts its ass all over the holy texts.<br><b>A mysterious force <u>is not pleased</u>!</b>"))
	...
	for (var/atom/A as anything in src.loc)
		if(A.event_handler_flags & IS_FARTABLE)
			if(istype(A,/mob/living))
				...
				src.visible_message(SPAN_ALERT("[src] farts on the holy texts.<br><b>A mysterious force smites [src]!</b>"))
				fart_on_other = 1
				src.fart_memory += A
				src.gib()
				break
