...
//Or if they have weapons and aren't security, arrest them.
	...
	explode()
		if (report_arrests)
			...
			pdaSignal.data = list("address_1"="00000000", "command"="text_message", "sender_name"="SECURITY-MAILBOT", "tag" = PDA_SEC_ALERT_TAG, "group"=MGD_SECURITY, "sender"="00000000", "message"="[message2send]")
	...
	onEnd()
		...
		if (get_dist(master, master.target) <= 1)
			...
			if (master.report_arrests && !uncuffable)
				...
				pdaSignal.data = list("address_1"="00000000", "command"="text_message", "sender_name"="SECURITY-MAILBOT", "tag" = PDA_SEC_ALERT_TAG, "group"=MGD_SECURITY, "sender"="00000000",\
