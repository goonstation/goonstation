	...
	bodyguard/heckle
		...
		task_act()
			...
			if (src.protected && prob(10))
				master.say(pick(buddy_heckle_phrases))
		...
		task_act()
			...
			if (src.protected && prob(5))
				...
				if (prob(40))
					...
					master.say(buddy_cheer_up_phrases[buddy_cheer_up_chooser])
					master.point(src.protected, 1)
				else
					var/buddy_cheer_up_chooser = rand(1, length(buddy_cheer_up_starters))
					master.say(buddy_cheer_up_starters[buddy_cheer_up_chooser])
					master.point(src.protected, 1)
					master.say(buddy_cheer_up_answers[buddy_cheer_up_chooser])
			...
			switch (state)
				...
				if (STATE_AT_BEACON)
					...
					if (next_beacon_id)
						...
						tour_delay = 30
						master.say("And that concludes the tour session.  Please visit the gift shop on your way out.")
					return
