...
/obj/machinery/bot/duckbot/proc/wakka_wakka()
	...
	if(src.emagged)
		if(ismob(src.annoy_target))
			...
			else if(!GET_COOLDOWN(src, DUCKBOT_ANNOY_PATHING_COOLDOWN))
				ON_COOLDOWN(src, DUCKBOT_ANNOY_PATHING_COOLDOWN, src.annoy_path_cooldown)
				var/turf/randwander = get_step_rand(get_turf(src.annoy_target))
				src.navigate_to(randwander, DUCKBOT_MOVE_SPEED, 0, 30)
		else
			for_by_tcl(M, /mob)
				if(IN_RANGE(src, M, 7))
					if(!GET_COOLDOWN(src, "[DUCKBOT_ANNOY_LOCKOUT_TIMEOUT]-[M.name]"))
