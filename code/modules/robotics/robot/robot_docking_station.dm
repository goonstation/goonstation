...
/obj/machinery/recharge_station/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || src.contents.Find(usr) || can_access_remotely(usr) || ((BOUNDS_DIST(src, usr) == 0) && istype(src.loc, /turf))))
		...
		if (href_list["rename"])
			...
			logTheThing("combat", usr, "uses a docking station to rename [constructTarget(R,"combat")] to [newname].")
			R.real_name = "[newname]"
			R.UpdateName()
			if (R.internal_pda)
		...
		if (href_list["removecell"]) //ZeWaka: Special snowflake fix for cell ejecting not working.
			...
			logTheThing("combat", usr, "removes [constructTarget(R,"combat")]'s power cell at [log_loc(usr)].")
			R.hud.update_charge()
...
/obj/machinery/recharge_station/proc/move_human_inside(mob/user, mob/victim)
	...
	logTheThing("combat", user, "puts [constructTarget(H,"combat")] into a conversion chamber at [log_loc(src)]")
