			...
			if (MENU_IN_RECORD)
				switch(lowertext(command))
					...
					if (FIELDNUM_NEWREC)
						...
						R["criminal"] = SECURITY::ARREST_STATE::NONE
						R["sec_flag"] = "None"
						R["mi_crim"] = "None"
				...
				switch(field_number)
					...
					if (FIELDNUM_SECFLAG)
						src.print_text("Please enter new value ([SECURITY::SECHUD_FLAG_MAX_CHARS] characters max), or \"None\".")
						src.menu = MENU_FIELD_INPUT
						return
				...
				switch (field_input)
					...
					if (FIELDNUM_CRIMSTAT)
						...
						if (lowertext(command) == "clown")
							src.active_secure["criminal"] = SECURITY::ARREST_STATE::CLOWN
						...
						switch (round( max( text2num_safe(command), 0) ))
							if (1)
								if (src.active_secure["criminal"] != SECURITY::ARREST_STATE::ARREST)
									src.report_status(src.active_general["name"], SECURITY::ARREST_STATE::ARREST)
								src.active_secure["criminal"] = SECURITY::ARREST_STATE::ARREST
							if (2)
								if (src.active_secure["criminal"] != SECURITY::ARREST_STATE::DETAIN)
									src.report_status(src.active_general["name"], SECURITY::ARREST_STATE::DETAIN)
								src.active_secure["criminal"] = SECURITY::ARREST_STATE::DETAIN
							if (3)
								src.active_secure["criminal"] = SECURITY::ARREST_STATE::NONE
								src.active_secure["sec_flag"] = "None"
							if (4)
								src.active_secure["criminal"] = SECURITY::ARREST_STATE::SUSPECT
							if (5)
								src.active_secure["criminal"] = SECURITY::ARREST_STATE::INCARCERATED
							if (6)
								src.active_secure["criminal"] = SECURITY::ARREST_STATE::PAROLE
							if (7)
								src.active_secure["criminal"] = SECURITY::ARREST_STATE::RELEASED
							if (0)
								src.menu = MENU_IN_RECORD
								return
							else
					...
					if (FIELDNUM_SECFLAG)
						...
						if (ckey(inputText))
							src.active_secure["sec_flag"] = copytext(inputText, 1, SECURITY::SECHUD_FLAG_MAX_CHARS + 1)
						else
							return
