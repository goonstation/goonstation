		...
		switch(menu)
			...
			if (MENU_FIELD_INPUT)
				...
				switch (field_input)
					...
					if (FIELDNUM_PRINT_R)
						if (ckey(inputText))
							src.active_general["fprint_r"] = copytext(inputText, 1, 33)
						else
							return

					if (FIELDNUM_PRINT_L)
						if (ckey(inputText))
							src.active_general["fprint_l"] = copytext(inputText, 1, 33)
						else
							return
	...
	proc
		...
		print_active_record()
			...
			var/view_string = {"\
				...
				<br>\[__\]<b>Photo</b>:       [istype(src.active_general["photo"], /datum/computer/file/image) ? "On File" : "None"]\
