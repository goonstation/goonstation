...
/obj/machinery/networked/telepad
	...
	proc/send(var/turf/target)
		...
		if (stuff.len)
			...
			if(ismob(which))
				logTheThing("station", usr, "sent [constructTarget(which,"station")] to [log_loc(target)] from [log_loc(src)] with a telepad")
	...
	proc/receive(var/turf/receiveturf)
		...
		if (stuff.len)
			var/atom/movable/which = pick(stuff)
			if(ismob(which))
				logTheThing("station", usr, "received [constructTarget(which,"station")] from [log_loc(which)] to [log_loc(src)] with a telepad")
	...
	proc/doubleportal(var/turf/target)
		...
		for(var/atom/movable/O in send)
			O.set_loc(target)
			if(ismob(O))
				logTheThing("station", usr, "sent [constructTarget(O,"station")] to [log_loc(target)] from [log_loc(src)] with a telepad")

		for(var/atom/movable/O in receive)
			if(ismob(O))
				logTheThing("station", usr, "received [constructTarget(O,"station")] from [log_loc(O)] to [log_loc(src)] with a telepad")
			O.set_loc(src.loc)
		showswirl(src.loc)
		showswirl(target)
		use_power(500000)
		...
		if(prob(5) && !locate(/obj/dfissure_to) in get_step(src, EAST))
			...
			if (start_portal)
				...
				logTheThing("station", usr, "created a portal to [log_loc(target)] at [log_loc(src.loc)] with a telepad")
	...
	proc/badsend()
		...
		logTheThing("station", usr, "sends the telepad at [log_loc(src)] on invalid coords, causing the [effect] effect.")
	...
	proc/badreceive()
		...
		logTheThing("station", usr, "receives the telepad at [log_loc(src)] on invalid coords, causing the [effect] effect.")
	...
	proc/processbadeffect(var/effect)
		switch(effect)
			...
			if("gib")
				for(var/mob/living/M in src.loc)
					logTheThing("combat", M, "was gibbed by a telescience fault at [log_loc(M)].")
					M.gib()
				return
			if("majormutate")
				for(var/mob/living/carbon/M in src.loc)
