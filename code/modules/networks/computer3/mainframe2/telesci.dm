						...
						if(!scanTurf)
							...
							if (isnum(scanTurf))
								...
								if(istype(scanTurf, /turf/simulated))
									...
									if(T.active_hotspot)
										burning = 1
								message_host("command=scan_reply&o2=[GM.oxygen]&tox=[GM.toxins]&n2=[GM.nitrogen]&co2=[GM.carbon_dioxide]&temp=[GM.temperature]&pressure=[MIXTURE_PRESSURE(GM)][(burning)?("&burning=1"):(null)]")
							else
