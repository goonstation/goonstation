...
/datum/computer/file/record/dwaine_help
	...
	New()
		...
		src.fields["history"] = "Controls the user's history file if it exists within their home directory.|nDisplay the history list with line numbers, prefixing each modified entry with the numeric index. An argument of N lists only the last N entries.|n-c to clear the entire file.|nUsage: history \[-c] or history \[arg]"
