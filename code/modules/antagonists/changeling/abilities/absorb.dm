/datum/action/bar/icon/abominationDevour
	...
	onEnd()
		...
		if(owner && ownerMob && target && BOUNDS_DIST(owner, target) == 0 && devour)
			...
			logTheThing("combat", ownerMob, "devours [constructTarget(target,"combat")] as a changeling in horror form [log_loc(owner)].")
...
/datum/action/bar/private/icon/changelingAbsorb
	...
	onEnd()
		...
		if(owner && ownerMob && target && BOUNDS_DIST(owner, target) == 0 && devour)
			...
			logTheThing("combat", ownerMob, "absorbs [constructTarget(target,"combat")] as a changeling [log_loc(owner)].")
