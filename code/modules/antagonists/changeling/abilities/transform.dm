	...
	cast(atom/target)
		...
		if (H.mutantrace)
			if (ismonkey(H))
				...
				logTheThing("combat", H, "leaves lesser form as a changeling, [log_loc(H)].")
				return 0
			...
			H.monkeyize()
			H.abilityHolder.updateButtons()
			logTheThing("combat", H, "enters lesser form as a changeling, [log_loc(H)].")
			return 0
...
/datum/targetable/changeling/transform
	...
	cast(atom/target)
		...
		logTheThing("combat", holder.owner, "transforms into [target_name] as a changeling [log_loc(holder.owner)].")
