/datum/targetable/changeling/sting
	...
	cast(atom/target)
		...
		if (isobj(target) && (target.can_receive() || istype(target,/obj/item/reagent_containers/food) || istype(target,/obj/item/reagent_containers/patch)))
	...
	dna
		...
		cast(atom/target)
			if (..())
				return 1
			if (target.can_receive() == 1 || istype(target,/obj/item/reagent_containers/food) || istype(target,/obj/item/reagent_containers/patch))
				if (target.reagents.total_volume >= target.reagents.maximum_volume)
					return 0
