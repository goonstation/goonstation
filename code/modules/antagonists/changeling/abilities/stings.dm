/datum/targetable/changeling/sting
	...
	cast(atom/target)
		...
		if (isobj(target) && (target.is_open_container() || istype(target,/obj/item/reagent_containers/food) || istype(target,/obj/item/reagent_containers/patch)))
			...
			if (istype(target,/obj/item/reagent_containers/patch))
				...
				if (P.medical)
					...
					P.medical = 0
			logTheThing("combat", holder.owner, "stings [target] with [name] as a changeling at [log_loc(holder.owner)].")
		...
		logTheThing("combat", holder.owner, "stings [constructTarget(MT,"combat")] with [name] as a changeling [log_loc(holder.owner)].")
