/datum/targetable/changeling/abomination
	...
	cast(atom/target)
		...
		if (isabomination(H))
			...
			logTheThing("combat", H, "enters horror form as a changeling, [log_loc(H)].")
			return 0
...
/mob/proc/revert_from_horror_form()
	if(ishuman(src))
		...
		logTheThing("combat", H, "voluntarily leaves horror form as a changeling, [log_loc(H)].")
		return 0
...
/datum/targetable/changeling/scream
	...
	cast(atom/target)
		...
		logTheThing("combat", holder.owner, "screeches as a changeling in horror form [log_loc(holder.owner)].")
