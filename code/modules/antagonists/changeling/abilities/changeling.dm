...
/datum/abilityHolder/changeling
	...
	proc/addDna(var/mob/living/carbon/human/M, var/headspider_override = 0)
		...
		if (O)
			...
			logTheThing("combat", owner, "absorbs [constructTarget(M,"combat")] as a changeling [log_loc(owner)].")
	...
	proc/return_control_to_master()
		if(master)
			logTheThing("combat", master, "has retaken control of the changeling body from [constructTarget(owner,"combat")].")
