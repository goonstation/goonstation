...
/datum/antagonist/subordinate/changeling_critter
	...
	give_equipment()
		...
		src.owner.current.ensure_speech_tree().AddSpeechModifier(SPEECH_MODIFIER_ORIGINAL_NAME)
	...
	remove_equipment()
		...
		src.owner.current.ensure_speech_tree().RemoveSpeechModifier(SPEECH_MODIFIER_ORIGINAL_NAME)
