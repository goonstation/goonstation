...
/datum/antagonist/subordinate/changeling_critter
	...
	announce()
		...
		if (!istype(critter))
			return ..()
		boutput(src.owner.current, "<h3><span class='alert'>You have reawakened to serve your host changeling, [src.master.current.real_name]! You must follow [his_or_her(src.master)] commands!</span></h3>")
