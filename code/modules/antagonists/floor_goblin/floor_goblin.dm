/mob/proc/make_floor_goblin()
	...
	logTheThing("admin", usr, "made [constructTarget(H,"admin")] a floor goblin.")
...
/datum/action/bar/icon/steal_shoes//Putting items on or removing items from others.
	...
	onStart()
		...
		logTheThing("combat", source, "tries to remove \an [shoes] from [constructTarget(target,"combat")] at [log_loc(target)].")
		var/name = "something"
		icon = shoes.icon
	...
	onEnd()
		...
		if(shoes)
			if(shoes.handle_other_remove(source, target))
				logTheThing("combat", source, "successfully removes \an [shoes] from [constructTarget(target,"combat")] at [log_loc(target)].")
				for(var/mob/O in AIviewers(owner))
