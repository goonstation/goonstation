/datum/antagonist/subordinate/mindhack
	...
	announce()
		. = ..()
		boutput(src.owner.current, "<h2>[SPAN_ALERT("You feel an unwavering loyalty to [src.master.current.real_name]! You feel you must obey [his_or_her(src.master.current)] every order! Do not tell anyone about this unless [src.master.current.real_name] tells you to!")]</h2>")
	...
	announce_removal(source)
		...
		switch (source)
			if (ANTAGONIST_REMOVAL_SOURCE_DEATH)
				...
				boutput(src.owner.current, "<h2>[SPAN_ALERT("As you have died, you are no longer mindhacked! Do not obey your former master's orders even if you've been brought back to life somehow.")]</h2>")
			...
			if (ANTAGONIST_REMOVAL_SOURCE_OVERRIDE)
				...
				boutput(src.owner.current, "<h2>[SPAN_ALERT("Your mindhack implant has been overridden by a new one, cancelling out your former allegiances!")]</h2>")
			...
			if (ANTAGONIST_REMOVAL_SOURCE_EXPIRED)
				...
				boutput(src.owner.current, "<h2>[SPAN_ALERT("Your mind is your own again! You no longer feel the need to obey your former master's orders.")]</h2>")
