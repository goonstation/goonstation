...
/datum/targetable/arcfiend/arcFlash
	...
	cast(atom/target)
		...
		logTheThing("combat", src.holder.owner, "[key_name(src.holder.owner)] used <b>[src.name]</b> on [key_name(target)] [log_loc(src.holder.owner)].")
		...
		for (var/i in 1 to src.chain_count)
			...
			logTheThing("combat", src.holder.owner, "[key_name(src.holder.owner)] hit [key_name(target)] with chain lightning [log_loc(src.holder.owner)].")
