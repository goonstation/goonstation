/datum/targetable/wraithAbility/spook
	...
	onAttach(datum/abilityHolder/holder)
		. = ..()
		src.object.contextLayout = new /datum/contextLayout/screen_HUD_default(2, 16, 16)//, -32, -32)
		if (!src.object.contextActions)
			object.contextActions = list()

		// this ability is never actually cast()
		// instead it's just context actions calling procs on this ability datum
		// i dont like it
		for(var/i=1, i<=8, i++)
			var/datum/contextAction/wraith_spook_button/newcontext = new /datum/contextAction/wraith_spook_button(i)
			src.object.contextActions += newcontext

	proc/haunt_pda(var/obj/item/device/pda2/pda)
		var/message = pick("boo", "git spooked", "BOOM",
							"there's a skeleton inside of you", "DEHUMANIZE YOURSELF AND FACE TO BLOODSHED",
