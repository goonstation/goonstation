...
/datum/targetable/wraithAbility/make_poltergeist
	...
	proc/make_poltergeist(var/mob/living/intangible/wraith/W, var/turf/T, var/tries = 0)
		...
		if (length(candidates) == 0)
			message_admins("Couldn't set up poltergeist; no ghosts responded. Source: [src.holder.owner]")
			logTheThing(LOG_ADMIN, null, "Couldn't set up poltergeist; no ghosts responded. Source: [src.holder.owner]")
			if (tries >= 1)
