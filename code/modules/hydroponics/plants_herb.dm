	...
	HYPspecial_proc(var/obj/machinery/plantpot/POT)
		...
		if (POT.growth > (P.growtime + DNA?.get_effective_value("growtime")) && prob(sting_prob)) //how frequently it injects people is based on endurance
			for (var/mob/living/M in range(1,POT))
				...
				M.reagents?.add_reagent("histamine", 5 * chem_protection) //separated from regular reagents so it's never more than 5 units, also no touch effect
				boutput(M, "<span class='notice'>You feel something brush against you.</span>")

				var/datum/reagents/reagents_temp = new/datum/reagents(length(assoc_reagents) + DNA?.get_effective_value("potency"))
				for (var/plantReagent in assoc_reagents) //amount of delivered chems is based on potency
					reagents_temp.add_reagent(plantReagent, chem_protection * max(1, (DNA?.get_effective_value("potency") / length(assoc_reagents))))
