...
/obj/machinery/vending/player
	...
	Topic(href, href_list)
		...
		if (href_list["loading"])
			...
			var/inp
			inp = tgui_input_number(usr, "Enter the new price:", "Item Price", 0, 1000000, 0)
			if (!(isdead(usr) || !can_act(usr) || !in_interact_range(src, usr) || isnull(inp)))
...
/obj/machinery/vending/air_vendor
	...
	Topic(href, href_list)
		...
		if(href_list["changepressure"])
			var/change = tgui_input_number(usr, "Target Pressure (10.1325-1013.25):", "Enter target pressure", target_pressure, 1013.25, 10.1325, round_input = FALSE)
			if(change)
				src.updateUsrDialog()
