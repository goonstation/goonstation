...
/datum/dj_panel/ui_act(action, params)
	...
	switch(action)
		...
		if("toggle-announce")
			...
			logTheThing("admin", usr, "set their DJ mode to [(usr.client.djmode ? "On" : "Off")]")
			logTheThing("diary", usr, "set their DJ mode to [(usr.client.djmode ? "On" : "Off")]", "admin")
		...
		if("play-ambience")
			logTheThing("admin", usr, "played ambient sound [loaded_sound]")
			logTheThing("diary", usr, "played ambient sound [loaded_sound]", "admin")
		...
		if("play-player")
			...
			if (!C) return FALSE
			logTheThing("admin", usr, "played sound [loaded_sound] to [C]")
			logTheThing("diary", usr, "played sound [loaded_sound] to [C]", "admin")
...
/datum/dj_panel/proc/toggledj(client/C, client/actor)
	...
	logTheThing("admin", actor, "has [C.non_admin_dj ? "given" : "removed"] the ability for [constructTarget(C,"admin")] to DJ and use dectalk.")
	logTheThing("diary", actor, "has [C.non_admin_dj ? "given" : "removed"] the ability for [constructTarget(C,"diary")] to DJ and use dectalk.", "admin")
