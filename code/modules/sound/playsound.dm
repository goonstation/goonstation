...
/client/proc/play_sound_real(S as sound, var/vol as num, var/freq as num)
	...
	logTheThing("admin", src, "played sound [S]")
	logTheThing("diary", src, "played sound [S]", "admin")
...
/client/proc/play_music_real(S as sound, var/freq as num)
	...
	SPAWN(0)
		...
		dj_panel.move_admin_sound_channel()
	logTheThing("admin", src, "started loading music [S]")
	logTheThing("diary", src, "started loading music [S]", "admin")
...
/client/proc/play_music_radio(soundPath, var/name)
	...
	logTheThing("admin", src, "started loading music [soundPath], by the name of: [name]")
	logTheThing("diary", src, "started loading music [soundPath], by the name of: [name]", "admin")
...
/proc/play_music_remote(data)
	...
	if (adminC)
		logTheThing("admin", adminC, "loaded remote music: [data["file"]] ([data["filesize"]])")
		logTheThing("diary", adminC, "loaded remote music: [data["file"]] ([data["filesize"]])", "admin")
		message_admins("[key_name(adminC)] loaded remote music: [data["title"]] ([data["duration"]] / [data["filesize"]])")
	else
		logTheThing("admin", data["key"], "loaded remote music: [data["file"]] ([data["filesize"]])")
		logTheThing("diary", data["key"], "loaded remote music: [data["file"]] ([data["filesize"]])", "admin")
...
/client/proc/play_youtube_audio()
	...
	if (response.errored || !response.body)
		...
		logTheThing("debug", null, "<b>Youtube Error</b>: No response from server with video: <b>[video]</b>")
		logTheThing("diary", null, "Youtube Error: No response from server with video: [video]", "debug")
		return
