/datum/random_event/major/antag/antagonist
	...
	event_effect(var/source)
		...
		logTheThing("admin", null, "Setting up Antagonist Spawn event ([src.antagonist_type]). Source: [source ? "[source]" : "random"]")
		...
		if (src.antagonist_type == "Blob (AI)")
			...
			if (BS)
				...
				logTheThing("admin", null, "Antagonist Spawn spawned an AI blob at [log_loc(BS)]. Source: [source ? "[source]" : "random"]")
				..() // Report spawn().
				src.post_event()
				return
			else
	...
	proc/do_event(var/source)
		...
		if (!islist(candidates) || candidates.len <= 0)
			...
			logTheThing("admin", null, "Couldn't set up Antagonist Spawn ([src.antagonist_type]); no ghosts responded. Source: [source ? "[source]" : "random"]")
			src.post_event()
			return
		...
		if (!(lucky_dude && istype(lucky_dude) && lucky_dude.current))
			...
			logTheThing("admin", null, "Couldn't set up Antagonist Spawn ([src.antagonist_type]); candidate selection failed (had [candidates.len] candidate(s)). Source: [source ? "[source]" : "random"]")
			src.post_event()
			return
		...
		if (failed != 0)
			...
			logTheThing("admin", null, "Couldn't set up Antagonist Spawn ([src.antagonist_type]); respawn failed. Source: [source ? "[source]" : "random"]")
			src.post_event()
			return
		...
		*/
		...
		logTheThing("admin", lucky_dude.current, "respawned as a random event [src.antagonist_type]. Source: [source ? "[source]" : "random"]")
		src.post_event()
		return
