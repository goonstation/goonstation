/datum/random_event/major/antag/pirates
	name = "Pirates"
	...
	admin_call(var/source)
		...
		src.antag_count = input(usr, "How many Pirates to spawn? ([length(eligible_dead_player_list(allow_dead_antags = TRUE))] players eligible)", src.name, 0) as num|null
	...
	event_effect(var/source)
		...
		if (src.admin_override != 1)
			if (!source && (!ticker.mode || ticker.mode.latejoin_antag_compatible == 0 || late_traitors == 0))
				message_admins("Pirates are disabled in this game mode, aborting.")
				return
	...
	proc/do_event(var/source)
		if (!src || !istype(src, /datum/random_event/major/antag/pirates))
			return

		// 1: alert | 2: alert (chatbox) | 3: alert acknowledged (chatbox) | 4: no longer eligible (chatbox) | 5: waited too long (chatbox)
		var/list/text_messages = list()
		text_messages.Add("Would you like to respawn as a pirate? Your name will be added to the list of eligible candidates and may be selected at random by the game.")
		text_messages.Add("You are eligible to be respawned as a pirate. You have [src.ghost_confirmation_delay / 10] seconds to respond to the offer.")
		...
		if (!islist(candidates) || !length(candidates))
			message_admins("Couldn't set Pirate; no ghosts responded. Source: [source ? "[source]" : "random"]")
			logTheThing(LOG_ADMIN, null, "Couldn't set up Pirate; no ghosts responded. Source: [source ? "[source]" : "random"]")
			src.post_event()
			return
		...
		if(!admin_override && (length(candidates) < src.minimum_count) )
			...
			logTheThing(LOG_ADMIN, null, "Couldn't set up Pirate; Insufficient ghosts ghosts responded. Source: [source ? "[source]" : "random"]")
			src.post_event()
			return
		...
		for(var/antag_idx in 1 to src.antag_count)
			...
			if (!(lucky_dude && istype(lucky_dude) && lucky_dude.current))
				message_admins("Couldn't set up Pirate; candidate selection failed (had [candidates.len] candidate(s)). Source: [source ? "[source]" : "random"]")
				logTheThing(LOG_ADMIN, null, "Couldn't set up Pirate); candidate selection failed (had [candidates.len] candidate(s)). Source: [source ? "[source]" : "random"]")
				src.post_event()
				return

			candidates -= lucky_dude
			log_respawn_event(lucky_dude, "Pirate", source)
			...
			var/pirate_rank = ROLE_PIRATE
			switch(pirates_assigned)
				if (0)
					pirate_rank = ROLE_PIRATE_CAPTAIN
				if (1)
					pirate_rank = ROLE_PIRATE_FIRST_MATE

			if (istype(L))
				L.mind?.wipe_antagonists()
				L.mind?.add_antagonist(pirate_rank, do_equip = TRUE, do_relocate = TRUE, source = ANTAGONIST_SOURCE_RANDOM_EVENT)
			else
				failed = TRUE

			if (failed)
				message_admins("Couldn't set up Pirate Spawn; respawn failed. Source: [source ? "[source]" : "random"]")
				logTheThing(LOG_ADMIN, null, "Couldn't set up Pirate Spawn; respawn failed. Source: [source ? "[source]" : "random"]")
				antag_idx--
				return

			lucky_dude.assigned_role = "MODE"
			lucky_dude.special_role = pirate_rank
			lucky_dude.random_event_special_role = 1
			if (!(lucky_dude in ticker.mode.Agimmicks))
				ticker.mode.Agimmicks.Add(lucky_dude)

			if (lucky_dude.current)
				lucky_dude.current.show_text("<h3>You have been respawned as a Pirate.</h3>", "blue")
			message_admins("[key_name(lucky_dude.key)] respawned as a random event Pirate. Source: [source ? "[source]" : "random"]")
		src.post_event()
