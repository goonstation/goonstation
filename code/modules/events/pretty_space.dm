...
/datum/random_event/major/pretty_space
	...
	admin_call(var/source)
		...
		color = usr.client?.input_data(list(DATA_INPUT_JSON), "Enter color matrix", null, null)
		duration = usr.client?.input_data(list(DATA_INPUT_NUM), "Enter duration (in seconds)", null, null) * 1 SECOND
