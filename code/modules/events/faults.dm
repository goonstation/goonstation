	...
	event_effect(source, amount)
		...
		if(length(possible_cameras) >= amount)
			...
			SPAWN(rand(30 SECONDS, 70 SECONDS))
				pda_msg( "Camera failure(s) detected.  Investigate: [jointext(areas, ", ")]", sender_name="CAMERA-DAEMON", group=list(MGT_AI, MGD_SILICON))
...
/datum/random_event/minor/fault/door
	...
	event_effect(source, amount)
		...
		if(length(possible_doors) >= amount)
			...
			SPAWN(rand(30 SECONDS, 70 SECONDS))
				pda_msg( "Door failure(s) detected. Investigate: [jointext(areas, ", ")]", sender_name="EQUIPMENT-DAEMON", group=list(MGD_SILICON, MGD_ENGINEER))
