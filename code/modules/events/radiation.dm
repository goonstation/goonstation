...
/obj/anomaly/neutron_burst
	...
	New(var/loc,var/lifespan = 45)
		...
		SPAWN(lifespan)
			playsound(src,pulse_sound,50,TRUE)
			irradiate_turf(get_turf(src))
			shoot_projectile_ST_pixel_spread(get_turf(src), new/datum/projectile/neutron(10), get_step_rand(get_turf(src)), spread_angle = 360, firemode = new/datum/firemode/ten_burst)
			for (var/turf/T in circular_range(src,pulse_range))
				irradiate_turf(T)
			SPAWN(0)
				qdel(src)
