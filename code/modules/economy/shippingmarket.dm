...
/datum/shipping_market
	...
	proc/sell_crate(obj/storage/crate/sell_crate, var/list/commodities_list)
		...
		if(scan && account)
			...
			pdaSignal.data = list("address_1"="00000000", "command"="text_message", "sender_name"="CARGO-MAILBOT", "tag" = PDA_CARGO_ALERT_TAG, "group"=MGD_CARGO, "sender"="00000000", "message"="Notification: [duckets] credits earned from last outgoing shipment. Splitting half of profits with [scan.registered].")
		else
			wagesystem.shipping_budget += duckets
			pdaSignal.data = list("address_1"="00000000", "command"="text_message", "sender_name"="CARGO-MAILBOT", "tag" = PDA_CARGO_ALERT_TAG, "group"=MGD_CARGO, "sender"="00000000", "message"="Notification: [duckets] credits earned from last outgoing shipment.")
	...
	proc/receive_crate(obj/storage/S)
		...
		var/datum/radio_frequency/transmit_connection = radio_controller.return_frequency("1149")
		var/datum/signal/pdaSignal = get_free_signal()
		pdaSignal.data = list("address_1"="00000000", "command"="text_message", "sender_name"="CARGO-MAILBOT", "tag" = PDA_CARGO_ALERT_TAG, "group"=MGD_CARGO, "sender"="00000000", "message"="Shipment arriving to Cargo Bay: [S.name].")
		pdaSignal.transmission_method = TRANSMISSION_RADIO
		transmit_connection.post_signal(null, pdaSignal)
