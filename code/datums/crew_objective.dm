...
/datum/objective/crew/bartender/shotgun
	...
	check_completion()
		if(owner.current?.check_contents_for(/obj/item/gun/kinetic/single_action/mts_255))
			return TRUE
		else
			return FALSE
...
/datum/objective/crew/geneticist/budget
	explanation_text = "End the round with the Medical budget over 30,000 credits."
	var/static/check_result = null
	check_completion()
		if(isnull(check_result))
			check_result = FALSE
			if (wagesystem.budgets[BUDGET_CAT_DEPT_MEDICAL] >= 30000)
				check_result = TRUE
		return check_result
