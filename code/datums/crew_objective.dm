...
/datum/objective/crew/headofsecurity
	...
	centcom
		...
		check_completion()
			for(var/datum/mind/M in ticker.minds)
				if(M.special_role && M.current && !isobserver(M.current) && in_centcom(M.current.loc) && M.current.hasStatus("handcuffed"))
					if(owner.current && !isdead(owner.current) && in_centcom(owner.current.loc)) //split this up as it was long
						return 1
			return 0
...
/datum/objective/crew/headofpersonnel
	vanish
		...
		medal_name = "Unperson"
		check_completion()
			if(owner.current && !isdead(owner.current) && owner.current.z != 1 && !in_centcom(owner.current.loc)) return 1
			else return 0
	...
	onfire
		...
		check_completion()
			if(owner.current && !isdead(owner.current) && ishuman(owner.current))
				var/mob/living/carbon/human/H = owner.current
				if(in_centcom(H.loc) && H.getStatusDuration("burning") > 1 && owner.current.reagents.has_reagent("silver_sulfadiazine")) return 1
				else return 0
...
/datum/objective/crew/staffassistant
	...
	wearbutt
		...
		check_completion()
			if(owner.current && !isdead(owner.current) && ishuman(owner.current))
				var/mob/living/carbon/human/H = owner.current
				if(in_centcom(H.loc) && H.head && H.head.name == "[H.real_name]'s butt") return 1
			return 0
	promotion
		...
		check_completion()
			if(owner.current && !isdead(owner.current) && ishuman(owner.current))
				var/mob/living/carbon/human/H = owner.current
				if(in_centcom(H.loc) && H.wear_id && H.wear_id:registered == H.real_name && H.wear_id:assignment != ("Technical Assistant" || "Staff Assistant" || "Medical Assistant")) return 1
				else return 0
	clown
	...
	monkey
		...
		check_completion()
			if(owner.current && !isdead(owner.current) && in_centcom(owner.current.loc) && ismonkey(owner.current)) return 1
			else return 0
...
/datum/objective/crew/technicalassistant
	...
	promotion
		...
		medal_name = "Glass ceiling"
		check_completion()
			if(owner.current && !isdead(owner.current) && in_centcom(owner.current.loc)) //checking basic stuff - they escaped alive and have an ID
				var/mob/living/carbon/human/H = owner.current
