		...
		if (src.temp)
			...
			<table>
				...
				<tbody>
					...
					<tr><th>Total Funds</th><th class='r'>[num2text(round(wagesystem.research_budget),50)][CREDIT_SIGN]</th></tr>
		...
		if ((usr.contents.Find(src) || (in_interact_range(src, usr) && istype(src.loc, /turf))) || (usr_is_robot))
			...
			if (src.authenticated)
				if (href_list["list"])
					...
					if (t1 > 10000)
						t1 = 10000
						boutput(usr, "<span class='alert'>Maximum wage is [CREDIT_SIGN]10,000.</span>")
					logTheThing("station", usr, null, "sets <b>[R["name"]]</b>'s wage to [t1][CREDIT_SIGN].")
					R["wage"] = t1
					...
					if (t2 == "Withdraw")
						...
						logTheThing("station", usr, null, "adds [t1][CREDIT_SIGN] to the station budget from <b>[R["name"]]</b>'s account.")
						...
						R["current_money"] += t1
						wagesystem.station_budget -= t1
						logTheThing("station", usr, null, "adds [t1][CREDIT_SIGN] to <b>[R["name"]]</b>'s account from the station budget.")
