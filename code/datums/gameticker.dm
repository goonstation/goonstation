...
/datum/controller/gameticker/proc/setup()
	...
	logTheThing("debug", null, "Chosen game mode: [mode] ([master_mode]) on map [getMapNameFromID(map_setting)].")
	...
	SPAWN(0)
		...
		logTheThing("ooc", null, "<b>Current round begins</b>")
...
/datum/controller/gameticker
			...
			if (player.ready)
				...
				if (player.mind && player.mind.assigned_role == "AI")
					...
					if (W)
						...
						logTheThing("debug", W, "<b>Late join</b>: assigned antagonist role: wraith.")
					...
					if (B)
						...
						logTheThing("debug", B, "<b>Late join</b>: assigned antagonist role: blob.")
					...
					if (F)
						...
						logTheThing("debug", F, "<b>Late join</b>: assigned antagonist role: flockmind.")
		...
		for (var/mob/player in mobs)
			...
			if (player.mind && !istype(player, /mob/new_player) && player.client)
				if (!(player.mind in ticker.minds))
					if (periodic_check == 1)
						logTheThing("debug", player, "<b>Gameticker fallback:</b> re-added player to ticker.minds.")
					else
						logTheThing("debug", player, "<b>Gameticker setup:</b> added player to ticker.minds. [player.mind.on_ticker_add_log()]")
				...
				if(istype(H))
					...
					SPAWN(5 SECONDS)
						if(H.organHolder && H.organHolder.skull)
							...
							logTheThing("debug", H, "has the dubious pleasure of having a key embedded in their skull.")
						else
							skull_key_assigned = 0
		else if(!skull_key_assigned)
			logTheThing("debug", null, "<B>SpyGuy/collar key:</B> Did not implant a key because there was not enough players.")
	...
	proc/process()
		...
		if(mode.check_finished())
			...
			//logTheThing("debug", null, "Zamujasa: [world.timeofday] Starting declare_completion.")
			try
				declare_completion()
			catch(var/exception/e)
				logTheThing("debug", null, "Game Completion Runtime: [e.file]:[e.line] - [e.name] - [e.desc]")
				logTheThing("diary", null, "Game Completion Runtime: [e.file]:[e.line] - [e.name] - [e.desc]", "debug")

			//logTheThing("debug", null, "Zamujasa: [world.timeofday] Finished declare_completion. The round is now over.")
			...
			SPAWN(5 SECONDS)
				//logTheThing("debug", null, "Zamujasa: [world.timeofday] game-ending spawn happening")
				...
				//logTheThing("debug", null, "Zamujasa: [world.timeofday] one minute delay, game should restart now")
				if (game_end_delayed == 1)
					...
					//logTheThing("debug", null, "Zamujasa: [world.timeofday] REBOOTING THE SERVER!!!!!!!!!!!!!!!!!")
					Reboot_server()
...
/datum/controller/gameticker/proc/declare_completion()
	...
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] statlog_traitors")
	statlog_traitors()
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] statlog_ailaws")
	statlog_ailaws(0)
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] round_end_data")
	...
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] Processing end-of-round generic medals")
...
#ifdef CREW_OBJECTIVES
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] Processing crew objectives")
	...
	for (var/datum/mind/crewMind in minds)
		...
		for (var/datum/objective/crew/CO in crewMind.objectives)
			...
			if(CO.check_completion())
				...
				logTheThing("diary", crewMind, "completed objective: [CO.explanation_text]")
				...
				logTheThing("diary", crewMind, "failed objective: [CO.explanation_text]. Bummer!")
				allComplete = 0
				crewMind.all_objs = 0
	...
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] mode.declare_completion()")
	mode.declare_completion()
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] mode.declare_completion() done - calculating score")

	score_tracker.calculate_score()
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] score calculated")
	...
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] ai law display")
	...
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] resetting gauntlet (why? who cares! the game is over!)")
	if (gauntlet_controller.state)
		gauntlet_controller.resetArena()
#ifdef CREW_OBJECTIVES
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] displaying completed crew objectives")
...
#endif
#ifdef MISCREANTS
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] displaying miscreants")
	...
	//logTheThing("debug", null, "Zamujasa: [world.timeofday] processing spacebux updates")

	var/time = world.time

	logTheThing("debug", null, "Revving up the spacebux loop...")
	...
	for(var/mob/player in mobs)
		if (player?.client && player.mind && !player.mind.joined_observer && !istype(player,/mob/new_player))
			logTheThing("debug", null, "Iterating on [player.client]")
			//logTheThing("debug", null, "Zamujasa: [world.timeofday] spacebux calc start: [player.mind.ckey]")
			...
			//logTheThing("debug", null, "Zamujasa: [world.timeofday] spacebux calc finish: [player.mind.ckey]")

			if(player.client)
				if (player_loses_held_item)
					logTheThing("debug", null, "[player.ckey] lost held item")
	...
	SPAWN(0)
		...
		logTheThing("debug", null, "Done with spacebux")
	...
	logTheThing("debug", null, "Done with books")

	award_archived_round_xp()

	logTheThing("debug", null, "Spawned XP")

	SPAWN(0)
		//logTheThing("debug", null, "Zamujasa: [world.timeofday] creds/new")
		var/chui/window/crew_credits/creds = new
		//logTheThing("debug", null, "Zamujasa: [world.timeofday] displaying tickets and scores")
		for(var/mob/E in mobs)
			if(E.client)
				if (E.client.preferences.view_tickets)
					//logTheThing("debug", null, "Zamujasa: [world.timeofday] sending tickets to [E.ckey]")
					E.showtickets()
					//logTheThing("debug", null, "Zamujasa: [world.timeofday] done sending tickets to [E.ckey]")

				if (E.client.preferences.view_score)
					//logTheThing("debug", null, "Zamujasa: [world.timeofday] sending crew credits to [E.ckey]")
					creds.Subscribe(E.client)
					//logTheThing("debug", null, "Zamujasa: [world.timeofday] done crew credits to [E.ckey]")
				SPAWN(0) show_xp_summary(E.key, E)

		//logTheThing("debug", null, "Zamujasa: [world.timeofday] done showing tickets/scores")

	logTheThing("debug", null, "Did credits")

	//logTheThing("debug", null, "Zamujasa: [world.timeofday] finished spacebux updates")
	...
	try
		...
		logTheThing("debug", null, "playtime was unable to be logged because of: [e.name]")
		logTheThing("diary", null, "playtime was unable to be logged because of: [e.name]", "debug")
