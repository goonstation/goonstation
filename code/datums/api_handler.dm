...
/datum/apiHandler
	...
	New()
		...
		if (!config.goonhub_api_endpoint)
			src.enabled = 0
			logTheThing("debug", null, "Goonhub endpoint doesn't exist, disabled api handler")
			logTheThing("diary", null, "Goonhub endpoint doesn't exist, disabled api handler", "debug")
	...
	proc/queryAPI(route = "", query = list(), forceResponse = 0, attempt = 1, forceErrorException = 0)
		...
		if (response.errored || !response.body)
			logTheThing("debug", null, "<b>API Error</b>: No response from server during query [!response.body ? "during" : "to"] <b>[safeReq]</b> (Attempt: [attempt])")
			logTheThing("diary", null, "API Error: No response from server during query [!response.body ? "during" : "to"] [safeReq] (Attempt: [attempt])", "debug")
		...
		if (forceResponse)
			...
			if (!data)
				logTheThing("debug", null, "<b>API Error</b>: JSON decode error during <b>[safeReq]</b> (Attempt: [attempt])")
				logTheThing("diary", null, "API Error: JSON decode error during [safeReq] (Attempt: [attempt])", "debug")
