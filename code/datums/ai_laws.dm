...
/datum/ai_rack_manager
	...
	proc/register_new_rack(var/obj/machinery/lawrack/new_rack)
		...
		logTheThing("station", src, "[src] registers a new law rack [constructName(new_rack)]")
		if(isnull(src.default_ai_rack) && !istype(new_rack,/obj/machinery/lawrack/syndicate)) //syndi rack can't be default
			...
			for (var/mob/living/silicon/S in mobs)
				if(!S.emagged && S.law_rack_connection == null && !S.syndicate)
					S.law_rack_connection = src.default_ai_rack
					logTheThing("station", new_rack, "[S.name] is connected to the rack [constructName(new_rack)]")
					...
					S.show_laws()
			#endif
			logTheThing("station", src, "the law rack [constructName(new_rack)] claims default rack!")
			...
			if(!src.first_registered)
				...
				logTheThing("station", src, "the law rack [constructName(new_rack)] claims first registered, and gets Asimov laws!")
		...
		if(isnull(src.default_ai_rack_syndie) && istype(new_rack,/obj/machinery/lawrack/syndicate)) //but it can be syndie default
			...
			for (var/mob/living/silicon/S in mobs)
				if(!S.emagged && S.law_rack_connection == null && S.syndicate)
					...
					S.show_laws()
			#endif
			logTheThing("station", src, "the law rack [constructName(new_rack)] claims default SYNDICATE rack!")
			...
			if(!src.first_registered_syndie)
				...
				logTheThing("station", src, "the law rack [constructName(new_rack)] claims first registered SYNDICATE, and gets Syndicate laws!")
	...
	proc/unregister_rack(var/obj/machinery/lawrack/dead_rack)
		logTheThing("station", src, "[src] unregisters the law rack [constructName(dead_rack)]")

		if(src.default_ai_rack == dead_rack)
			//ruhoh
			src.default_ai_rack = null
			logTheThing("station", src, "[src] unregisters the DEFAULT law rack [constructName(dead_rack)]")
		if(src.default_ai_rack_syndie == dead_rack)
			//ruhoh
			src.default_ai_rack_syndie = null
			logTheThing("station", src, "[src] unregisters the DEFAULT SYNDICATE law rack [constructName(dead_rack)]")
		//remove from list
		...
		for (var/mob/living/intangible/aieye/E in mobs)
			if(E.mainframe?.law_rack_connection == dead_rack)
				...
				logTheThing("station", E.mainframe, "[E.mainframe.name] loses connection to the rack [constructName(dead_rack)] and now has no laws")
