...
/datum/intercept_text/proc/pick_fingerprints()
	...
	if (!istype(dude) || !dude.bioHolder)
		return "CLASSIFIED"
	var/print = "[dude.bioHolder.default_fingerprints?.id]"
	return print
...
/datum/intercept_text/proc/build_rev(var/datum/mind/correct_mind)
	...
	if(prob(prob_right_job) && correct_mind?.assigned_role != "MODE")
		...
		job_tmp.Remove("Captain", "Security Guard", "Security Assistant", "Vice Officer", "Detective", "Head Of Security", "Head of Personnel", "Chief Engineer", "Research Director", "MODE")
