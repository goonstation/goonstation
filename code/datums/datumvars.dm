...
/client/proc/modify_variable(datum/D, variable, set_global = 0)
	...
	var/class = input("What kind of variable?","Variable Type",default) as null|anything in list("text",
		"num","type","reference","mob reference","turf by coordinates","reference picker","new instance of a type","icon","file","color","list","number list","edit referenced object","create new list", "matrix","null", "ref", "restore to default")
	...
	switch(class)
		...
		if("number list")
			var/list/the_list = src.create_number_list()
			if(set_global)
				for(var/x in world)
					if(!istype(x, D.type)) continue
					x:vars[variable] = the_list
					LAGCHECK(LAG_LOW)
			else
				if(D == "GLOB")
					global.vars[variable] = the_list
				else
					D.vars[variable] = the_list
