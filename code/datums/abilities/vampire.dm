...
/datum/abilityHolder/vampire
	...
	proc/change_vampire_blood(var/change = 0, var/total_blood = 0, var/set_null = 0)
		if (total_blood)
			if (src.vamp_blood < 0)
				src.vamp_blood = 0
				if (haine_blood_debug) logTheThing("debug", owner, "<b>HAINE BLOOD DEBUG:</b> [owner]'s vamp_blood dropped below 0 and was reset to 0")
			...
			if (src.points < 0)
				src.points = 0
				if (haine_blood_debug) logTheThing("debug", owner, "<b>HAINE BLOOD DEBUG:</b> [owner]'s vamp_blood_remaining dropped below 0 and was reset to 0")
	...
	proc/transmit_thrall_msg(var/message,var/mob/sender)
		...
		logTheThing("diary", sender, "(GHOULSPEAK): [message]", "ghoulsay")
	...
	proc/make_thrall(var/mob/victim)
		if (ishuman(victim))
			...
			logTheThing("combat", owner, "enthralled [constructTarget(M,"combat")] at [log_loc(owner)].")
