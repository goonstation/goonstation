...
/obj/storage/closet/coffin/vampire
	...
	attackby(obj/item/I, mob/user)
		...
		if (_health <= 0)
			logTheThing("combat", user, "destroyed [src] at [log_loc(src)]")
			bust_out()
...
/datum/targetable/vampire/mark_coffin
	...
	cast(turf/target)
		...
		logTheThing("combat", M, "marks coffin on tile on [constructTarget(target,"combat")] at [log_loc(M)].")
		return 0
...
/datum/targetable/vampire/coffin_escape
	...
	cast(mob/target)
		...
		logTheThing("combat", M, "begins escaping to a coffin from [log_loc(M)] to [log_loc(V.coffin_turf)].")
