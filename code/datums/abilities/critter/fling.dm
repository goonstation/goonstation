//it's wrestler throw, but for gorillas
/datum/targetable/critter/fling
	name = "Fling"
	desc = "Spin a grabbed opponent around and fling them, ripping off a limb"
	...
	cast(mob/target)
		...
		if (M && HH)
			...
			logTheThing(LOG_COMBAT, M, "uses the [name] wrestling move on [constructTarget(target,"combat")] at [log_loc(M)].")
