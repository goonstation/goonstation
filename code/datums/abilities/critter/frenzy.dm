...
/datum/targetable/critter/frenzy
	...
	cast(atom/target)
		...
		if (!is_incapacitated(MT))
			...
			return 1
		playsound(holder.owner, "sound/voice/animal/greslours_roar.ogg", 80, 1)
		disabled = 1
		SPAWN_DBG(0)
			...
			while (frenz > 0 && MT && !MT.disposed)
				...
				if (is_incapacitated(holder?.owner))
					break
				playsound(holder.owner, "sound/voice/animal/greslours_maul.ogg", 80, 1)
