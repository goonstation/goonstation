...
/datum/action/invisibility
	...
	onUpdate()
		...
		if (ability && owner && state == ACTIONSTATE_RUNNING)
			var/mob/M = owner
			APPLY_ATOM_PROPERTY(M, PROP_INVISIBILITY, ability, ability.inv_level)
	...
	onInterrupt(var/flag = 0)
		...
		if (ability)
			...
			var/mob/M = owner
			REMOVE_ATOM_PROPERTY(M, PROP_INVISIBILITY, ability)
		if (iicon)
			del iicon
		qdel(src)
...
/datum/targetable/critter/fadeout
	...
	cast(atom/target)
		...
		SPAWN(wait)
			APPLY_ATOM_PROPERTY(holder.owner, PROP_INVISIBILITY, src, inv_level)
	...
	proc/fade_in()
		if (holder.owner)
			...
			doCooldown()
			SPAWN(linger_time)
				REMOVE_ATOM_PROPERTY(holder.owner, PROP_INVISIBILITY, src)
