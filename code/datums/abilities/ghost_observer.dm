...
/datum/targetable/ghost_observer/manifest
	...
	proc/start_spooking()
		...
		if (istype(holder, /datum/abilityHolder/ghost_observer))
			...
			GAH.spooking = 1
		REMOVE_ATOM_PROPERTY(src.holder.owner, PROP_INVISIBILITY, src.holder.owner)
	...
	proc/stop_spooking()
		...
		if (istype(holder, /datum/abilityHolder/ghost_observer))
			var/datum/abilityHolder/ghost_observer/GAH = holder
			GAH.spooking = 0
		APPLY_ATOM_PROPERTY(src.holder.owner, PROP_INVISIBILITY, src.holder.owner, ghost_invisibility)
