...
/datum/action/bar/private/icon/werewolf_feast
	...
	onUpdate()
		...
		if (complete >= 0.7 && last_complete < 0.7)
			...
			if (!isdead(target) && !(istype(target, /mob/living/carbon/human/npc/monkey)) // Can't farm NPC monkeys. Player monkeys are still able to be mauled. Also removed redundant bioholder check.
