/datum/abilityHolder/revenant
	...
	pointCheck(cost)
		...
		if (relay.points < 0) // Just-in-case fallback.
			logTheThing("debug", usr, "'s ability holder ([relay.type]) was set to an invalid value (points less than 0), resetting.")
			relay.points = 0
...
/datum/bioEffect/hidden/revenant
	...
	proc/wraithPossess(var/mob/wraith/W)
		...
		logTheThing("combat", usr, "possessed the corpse of [owner] as a revenant at [log_loc(owner)].")
	...
	proc/RevenantDeath()
		...
		logTheThing("combat", usr, "died as a revenant at [log_loc(owner)].")
		if (owner.mind)
		...
		SPAWN(0.5 SECONDS)
			...
			while (holder.owner.loc == location && isalive(holder.owner) && !holder.owner.equipped())
				...
				if (iterations > 12 && prob((iterations - 12) * 5))
					...
					logTheThing("combat", holder.owner, "gibs [constructTarget(H,"combat")] with the Revenant crush ability at [log_loc(holder.owner)].")
					H.gib()
					return
				sleep(0.7 SECONDS)
			holder.owner.show_message("<span class='alert'>You were interrupted!</span>")
