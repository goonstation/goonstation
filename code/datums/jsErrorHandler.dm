...
* 2. Include the "browserassets/src/js/errorHandler.js" file in your html file (if using chui, skip this step) (look at browserOutput.html for an example)
...
/datum/debugFileOutput
	...
	proc/error(fileName, message, client/C)
		...
		message = "\[[time2text(world.realtime, "YYYY-MM-DD hh:mm:ss")]\] Client: \[[C?.get_key() ? C.get_key() : "Unknown Client"]\] triggered: [message]"
		logFile << message
		return 1
