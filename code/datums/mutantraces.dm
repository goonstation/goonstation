...
/datum/mutantrace/lizard
	...
	emote(var/act, var/voluntary)
		switch(act)
			...
			// if anyone wants to figure out how to get the tails to flip direction, heres the emote base for it
			if ("hiss")
				if (mob.emote_check(voluntary, 50))
					. = list("<B>[mob]</B> hisses", "<i>hisses</i>")
					playsound(get_turf(mob), "sound/voice/animal/cat_hiss.ogg", 40, 0, 0, mob.get_age_pitch(), channel=VOLUME_CHANNEL_EMOTE)
			if ("tongue")
				if (mob.emote_check(voluntary, 50))
...
/datum/mutantrace/roach
	...
	emote(var/act, var/voluntary)
		switch(act)
			if ("chirp")
				if (mob.emote_check(voluntary, 40))
					. = list("<B>[mob]</B> chirps", "<i>chirps</i>")
					playsound(get_turf(mob), "sound/voice/animal/bugchitter.ogg", 100, 0, 0, mob.get_age_pitch(), channel=VOLUME_CHANNEL_EMOTE)
			if ("hiss")
				if (mob.emote_check(voluntary, 50))
					. = list("<B>[mob]</B> hisses", "<i>hisses</i>")
					playsound(get_turf(mob), "sound/voice/animal/cat_hiss.ogg", 40, 0, 0, mob.get_age_pitch(), channel=VOLUME_CHANNEL_EMOTE)
			if ("click")
				if (mob.emote_check(voluntary, 50))
					. = list("<B>[mob]</B> clicks", "<i>clicks</i>")
					playsound(get_turf(mob), pick("sound/effects/fingersnap_echo.ogg","sound/voice/animal/butterflyscream.ogg"), 50, 0, 0, mob.get_age_pitch(), channel=VOLUME_CHANNEL_EMOTE)
			else
				.= ..()
		...
		switch(act)
			...
			if ("swish")
				if (mob.emote_check(voluntary, 50))
					...
					playsound(get_turf(mob), "sound/effects/poof.ogg", 100, 1, channel=VOLUME_CHANNEL_EMOTE)
			else
				.= ..()
