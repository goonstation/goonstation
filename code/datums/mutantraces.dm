...
/datum/mutantrace/abomination
	...
	New(var/mob/living/carbon/human/M)
		if(ruff_tuff_and_ultrabuff && ishuman(M))
			...
			APPLY_ATOM_PROPERTY(M, PROP_STAMINA_REGEN_BONUS, "abomination", 1000)
			APPLY_ATOM_PROPERTY(M, PROP_STUN_RESIST, "abomination", 100)
			APPLY_ATOM_PROPERTY(M, PROP_STUN_RESIST_MAX, "abomination", 100)
...
/datum/mutantrace/pug
	...
	New(var/mob/living/carbon/human/H)
		...
		if (ishuman(mob))
			mob.mob_flags |= SHOULD_HAVE_A_TAIL
			SPAWN(0)
				APPLY_ATOM_PROPERTY(mob, PROP_FAILED_SPRINT_FLOP, src)
		if (prob(50))
			voice_override = "pugg"
	...
	disposing()
		if (ishuman(mob))
			if (mob.mob_flags & SHOULD_HAVE_A_TAIL)
				mob.mob_flags &= ~SHOULD_HAVE_A_TAIL
			REMOVE_ATOM_PROPERTY(mob, PROP_FAILED_SPRINT_FLOP, src)
