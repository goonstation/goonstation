...
/datum/antagWeighter
	...
	proc/debugLog(msg)
		out(world, msg)
		//logTheThing("debug", null, "<b>AntagWeighter</b> [msg]")
	...
	*/
	...
	proc/choose(list/pool = list(), role = "", amount = 0, recordChosen = 0)
		...
		logTheThing("debug", null, "<b>AntagWeighter</b> Selecting [amount] out of [ckeyMinds.len] candidates for [role].")
		...
		if (recordChosen)
			...
			for (var/datum/mind/M in chosen)
				record[M.ckey] = role
				logTheThing("debug", null, "<b>AntagWeighter</b> Selected [M.ckey] for [role]. (Weight: [chosen[M]["weight"]], Seen: [chosen[M]["seen"]])")
			for (var/datum/mind/M in pool)
				if(!M.ckey)
					continue
				if(M in chosen)
					continue
				logTheThing("debug", null, "<b>AntagWeighter</b> Did <b>not</b> select [M.ckey] for [role]. (Weight: [history[M.ckey]["weight"]], Seen: [history[M.ckey]["seen"]])")
