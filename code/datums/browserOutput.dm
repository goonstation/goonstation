	...
	if (cookie != "none")
		...
		if (json_decode_crasher.Find(cookie))
			if (src.owner)
				...
				logTheThing("admin", src.owner, "just attempted to crash the server using at least 5 '\['s in a row.", "admin")
		...
		if (connData && islist(connData) && connData.len > 0 && connData["connData"])
			...
			if (found.len > 0 && found["ckey"] != src.owner.ckey)
				...
				logTheThing("debug", src.owner, "has a cookie from a banned account! (Matched: [found["ckey"]], [found["ip"]], [found["compid"]])")
				logTheThing("diary", src.owner, "has a cookie from a banned account! (Matched: [found["ckey"]], [found["ip"]], [found["compid"]])", "debug")
...
/datum/chatOutput/proc/handleContextMenu(command, target)
	...
	switch(command)
		...
		if ("gib")
			...
			logTheThing("admin", src.owner, "gibbed [constructTarget(targetMob,"admin")].")
		if ("popt")
			if(src.owner.holder)
