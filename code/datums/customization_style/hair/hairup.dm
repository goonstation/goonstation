ABSTRACT_TYPE(/datum/customization_style/hair/hairup)

/datum/customization_style/hair/hairup/bun
	name = "Bun"
	id = "bun"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/bundercut
	name = "Bun Undercut"
	id = "bundercut"
	gender = CUSTOMIZATION::GENDER::MASCULINE

/datum/customization_style/hair/hairup/sakura
	name = "Captor"
	id = "sakura"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/croft
	name = "Croft"
	id = "croft"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/indian
	name = "Double Braids"
	id = "indian"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/doublebun
	name = "Double Buns"
	id = "doublebun"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/drill
	name = "Drill"
	id = "drill"

/datum/customization_style/hair/hairup/fun_bun
	name = "Fun Bun"
	id = "fun_bun"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/charioteers
	name = "High Flat Top"
	id = "charioteers"
	gender = CUSTOMIZATION::GENDER::MASCULINE

/datum/customization_style/hair/hairup/spud
	name = "High Ponytail"
	id = "spud"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/longtailed
	name = "Long Mini Tail"
	id = "longtailed"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/longtwintail
	name = "Long Twin Tails"
	id = "longtwintail"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/longtwintail_half
	name = "Long Twin Tails: Split"
	id = "longtwintail_half"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/glamponytail
	name = "Glam Ponytail"
	id = "glamponytail"

/datum/customization_style/hair/hairup/glamponytail_half
	name = "Glam Ponytail: Split"
	id = "glamponytail_half"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/rockponytail
	name = "Rock Ponytail"
	id = "rockponytail"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/rockponytail_fade
	name = "Rock Ponytail: Faded"
	id = "rockponytail_fade"

/datum/customization_style/hair/hairup/rockponytail_half
	name = "Rock Ponytail: Split"
	id = "rockponytail_half"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/spikyponytail
	name = "Spiky Ponytail"
	id = "spikyponytail"
	gender = CUSTOMIZATION::GENDER::NEUTER

/datum/customization_style/hair/hairup/messyponytail
	name = "Messy Ponytail"
	id = "messyponytail"
	gender = CUSTOMIZATION::GENDER::NEUTER

/datum/customization_style/hair/hairup/untidyponytail
	name = "Untidy Ponytail"
	id = "untidyponytail"
	gender = CUSTOMIZATION::GENDER::NEUTER

/datum/customization_style/hair/hairup/lowpig
	name = "Low Pigtails"
	id = "lowpig"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/band
	name = "Low Ponytail"
	id = "band"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/minipig
	name = "Mini Pigtails"
	id = "minipig"
	gender = CUSTOMIZATION::GENDER::NEUTER

/datum/customization_style/hair/hairup/pig
	name = "Pigtails"
	id = "pig"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/pig_half
	name = "Pigtails: Split"
	id = "pig_half"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/pompompigtail
	name = "Pompom Pigtails"
	id = "pompompigtail"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/ponytail
	name = "Ponytail"
	id = "ponytail"
	gender = CUSTOMIZATION::GENDER::NEUTER

/datum/customization_style/hair/hairup/geisha_s
	name = "Shimada"
	id = "geisha_s"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/twotail
	name = "Split-Tails"
	id = "twotail"
	gender = CUSTOMIZATION::GENDER::MASCULINE

/datum/customization_style/hair/hairup/wavy_tail
	name = "Wavy Ponytail"
	id = "wavy_tail"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/wavy_tail_half
	name = "Wavy Ponytail: Split"
	id = "wavy_tail_half"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/puffball
	name = "Puffball"
	id = "puffball"

/datum/customization_style/hair/hairup/puffball_fade
	name = "Puffball: Faded"
	id = "puffball_fade"

/datum/customization_style/hair/hairup/puffball_split
	name = "Puffball: Split"
	id = "puffball_split"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/crown_braid
	name = "Crown Braid"
	id = "crown_braid"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/high_bun
	name = "High Bun"
	id = "high_bun"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/massive_braids
	name = "Massive Braids"
	id = "mega_braids"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/massive_braids_split
	name = "Massive Braids: Split"
	id = "mega_braids_split"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/chignon
	name = "Chignon"
	id = "chignon"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/french_twist
	name = "French Twist"
	id = "french_twist"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/hairpart_highpony
	name = "Hair Parts: High Ponytail"
	id = "spud_part"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/hairpart_wavypony
	name = "Hair Parts: Wavy Ponytail"
	id = "wavyponytail_part"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/hairpart_spikypony
	name = "Hair Parts: Spiky Ponytail"
	id = "spikyponytail_part"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/hairpart_messypony
	name = "Hair Parts: Messy Ponytail"
	id = "messyponytail_part"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/hairpart_pigtails
	name = "Hair Parts: Pigtails"
	id = "pig_part"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/hairpart_doublebuns
	name = "Hair Part: Double Buns"
	id = "doublebun_part"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/hairpart_highbun
	name = "Hair Part: High Bun"
	id = "high_bun_part"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/hairpart_crownbraid
	name = "Hair Part: Crown Braid"
	id = "crownbraid_part"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/scorpion_braid
	name = "Scorpion Braid"
	id = "scorpion_braid"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/scorpion_braid_alternating
	name = "Scorpion Braid: Alternating"
	id = "scorpion_braid_alternating"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/Side_Pigtails
	name = "Side Pigtails"
	id = "side_pigtails"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/Long_Top_Bun
	name = "Long Top Bun"
	id = "long_top_bun"
	gender = CUSTOMIZATION::GENDER::FEMININE

/datum/customization_style/hair/hairup/low_bun
	name = "Low Bun"
	id = "low_bun"

/datum/customization_style/hair/hairup/curtained_ponytail
	name = "Curtained Ponytail"
	id = "curtained_ponytail"

/datum/customization_style/hair/hairup/side_pigtailspart
	name = "Hair Parts: Side Pigtails"
	id = "side_pigtails_part"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/scorpion_braid_part
	name = "Hair Parts: Scorpion Braid"
	id = "scorpion_braid_part"
	random_allowed = FALSE

/datum/customization_style/hair/hairup/scorpion_braid_alternating_part
	name = "Hair Parts: Scorpion Braid Alternating"
	id = "scorpion_braid_alternating_part"
	random_allowed = FALSE
