	...
	proc/automatic_profiling(force_stop=FALSE, force_start=FALSE)
		...
		if(automatic_profiling_on)
			...
			if(highCpuCount <= 0 || force_stop)
				...
				logTheThing("debug", null, "Automatic profiling finished, CPU at [world.cpu], saved as [fname].")
			...
			if(highCpuCount >= CPU_START_PROFILING_COUNT || force_start)
				...
				logTheThing("debug", null, "Automatic profiling started, CPU at [world.cpu], map CPU at [world.map_cpu], last tick time at [time_since_last].")
