	...
	doWork()
		...
		for (var/datum/statusEffect/S as anything in globalStatusInstances)
			...
			if(S.owner)
				...
				logTheThing("debug", null, "Deleting orphaned status effect - type:[S.type], duration:[S.duration], OwnerInfo(was):[S.archivedOwnerInfo]")
				try
					S.onRemove()
				catch()
					logTheThing("debug", null, "Orphaned onRemove failed - type:[S.type]")
				globalStatusInstances -= S
