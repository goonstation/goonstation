	...
	onStart()
		...
		if(item)
			...
			if(existing_item) // if they have something there, smack it with held item
				...
				actions.start(new /datum/action/bar/icon/callback(source, target, 2.5 SECONDS, /mob/proc/click, list(existing_item, list()),  item.icon, item.icon_state, null, null, source), source) //this is messier
				interrupt(INTERRUPT_ALWAYS)
				return
