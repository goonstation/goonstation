...
/datum/job_controller/
	...
	Topic(href, href_list[])
		...
		if(href_list["AlterCap"])
			...
			logTheThing("admin", usr, "altered [JOB.name] job cap to [newcap]")
			logTheThing("diary", usr, "altered [JOB.name] job cap to [newcap]", "admin")
			src.job_config()
		...
		if(href_list["RemoveJob"])
			...
			logTheThing("admin", usr, "removed special job [JOB.name]")
			logTheThing("diary", usr, "removed special job [JOB.name]", "admin")
		...
		if(href_list["SpecialToggle"])
			...
			logTheThing("admin", usr, "toggled Special Jobs [src.allow_special_jobs ? "On" : "Off"]")
			logTheThing("diary", usr, "toggled Special Jobs [src.allow_special_jobs ? "On" : "Off"]", "admin")
			src.job_config()
		...
		if(href_list["CreateJob"])
			...
			if (match_check)
				...
				logTheThing("admin", usr, "created special job [JOB.name]")
				logTheThing("diary", usr, "created special job [JOB.name]", "admin")
...
/proc/find_job_in_controller_by_string(var/string,var/staple_only = 0)
	if (!string || !istext(string))
		logTheThing("debug", null, "<b>Job Controller:</b> Attempt to find job with bad string in controller detected")
		return null
	...
	if (!staple_only)
		...
		for (var/datum/job/J in job_controls.hidden_jobs)
			if (J.name == string || (string in J.alias_names))
				return J
	logTheThing("debug", null, "<b>Job Controller:</b> Attempt to find job by string \"[string]\" in controller failed")
	return null

/proc/find_job_in_controller_by_path(var/path)
	if (!path || !ispath(path) || !istype(path,/datum/job/))
		logTheThing("debug", null, "<b>Job Controller:</b> Attempt to find job with bad path in controller detected")
		return null
	for (var/datum/job/J in job_controls.staple_jobs)
		if (J.type == path)
			return J
	for (var/datum/job/J in job_controls.special_jobs)
		if (J.type == path)
			return J
	logTheThing("debug", null, "<b>Job Controller:</b> Attempt to find job by path \"[path]\" in controller failed")
	return null
