		...
		switch(action)
			...
			if ("update-pin")
				if (params["random"])
					...
					var/new_pin = tgui_input_number(usr, "Please select a PIN between [PIN_MIN] and [PIN_MAX]", "Character Generation", src.pin || PIN_MIN, PIN_MAX, PIN_MIN)
					if (new_pin)
						src.pin = clamp(round(text2num(new_pin)), PIN_MIN, PIN_MAX)
						src.profile_modified = TRUE
						return TRUE
