/atom/movable/screen/hud/adronestorage
...
/datum/hud/silicon/adrone
	...
	var/mob/living/silicon/adrone/master
	...
	New(M)
		...
		remove_screen(close)
		for (var/i = 1, i <= 7, i++)
			var/S = create_screen("object[i]", "object", null, null, "1, [10 - i]", HUD_LAYER + 1, customType = /atom/movable/screen/hud/adronestorage)
			remove_screen(S)
			screen_tools += S

		create_screen("", "", 'icons/mob/hud_common.dmi', "hotbar_bg", "CENTER-4, SOUTH to CENTER+4, SOUTH", HUD_LAYER)
		create_screen("", "", 'icons/mob/hud_common.dmi', "hotbar_side", "CENTER-4, SOUTH+1 to CENTER+4, SOUTH+1", HUD_LAYER, SOUTH)
		create_screen("", "", 'icons/mob/hud_common.dmi', "hotbar_side", "CENTER-5, SOUTH+1", HUD_LAYER, SOUTHWEST)
		create_screen("", "", 'icons/mob/hud_common.dmi', "hotbar_side", "CENTER-5, SOUTH", HUD_LAYER, EAST)
		create_screen("", "", 'icons/mob/hud_common.dmi', "hotbar_side", "CENTER+5, SOUTH+1", HUD_LAYER, SOUTHEAST)
		create_screen("", "", 'icons/mob/hud_common.dmi', "hotbar_side", "CENTER+5, SOUTH", HUD_LAYER, WEST)

		mod1 = create_screen("mod1", "Module 1", icon_hud, "mod10", "CENTER-4, SOUTH", HUD_LAYER+1)
		mod2 = create_screen("mod2", "Module 2", icon_hud, "mod20", "CENTER-3, SOUTH", HUD_LAYER+1)

		create_screen("store-d", "Store", icon_hud, "store-d", "CENTER-2, SOUTH", HUD_LAYER+1)
		...
		pulling = create_screen("pulling", "Pulling", icon_hud, "pull0-d", "CENTER+4, SOUTH", HUD_LAYER+1)

		health = create_screen("health", "Health", icon_hud, "health0-d", "EAST, NORTH")
	...
	update_health()
		...
		if (!isdead(master))
			...
			switch(master.health)
				if(25 to INFINITY)
					health.icon_state = "health0-d"
				if(20 to 25)
					health.icon_state = "health1-d"
				if(15 to 20)
					health.icon_state = "health2-d"
				if(10 to 15)
					health.icon_state = "health3-d"
				if(5 to 10)
					health.icon_state = "health4-d"
				if(0 to 5)
					health.icon_state = "health5-d"
				else
	...
	proc
		...
		update_tools()
			...
			if (tool1)
				add_object(tool1, HUD_LAYER+2, "CENTER-4, SOUTH")
			if (tool2)
				add_object(tool2, HUD_LAYER+2, "CENTER-3, SOUTH")
		...
		maptext_health_percent()

			var/dmg = master.bruteloss + master.fireloss
			var/pct = 100 - clamp(dmg / master.max_health * 100, 0, 100)
			return "<span style='color: [rgb(255 * clamp((100 - pct) / 50, 0, 1), 255 * clamp(pct / 50, 1, 0), 0)];'>[!mini_health ? "[add_lspace(round(pct), 3)]%" : "[add_lspace(round(master.max_health - dmg), 3)]</span>/<span style='color: #ffffff;'>[add_lspace(round(master.max_health), 3)]"]</span>"


		update_pulling()
			pulling.icon_state = "pull[master.pulling ? 1 : 0]-d"
...
/mob/living/silicon/adrone
	updateStatusUi()
		if(src.hud && istype(src.hud, /datum/hud/silicon/adrone))
			var/datum/hud/silicon/adrone/H = src.hud
