...
/datum/hud/human
	...
	relay_click(id, mob/user, list/params)
		switch (id)
			...
			if("belt")
				master.attempt_equip(SLOT_BELT, params)
			if("storage1")
				master.attempt_equip(SLOT_L_STORE, params)
			if("storage2")
				master.attempt_equip(SLOT_R_STORE, params)
			if("back")
				master.attempt_equip(SLOT_BACK, params)
			if("shoes")
				master.attempt_equip(SLOT_SHOES, params)
			if("gloves")
				master.attempt_equip(SLOT_GLOVES, params)
			if("id")
				master.attempt_equip(SLOT_WEAR_ID, params)
			if("under")
				master.attempt_equip(SLOT_W_UNIFORM, params)
			if("suit")
				master.attempt_equip(SLOT_WEAR_SUIT, params)
			if("glasses")
				master.attempt_equip(SLOT_GLASSES, params)
			if("ears")
				master.attempt_equip(SLOT_EARS, params)
			if("mask")
				master.attempt_equip(SLOT_WEAR_MASK, params)
			if("head")
				master.attempt_equip(SLOT_HEAD, params)
