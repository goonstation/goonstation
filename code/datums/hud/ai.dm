/datum/hud/silicon/ai
	...
	update_health()
		...
		health.maptext = "<span class='ol vga r' style='color: [rgb(255 * clamp((100 - pct) / 50, 0, 1), 255 * clamp(pct / 50, 1, 0), 0)];'>[add_leading(pct, 3)]%</span>"
		if (pct > 25)
	...
	proc
		...
		update_charge()
			if (master.cell)
				...
				cell.maptext = "<span class='ol vga r' style='color: [rgb(255 * clamp((100 - pct) / 50, 0, 1), 255 * clamp(pct / 50, 1, 0), 0)];'>[powertext][add_leading(pct,3)]%</span>"
