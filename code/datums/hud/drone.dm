/atom/movable/screen/hud/dronestorage
...
/datum/hud/silicon/drone
	...
	New(M)
		...
		remove_screen(close)
		for (var/i = 1, i <= 7, i++)
			var/S = create_screen("object[i]", "object", null, null, "1, [10 - i]", HUD_LAYER + 1, customType = /atom/movable/screen/hud/dronestorage)
			remove_screen(S)
			screen_tools += S
...
/mob/living/silicon/drone
	updateStatusUi()
		if(src.hud && istype(src.hud, /datum/hud/silicon/drone))
			var/datum/hud/silicon/drone/H = src.hud
