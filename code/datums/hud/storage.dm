/datum/hud/storage
	...
	proc/get_clicked_position(mob/user, list/params)
		...
		var/turfd = (isturf(master.linked_item.loc) && !istype(master.linked_item, /obj/item/holy_texts))
...
*/
	proc/update(mob/user = usr)
		...
		if (isturf(master.linked_item?.loc) && !istype(master.linked_item, /obj/item/holy_texts)) // goddamn TEXTS (prevents conflicting positions within different texts)
			x = 7
			y = 8
			sx = (num_contents + 1) / 2
			sy = 2
		...
		if (user && user.client?.tg_layout) //MBC TG OVERRIDE IM SORTY
			...
			if (turfd) // goddamn TEXTS (prevents conflicting positions within different texts)
				x = 8
				y = 8
				sx = (num_contents + 1) / 2
				sy = 2
