...
/datum/component/loctargeting/medium_light
	dupe_mode = COMPONENT_DUPE_UNIQUE_PASSARGS
	loctype = /mob
	var/r = 255
	var/g = 255
...
TYPEINFO(/datum/component/loctargeting/medium_light)
	initialization_args = list(
		ARG_INFO("r", DATA_INPUT_NUM, "Value of red component \[0-255\]", 255),
		ARG_INFO("g", DATA_INPUT_NUM, "Value of green component \[0-255\]", 255),
		ARG_INFO("b", DATA_INPUT_NUM, "Value of blue component \[0-255\]", 255),
		ARG_INFO("a", DATA_INPUT_NUM, "Alpha (brightness) component \[0-255\]", 127),
		ARG_INFO("enabled", DATA_INPUT_NUM, "Initial state of the medium light (bool)", FALSE)
	)
/datum/component/loctargeting/medium_light/Initialize(r = 255, g = 255, b = 255, a = 127, enabled = FALSE)
	. = ..()
	if(. == COMPONENT_INCOMPATIBLE)
		return
	src.r = r
...
/datum/component/loctargeting/medium_light/proc/update(var/new_enabled = NO_CHANGE)
	if(new_enabled != NO_CHANGE)
		src.enabled = new_enabled
	if(!src.enabled)
		light_target.remove_medium_light(src.light_name)
	else
		light_target.add_medium_light(src.light_name, list(r, g, b, a))

/datum/component/loctargeting/medium_light/RegisterWithParent()
	src.light_target = src.parent
	src.update()
	. = ..()

/datum/component/loctargeting/medium_light/UnregisterFromParent()
	src.update(FALSE)
	. = ..()

/datum/component/loctargeting/medium_light/proc/set_color(var/r, var/g, var/b)
	src.r = r
	src.g = g
	src.b = b
	src.update(FALSE)

/datum/component/loctargeting/medium_light/on_added(datum/source, old_loc)
	. = ..()
	if(!src.enabled)
		src.light_target = current_loc
		return
...
/datum/component/loctargeting/medium_light/on_removed(datum/source, old_loc)
	. = ..()
	var/obj/item/I = src.parent
	if(!src.enabled)
		src.light_target = I
...
/datum/component/loctargeting/medium_light/proc/enable()
	src.update(TRUE)

/datum/component/loctargeting/medium_light/proc/disable()
	src.update(FALSE)
