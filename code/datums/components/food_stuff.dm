		...
		if(istype(I, thing_i_ate))
			...
			switch(I.type)
				...
				if (/obj/item/organ/brain/synth)
					boutput(L, SPAN_NOTICE("Not quite as tasty as a <i>real</i> brain, but it tastes a lot less... prion-y."))
	...
	for(var/thing_i_ate in organ2hp)
		if(istype(I, thing_i_ate))
			...
			switch(I.type)
				...
				if (/obj/item/organ/brain/synth)
					boutput(M, SPAN_NOTICE("Not quite as tasty as a <i>real</i> brain, but it tastes a lot less... prion-y."))
				if (/obj/item/organ/chest)
