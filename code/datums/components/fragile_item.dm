...
TYPEINFO(/datum/component/fragile_item)
	initialization_args = list(
		...
		ARG_INFO("type_to_break_into", DATA_INPUT_NULL, "Path of stuff to break into", /obj/item/material/shard/glass),
...
/datum/component/fragile_item/Initialize(var/safe_hits = 3, var/probability_of_breaking = 40, var/stay_in_hand = 1, var/type_to_break_into = /obj/item/material/shard/glass, var/sound_to_play_on_breaking = 'sound/impact_sounds/Crystal_Shatter_1.ogg')
	. = ..()
	...
	if(safe_hits > 0)
		...
		if(prob(probability_of_breaking))
			...
			if(!type_to_break_into)
				user.visible_message(SPAN_ALERT("As [user] swings with the [I], a shattering sound echoes, leaving behind nothing but dust!"))
			else
				...
				user.visible_message(SPAN_ALERT("As [user] swings with the [I], a shattering sound echoes, leaving behind \a [new_object]!"))
...
/datum/component/fragile_item/proc/potentially_break_thrown(var/obj/item/thrown_item)
	if(safe_hits > 0)
		...
		if(prob(probability_of_breaking))
			SPAWN(0)
				if(!type_to_break_into)
					thrown_item.visible_message(SPAN_ALERT("As [thrown_item] stops, a shattering sound echoes, leaving nothing but dust!"))
				else
					var/new_object = new type_to_break_into(get_turf(thrown_item))
					thrown_item.visible_message(SPAN_ALERT("As [thrown_item] stops, a shattering sound echoes, leaving behind \a [new_object]!"))
