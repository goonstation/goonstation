...
/datum/component/bullet_holes
	...
	var/insert_idx = 1 // used so the first bullet hole is the first to be removed when we're at the limit
...
/datum/component/bullet_holes/proc/handle_impact(rendering_on, obj/projectile/shot)
	...
	// TODO let parent specify custom offsets
	...
	if (insert_idx > max_holes) // oh no, we're full. start inserting at the head of the array again
		insert_idx = 1
	src.impact_images[insert_idx++] = impact
	src.redraw_impacts()
