...
/datum/limb/bear
	...
	disarm(mob/target, var/mob/living/user)
		logTheThing("combat", user, "mauls [constructTarget(target,"combat")] with bear limbs (disarm intent) at [log_loc(user)].")
	...
	grab(mob/target, var/mob/living/user)
		logTheThing("combat", user, "mauls [constructTarget(target,"combat")] with bear limbs (grab intent) at [log_loc(user)].")
	...
	harm(mob/target, var/mob/living/user, var/no_logs = 0)
		if (no_logs != 1)
			logTheThing("combat", user, "mauls [constructTarget(target,"combat")] with bear limbs at [log_loc(user)].")
		var/obj/item/affecting = target.get_affecting(user)
...
/datum/limb/bear/zombie
	...
	disarm(mob/target, var/mob/living/user)
		logTheThing("combat", user, "mauls [constructTarget(target,"combat")] with zomb limbs (disarm intent) at [log_loc(user)].")
		user.visible_message("<span class='alert'>[user] mauls [target] while trying to disarm them!</span>")
		harm(target, user, 1)
...
/datum/limb/dualsaw
	...
	disarm(mob/target, var/mob/living/user)
		logTheThing("combat", user, "slashes [constructTarget(target,"combat")] with dual saw (disarm intent) at [log_loc(user)].")
		user.visible_message("<span class='alert'>[user] slashes [target] while trying to disarm them!</span>")
		harm(target, user, 1)

	grab(mob/target, var/mob/living/user)
		logTheThing("combat", user, "slashes [constructTarget(target,"combat")] with dual saw (grab intent) at [log_loc(user)].")
	...
	harm(mob/target, var/mob/living/user, var/no_logs = 0)
		...
		if (no_logs != 1)
			logTheThing("combat", user, "slashes [constructTarget(target,"combat")] with dual saw at [log_loc(user)].")
		var/obj/item/affecting = target.get_affecting(user)
		var/datum/attackResults/msgs = user.calculate_melee_attack(target, affecting, 6, 10, 0)
		user.attack_effects(target, affecting)
...
/datum/limb/brullbar
	...
	proc/accident(mob/target, mob/living/user)
		if(check_target_immunity( target ))
			return 0
		if (prob(25))
			logTheThing("combat", user, "accidentally harms [constructTarget(target,"combat")] with [src] at [log_loc(user)].")
	...
	harm(mob/target, var/mob/living/user, var/no_logs = 0)
		...
		var/quality = src.holder.quality
		if (no_logs != 1)
			logTheThing("combat", user, "mauls [constructTarget(target,"combat")] with [src] at [log_loc(user)].")
		var/obj/item/affecting = target.get_affecting(user)
...
/datum/limb/hot //because
	...
	proc/accident(mob/target, mob/living/user)
		if(check_target_immunity( target ))
			return 0
		if (prob(15))
			logTheThing("combat", user, "accidentally harms [constructTarget(target,"combat")] with hot hands at [log_loc(user)].")
	...
	harm(mob/target, var/mob/living/user, var/no_logs = 0)
		if(check_target_immunity( target ))
			return 0
		if (no_logs != 1)
			logTheThing("combat", user, "melts [constructTarget(target,"combat")] with hot hands at [log_loc(user)].")
		var/obj/item/affecting = target.get_affecting(user)
...
/datum/limb/abomination
	...
	disarm(mob/target, var/mob/living/user)
		...
		logTheThing("combat", user, "diarms [constructTarget(target,"combat")] with [src.weak == 1 ? "werewolf" : "abomination"] arms at [log_loc(user)].")
	...
	harm(mob/target, var/mob/living/user)
		...
		logTheThing("combat", user, "punches [constructTarget(target,"combat")] with [src.weak == 1 ? "werewolf" : "abomination"] arms at [log_loc(user)].")
		user.attack_effects(target, affecting)
		msgs.flush(SUPPRESS_LOGS)
...
/datum/limb/claw
	...
	proc/accident(mob/target, mob/living/user)
		if(check_target_immunity( target ))
			return 0
		if (prob(25))
			logTheThing("combat", user, "accidentally harms [constructTarget(target,"combat")] with claw arms at [log_loc(user)].")
			user.visible_message("<span class='alert'><b>[user] accidentally claws [target] while trying to [user.a_intent] them!</b></span>", "<span class='alert'><b>You accidentally claw [target] while trying to [user.a_intent] them!</b></span>")
			harm(target, user, 1)
			return 1
		return 0
	...
	harm(mob/target, var/mob/living/user, var/no_logs = 0)
		...
		var/quality = src.holder.quality
		if (no_logs != 1)
			logTheThing("combat", user, "claws [constructTarget(target,"combat")] with claw arms at [log_loc(user)].")
		var/obj/item/affecting = target.get_affecting(user)
...
/datum/limb/leg_hand
	...
	harm(mob/target, var/mob/living/user, var/no_logs = 0)
		...
		var/quality = src.holder.quality
		if (no_logs != 1)
			logTheThing("combat", user, "kicks [constructTarget(target,"combat")] at [log_loc(user)].")
		var/obj/item/affecting = target.get_affecting(user)
...
/datum/limb/small_critter
	...
	harm(mob/target, var/mob/living/user, var/no_logs = 0)
		...
		var/quality = src.holder.quality
		if (no_logs != 1)
			logTheThing("combat", user, "attacks [constructTarget(target,"combat")] with a critter arm at [log_loc(user)].")
		var/obj/item/affecting = target.get_affecting(user)
...
/datum/limb/small_critter/med/dash
	...
	harm(mob/target, var/mob/living/user, var/no_logs = 0)
		...
		var/quality = src.holder.quality
		if (no_logs != 1)
			logTheThing("combat", user, "slashes [constructTarget(target,"combat")] with dash arms at [log_loc(user)].")
