/datum/control_point
	...
	proc/do_item_delivery()
		if (!src.computer)
			...
			logTheThing("debug", null, "PW CONTROL POINT has null computer var.!!!owner_team=[owner_team]")
			return 0
		if (src.owner_team == 0)
			return 0
		...
		if (spawned_crate == TRUE && istype(ticker.mode, /datum/game_mode/pod_wars))
			...
			if (!other_team)
				...
				logTheThing("debug", null, "Can't grab the opposite team for control point [src.name]. It's owner_team value is:[src.owner_team]")
				return 0
