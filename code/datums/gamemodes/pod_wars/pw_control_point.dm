/datum/control_point
	...
	proc/capture(var/mob/user, var/team_num)
		...
		switch(team_num)
			if (TEAM_NANOTRASEN)
				minimap_renderer.recolour_area(src.capture_area.dynamic_map_colour_group, MAPC_NANOTRASEN)
			if (TEAM_SYNDICATE)
				minimap_renderer.recolour_area(src.capture_area.dynamic_map_colour_group, MAPC_SYNDICATE)
