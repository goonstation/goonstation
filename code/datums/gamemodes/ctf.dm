...
/datum/game_mode/ctf/announce()
	boutput(world, "<B>The current game mode is - Capture the Flag!</B>")
	boutput(world, "<B>Capture the other teams flag and bring it back to your base!</B>")
	boutput(world, "Respawn is on")
			...
			SPAWN_DBG(0)
				if(M.client)
					...
					if(G)
						...
						boutput(world, "No green team spawn point detected")
					M.client.team = "Green"
...
/datum/game_mode/ctf/post_setup()
	...
	SPAWN_DBG (50)
		...
		if (L)
			...
			boutput(world, "No red team spawn injector point detected")

		L = locate("landmark*The-Green-Team")
		if (L)
			new /obj/machinery/green_injector(L.loc)
		else
			boutput(world, "No green team injector spawn point detected")
