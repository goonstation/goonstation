...
/datum/game_mode/restructuring/announce()
	boutput(world, "<span style=\"color:red\"><B>GLOBAL TRANSMISSION FROM HEAD OFFICE: A CORPORATE RESTRUCTURING IS TO TAKE PLACE</B></span>")
	boutput(world, "<span style=\"color:red\"><B>Stay tuned for further news; note that we do care for our employees and any layoffs will be dealt with discretion and compassion</B></span>")
...
/datum/game_mode/restructuring/proc/pick_target(who)
	...
	if (!who)
		...
		boutput(world, "<span style=\"color:red\"><B>HEAD OFFICE: [target_desc] is accused of attempting to start a Union and is now considered a threat to the station. Terminate the employee immediately.</b></span>")
	else
		target = who
		target_desc = get_target_desc(target)
		boutput(world, "<span style=\"color:red\"><B>HEAD OFFICE: [target_desc] is accused of being a giant shitty homophobe or some shit. Terminate the employee immediately.</b></span>")
...
/datum/game_mode/restructuring/check_win()
	...
	if (left_alive.len == 1)
		...
		boutput(world, "<span style=\"color:red\"><B>HEAD OFFICE: Thanks to his superior brown-nosing abilities, [thewinner] has been promoted to senior management! Congratulations!</span>")
		return 1
