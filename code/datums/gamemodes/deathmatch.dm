...
/datum/game_mode/deathmatch
	...
	announce()
		boutput(world, "<B>The current game mode is - Death Commando Deathmatch!</B>")
		boutput(world, "<B>Just kill everyone else. They're gonna try to kill you, after all. Respawning is enabled.</B>")
		...
		for(var/mob/living/carbon/human/M in mobs)
			SPAWN_DBG(0)
				if(M.client)
					...
					var/newname = input(M,"You are a death commando. Would you like to change your name?", "Character Creation", randomname) as text
					if(!length(newname))
						newname = randomname
					...
					M.equip_if_possible(new /obj/item/clothing/suit/armor/death_commando(M), M.slot_wear_suit)
					...
					M.equip_if_possible(new /obj/item/gun/energy/pulse_rifle(M), M.slot_l_hand)
					M.equip_if_possible(new /obj/item/m_pill/cyanide(M), M.slot_l_store)
					M.equip_if_possible(new /obj/item/flashbang(M), M.slot_r_store)
