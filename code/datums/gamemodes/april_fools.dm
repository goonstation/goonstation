/datum/game_mode/mixed/april_fools
	name = "April Fools"
	config_tag = "april_fools"
	antag_token_support = FALSE

/datum/game_mode/mixed/april_fools/announce()
	boutput(world, "<B>The current game mode is - April Fools!</B>")
	boutput(world, "<B>Entirely random number of antagonists!</B>")

/datum/game_mode/mixed/april_fools/pre_setup()
	var/num_players = src.roundstart_player_count()

	var/num_enemies = rand(1, max(num_players, 1))
	...
	for(var/j = 0, j < num_enemies, j++)
		switch(weighted_pick(all_antag_types))
			if(ROLE_TRAITOR) num_traitors++
			if(ROLE_CHANGELING) num_changelings++
			if(ROLE_VAMPIRE) num_vampires++
			if(ROLE_WIZARD) num_wizards++
			if(ROLE_WRAITH) num_wraiths++
			if(ROLE_BLOB) num_blobs++
			if(ROLE_FLOCKMIND) num_flockminds++
