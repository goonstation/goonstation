...
// former roles are just stored as a list of id strings so we need to decode them to english
/datum/crewCredits/proc/special_role_to_english(special_role)
	switch(special_role)
		...
		if( ROLE_SPY_THIEF )
			return "spy-thief"
		if( ROLE_NUKEOP )
			return "nuclear operative"
		if( ROLE_NUKEOP_COMMANDER )
			return "nuclear commander"
		if( ROLE_SYNDICATE_AGENT )
		...
		if( ROLE_POLTERGEIST )
			return "poltergeist"
		if( ROLE_HARBINGER_SUMMON )
			return "harbinger summon"
		if( ROLE_PLAGUE_RAT )
			return "plague rat"
		if( ROLE_HEAD_REVOLUTIONARY )
			return "head revolutionary"
		...
		if( ROLE_SALVAGER )
			return "salvager"
		if( ROLE_ANTAGONIST_CRITTER )
			return "antagonist critter"
		if( ROLE_MISC )
			return "miscellanous"
		if( ROLE_BASKETBALL_WIZARD )
			return "basketball wizard"
		...
		if( ROLE_WRESTLER )
			return "wrestler"
		if( ROLE_PHOENIX )
			return "space phoenix"
		if( ROLE_EMAGGED_ROBOT )
			return "emagged robot"
		if( ROLE_SYNDICATE_ROBOT )
			return "syndicate robot"
		if( ROLE_CHANGELING_HIVEMIND_MEMBER )
			return "changeling hivemind member"
		if( ROLE_HANDSPIDER )
			return "handspider"
		if( ROLE_EYESPIDER )
			return "eyespider"
		...
		if( ROLE_SLEEPER_AGENT )
			return "sleeper agent"
		if( ROLE_HARDMODE_TRAITOR )
			return "hard-mode traitor"
		if( ROLE_MINDHACK )
			return "mindhack"
		if( ROLE_NUKEOP_GUNBOT )
			return "nuclear gunbot"
		if( ROLE_ZOMBIE )
			return "zombie"
		if( ROLE_BROKEN )
			return "broken"
		...
		if( ROLE_SLASHER )
			return "slasher"
		if( ROLE_LICENSED )
			return "licensed traitor"
		if( ROLE_PIRATE )
			return "pirate"
		if( ROLE_PIRATE_FIRST_MATE )
			return "pirate first mate"
		if( ROLE_PIRATE_CAPTAIN )
			return "pirate captain"
		if( ROLE_CONFIRMED_CRIMINAL )
			return "confirmed criminal"
