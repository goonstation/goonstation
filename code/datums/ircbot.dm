	...
	proc
		...
		load()
			if (config)
				...
				loadTries++
				if (loadTries >= 5)
					logTheThing("debug", null, "<b>IRCBOT:</b> Reached 5 failed config load attempts")
					logTheThing("diary", null, "<b>IRCBOT:</b> Reached 5 failed config load attempts", "debug")
				return 0
		...
		export(iface, args)
			...
			if (!config || !src.loaded)
				...
				if (response.errored || !response.body)
					logTheThing("debug", null, "<b>IRCBOT:</b> No return data from export. <b>errored:</b> [response.errored] <b>status_code:</b> [response.status_code] <b>iface:</b> [iface]. <b>args:</b> [text_args(args)] <br> <b>error:</b> [response.error]")
					return
				...
				if (!contentJson["status"])
					logTheThing("debug", null, "<b>IRCBOT:</b> Object missing status parameter in export response: [json_encode(contentJson)]")
					return 0
				if (contentJson["status"] == "error")
					...
					logTheThing("debug", null, "<b>IRCBOT:</b> [log]")
				return 1
		...
		logDebug(log)
			if (!log) return 0
			logTheThing("debug", null, "<b>IRCBOT DEBUGGING:</b> [log]")
			return 1
