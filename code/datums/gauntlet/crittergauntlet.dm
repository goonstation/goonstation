...
/datum/arena/gauntletController
	...
	proc/finishStaging()
		...
		SPAWN(0)
			...
			for (var/mob/living/M in gauntlet)
				...
				if (M.client)
					moblist_names += " ([M.client.key])"
			logTheThing("debug", null, "<b>Marquesas/Critter Gauntlet</b>: Starting arena game with players: [moblist_names]")
	...
	proc/startWave()
		...
		logTheThing("debug", null, "<b>Marquesas/Critter Gauntlet:</b> On level [current_level]. Spending [points] points, composed of 1 base, [round(current_level * 0.1) * 1.5] major and [(current_level % 10) / 20] minor.")
...
/datum/gauntletEvent
	...
	lightningstrikes
		...
		setUp()
			...
			if (!T)
				logTheThing("debug", null, "Gauntlet event Lightning Strikes failed setup.")
			D1 = new(T)
			D2 = new()
...
/proc/queryGauntletMatches(data)
	if (islist(data) && data["data_hub_callback"])
		...
		for (var/userkey in data["keys"])
			logTheThing("debug", null, "<b>Marquesas/Gauntlet Query:</b> Got key [userkey].")
			var/matches = data[userkey]
			logTheThing("debug", null, "<b>Marquesas/Gauntlet Query:</b> Matches for [userkey]: [matches].")
			var/obj/item/card/id/gauntlet/G = locate("gauntlet-id-[userkey]") in world
			if (G && istype(G))
				G.SetMatchCount(text2num(matches))
			else
				logTheThing("debug", null, "<b>Marquesas/Gauntlet Query:</b> Could not locate ID 'gauntlet-id-[userkey]'.")
				return 1
