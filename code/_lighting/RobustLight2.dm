	...
	proc
		...
		apply()
			...
			if(src.apply_level != 1)
				...
				logTheThing("debug", src, "<b>Light:</b> [src] (at [src.x] [src.y] [src.z]) is at apply level [src.apply_level] after an apply.")
#endif
		...
		strip(generation)
			...
			if(src.apply_level != 0)
				//CRASH("Light [src]'s apply level at [src.x], [src.y], [src.z] is [src.apply_level]")
				logTheThing("debug", src, "<b>Light:</b> [src] (at [src.x] [src.y] [src.z]) is at apply level [src.apply_level] after a strip.")
#endif
...
proc
	...
	RL_Suspend()
		RL_Suspended = 1
#ifdef DEBUG_LIGHT_STRIP_APPLY
		logTheThing("debug", src, "<b>Light:</b> Suspended lighting.")
#endif
		//TODO
	...
	RL_Resume()
		RL_Suspended = 0
#ifdef DEBUG_LIGHT_STRIP_APPLY
		logTheThing("debug", src, "<b>Light:</b> Unsuspended lighting.")
#endif
		// TODO
