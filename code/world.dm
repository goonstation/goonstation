...
/world/proc/load_intra_round_value(var/field) //Currently for solarium effects, could also be expanded to that pickle jar idea.
	...
	if (!F)
		logTheThing("debug", null, "Failed to load intra round value \"[field]\". Save file exists but may be locked by another process.")
		return
	F["[field]"] >> .
...
/world/proc/save_intra_round_value(var/field, var/value)
	...
	if (!F)
		logTheThing("debug", null, "Unable to save intra round value to field \"[field]\". Save file may be locked by another process.")
		return
	if (F.Lock(10))
		F["[field]"] << value
		return 0
	else
		logTheThing("debug", null, "Unable to save intra round value to field \"[field]\". Failed to obtain an exclusive save file lock.")
...
/world/proc/installUpdate()
	...
	logTheThing("diary", null, "Checking for updated [config.dmb_filename].dmb...", "admin")
	if(fexists("update/[config.dmb_filename].dmb"))
		logTheThing("diary", null, "Updated [config.dmb_filename].dmb found. Updating...", "admin")
		for(var/f in flist("update/"))
			if (IS_DIR_FNAME("update/[f]"))
				logTheThing("diary", null, "\tClearing [f]...", "admin")
				fdel(f)

			logTheThing("diary", null, "\tMoving [f]...", "admin")
			fcopy("update/[f]", "[f]")
			fdel("update/[f]")
		...
		logTheThing("diary", null, "Update complete.", "admin")
	else
		logTheThing("diary", null, "No update found. Skipping update process.", "admin")
	...
	if (T == "ping")
		...
		switch(plist["type"])
			if("irc")
				...
				logTheThing("ooc", null, "Discord OOC: [nick]: [msg]")
			...
			if("ooc")
				...
				msg = trim(copytext(sanitize(msg), 1, MAX_MESSAGE_LEN))
				msg = discord_emojify(msg)
				logTheThing("ooc", nick, "OOC: [msg]")
				logTheThing("diary", nick, ": [msg]", "ooc")
			...
			if("asay")
				...
				logTheThing("admin", null, "Discord ASAY: [nick]: [msg]")
				logTheThing("diary", null, "Discord ASAY: [nick]: [msg]", "admin")
			...
			if("fpm")
				...
				logTheThing("admin", null, "[server_name] PM: [nick]: [msg]")
				logTheThing("diary", null, "[server_name] PM: [nick]: [msg]", "admin")
			...
			if("pm")
				...
				if (M?.client)
					...
					logTheThing("admin_help", null, "Discord: [nick] PM'd [constructTarget(M,"admin_help")]: [msg]")
					logTheThing("diary", null, "Discord: [nick] PM'd [constructTarget(M,"diary")]: [msg]", "ahelp")
			...
			if("mentorpm")
				...
				if (M?.client)
					...
					logTheThing("admin", null, "Discord: [nick] Mentor PM'd [constructTarget(M,"admin")]: [msg]")
					logTheThing("diary", null, "Discord: [nick] Mentor PM'd [constructTarget(M,"diary")]: [msg]", "admin")
					for (var/client/C)
...
*/
			if ("heal")
				...
				for (var/mob/M in mobs)
					if (M.ckey && (findtext(M.real_name, who) || findtext(M.ckey, who)))
						M.full_heal()
						logTheThing("admin", nick, "healed / revived [constructTarget(M,"admin")]")
						logTheThing("diary", nick, "healed / revived [constructTarget(M,"diary")]", "admin")
			...
			if ("hubCallback")
				...
				if (!plist["data"])
					logTheThing("debug", null, "<b>API Error (Temp):</b> Didnt get data.")
					return 0
				if (!plist["proc"])
					logTheThing("debug", null, "<b>API Error (Temp):</b> Didnt get proc.")
					return 0
				...
				try
					...
					logTheThing("debug", null, "<b>API Error:</b> Invalid JSON detected: [plist["data"]]")
					return 0
				...
				if (rVal)
					logTheThing("debug", null, "<b>Callback Error</b> - Hub callback failed in [theDatum ? "<b>[theDatum]</b> " : ""]<b>[theProc]</b> with message: <b>[rVal]</b>")
					logTheThing("diary", null, "<b>Callback Error</b> - Hub callback failed in [theDatum ? "[theDatum] " : ""][theProc] with message: [rVal]", "debug")
					return 0
				else
					return 1
			...
			if ("delay")
				...
				if (game_end_delayed == 0)
					...
					logTheThing("admin", null, "[game_end_delayer] delayed the server restart from Discord.")
					logTheThing("diary", null, "[game_end_delayer] delayed the server restart from Discord.", "admin")
			...
			if ("undelay")
				...
				if (game_end_delayed == 0)
					...
					game_end_delayed = 0
					game_end_delayer = plist["nick"]
					logTheThing("admin", null, "[game_end_delayer] removed the restart delay from Discord.")
					logTheThing("diary", null, "[game_end_delayer] removed the restart delay from Discord.", "admin")
					...
					game_end_delayer = plist["nick"]
					logTheThing("admin", null, "[game_end_delayer] removed the restart delay from Discord and triggered an immediate restart.")
					logTheThing("diary", null, "[game_end_delayer] removed the restart delay from Discord and triggered an immediate restart.", "admin")
			...
			if ("triggerMapSwitch")
				...
				logTheThing("admin", nick, "set the next round's map to [mapName] from Discord")
				logTheThing("diary", nick, "set the next round's map to [mapName] from Discord", "admin")
			...
			if ("whitelistChange")
				...
				if (msg)
					logTheThing("admin", null, msg)
					logTheThing("diary", null, msg, "admin")
