...
/client/proc/toggleResourceCache()
	...
	logTheThing("admin", usr, "toggled the resource cache [disableResourceCache ? "off" : "on"]")
	logTheThing("diary", usr, "toggled the resource cache [disableResourceCache ? "off" : "on"]", "admin")
