...
/proc/scan_geology_targeted(var/atom/target, var/mob/user, var/visible = FALSE)
	...
	if(istype(target, /turf/simulated/wall/auto/asteroid))
		...
		if(event && event.analysis_string)
...
/proc/scan_appraisal(atom/target, mob/user as mob, var/visible = FALSE)
	...
	else if (istype(target, /obj/storage/crate))
		sell_value = -1
	...
	else if (istype(target, /obj/item/satchel))
		out_text = "<span class='alert'>Contents must be placed in a crate to be sold!</span><br>"
		sell_value = shippingmarket.appraise_value(target.contents, sell = 0)
	...
	if (user.client && !user.client.preferences?.flying_chat_hidden)
		...
		chat_text = make_chat_maptext(target, popup_text, alpha = 180, force = 1, time = 1.5 SECONDS)
