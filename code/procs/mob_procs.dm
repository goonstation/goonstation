...
/mob/proc/saylist(var/message, var/list/heard, var/list/olocs, var/thickness, var/italics, var/list/processed, var/use_voice_name = 0, var/image/chat_maptext/assoc_maptext = null)
	...
	var/rendered = "<span class='say'>[my_name] [SPAN_MESSAGE("[message_a]")]</span>"
	...
	if (length(olocs))
		...
		if (thickness < 0)
			...
			rendered_outside = "<span class='say'>[my_name] (on [bicon(outermost)] [outermost]) [SPAN_MESSAGE("[message_a]")]</span>"
		else if (thickness < 10)
			rendered_outside = "<span class='say'>[my_name] (inside [bicon(outermost)] [outermost]) [SPAN_MESSAGE("[message_a]")]</span>"
		else if (thickness < 20)
			rendered_outside = "<span class='say'>muffled <span class='name' data-ctx='\ref[src.mind]'>[src.voice_name]</span> (inside [bicon(outermost)] [outermost]) [SPAN_MESSAGE("[message_a]")]</span>"
	...
	for (var/mob/M in heard)
		...
		if (olocs.len && !(M.loc in olocs))
			...
			if (isghostdrone(M) && !isghostdrone(src) && !istype(M, /mob/living/silicon/ghostdrone/deluxe))
				thisR = "<span class='say'><span class='name' data-ctx='\ref[src.mind]'>[src.voice_name]</span> [SPAN_MESSAGE("[message_a]")]</span>"
