proc/explosion(atom/source, turf/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, turf_safe = FALSE,
			   range_cutoff_fraction=1, effects=null)
	...
	explosion_new(source, epicenter, (power*1.5)**2, max(light_impact_range/(power*1.5), 1), turf_safe = turf_safe,
				  range_cutoff_fraction=range_cutoff_fraction, effects=effects)
	//boutput(world, SPAN_ALERT("[power]"))

proc/explosion_new(atom/source, turf/epicenter, power, brisance=1, angle = 0, width = 360, turf_safe=FALSE, range_cutoff_fraction=1, effects=null)
	explosions.explode_at(source, epicenter, power, brisance, angle, width, turf_safe=turf_safe, range_cutoff_fraction=range_cutoff_fraction, effects=effects)
