...
/proc/equip_spy_theft(mob/living/carbon/human/traitor_mob)
	...
	else if (!R && istype(traitor_mob.l_hand, /obj/item/storage))
		var/list/cont = list()
		SEND_SIGNAL(traitor_mob.l_hand, COMSIG_STORAGE_GET_CONTENTS, cont)
		for (var/obj/item/device/radio/foo in cont)
			R = foo
			loc = "in the [traitor_mob.l_hand.name] in your left hand"
			break
	else if (!R && istype(traitor_mob.r_hand, /obj/item/storage))
		var/list/cont = list()
		SEND_SIGNAL(traitor_mob.r_hand, COMSIG_STORAGE_GET_CONTENTS, cont)
		for (var/obj/item/device/radio/foo in cont)
			R = foo
			...
			break
	else if (!R && istype(traitor_mob.back, /obj/item/storage))
		var/list/cont = list()
		SEND_SIGNAL(traitor_mob.back, COMSIG_STORAGE_GET_CONTENTS, cont)
		for (var/obj/item/device/radio/foo in cont)
			R = foo
