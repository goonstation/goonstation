...
/mob/living/electric_expose(var/power = 1)
	...
	if (power > 1) // pretty light damage and stam damage :)
		...
		if (src.hasStatus("knockdown"))
			src.do_disorient(stamina_damage = 15 + power * 8, knockdown = 0, stunned = 0, paralysis = 0, disorient = (power * (0.5 SECONDS)), remove_stamina_below_zero = 0, target_type = DISORIENT_BODY)
		else
			src.do_disorient(stamina_damage = 15 + power * 8, knockdown = 1 SECONDS + (power * (0.1 SECONDS)), stunned = 0, paralysis = 0, disorient = (power * (0.5 SECONDS)), remove_stamina_below_zero = 0, target_type = DISORIENT_BODY)
