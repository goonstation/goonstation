...
/obj/projection
	...
	proc/Life()
		if(src.pointer)
			...
			logTheThing("admin", null, "[src]/(%coords([src.x], [src.y], [src.z])%) not defined properly with ID = [src.id] and PTP = [src.pointer]")
			logTheThing("diary", null, "[src]/(%coords([src.x], [src.y], [src.z])%) not defined properly with ID = [src.id] and PTP = [src.pointer]", "admin")
