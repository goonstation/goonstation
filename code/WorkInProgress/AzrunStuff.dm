...
/datum/targetable/ai/module/turret/target
	...
	cast(atom/target)
		...
		if(target == expansion.turret.target)
			...
			logTheThing("combat", holder.owner, "[key_name(holder.owner)] used <b>[src.name]</b> on [key_name(target)] [log_loc(holder.owner)].")
...
/datum/targetable/ai/module/turret/swap_bullets
	...
	cast(atom/target)
		...
		logTheThing("combat", holder.owner, "[key_name(holder.owner)] set deployable turret to [mode].")
