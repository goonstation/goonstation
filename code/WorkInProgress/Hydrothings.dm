...
/obj/item/lilgreg
		...
		if(cantalk == 1)
			...
			if(prob(5))
				...
				if (istype(get_area(src), /area/solarium) && seensol == 0)
					user.visible_message("<span class='say'>[SPAN_NAME("[src]")] says, \"Woah, so thats what the sun looks like. It's kind of smaller then I expected though?\"")
					sleep(1 SECOND)
	...
	hear_talk(var/mob/living/carbon/speaker, text, real_name)
		if(prob(10))
			usr.visible_message("<span class='say'>[SPAN_NAME("[src]")] says, \"Woah [real_name] thats [pick("radical", "awesome", "sweet", "delicious", "100% spectacular", "better then sliced bread", "hootacular", "horrible", "hootastic", "dab worthy")]!\"")
			return
