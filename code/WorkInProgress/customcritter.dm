		...
		for (var/i = 1, i <= abil.len, i++)
			...
			if (T)
				...
				if (istype(A))
					...
					logTheThing("debug", null, "<b>Marquesas/CritterCreator:</b> Cannot deserialize type [T].")
...
/datum/critterAbility
	...
	proc/deserialize(var/savefile/F, var/path, var/datum/sandbox/sandbox)
		...
		if (ET)
			...
			logTheThing("debug", "usr", "<b>Marquesas/CritterCreator: </b> Failed to deserialize event for ability.")
...
/datum/critterAbility/criticalStrike
	...
	use_ability()
		if (C.target && ismob(C.target))
			...
			logTheThing("debug", null, "<b>Marquesas/CritterCreator: </b> Cannot reagent inject target, target is [C.target].")
			return 0
...
/datum/critterAbility/reagent
	...
	use_ability()
		if (!C)
			logTheThing("debug", null, "<b>Marquesas/CritterCreator: </b> Error using ability \ref[src] ([type]). C: \ref[C] [C].")
			return 0
		if (C.target && ismob(C.target))
			var/mob/M = C.target
			if (melee && GET_DIST(C, M) > C.attack_range)
