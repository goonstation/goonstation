...
/obj/monkeyplant
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if (narrator_mode)
			playsound(get_turf(src), 'sound/vox/scream.ogg', 10, 1, -1, channel=VOLUME_CHANNEL_SCREAMFART)
		else
			playsound(get_turf(src), 'sound/voice/screams/monkey_scream.ogg', 10, 1, -1, channel=VOLUME_CHANNEL_SCREAMFART)
		..()
		return
