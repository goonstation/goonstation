	...
	attack_hand(mob/user)
		...
		if(position == CONVEYOR_STOPPED)
			if (last_pos == CONVEYOR_REVERSE)
				...
				last_pos = CONVEYOR_STOPPED
			logTheThing("station", user, "turns the conveyor switch on in [last_pos == CONVEYOR_REVERSE ? "forward" : "reverse"] mode at [log_loc(src)].")
		else
			last_pos = position
			position = CONVEYOR_STOPPED
			logTheThing("station", user, "turns the conveyor switch off at [log_loc(src)].")
		UpdateIcon()
