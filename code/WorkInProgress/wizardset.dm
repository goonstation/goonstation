...
/obj/item/potion
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob)
		...
		if (user == target)
			...
			logTheThing("combat", user, "drinks [src] ([potion_name] -- [reagent])")
			drink(user)
		else if (ishuman(target))
			user.visible_message("<span class='alert'>[user] attempts to force [target] to drink [src].</span>")
			logTheThing("combat", user, "tries to force [constructTarget(target,"combat")] to drink [src] ([potion_name] -- [reagent]).")
			if (do_after(user, 3 SECONDS))
				if (reagent)
					user.visible_message("<span class='alert'>[user] forces [target] to drink [src].</span>")
					logTheThing("combat", user, "forces [constructTarget(target,"combat")] to drink [src] ([potion_name] -- [reagent]).")
					drink(target)
