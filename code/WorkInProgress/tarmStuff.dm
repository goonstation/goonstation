	...
	attackby(obj/item/I, mob/user)
		if(istype(I, /obj/item/paper))
			...
			if(P.info && !taken_suggestion)
				message_admins("[user] ([user?.get_ckey()]) has made a suggestion in [src]:<br>[P.name]<br><br>[copytext(P.info,1,MAX_MESSAGE_LEN)]")
				var/ircmsg[] = new()
				ircmsg["msg"] = "[user] ([user?.get_ckey()]) has made a suggestion in [src]:\n**[P.name]**\n[strip_html_tags(P.info)]"
