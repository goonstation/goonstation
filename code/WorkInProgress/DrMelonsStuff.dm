...
/obj/machinery/bathtub
	...
	attack_hand(mob/user as mob)
		if (!can_act(user, 0) || isAI(user)) return
		if (src.myuser)
	...
	MouseDrop_T(mob/living/carbon/human/target, mob/user)
		if (src.myuser || !istype(target) || target.buckled || LinkBlocked(target.loc,src.loc) || get_dist(user, src) > 1 || get_dist(user, target) > 1 || !can_act(user, 0) || isAI(user))
			return
