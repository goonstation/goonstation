...
/client/var/datum/test_for_tgui/secret_test_ui

/client/verb/secret_interface_test()
	set category = "Debug"
	set name = "Secret Interface Test"
	set desc = "Open the sample secret TGUI interface."
	if(!mob)
		return
	if(!secret_test_ui)
		secret_test_ui = new
	secret_test_ui.ui_interact(mob)

/datum/test_for_tgui
	var/counter = 0

/datum/test_for_tgui/ui_state(mob/user)
	return tgui_always_state

/datum/test_for_tgui/ui_data(mob/user)
	return list(
		counter = counter,
