	...
	execute_ability()
		...
		switch (alert(usr, "Which arm would you like to replace with [the_item]?",,"Left","Right","Cancel"))
			...
			if ("Right")
				...
				SETUP_GENERIC_ACTIONBAR(H, the_item, 3 SECONDS, TYPE_PROC_REF(/obj/item/saw/syndie, end_replace_arm), list("r_arm", H), the_item.icon, the_item.icon_state,"", INTERRUPT_MOVE | INTERRUPT_STUNNED | INTERRUPT_ACTION)
			if ("Left")
				if (!H.limbs.l_arm)
					var/obj/item/saw/syndie/S = the_item
					...
					return
				boutput(H, "<span class='alert'>You need to hold still...</span>")
				SETUP_GENERIC_ACTIONBAR(H, the_item, 3 SECONDS, TYPE_PROC_REF(/obj/item/saw/syndie, end_replace_arm), list("l_arm", H), the_item.icon, the_item.icon_state,"", INTERRUPT_MOVE | INTERRUPT_STUNNED | INTERRUPT_ACTION)
		..()
