...
/datum/player
	...
	proc/cloud_fetch_target_ckey(target)
		...
		if (response.errored || !response.body)
			logTheThing("debug", target, "failed to have their cloud data loaded: Couldn't reach Goonhub")
			return

		var/list/ret = json_decode(response.body)
		if(ret["status"] == "error")
			logTheThing("debug", target, "failed to have their cloud data loaded: [ret["error"]["error"]]")
			return
		else
			return ret
#else
