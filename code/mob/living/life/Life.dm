...
/mob/living/
	...
	handle_stamina_updates()
		...
		var/final_mod = (src.stamina_regen + GET_ATOM_PROPERTY(src, PROP_STAMINA_REGEN_BONUS)) * (stam_time_passed / tick_spacing)
		if (final_mod > 0)
...
/mob/living/carbon/human
	...
	get_cold_protection()
		...
		thermal_protection += GET_ATOM_PROPERTY(src, PROP_COLDPROT)
...
/*
	...
	get_rad_protection()
		...
		if (!src)
			return 0
		return clamp(GET_ATOM_PROPERTY(src, PROP_RADPROT), 0, 100)
	...
	get_ranged_protection()
		...
		// Resistance from Clothing
		protection += GET_ATOM_PROPERTY(src, PROP_RANGEDPROT)
		protection += GET_ATOM_PROPERTY(src, PROP_ENCHANT_ARMOR)/10 //enchanted clothing isn't that bulletproof at all
		return protection
	...
	get_melee_protection(zone, damage_type)
		...
		if(a_zone == "All")
			protection = (5 * GET_ATOM_PROPERTY(src, PROP_MELEEPROT_BODY) + GET_ATOM_PROPERTY(src, PROP_MELEEPROT_HEAD))/6
		if (a_zone == "chest")
			protection = GET_ATOM_PROPERTY(src, PROP_MELEEPROT_BODY)
		else //can only be head
			protection = GET_ATOM_PROPERTY(src, PROP_MELEEPROT_HEAD)
		protection += GET_ATOM_PROPERTY(src, PROP_ENCHANT_ARMOR)/2
	...
	get_deflection()
		if (!src)
			return 0
		return min(GET_ATOM_PROPERTY(src, PROP_DISARM_RESIST), 90)
	...
	get_heat_protection()
		...
		// Resistance from Clothing
		thermal_protection += GET_ATOM_PROPERTY(src, PROP_HEATPROT)
