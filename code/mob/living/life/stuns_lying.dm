...
/datum/lifeprocess/stuns_lying
	...
	process()
		...
		var/cant_lie = (owner.buckled && !istype(owner.buckled, /obj/stool/bed)) || robot_owner || hivebot_owner || istype(owner.loc, /obj/machinery/atmospherics/unary/cryo_cell)|| (human_owner && (human_owner.limbs && (istype(human_owner.limbs.l_leg, /obj/item/mob_part/humanoid_part/silicon_part/leg/left/treads) || istype(human_owner.limbs.r_leg, /obj/item/mob_part/humanoid_part/silicon_part/leg/right/treads)) && !locate(/obj/table, human_owner.loc) && !locate(/obj/machinery/optable, human_owner.loc)))
