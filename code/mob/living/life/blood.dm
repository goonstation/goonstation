...
/datum/lifeprocess/blood
	process(var/datum/gas_mixture/environment)
		...
		owner.blood_pressure["status"] = (current_blood_amt <= owner.hypotension_threshold) ? "HYPOTENSIVE" : (current_blood_amt >= owner.hypertension_threshold) ? "HYPERTENSIVE" : "NORMAL"
		...
		if (current_blood_amt <= 0) // welp
		...
		else if (current_blood_amt <= 299) // very low (70/50)
		...
		else if (current_blood_amt <= owner.hypotension_threshold) // low (100/65)
			if (prob(2))
		...
		else if (current_blood_amt < owner.hypertension_threshold) // normal (120/80)
			...
			return ..()
		else if (current_blood_amt <= 665) // high (140/90)
			if (prob(2))
		...
		else if (current_blood_amt <= 749) // very high (160/100)
			if (prob(2))
