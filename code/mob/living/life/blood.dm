...
/datum/lifeprocess/blood
	...
	process(var/datum/gas_mixture/environment)
		...
		if (owner.reagents?.total_volume > 0)
			anticoag_amt = owner.reagents.get_reagent_amount("heparin") + owner.reagents.get_reagent_amount("acetylsalicylic_acid")/2 + owner.reagents.get_reagent_amount("ginger")/3
		...
		if (owner.bleeding)
			...
			if (length(owner.surgeryHolder.get_active_surgeries()))
				decrease_chance -= 1
			...
			if (probmult(surgery_increase_chance) && owner.surgeryHolder.get_active_surgeries())
				owner.bleeding += 1
