...
////Dead sight
		...
		if ((isdead(owner) || HAS_ATOM_PROPERTY(owner, PROP_XRAYVISION) || HAS_ATOM_PROPERTY(owner, PROP_XRAYVISION_WEAK)) && (T && !isrestrictedz(T.z)))
			owner.sight |= SEE_TURFS
			owner.sight |= SEE_MOBS
			owner.sight |= SEE_OBJS
			...
			else if(HAS_ATOM_PROPERTY(owner, PROP_XRAYVISION_WEAK))
				owner.sight &= ~SEE_BLACKNESS
				owner.sight &= ~SEE_MOBS
			else
...
////Ship sight
		...
		else if (HAS_ATOM_PROPERTY(owner, PROP_NIGHTVISION_WEAK))
			owner.render_special.set_centerlight_icon("thermal", rgb(0.5 * 255, 0.5 * 255, 0.5 * 255))
