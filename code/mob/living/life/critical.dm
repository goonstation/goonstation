...
/datum/lifeprocess/critical //for mobs that use crit (humans only right now)
	process(var/datum/gas_mixture/environment)
		...
		if (owner.health <= -100)	// Deathchance altered to account for deeper braincrit
			if (owner.reagents.has_reagent("synaptizine") && owner.reagents.has_reagent("atropine"))
				var/deathchance = min(99, ((owner.get_brain_damage() * -1) + (owner.health + (owner.get_oxygen_deprivation() / 2))) * -0.001)
				if (prob(deathchance))
					owner.death()
			else
				var/deathchance = min(99, ((owner.get_brain_damage() * -1) + (owner.health + (owner.get_oxygen_deprivation() / 2))) * -0.01)
				if (prob(deathchance))
					owner.death()
