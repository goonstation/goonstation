...
/datum/lifeprocess/critical //for mobs that use crit (humans only right now)
	process(var/datum/gas_mixture/environment)
		...
		if (owner.health <= -100)
			if ((owner.reagents && owner.reagents.has_reagent("synaptizine") && owner.reagents.has_reagent("atropine")) || (owner.bodytemperature < owner.base_body_temp - 100 && (owner.reagents && owner.reagents.has_reagent("cryoxadone"))))
