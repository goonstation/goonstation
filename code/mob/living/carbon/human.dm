...
/mob/living/carbon/human/is_can_receive()
...
/mob/living/carbon/human/throw_item(atom/target, list/params)
	...
	if (I)
		...
		if (iscarbon(I))
			...
			logTheThing(LOG_COMBAT, src, "throws [I] [I.is_can_receive() ? "[log_reagents(I)]" : ""] [dir2text(throw_dir)] at [log_loc(src)].")
...
/mob/living/carbon/human/hitby(atom/movable/AM, datum/thrown_thing/thr)
	...
	if(((src.in_throw_mode && src.a_intent == "help") || src.client?.check_key(KEY_THROW)) && !src.equipped())
		if((src.hand && (!src.limbs.l_arm)) || (!src.hand && (!src.limbs.r_arm)) || src.hasStatus("handcuffed") || (prob(60) && src.bioHolder.HasEffect("clumsy")) || ismob(AM) || (thr?.get_throw_travelled() <= 1 && AM.last_throw_x == AM.x && AM.last_throw_y == AM.y))
			...
			logTheThing(LOG_COMBAT, src, "is struck by [AM] [AM.is_can_receive() ? "[log_reagents(AM)]" : ""] at [log_loc(src)] (likely thrown by [thr?.user ? thr.user : "a non-mob"]).")
			...
			logTheThing(LOG_COMBAT, src, "catches [AM] [AM.is_can_receive() ? "[log_reagents(AM)]" : ""] at [log_loc(src)] (likely thrown by [thr?.user ? constructName(thr.user) : "a non-mob"]).")
			src.throw_mode_off()
			#ifdef DATALOGGER
		...
		if (AM.throwing & THROW_CHAIRFLIP)
			...
			src.visible_message("<span class='alert'>[src] has been hit by [AM].</span>")
			random_brute_damage(src, AM.throwforce,1)
			logTheThing(LOG_COMBAT, src, "is struck by [AM] [AM.is_can_receive() ? "[log_reagents(AM)]" : ""] at [log_loc(src)] (likely thrown by [thr?.user ? constructName(thr.user) : "a non-mob"]).")
			if(thr?.user)
				src.was_harmed(thr.user, AM)
