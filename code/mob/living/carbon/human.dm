	...
	proc/replace_with(var/target, var/new_type, var/mob/user, var/show_message = 1)
		...
		if (istext(target) && ispath(new_type))
			if (target == "both_arms" || target == "l_arm")
				...
				if (user)
					logTheThing("admin", user, "replaced [constructTarget(src.holder,"admin")]'s left arm with [new_type]")
				. ++
			...
			if (target == "both_arms" || target == "r_arm")
				...
				if (user)
					logTheThing("admin", user, "replaced [constructTarget(src.holder,"admin")]'s right arm with [new_type]")
				. ++
			...
			if (target == "both_legs" || target == "l_leg")
				if (ispath(new_type, /obj/item/parts/human_parts/leg) || ispath(new_type, /obj/item/parts/robot_parts/leg))
					...
					if (user)
						logTheThing("admin", user, "replaced [constructTarget(src.holder,"admin")]'s left leg with [new_type]")
					. ++
			...
			if (target == "both_legs" || target == "r_leg")
				if (ispath(new_type, /obj/item/parts/human_parts/leg) || ispath(new_type, /obj/item/parts/robot_parts/leg))
					...
					if (user)
						logTheThing("admin", user, "replaced [constructTarget(src.holder,"admin")]'s right leg with [new_type]")
					. ++
			if (.)
...
/mob/living/carbon/human/death(gibbed)
	...
	if (src.unkillable || src.spell_soulguard)
		if (src.unkillable && src.mind.dnr) //Unless they have dnr set in which case rip for good
			logTheThing("combat", src, "was about to be respawned (Unkillable) but had DNR set.")
			if (!gibbed)
				src.gib()
			...
			logTheThing("combat", src, "respawns ([src.spell_soulguard ? "Soul Guard" : "Unkillable"])")
	...
	if (C)
		...
		//Changelings' heads pop off and crawl away - but only if they're not gibbed and have some spare DNA points. Oy vey!
			SPAWN(0)
				...
				logTheThing("combat", src, "became a headspider at [log_loc(src)].")
	...
	logTheThing("combat", src, "dies [log_health(src)] at [log_loc(src)].")
	//src.icon_state = "dead"
...
/mob/living/carbon/human/throw_item(atom/target, list/params)
	...
	if (I)
		...
		if (iscarbon(I))
			...
			logTheThing("combat", src, "throws [constructTarget(C,"combat")] [dir2text(throw_dir)] at [log_loc(src)].")
			...
			logTheThing("combat", src, "throws [I] [I.is_open_container() ? "[log_reagents(I)]" : ""] [dir2text(throw_dir)] at [log_loc(src)].")
...
/mob/living/carbon/human/whisper(message as text, forced=FALSE)
	...
	logTheThing("diary", src, "(WHISPER): [message]", "whisper")
	logTheThing("whisper", src, "SAY: [message] (WHISPER) [log_loc(src)]")
...
/mob/living/carbon/human/infected(var/datum/pathogen/P)
	...
	if (!(P.pathogen_uid in src.pathogens))
		...
		Q.infected = src
		logTheThing("pathology", src, "is infected by [Q].")
		return 1
	else
		...
		if (C.generation < P.generation)
			var/datum/pathogen/Q = new /datum/pathogen
			Q.setup(0, P, 1)
			logTheThing("pathology", src, "'s pathogen mutation [C] is replaced by mutation [Q] due to a higher generation number.")
			pathogen_controller.mob_infected(Q, src)
...
/mob/living/carbon/human/cured(var/datum/pathogen/P)
	if (P.pathogen_uid in src.pathogens)
		...
		if (M.auto_immunize)
			immunity(P)
		qdel(Q)
		logTheThing("pathology", src, "is cured of [pname].")
...
/mob/living/carbon/human/remission(var/datum/pathogen/P)
	...
	if (P.pathogen_uid in src.pathogens)
		var/datum/pathogen/Q = src.pathogens[P.pathogen_uid]
		Q.remission()
		logTheThing("pathology", src, "'s pathogen [Q] enters remission.")
...
/mob/living/carbon/human/immunity(var/datum/pathogen/P)
	...
	if (!(P.pathogen_uid in src.immunities))
		...
		logTheThing("pathology", src, "gains immunity to pathogen [P].")
...
/mob/living/carbon/human/proc/drop_juggle()
	...
	src.update_body()
	logTheThing("combat", src, "drops the items they were juggling")
...
/mob/living/carbon/human/proc/add_juggle(var/obj/thing as obj)
	...
	if (isitem(thing))
		...
		i.on_spin_emote(src)
	src.update_body()
	logTheThing("combat", src, "juggles [thing]")
...
/mob/living/carbon/human/proc/activate_chest_item_on_attack(mob/living/carbon/human/M) // Let's only have humans do this, ok?
	...
	if (M && M.zone_sel && M.zone_sel.selecting == "chest" && src.chest_item != null && (src.chest_item in src.contents))
		logTheThing("combat", M, "activates [src.chest_item] embedded in [src]'s chest cavity at [log_loc(src)]")
...
/mob/living/carbon/human/proc/chest_item_dump_reagents_on_flip()
	...
	if (liquidReagentContainer && src.chest_item.reagents.total_volume > 0)			// If container type is OK and has reagents...
		...
		if (maxVolumeAdd > 0)	// If we can add reagents to human, print message and dump shit into human
			...
			logTheThing("combat", src, "transfers chemicals from [src.chest_item] [log_reagents(src.chest_item)] to [src] at [log_loc(src)]")
...
/mob/living/carbon/human/proc/chest_item_attack_self_on_fart()
	...
	if (src.chest_item_sewn == 0 || istype(src.chest_item, /obj/item/cloaking_device))	// If item isn't sewn in, poop it onto the ground. No fartcloaks allowed
		...
		// Other object is pooped out
			...
			take_bleeding_damage(src, src, 5)
		// added log - cirr
		logTheThing("combat", src, "takes damage from farting out [src.chest_item] embedded in [src]'s chest cavity at [log_loc(src)]")
...
/mob/living/carbon/human/hitby(atom/movable/AM, datum/thrown_thing/thr)
	...
	if(((src.in_throw_mode && src.a_intent == "help") || src.client?.check_key(KEY_THROW)) && !src.equipped())
		if((src.hand && (!src.limbs.l_arm)) || (!src.hand && (!src.limbs.r_arm)) || src.hasStatus("handcuffed") || (prob(60) && src.bioHolder.HasEffect("clumsy")) || ismob(AM) || (thr?.get_throw_travelled() <= 1 && AM.last_throw_x == AM.x && AM.last_throw_y == AM.y))
			...
			logTheThing("combat", src, "is struck by [AM] [AM.is_open_container() ? "[log_reagents(AM)]" : ""] at [log_loc(src)] (likely thrown by [thr?.user ? thr.user : "a non-mob"]).")
			...
			logTheThing("combat", src, "catches [AM] [AM.is_open_container() ? "[log_reagents(AM)]" : ""] at [log_loc(src)] (likely thrown by [thr?.user ? constructName(thr.user) : "a non-mob"]).")
			src.throw_mode_off()
			#ifdef DATALOGGER
		...
		if (AM.throwing & THROW_CHAIRFLIP)
			...
			src.visible_message("<span class='alert'>[src] has been hit by [AM].</span>")
			random_brute_damage(src, AM.throwforce,1)
			logTheThing("combat", src, "is struck by [AM] [AM.is_open_container() ? "[log_reagents(AM)]" : ""] at [log_loc(src)] (likely thrown by [thr?.user ? constructName(thr.user) : "a non-mob"]).")
			if(thr?.user)
				src.was_harmed(thr.user, AM)
