	...
	switch(ai_state)
		...
		if(AI_ATTACKING)	//Gonna kick your ass.
			...
			if(iscarbon(ai_target))
				...
				if((carbon_target.getStatusDuration("weakened") || carbon_target.getStatusDuration("stunned") || carbon_target.getStatusDuration("unconscious")) && distance <= 1 && !ai_incapacitated())
...
/mob/living/carbon/human/proc/ai_incapacitated()
	if(stat || hasStatus(list("stunned", "unconscious", "weakened")) || !sight_check(1)) return 1
	else return 0
...
/mob/living/carbon/human/proc/ai_freeself()
	if(istype(src.loc, /obj/machinery/disposal))
		...
		if (V.rider == src)
			if(!(src.getStatusDuration("unconscious") || src.getStatusDuration("stunned") || src.getStatusDuration("weakened") || src.stat))
				V.eject_rider(0, 1)
