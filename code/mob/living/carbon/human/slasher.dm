	...
	proc
		...
		take_control(mob/living/carbon/human/M)
			...
			SPAWN(0)
				...
				if(prob(33))
					M.emote("faint")
					M.setStatusMax("weakened", 4 SECONDS)
				else
					M.emote("tremble")
				sleep(20 SECONDS)
				...
				if(prob(50))
					M.emote("shudder")
					M.setStatusMax("weakened", 1 SECONDS)
					M.setStatusMax("paralysis", 1 SECONDS)
					M.force_laydown_standup()
				else
					M.emote("faint")
					M.setStatusMax("weakened", 8 SECONDS)
				...
				M.emote("faint")
				M.setStatusMax("weakened", 8 SECONDS)
				M.setStatusMax("paralysis", 8 SECONDS)
				sleep(8 SECONDS)
