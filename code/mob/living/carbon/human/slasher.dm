/mob/living/carbon/human/slasher
	...
	New(loc)
		...
		APPLY_ATOM_PROPERTY(src, PROP_STUN_RESIST, "slasher_stun_resistance", 80)
		APPLY_ATOM_PROPERTY(src, PROP_STUN_RESIST_MAX, "slasher_stun_resistance", 80)
		START_TRACKING
		APPLY_ATOM_PROPERTY(src, PROP_NO_SELF_HARM, src)
		APPLY_ATOM_PROPERTY(src, PROP_AI_UNTRACKABLE, src)
	...
	proc
		...
		corporealize()
			if(src.hasStatus("incorporeal"))
				...
				SPAWN(1.5 SECONDS)
					...
					REMOVE_ATOM_PROPERTY(src, PROP_NEVER_DENSE, src)
					REMOVE_ATOM_PROPERTY(src, PROP_NO_MOVEMENT_PUFFS, src)
					REMOVE_ATOM_PROPERTY(src, PROP_NOCLIP, src)
					src.alpha = 254
		...
		take_control(mob/living/carbon/human/M)
			...
			SPAWN(0)
				...
				APPLY_ATOM_PROPERTY(M, PROP_NO_SELF_HARM, src)
				...
				REMOVE_ATOM_PROPERTY(M, PROP_NO_SELF_HARM, src)
				qdel(WG)
				remove_equipment(M)
		...
		blood_trail()
			...
			if(!src.trailing_blood)
				...
				src.track_blood()
				trailing_blood = TRUE
				APPLY_ATOM_PROPERTY(src, PROP_BLOOD_TRACKING_ALWAYS, src)
			else
				REMOVE_ATOM_PROPERTY(src, PROP_BLOOD_TRACKING_ALWAYS, src)
