			...
			if ("uguu")
				if (istype(src.wear_mask, /obj/item/clothing/mask/anime) && !src.stat)
					...
					SPAWN(1 SECOND)
						...
						src.wear_mask = null
						logTheThing("combat", src, "was gibbed by emoting uguu at [log_loc(src)].")
						src.gib()
						return
				else
			...
			if ("tip")
				if (!src.restrained() && !src.stat)
					...
					if (istype(src.head, /obj/item/clothing/head/fedora))
						...
						SPAWN(1 SECOND)
							src.add_karma(-10)
							logTheThing("combat", src, "was gibbed by emoting fedora tipping at [log_loc(src)].")
							src.gib()
							...
							if (!flipped_a_guy)
								for (var/mob/living/M in view(1, null))
									...
									if (src.reagents?.get_reagent_amount("ethanol") > 10 && src.can_drunk_act())
										...
										logTheThing("combat", src, "flips into [constructTarget(M,"combat")]")
...
/mob/living/proc/do_suplex(obj/item/grab/G)
	...
	logTheThing("combat", src, "suplexes [constructTarget(G.affecting,"combat")][tabl ? " into \an [tabl]" : null] [log_loc(src)]")
