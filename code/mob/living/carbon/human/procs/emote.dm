			...
			if ("monologue")
				...
				if (src.mind && src.mind.assigned_role == "Detective")
					...
					if (G && ishuman(G.affecting))
						src.say_verb("I'll stare the bastard in the face as [he_or_she(G.affecting)] screams to God, and I'll laugh harder when [he_or_she(G.affecting)] whimpers like a baby. And when [G.affecting]'s eyes go dead, the hell I send him [him_or_her(G.affecting)] will seem like heaven after what I've done to [him_or_her(G.affecting)].")
					else if (istype(src.loc.loc, /area/station/security/detectives_office))
						src.say_verb("As I looked out the door of my office, I realised it was a night when you didn't know your friends but strangers looked familiar. A night like this, the smartest thing to do is nothing: stay home. It was like the wind carried people along with it. But I had to get out there.")
					else if (istype(src.loc.loc, /area/station/maintenance))
						src.say_verb("The dark maintenance corridoors of this place were always the same, home to the most shady characters you could ever imagine. Walk down the right back alley in [station_name(1)], and you can find anything.")
					else if (istype(src.loc.loc, /area/station/hydroponics))
						src.say_verb("A gang of space farmers growing psilocybin mushrooms, cannabis, and of course those goddamned george melons. A shady bunch, whose wiles had earned them the trust of many. The Chef. The Barman. But not me. frown No, their charms don't work on a [man_or_woman(src)] of values and principles.")
					else if (istype(src.loc.loc, /area/station/mailroom))
						src.say_verb("The post office, an unused room habited by a brainless monkey, a cynical postman, and now, me. I've never trusted postal workers, with their crisp blue suits and their peaked caps. There's never any mail sent, excepting the ticking packages I gotta defuse up in the bridge.")
					else if (istype(src.loc.loc, /area/centcom))
						src.say_verb("Central Command. I was tired as hell but I could afford to be tired now... I needed it to be morning. I wanted to hear doors opening, cars start, and human voices talking about the Space Olympics. I wanted to make sure there were still folks out there facing life with nothing up their sleeves but their arms. They didn't know it yet, but they had a better shot at happiness and a fair shake than they did yesterday.")
					else if (istype(src.loc.loc, /area/station/chapel))
						src.say_verb("The self-pontificating bastard who calls himself our chaplain conducts worship here. If you can call the summoning of an angry god who pelts us with toolboxes, bolts of lightning, and occasionally rips our bodies in twain 'worship'.")
					else if (istype(src.loc.loc, /area/station/bridge))
						src.say_verb("The bridge. The home of the Captain and Head of Personnel. I tried to tell myself I was the sturdy leg in our little triangle. I was worried it was true.")
					else if (istype(src.loc.loc, /area/station/security/main))
						src.say_verb("I had dreams of being security before I got into the detective game. I wanted to meet stimulating and interesting people of an ancient space culture, and kill them. I wanted to be the first kid on my ship to get a confirmed kill.")
					else if (istype(src.loc.loc, /area/station/crew_quarters/bar))
						src.say_verb("The station bar, full of the best examples of lowlifes and drunks I'll ever find. I need a drink though, and there are no better places to find a beer than here.")
					else if (istype(src.loc.loc, /area/station/medical))
						src.say_verb("Medical. In truth it's full of the biggest bunch of cut throats on the station, most would rather cut you up than sow you up, but if I've got a slug in my ass, I don't have much choice.")
					else if (istype(src.loc.loc, /area/station/hallway/primary/))
						src.say_verb("The halls of the station assault my nostrils like a week old meal left festering in the sink. A thug around every corner, and reason enough themselves to keep my gun in my hand.")
					else if (istype(src.loc.loc, /area/station/hallway/secondary/exit))
						src.say_verb("The only way off this hellhole and it's the one place I don't want to be, but sometimes you have to show your friends that you're worth a damn. Sometimes that means dying, sometimes it means killing a whole lot of people to escape alive.")
					else if (istype(src.loc.loc, /area/station/hallway/secondary/entry))
						src.say_verb("The entrance to [station_name(1)]. You will never find a more wretched hive of scum and villainy. I must be cautious.")
					else if (istype(src.loc.loc, /area/station/engine/))
						src.say_verb("The churning, hellish heart of the station that just can't help missing the beat. Full of the dregs of society, and not the right place to be caught unwanted. I better watch my back.")
					else if (istype(src.loc.loc, /area/station/maintenance/disposal))
						src.say_verb("Disposal. Usually bloodied, full of grey-suited corpses and broken windows. Down here, you can hear the quiet moaning of the station itself. It's like it's mourning. Mourning better days long gone, like assistants through these pipes.")
					else if (istype(src.loc.loc, /area/station/crew_quarters/cafeteria))
						src.say_verb("A place to eat, but not an appealing one. I've heard rumours about this place, and if there's one thing I know, it's that it's not normal to eat people.")
