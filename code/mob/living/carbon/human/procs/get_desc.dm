	...
	if (src.organHolder)
		...
		if (src.organHolder.chest.op_stage > 0.0)
			...
			if (src.organHolder.chest.op_stage >= 10.0 && !istype(src.mutantrace, /datum/mutantrace/skeleton))
				if ((!src.mutantrace || src.mutantrace.tail == NO_TAIL) && src.butt_op_stage >= 4)
					. += "<br><span class='alert'><B>[src.name] has a large incision at the base of [t_his] back!</B></span>"
				else if ((!src.mutantrace || src.mutantrace.tail == NO_TAIL) && src.butt_op_stage < 4)
					. += "<br><span class='alert'><B>[src.name] has a large incision above [t_his] butt!</B></span>"

				else if (src.organHolder.tail && src.mutantrace && src.mutantrace.tail > NO_TAIL)
					. += "<br><span class='alert'><B>[src.name] has a long incision around the base of [t_his] tail!</B></span>"
				else if (!src.organHolder.tail && src.mutantrace && src.mutantrace.tail > NO_TAIL && src.butt_op_stage >= 4)
		...
		if (src.organHolder.chest.op_stage == 0.0)
			if (src.organHolder.tail && (src.organHolder.tail.type != /obj/item/organ/tail/bone) && (!src.mutantrace || src.mutantrace.tail == NO_TAIL) && src.butt_op_stage >= 4)
			...
			if (!src.organHolder.tail && src.mutantrace && src.mutantrace.tail > NO_TAIL)
