...
/mob/living/carbon/human/get_desc()
	...
	if (src.hasStatus("handcuffed"))
		. +=  "<br><b class='alert'>[name_to_use] is [bicon(src.handcuffs)] handcuffed!</b>"

	if (src.w_uniform && !(src.wear_suit?.hides_from_examine & C_UNIFORM))
		. += "<br><span class='[src.w_uniform.blood_DNA ? "alert" : "notice"]'>[name_to_use] is wearing [bicon(src.w_uniform)] \an [src.w_uniform.name].</span>"

	if (src.wear_suit)
		. += "<br><span class='[src.wear_suit.blood_DNA ? "alert" : "notice"]'>[name_to_use] has [bicon(src.wear_suit)] \an [src.wear_suit.name] on.</span>"
	...
	if (src.ears && !(src.wear_suit?.hides_from_examine & C_EARS) && !(src.head?.hides_from_examine & C_EARS))
		if (istype(src.ears, /obj/item/clothing/))
			. += "<br><span class='[src.ears.blood_DNA ? "alert" : "notice"]'>[name_to_use] has [bicon(src.ears)] \an [src.ears.name] by [t_his] mouth.</span>"
		else
			. += "<br><span class='[src.ears.blood_DNA ? "alert" : "notice"]'>[name_to_use] has [bicon(src.ears)] [src.ears.blood_DNA ? "a bloody [src.ears.name]" : "\an [src.ears.name]"] by [t_his] mouth.</span>"

	if (src.head)
		. += "<br><span class='[src.head.blood_DNA ? "alert" : "notice"]'>[name_to_use] has [bicon(src.head)] \an [src.head.name] on [t_his] head.</span>"
	...
	if (src.wear_mask && !(src.wear_suit?.hides_from_examine & C_MASK) && !(src.head?.hides_from_examine & C_MASK))
		if (istype(src.l_hand, /obj/item/clothing/))
			. += "<br><span class='[src.wear_mask.blood_DNA ? "alert" : "notice"]'>[name_to_use] has [bicon(src.wear_mask)] [src.wear_mask.blood_DNA ? "a bloody [src.wear_mask.name]" : "\an [src.wear_mask.name]"] on [t_his] face.</span>"
		else
			. += "<br><span class='[src.wear_mask.blood_DNA ? "alert" : "notice"]'>[name_to_use] has [bicon(src.wear_mask)] \an [src.wear_mask.name] on [t_his] face.</span>"
	...
	if (src.r_hand)
		if (!ignore_checks && dist > MAX_NAMETAG_RANGE)
			...
			. += "<br><span class='[src.r_hand.blood_DNA ? "alert" : "notice"]'>[name_to_use] has [bicon(src.r_hand)] [src.r_hand.blood_DNA ? "a bloody [src.r_hand.name]" : "\an [src.r_hand.name]"] in [t_his] right hand.</span>"

	if (src.belt)
		. += "<br><span class='[src.belt.blood_DNA ? "alert" : "notice"]'>[name_to_use] has [bicon(src.belt)] [src.belt.blood_DNA ? "a bloody [src.belt.name]" : "\an [src.belt.name]"] on [t_his] belt.</span>"

	if (src.gloves && !src.gloves.nodescripition)
		if(!(src.wear_suit && src.wear_suit?.hides_from_examine & C_GLOVES))
			. += "<br><span class='[src.gloves.blood_DNA ? "alert" : "notice"]'>[name_to_use] has [bicon(src.gloves)] [src.gloves.name] on [t_his] hands.</span>"
	else if (src.blood_DNA)
		. += "<br>[SPAN_ALERT("[name_to_use] has bloody hands!")]"

	if (src.shoes && !(src.wear_suit?.hides_from_examine & C_SHOES))
		. += "<br><span class='[src.shoes.blood_DNA ? "alert" : "notice"]'>[name_to_use] has [bicon(src.shoes)] [src.shoes.name] on [t_his] feet.</span>"
	else if (islist(src.tracked_blood))
		. += "<br>[SPAN_ALERT("[name_to_use] has bloody feet!")]"
	...
	if (src.wear_id)
		if (!ignore_checks && dist > MAX_NAMETAG_RANGE)
			...
			if (src.wear_id:registered != src.real_name && in_interact_range(src, usr) && prob(10))
				. += "<br>[SPAN_ALERT("[name_to_use] is wearing [bicon(src.wear_id)] [src.wear_id.name] yet doesn't seem to be that person!!!")]"
			else
				. += "<br>[SPAN_NOTICE("[name_to_use] is wearing [bicon(src.wear_id)] [src.wear_id.name].")]"
			...
			if (desc_id_card)
				if (desc_id_card.registered != src.real_name && in_interact_range(src, usr) && prob(10))
					. += "<br>[SPAN_ALERT("[name_to_use] is wearing [bicon(src.wear_id)] [src.wear_id.name] with [bicon(desc_id_card)] [desc_id_card.name] in it yet doesn't seem to be that person!!!")]"
				else
					. += "<br>[SPAN_NOTICE("[name_to_use] is wearing [bicon(src.wear_id)] [src.wear_id.name] with [bicon(desc_id_card)] [desc_id_card.name] in it.")]"
	...
	if(global.client_image_groups?[CLIENT_IMAGE_GROUP_ARREST_ICONS]?.subscribed_mobs_with_subcount[usr]) // are you in the list of people who can see arrest icons??
		var/datum/db_record/sec_record = data_core.security.find_record("name", name_to_use)
		if(sec_record)
			var/sechud_flag = sec_record["sec_flag"]
			if (lowertext(sechud_flag) != "none")
				. += "<br>[SPAN_NOTICE("[name_to_use] has a Security HUD flag set:")] [SPAN_ALERT("[sechud_flag]")]"
	...
	if (locate(/obj/item/implant/projectile/body_visible/dart) in src.implant)
		...
		for (var/obj/item/implant/projectile/body_visible/dart/P in src.implant)
			count++
		. += "<br>[SPAN_ALERT("[name_to_use] has [count > 1 ? "darts" : "a dart"] stuck in [him_or_her(src)]!")]"
	...
	if (locate(/obj/item/implant/projectile/body_visible/syringe) in src.implant)
		...
		for (var/obj/item/implant/projectile/body_visible/syringe/P in src.implant)
			count++
		. += "<br>[SPAN_ALERT("[name_to_use] has [count > 1 ? "syringes" : "a syringe"] stuck in [him_or_her(src)]!")]"
	...
	if (locate(/obj/item/implant/projectile/body_visible/arrow) in src.implant)
		...
		for (var/obj/item/implant/projectile/body_visible/arrow/P in src.implant)
			count++
		. += "<br>[SPAN_ALERT("[name_to_use] has [count > 1 ? "arrows" : "an arrow"] stuck in [him_or_her(src)]!")]"
	...
	if (locate(/obj/item/implant/projectile/body_visible/seed) in src.implant)
		...
		for (var/obj/item/implant/projectile/body_visible/seed/P in src.implant)
			count++
		. += "<br>[SPAN_ALERT("[name_to_use] has [count > 1 ? "seeds" : "a seed"] stuck in [him_or_her(src)]!")]"

	if (src.is_jittery)
		switch(src.jitteriness)
			if (300 to INFINITY)
				. += "<br>[SPAN_ALERT("[name_to_use] is violently convulsing.")]"
			if (200 to 300)
				. += "<br>[SPAN_ALERT("[name_to_use] looks extremely jittery.")]"
			if (100 to 200)
				. += "<br>[SPAN_ALERT("[name_to_use] is twitching ever so slightly.")]"

	if (src.organHolder)
		if (src.organHolder.head)
			if (face_visible())
				if (!src.organHolder.skull)
					. += "<br>[SPAN_ALERT("<B>[name_to_use] no longer has a skull in [t_his] head, [t_his] face is just empty skin mush!</B>")]"

				if (!src.organHolder.right_eye && !src.organHolder.left_eye)
					. += "<br>[SPAN_ALERT("<B>[name_to_use]'s eyes are missing!</B>")]"
				else
					if (!src.organHolder.left_eye)
						. += "<br>[SPAN_ALERT("<B>[name_to_use]'s left eye is missing!</B>")]"
					else if (src.organHolder.left_eye.show_on_examine)
						. += "<br>[SPAN_NOTICE("[name_to_use] has [bicon(src.organHolder.left_eye)] \an [src.organHolder.left_eye.organ_name] in [t_his] left eye socket.")]"
					if (!src.organHolder.right_eye)
						. += "<br>[SPAN_ALERT("<B>[name_to_use]'s right eye is missing!</B>")]"
					else if (src.organHolder.right_eye.show_on_examine)
						. += "<br>[SPAN_NOTICE("[name_to_use] has [bicon(src.organHolder.right_eye)] \an [src.organHolder.right_eye.organ_name] in [t_his] right eye socket.")]"

				if (src.organHolder.head.scalp_op_stage > 0)
					if (src.organHolder.head.scalp_op_stage >= 5.0)
						if (!src.organHolder.skull)
							. += "<br>[SPAN_ALERT("<B>There's a gaping hole in [name_to_use]'s head and [t_his] skull is gone!</B>")]"
						else if (!src.organHolder.brain)
							. += "<br>[SPAN_ALERT("<B>There's a gaping hole in [name_to_use]'s head and [t_his] brain is gone!</B>")]"
						else
							. += "<br>[SPAN_ALERT("<B>There's a gaping hole in [name_to_use]'s head!</B>")]"
					else if (src.organHolder.head.scalp_op_stage >= 4.0)
						if (!src.organHolder.brain)
							. += "<br>[SPAN_ALERT("<B>[name_to_use]'s head has been cut open and [t_his] brain is gone!</B>")]"
						else
							. += "<br>[SPAN_ALERT("<B>[name_to_use]'s head has been cut open!</B>")]"
					else
						. += "<br>[SPAN_ALERT("<B>[name_to_use] has an open incision on [t_his] head!</B>")]"

				if (src.organHolder.head.op_stage > 0.0)
					if (src.organHolder.head.op_stage >= 3.0)
						. += "<br>[SPAN_ALERT("<B>[name_to_use]'s head is barely attached!</B>")]"
					else
						. += "<br>[SPAN_ALERT("<B>[name_to_use] has a huge incision across [t_his] neck!</B>")]"

		else
			. += "<br>[SPAN_ALERT("<B>[name_to_use] has been decapitated!</B>")]"


		if (src.organHolder.chest)
			if (src.organHolder.chest.op_stage > 0.0)
				if (src.organHolder.chest.op_stage < 2.0)
					. += "<br>[SPAN_ALERT("<B>[name_to_use] has an indeterminate number of small surgical scars on [t_his] chest!</B>")]"
				if (src.organHolder.chest.op_stage >= 2.0)
					if (src.organHolder.heart)
						. += "<br>[SPAN_ALERT("<B>[name_to_use]'s chest is cut wide open!</B>")]"
					else
						. += "<br>[SPAN_ALERT("<B>[name_to_use]'s chest is cut wide open and [t_his] heart has been removed!</B>")]"
					if (!src.chest_cavity_clamped)
						. += "<br>[SPAN_ALERT("<B>Blood is slowly seeping out of [name_to_use]'s un-clamped chest wound.</B>")]"
			//tailstuff
			if (src.organHolder.tail) // Has a tail?
				...
				if (src.organHolder.tail && (!(src.mob_flags & SHOULD_HAVE_A_TAIL) || src.organHolder.tail?.donor_original != src))
					if (!src.organHolder.butt) // no butt?
						. += "<br>[SPAN_NOTICE("[name_to_use] has [src.organHolder.tail.name] attached just above the spot where [t_his] butt should be.")]"
					else
						. += "<br>[SPAN_NOTICE("[name_to_use] has [src.organHolder.tail.name] attached just above [t_his] butt.")]"
				...
				if (src.organHolder.back_op_stage >= BACK_SURGERY_OPENED && src.mob_flags & ~IS_BONEY) // assive ass wound? and not a skeleton?
					. += "<br>[SPAN_ALERT("<B>[name_to_use] has a long incision around the base of [t_his] tail!</B>")]"
				...
				if (src.organHolder.back_op_stage >= BACK_SURGERY_OPENED) // first person to call this a tailhole is getting dropkicked into the sun
					if (src.mob_flags & SHOULD_HAVE_A_TAIL) // Are they supposed to have a tail?
						if (!src.organHolder.butt) // Also missing a butt?
							. += "<br>[SPAN_ALERT("<B>[name_to_use] has a large incision at the base of [t_his] back where [t_his] tail should be!</B>")]"
						else // has butt
							. += "<br>[SPAN_ALERT("<B>[name_to_use] has a large incision above [t_his] butt where [t_his] tail should be!</B>")]"
					else // Do they normally not have a tail?
						if (!src.organHolder.butt) // Also missing a butt?
							. += "<br>[SPAN_ALERT("<B>[name_to_use] has a large incision at the base of [t_his] back!</B>")]"
						else // has butt
							. += "<br>[SPAN_ALERT("<B>[name_to_use] has a large incision above [t_his] butt!</B>")]"
				else if (src.mob_flags & SHOULD_HAVE_A_TAIL) // No tail, no ass wound? Supposed to have a tail?
					. += "<br>[SPAN_ALERT("<B>[name_to_use] is missing [t_his] tail!</B>")]" // oh no my tails gone!!
					// Commenting on someone not having a tail when they shouldnt have a tail will be left up to the player
		else
			. += "<br>[SPAN_ALERT("<B>[name_to_use]'s entire chest is missing!</B>")]"

		if (!src.organHolder.butt)
			. += "<br>[SPAN_ALERT("<B>[name_to_use]'s butt seems to be missing!</B>")]"
		else if (src.organHolder.back_op_stage > BACK_SURGERY_CLOSED)
			. += "<br>[SPAN_ALERT("<B>[name_to_use] has an open incision on [t_his] butt!</B>")]"

	if (src.limbs)
		if (!src.limbs.l_arm)
			. += "<br>[SPAN_ALERT("<B>[name_to_use]'s left arm is completely severed!</B>")]"
		else
		...
		if (!src.limbs.r_arm)
			. += "<br>[SPAN_ALERT("<B>[name_to_use]'s right arm is completely severed!</B>")]"
		else
			var/limbtxt = src.limbs.r_arm.on_holder_examine()
			if (limbtxt)
				. += "<br>[SPAN_NOTICE("[name_to_use] [limbtxt] right arm.")]"

		if (!src.limbs.l_leg)
			. += "<br>[SPAN_ALERT("<B>[name_to_use]'s left leg is completely severed!</B>")]"
		else
			var/limbtxt = src.limbs.l_leg.on_holder_examine()
			if (limbtxt)
				. += "<br>[SPAN_NOTICE("[name_to_use] [limbtxt] left leg.")]"

		if (!src.limbs.r_leg)
			. += "<br>[SPAN_ALERT("<B>[name_to_use]'s right leg is completely severed!</B>")]"
		else
			var/limbtxt = src.limbs.r_leg.on_holder_examine()
			if (limbtxt)
				. += "<br>[SPAN_NOTICE("[name_to_use] [limbtxt] right leg.")]"
	if (src.bleeding && !isdead(src))
		switch (src.bleeding)
			if (1 to 2)
				. += "<br>[SPAN_ALERT("[name_to_use] is bleeding a little bit.")]"
			if (2 to 3)
				. += "<br>[SPAN_ALERT("<B>[name_to_use] is bleeding!</B>")]"
			if (3 to 4)
				. += "<br>[SPAN_ALERT("<B>[name_to_use] is bleeding a lot!</B>")]"
			if (4 to INFINITY)
				. += "<br>[SPAN_ALERT("<B>[name_to_use] is bleeding very badly!</B>")]"

	if (!isvampire(src)) // Added a check for vampires (Convair880).
		switch (src.blood_pressure["total"])
			if (-INFINITY to 0) // welp
				. += "<br>[SPAN_ALERT("<B>[name_to_use] is pale as a ghost!</B>")]"
			if (1 to 299) // very low (70/50)
				. += "<br>[SPAN_ALERT("<B>[name_to_use] is very pale!</B>")]"
			if (300 to 414) // low (100/65)
				. += "<br>[SPAN_ALERT("<B>[name_to_use] is pale.</B>")]"
			if (585 to 666) // high (140/90)
				. += "<br>[SPAN_ALERT("[name_to_use] is a little sweaty and red in the face.")]"
			if (666 to 750) // very high (160/100)
				. += "<br>[SPAN_ALERT("<B>[name_to_use] is very sweaty and red in the face!</B>")]"
			if (750 to 1500) // critically high (180/110)
				. += "<br>[SPAN_ALERT("<B>[name_to_use] is sweating like a pig and red as a tomato!</B>")]"
	...
	if ((isdead(src)) || changeling_fakedeath || src.bioHolder?.HasEffect("dead_scan") == 2 || (src.reagents.has_reagent("capulettium") && is_incapacitated(src)) || (src.reagents.has_reagent("capulettium_plus") && src.hasStatus("resting")))
		...
		if (brute >= 5)
			if (brute < 30)
				. += "<br>[SPAN_ALERT("[name_to_use] looks a little injured.")]"
			else
				. += "<br>[SPAN_ALERT("<B>[name_to_use] looks severely injured!</B>")]"

		var/burn = src.get_burn_damage()
		if (burn >= 5)
			if (burn < 30)
				. += "<br>[SPAN_ALERT("[name_to_use] looks a little burnt.")]"
			else
				. += "<br>[SPAN_ALERT("<B>[name_to_use] looks severely burned!</B>")]"

		if (src.stat || src.hasStatus("paralysis"))
			. += "<br>[SPAN_ALERT("[name_to_use] doesn't seem to be responding to anything around [t_him], [t_his] eyes closed as though asleep.")]"
		else
			if (src.get_brain_damage() >= BRAIN_DAMAGE_LETHAL || src.reagents?.has_reagent("expresso"))
				. += "<br>[SPAN_ALERT("[name_to_use] has a blank expression on [his_or_her(src)] face.")]"
			...
			if (!src.client && !src.ai_active)
				...
				if (using_vr_goggles)
					if (!(src.wear_suit?.hides_from_examine & C_GLASSES) && !(src.head?.hides_from_examine & C_GLASSES))
						. += "<br><span style='color:#8600C8'>[name_to_use]'s mind is elsewhere.</span>"
				else
					. += "<br>[name_to_use] seems to be staring blankly into space. "
	...
	switch (src.decomp_stage)
		if (DECOMP_STAGE_BLOATED)
			. += "<br>[SPAN_ALERT("[name_to_use] looks bloated and smells a bit rotten!")]"
		if (DECOMP_STAGE_DECAYED)
			. += "<br>[SPAN_ALERT("[name_to_use]'s flesh is starting to rot away from [t_his] bones!")]"
		if (DECOMP_STAGE_HIGHLY_DECAYED)
			. += "<br>[SPAN_ALERT("[name_to_use]'s flesh is almost completely rotten away, revealing parts of [t_his] skeleton!")]"
		if (DECOMP_STAGE_SKELETONIZED)
			. += "<br>[SPAN_ALERT("[name_to_use]'s remains are completely skeletonized.")]"
