		...
		if (src.organHolder.head)
			if (((src.wear_mask && src.wear_mask.see_face) || !src.wear_mask) && ((src.head && src.head.see_face) || !src.head))
				...
				if (src.organHolder.head.op_stage > 0.0)
					if (src.organHolder.head.op_stage == 3.0) //fix me
			...
			if (src.organHolder.brain.installed_aug && src.head && src.head.c_flags & BLOCKCHOKE)
				. += "<br><span class='alert'>[src.name] has a [bicon(src.organHolder.brain.installed_aug)] small, round machine installed at the base of [t_his] skull.</span>"
			else if (src.organHolder.brain.installed_aug)
				. += "<br><span class='alert'>[src.name] has [bicon(src.organHolder.brain.installed_aug)] [src.organHolder.brain.installed_aug] installed at the base of [t_his] skull.</span>"
