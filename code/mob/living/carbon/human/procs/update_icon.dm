...
/mob/living/carbon/human/update_face()
	...
	if (src?.organHolder?.head)
		...
		var/y_to_offset = AHH.customizations[1].offset_y
...
/mob/living/carbon/human/proc/update_hair_layer()
	if ((src.wear_suit && src.wear_suit.over_hair) || (src.head && src.head.seal_hair) || (src.wear_suit && src.wear_suit.body_parts_covered & HEAD))
		...
		src.image_cust_one?.layer = src.bioHolder.mobAppearance.customizations[1].style.default_layer
		src.image_cust_two?.layer = src.bioHolder.mobAppearance.customizations[2].style.default_layer
		src.image_cust_three?.layer = src.bioHolder.mobAppearance.customizations[3].style.default_layer
			...
			if (!src.decomp_stage)
				...
				if(AHOLD.mob_appearance_flags & HAS_EXTRA_DETAILS)
					...
					switch(AHOLD.mob_detail_1_color_ref)
						if(CUST_1)
							human_image.color = AHOLD.customizations[1].color
						if(CUST_2)
							human_image.color = AHOLD.customizations[2].color
						if(CUST_3)
							human_image.color = AHOLD.customizations[3].color
						else
				...
				if(AHOLD.mob_appearance_flags & HAS_OVERSUIT_DETAILS)	// need more oversuits? Make more of these!
					...
					switch(AHOLD.mob_oversuit_1_color_ref)
						if(CUST_1)
							human_detail_image.color = AHOLD.customizations[1].color
						if(CUST_2)
							human_detail_image.color = AHOLD.customizations[2].color
						if(CUST_3)
							human_detail_image.color = AHOLD.customizations[3].color
						else
