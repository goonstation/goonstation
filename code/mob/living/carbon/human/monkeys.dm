...
/mob/living/carbon/human/npc/monkey // :getin:
	...
	proc/ai_pickpocket(priority_only=FALSE)
		if (src.getStatusDuration("weakened") || src.getStatusDuration("stunned") || src.getStatusDuration("unconscious") || src.stat || src.ai_picking_pocket)
			return
	...
	proc/ai_knock_from_hand(priority_only=FALSE)
		if (src.getStatusDuration("weakened") || src.getStatusDuration("stunned") || src.getStatusDuration("unconscious") || src.stat || src.ai_picking_pocket || src.r_hand)
			return
		var/list/possible_targets = list()
		var/list/priority_targets = list()
		for (var/mob/living/carbon/human/H in view(1, src))
