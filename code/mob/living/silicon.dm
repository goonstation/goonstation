...
/mob/living/silicon/click(atom/target, params, location, control)
	...
	if (src.client.check_any_key(KEY_OPEN | KEY_EXAMINE | KEY_POINT) || (equipped && (inrange || (equipped.flags & EXTRADELAY))) || istype(target, /turf) || ishelpermouse(target)) // slightly hacky, oh well, tries to check whether we want to click normally or use attack_ai
		..()
	else
