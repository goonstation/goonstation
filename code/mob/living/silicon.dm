...
/mob/living/silicon/New()
	...
	if(src.syndicate)
		...
		logTheThing("station", src, "New cyborg [src] connects to default SYNDICATE rack [constructName(src.law_rack_connection)]")
	else
		src.law_rack_connection = ticker?.ai_law_rack_manager.default_ai_rack
		logTheThing("station", src, "New cyborg [src] connects to default rack [constructName(src.law_rack_connection)]")
...
/mob/living/silicon/proc/make_syndicate(var/cause)
	...
	if (src.syndicate || src.syndicate_possible)
		...
		logTheThing("station", src, "[src] was made a Syndicate robot at [log_loc(src)]. [cause ? " Source: [constructTarget(cause,"combat")]" : ""]")

		src.law_rack_connection = ticker?.ai_law_rack_manager?.default_ai_rack_syndie
		logTheThing("station", src, "[src.name] is connected to the default Syndicate rack [constructName(src.law_rack_connection)] [cause ? " Source: [constructTarget(cause,"combat")]" : ""]")
		src.show_laws()
...
/mob/living/silicon/proc/remove_syndicate(var/cause)
	...
	logTheThing("station", src, "[src]'s status as a [role != "" ? "[role]" : "rogue robot"] was removed[persistent == 1 ? " (actual antagonist role unchanged)" : ""].[cause ? " Source: [constructTarget(cause,"combat")]" : ""]")
	...
	src.law_rack_connection = ticker?.ai_law_rack_manager?.default_ai_rack
	logTheThing("station", src, "[src.name] is connected to the default rack [constructName(src.law_rack_connection)] [cause ? " Source: [constructTarget(cause,"combat")]" : ""]")
	src.syndicate = FALSE
	return TRUE
...
/mob/living/silicon/hitby(atom/movable/AM, datum/thrown_thing/thr)
	...
	logTheThing("combat", src, "is struck by [AM] [AM.is_open_container() ? "[log_reagents(AM)]" : ""] at [log_loc(src)].")
