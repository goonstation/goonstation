...
/mob/living/intangible/blob_overmind/ai
	...
	proc/ai_process()
		if (refresh_lists > 50 && state > 1 || length(open) + length(open_low) + length(open_medium) + length(closed) <= 1)
			logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Refreshing lists.")
			refresh_lists = 0
		...
		if (state > 1)
			...
			if (fireres_up)
				if (fireres_up.check_requirements())
					...
					fireres_up = null
					logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Took fire resistance upgrade.")
					...
					if (!(locate(/obj/blob) in H.loc))
						...
						if (has_adjacent_blob(T) && prob(50))
							...
							if (T.can_blob_spread_here())
								spread_to(T, 0)
							logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Can't absorb [H] (no blob on tile), attacking instead at [log_loc(H)].")
						continue
					else
		...
		switch (state)
			...
			if (STATE_DEPLOYING)
				...
				if (counter <= 0)
					...
					set_loc(T)
					if (!deploy)
						logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Invalid state for [src]: Cannot find deploy ability in state DEPLOYING.")
						state = 0
						return
					...
					if (deploy in abilities)
						logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Deploy failed.")
						return
					...
					logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Deployed blob to ([T.x], [T.y], [T.z]).")
					counter = 0
			if (STATE_EXPANDING)
				refresh_lists++
				...
				if (gen_up)
					if (gen_up.check_requirements())
						gen_up.take_upgrade()
						logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Took generation rate upgrade while expanding.")
				if (spread_up)
					if (spread_up.check_requirements())
						spread_up.take_upgrade()
						logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Took spread upgrade while expanding.")
				...
				if (ST && !destroying)
					if (!spread)
						logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Invalid state for [src]: Cannot find spread ability in state EXPANDING.")
					spread_to(ST, 1)
					return
				if ((open_medium.len && !destroying) || (destroying && destroy_level == 1))
					...
					if (ST)
						...
						set_loc(ST)
						attack.onUse(ST)
						logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Attacking tile at [log_loc(ST)].")
			...
			if (STATE_DO_LIPIDS)
				...
				if (!A)
					state = STATE_EXPANDING
					logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Failed to find suitable ribosome candidate in 20 attempts.")
					return
				...
				if (L)
					ribosome_count++
				logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Creating ribosome at [log_loc(T)].")
				state = STATE_EXPANDING
			if (STATE_FORTIFYING)
				if (!fortifying)
					state = STATE_EXPANDING
				...
				if (attacker)
					...
					for (var/obj/reagent_dispensers/fueltank/FU in view(attacker))
						if (has_adjacent_blob(FU.loc))
							...
							logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Attacking fuel tank at [log_loc(FU)] in response to attack force.")
					if (has_adjacent_blob(AT))
						attack_now(AT, attack_used)
						logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Hitting [attacker] at [log_loc(attacker.loc)] [attacks - attack_used] times.")
					...
					if (!spreaded)
						for (var/turf/T in range(2, attacker))
							if (T.can_blob_spread_here())
								spread_to(T, 0)
								logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Spreading near [attacker] to [log_loc(T)] in response to attack force.")
								break
				else if (!attacker)
					var/obj/blob/F = null
					//var/preferred = 0
					...
					for (var/turf/T in range(5, nearest))
						if (T.can_blob_spread_here())
							spread_to(T, 0)
							logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Spreading near nearest [nearest] to [log_loc(T)] in response to attack force.")
							break
	...
	proc/spread_to(var/turf/ST, var/is_calm)
		...
		if (locate(/obj/blob) in ST)
			...
			if (is_calm)
				last_spread = ST
			logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Spreading to [log_loc(ST)].")
		else
			return
		update_lists(ST)
		if (!is_calm)
	...
	proc/create_mitochondria_if_possible(var/turf/T)
		if (bio_points >= mito.bio_point_cost && mito.last_used <= world.time)
			set_loc(T)
			mito.onUse(T)
			logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Creating mitochondria at [log_loc(T)].")
			return 1
		return 0
	...
	proc/create_wall_if_possible(var/turf/T)
		if (bio_points >= wall.bio_point_cost && wall.last_used <= world.time)
			set_loc(T)
			wall.onUse(T)
			logTheThing("debug", src, "<b>Marquesas/AI Blob:</b> Creating wall at [log_loc(T)].")
			return 1
		return 0
