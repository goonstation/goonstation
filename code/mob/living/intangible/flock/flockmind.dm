...
/mob/living/intangible/flock/flockmind/proc/partition(free = FALSE)
	...
	if (src.disposed)
		...
		logTheThing("admin", null, "No Flocktraces were created for [src.real_name] due to their death.")
		return TRUE

	if (!length(candidates))
		message_admins("No ghosts responded to a Flocktrace offer from [src.real_name]")
		logTheThing("admin", null, "No ghosts responded to Flocktrace offer from [src.real_name]")
	...
	if (!free && !src.abilityHolder.pointCheck(FLOCKTRACE_COMPUTE_COST))
		...
		logTheThing("admin", null, "Flocktrace offer from [src.real_name] failed due to lack of compute.")
	...
	logTheThing("admin", picked.key, "respawned as a Flocktrace under [src.real_name].")
...
/mob/living/intangible/flock/flockmind/proc/receive_ghosts(var/list/ghosts)
	...
	if(winner) // probably a paranoid check
		...
		logTheThing("admin", src, "made [key_name(trace)] a flocktrace via ghost volunteer respawn.")
