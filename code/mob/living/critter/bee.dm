...
/datum/component/firefly_glow/Initialize(atom/A, mob/living/critter/small_animal/firefly/F, mob/living/carbon/human/user)
	if(add_firefly(A, F, user))
		...
		RegisterSignal(parent, list(COMSIG_ATOM_REAGENT_CHANGE), .proc/bye_fireflies)
...
/datum/component/firefly_glow/UnregisterFromParent()
	...
	update_glow(A, user)
	UnregisterSignal(parent, list(COMSIG_ITEM_PICKUP, COMSIG_ITEM_DROPPED, COMSIG_ATOM_POST_UPDATE_ICON, COMSIG_ATOM_REAGENT_CHANGE))
	. = ..()
