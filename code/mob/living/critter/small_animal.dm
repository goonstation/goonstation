...
/mob/living/critter/small_animal/mouse/weak/mentor
	...
	proc/into_pocket(mob/M, var/voluntary = 1)
		...
		logTheThing("admin", src, "jumps into [constructTarget(M, "admin")]'s pocket as a mentor mouse at [log_loc(M)].")
...
/datum/targetable/critter/mentordisappear
	...
	cast(mob/target)
		...
		M.ghostize()
		qdel(M)
		logTheThing("admin", src, "turned from a mentor mouse to a ghost") // I can remove this but it seems like a good thing to have
		...
		if (act == "flip")
			for (var/obj/item/grab/G in src.equipped_list(check_for_magtractor = 0))
				...
				if (G.state >= GRAB_STRONG && isturf(src.loc) && isturf(G.affecting.loc))
					src.emote("scream")
					logTheThing("combat", src, "crunches [constructTarget(G.affecting,"combat")] [log_loc(src)]")
					M.lastattacker = src
