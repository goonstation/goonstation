...
/datum/action/bar/icon/mechanimateAbility
	...
	onEnd()
		...
		if(ownerMob && target && (BOUNDS_DIST(owner, target) == 0) && mechanimate?.cooldowncheck())
			logTheThing("combat", ownerMob, "injects [constructTarget(target,"combat")]. Crawler transformation")
...
/datum/targetable/critter/dissect
	...
	cast(mob/target)
		...
		logTheThing("combat", M, "uses dissect on [constructTarget(target,"combat")] at [log_loc(M)].")
		return 0
