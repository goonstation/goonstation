...
/datum/limb/king_crab/proc/accident(mob/target, mob/living/user)
	if(check_target_immunity( target ))
		return 0
	if (prob(15))
		logTheThing("combat", user, "accidentally slashes [constructTarget(target,"combat")] with pincers at [log_loc(user)].")
...
/datum/limb/king_crab/harm(mob/target, var/mob/living/user, var/no_logs = 0)
	if (no_logs != 1)
		logTheThing("combat", user, "slashes [constructTarget(target,"combat")] with pincers at [log_loc(user)].")
