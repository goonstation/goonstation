...
/datum/ai_camera_tracker
	...
	proc/can_track(mob/target as mob)
		...
		return (target.loc?.z == 1) \
				...
				|| (hasvar(target, "wear_id") && (istype(target:wear_id, /obj/item/device/pda2) || istype(target:wear_id, /obj/item/clothing/lanyard)) && target:wear_id:ID_card && istype(target:wear_id:ID_card, /obj/item/card/id/syndicate)) \
