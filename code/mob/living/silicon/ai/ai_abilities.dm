	...
	cast(atom/target)
		...
		for(var/obj/machinery/camera/cam in T.cameras)
			...
			if(!ON_COOLDOWN(cam,"[src.type]", 15 SECONDS))
				...
				logTheThing("combat", holder.owner, "[key_name(holder.owner)] fires [src.name], creating metal foam at [log_loc(T)].")
	...
	cast(atom/target)
		...
		for(var/obj/machinery/camera/cam in T.cameras)
			...
			if(!ON_COOLDOWN(cam,"[src.type]", src.projectile_cd))
				...
				SPAWN(charge_time)
					logTheThing("combat", holder.owner, "[key_name(holder.owner)] fires a camera projectile [src.name], targeting [key_name(target)] [log_loc(target)].")
...
/datum/targetable/ai/module/flash
	...
	cast(atom/target)
		...
		if(C)
			logTheThing("combat", holder.owner, "[key_name(holder.owner)] activates AI [src.name], targeting [log_loc(target)].")
