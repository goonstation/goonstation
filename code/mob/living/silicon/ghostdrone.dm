	...
	emote(var/act, var/voluntary = 1)
		...
		switch(lowertext(act))
			...
			if ("scream")
				if (src.emote_check(voluntary, 50))
					if (narrator_mode)
						playsound(get_turf(src), 'sound/vox/scream.ogg', 50, 1, 0, src.get_age_pitch(), channel=VOLUME_CHANNEL_SCREAMFART)
					else
			...
			if ("fart")
				if (src.emote_check(voluntary))
					...
					if (narrator_mode)
						playsound(get_turf(src), 'sound/vox/fart.ogg', 50, 1, channel=VOLUME_CHANNEL_SCREAMFART)
					else
						playsound(get_turf(src), src.sound_fart, 50, 1, channel=VOLUME_CHANNEL_SCREAMFART)
#ifdef DATALOGGER
