...
/mob/living/silicon/ai/proc/de_electrify_verb()
	...
	if(alert("Are you sure?",,"Yes","No") == "Yes")
		for_by_tcl(D, /obj/machinery/door/airlock)
			if (D.z == 1 && D.canAIControl() && !D.isWireCut(9 /*AIRLOCK_WIRE_ELECTRIFY*/) && D.secondsElectrified != 0 )
...
/mob/living/silicon/ai/proc/unbolt_all_airlocks()
	...
	if(alert("Are you sure?",,"Yes","No") == "Yes")
		for_by_tcl(D, /obj/machinery/door/airlock)
			if (D.z == 1 && D.canAIControl() && D.locked && !D.isWireCut(4 /*AIRLOCK_WIRE_DOOR_BOLTS*/) && D.arePowerSystemsOn())
				D.locked = 0
				D.update_icon()
				count++
