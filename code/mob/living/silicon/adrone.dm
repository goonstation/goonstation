#define ADRONE_BATTERY_DISTRESS_INACTIVE 0
#define ADRONE_BATTERY_DISTRESS_ACTIVE 1
...
/mob/living/silicon/adrone
	...
	var/datum/hud/silicon/adrone/hud
...
// Pieces and parts
	...
	var/opened = 0
	var/batteryDistress = ADRONE_BATTERY_DISTRESS_INACTIVE
	...
	New(loc, var/obj/item/parts/robot_parts/adrone_frame/frame = null, var/starter = 0, var/syndie = 0, var/frame_emagged = 0)
		...
		if (starter && !(src.dependent || src.shell))
			...
			if (!frame)
				...
				logTheThing(LOG_DEBUG, null, "<b>I Said No/Composite Adrone:</b> Composite adrone attempted to spawn with null frame")
				qdel(src)
				return
			else
				if (!frame.cell || !frame.brain)
					logTheThing(LOG_DEBUG, null, "<b>I Said No/Composite Adrone:</b> Composite adrone attempted to spawn from incomplete frame")
					qdel(src)
					return
		...
		SPAWN(1.5 SECONDS)
			if (!src.brain && src.key && !(src.dependent || src.shell || src.ai_interface))
				...
				if (!B.owner) //Oh no, they have no mind!
					logTheThing(LOG_DEBUG, null, "<b>Mind</b> Adrone spawn forced to create new mind for key \[[src.key ? src.key : "INVALID KEY"]]")
...
//////////////////////////
	...
	use_power()
		...
		if (src.cell)
			if(src.cell.charge <= 0)
				...
				if(src.cell.charge < (src.cell.maxcharge - ADRONE_BATTERY_WIRELESS_CHARGERATE))
					for_by_tcl(IX, /obj/machinery/interdictor)
						if (IX.expend_interdict(round(ADRONE_BATTERY_WIRELESS_CHARGERATE*1.7),src,TRUE,ITDR_NIMBUS))
							//multiplier to charge rate is an efficiency penalty due to over-the-air charging
							src.cell.give(ADRONE_BATTERY_WIRELESS_CHARGERATE)
							break

				if (fix)
					HealDamage("All", 6, 6)

			if (src.cell.charge <= ADRONE_BATTERY_DISTRESS_THRESHOLD)
				batteryDistress() // Execute distress mode
			else if (src.batteryDistress == ADRONE_BATTERY_DISTRESS_ACTIVE)
	...
	proc/update_appearance()
		...
		if (src.emagged)
			src.i_details.icon_state = "adrone-emagged"
			UpdateOverlays(src.i_details, "adrone-emagged")
		else
...
/mob/living/silicon/adrone/var/image/i_batterydistress

/mob/living/silicon/adrone/proc/batteryDistress()
	...
	if (src.batteryDistress == ADRONE_BATTERY_DISTRESS_INACTIVE) // We only need to apply the indicator when we first enter distress
		...
		src.batteryDistress = ADRONE_BATTERY_DISTRESS_ACTIVE
...
/mob/living/silicon/adrone/set_a_intent(intent)
	. = ..()
	src.hud?.update_intent()

/mob/living/silicon/adrone/proc/clearBatteryDistress()
...
/mob/living/silicon/adrone/verb/cmd_return_mainframe()
	set category = "Drone Commands"
...
/mob/living/silicon/adrone/find_in_hand(var/obj/item/I, var/this_hand)
	if (!I)
		return 0
...
/mob/living/silicon/adrone/find_type_in_hand(var/obj/item/I, var/this_hand)
	if (!I)
		return 0
...
/mob/living/silicon/adrone/find_tool_in_hand(var/tool_flag, var/hand)
	if (hand)
		var/i = 0
...
#undef can_step_sfx
#undef ADRONE_BATTERY_DISTRESS_INACTIVE
#undef ADRONE_BATTERY_DISTRESS_ACTIVE
#undef ADRONE_BATTERY_DISTRESS_THRESHOLD
