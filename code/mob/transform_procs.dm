...
/mob/proc/humanize(var/tele_to_arrival_shuttle = FALSE, var/equip_rank = TRUE, var/random_human = TRUE)
	...
	if (character && istype(character))
		...
		logTheThing("debug", respawned, "Humanize() failed. Player was respawned instead.")
...
/mob/proc/critterize(var/CT)
	if (src.mind || src.client)
		...
		logTheThing("admin", usr, "made [constructTarget(src,"admin")] a critter ([CT]).")
...
/mob/proc/blobize()
	if (src.mind || src.client)
		...
		logTheThing("admin", usr, "made [constructTarget(src,"admin")] a blob.")
...
/mob/proc/machoize(var/shitty = 0)
	if (src.mind || src.client)
		if (shitty)
			...
			logTheThing("admin", null, "[constructTarget(src,"admin")] has been made a faustian macho man.")
		else
			message_admins("[key_name(usr)] made [key_name(src)] a macho man.")
			logTheThing("admin", usr, "made [constructTarget(src,"admin")] a macho man.")
...
/mob/proc/cubeize(var/life = 10, var/CT)
	...
	if (src.mind || src.client)
		...
		logTheThing("admin", usr, "made [constructTarget(src,"admin")] a cube ([CT]) with a lifetime of [life].")
...
/mob/dead/observer/verb/respawn_as_mentor_mouse()
	...
	logTheThing("admin", C, "respawned as a mentor mouse at [log_loc(C)].")
...
/mob/proc/flockerize(var/datum/flock/flock) // this will not host your web apps for you
	if (src.mind || src.client)
		if(flock == null)
			...
			logTheThing("admin", usr, "made [constructTarget(src,"admin")] a flockmind ([src.real_name]).")
			...
			logTheThing("admin", usr, "made [constructTarget(src,"admin")] a flocktrace ([flock.name]).")
