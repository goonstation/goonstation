	...
	if (src.use_stamina)
		...
		if (world.time < src.next_move + SUSTAINED_RUN_GRACE)
			if(move_dir & last_move_dir)
				if (sustained_moves < SUSTAINED_RUN_REQ+1 && sustained_moves + steps >= SUSTAINED_RUN_REQ+1 && !(src.mob_flags & NO_MOVEMENT_PUFFS))
				...
				if (sustained_moves >= SUSTAINED_RUN_REQ+1 && !isFlying && !(src.mob_flags & NO_MOVEMENT_PUFFS))
					...
					if(!(src.mob_flags & NO_MOVEMENT_PUFFS))
...
/mob/living/Move(var/turf/NewLoc, direct)
	...
	if (. && move_dir && !(direct & move_dir) && src.use_stamina)
		if (sustained_moves >= SUSTAINED_RUN_REQ+1 && !(src.mob_flags & NO_MOVEMENT_PUFFS))
			sprint_particle_small(src,get_step(NewLoc,turn(move_dir,180)),turn(move_dir,180))
			playsound(src.loc,"sound/effects/sprint_puff.ogg", 9, 1,extrarange = -25, pitch=2.8)
		sustained_moves = 0
	...
	if (special_sprint && src.client)
		...
		if (!next_step_delay && world.time >= next_sprint_boost)
			if (!HAS_MOB_PROPERTY(src, PROP_CANTMOVE))
				...
				if ((src.loc != last || force_puff) && !(src.mob_flags & NO_MOVEMENT_PUFFS)) //ugly check to prevent stationary sprint weirds
