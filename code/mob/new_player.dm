...
mob/new_player
	...
	proc/IsJobAvailable(var/datum/job/JOB)
		...
		if (JOB.needs_college && !src.has_medal("Unlike the director, I went to college"))
			return 0
		if (src.client && src.client.player)
			var/round_num = src.client.player.get_rounds_participated()
			if (!isnull(round_num) && (round_num < JOB.rounds_needed_to_play || round_num > JOB.max_rounds_to_play)) //they havent played enough rounds!
				return 0
