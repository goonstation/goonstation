	...
	for (var/client/C as anything in clients)
		...
		if (C.holder)
			if (usr.client.holder) //The viewer is an admin, we can show them stuff
				var/thisW = "<a href='byond://?src=\ref[usr.client.holder];action=adminplayeropts;targetckey=[C.get_ckey()]' class='adminooc text-normal'>"
				if (C.stealth)
					if (C.fakekey)
		...
		//Mentors
			...
			if (usr.client.holder)
				thisW += "<a href='byond://?src=\ref[usr.client.holder];action=adminplayeropts;targetckey=[C.get_ckey()]' class='mentorooc text-normal'>"
			else
		...
		//Normies
			...
			var/thisW
			if (usr.client.holder)
				thisW += "<a href='byond://?src=\ref[usr.client.holder];action=adminplayeropts;targetckey=[C.get_ckey()]' class='ooc text-normal'>"
			else
		...
		if (length(whoAdmins) < 1)
			for (var/client/C as anything in clients)
				if (C?.holder?.adminwho_alerts && !C.player_mode)
					var/msg = SPAN_ADMIN("ADMIN LOG: [key_name(usr)] used Who and saw [length(whoAdmins)] admins.")
...
/client/verb/adminwho()
	...
	if(!usr.client.holder)
		...
		for(var/client/C as anything in clients)
			if(C?.holder?.adminwho_alerts && !C.player_mode)
				var/msg = SPAN_ADMIN("ADMIN LOG: [key_name(usr)] used adminwho and saw [adwnum] admins.")
				boutput(C, replacetext(replacetext(msg, "%admin_ref%", "\ref[C?.holder]"), "%client_ref%", "\ref[C]"))
