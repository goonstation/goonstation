...
/mob/proc/process_move(keys)
	...
	if (src.move_dir)
		...
		if ((keys & KEY_RUN) && \
		      ((H.get_stamina() > STAMINA_COST_SPRINT && HAS_ATOM_PROPERTY(src, PROP_FAILED_SPRINT_FLOP)) ||  H.get_stamina() > STAMINA_SPRINT) && \
			  !HAS_ATOM_PROPERTY(src, PROP_CANTSPRINT))
			running = 1
		...
		if (src.client && src.client.flying || (ismob(src) && HAS_ATOM_PROPERTY(src, PROP_NOCLIP)))
			...
			if (src.buckled && !istype(src.buckled, /obj/stool/chair))
				...
				if (!spacemove) // buh
					...
					if (src.loc != old_loc)
						...
						if(src.get_stamina() < STAMINA_COST_SPRINT && HAS_ATOM_PROPERTY(src, PROP_FAILED_SPRINT_FLOP)) //Check after move rather than before so we cleanly transition from sprint to flop
