...
/mob/wraith
	...
	death(gibbed)
		...
		if (deaths < 2)
			...
			logTheThing("combat", src, "lost a life as a wraith at [log_loc(src.loc)].")
			src.justdied = 1
			...
			logTheThing("combat", src, "died as a wraith with no more respawns at [log_loc(src.loc)].")
			if (src.mind)
				for (var/datum/objective/specialist/wraith/WO in src.mind.objectives)
					WO.onBanished()
	...
	say(var/message)
		...
		if (src.density) //If corporeal speak to the living (garbled)
			logTheThing("diary", src, "(WRAITH): [message]", "say")
...
/mob/proc/wraithize()
	if (src.mind || src.client)
		...
		logTheThing("admin", usr, "made [constructTarget(src,"admin")] a wraith.")
		return make_wraith()
	return null
