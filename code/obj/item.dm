...
/obj/item/proc/Eat(var/mob/M as mob, var/mob/user, var/by_matter_eater=FALSE)
	...
	if (M == user)
		...
		logTheThing("combat", user, "attempts to feed [constructTarget(M,"combat")] [src] [log_reagents(src)]")
		...
		logTheThing("combat", user, "feeds [constructTarget(M,"combat")] [src] [log_reagents(src)]")
	...
	if (istype(src, /obj/item/plant))
		if (!GET_COOLDOWN(global, "hotbox_adminlog"))
			...
			if (length(hotbox_plants) >= 5) //number is up for debate, 5 seemed like a good starting place
				...
				message_admins(msg)
				logTheThing("bombing", W?.fingerprintslast, msg)
	...
	if(!src.anchored)
		if (iswraith(usr))
			...
			if (W.weak_tk && !IN_RANGE(src, W, 2))
				...
				logTheThing("combat", usr, "moves [src] [dir2text(get_dir(usr, over_object))] with wtk.")
				return
			src.throw_at(over_object, 7, 1)
			logTheThing("combat", usr, "throws [src] [dir2text(get_dir(usr, over_object))] with wtk.")
		else if (ismegakrampus(usr))
			src.throw_at(over_object, 7, 1)
			logTheThing("combat", usr, "throws [src] [dir2text(get_dir(usr, over_object))] with k_tk.")
			...
			src.throw_at(over_object, 7, 1)
			logTheThing("combat", usr, "throws [src] [dir2text(get_dir(usr, over_object))] with tk.")
			...
			if (istype(usr:abilityHolder, /datum/abilityHolder/ghost_observer))
				...
				if (GH.spooking)
					...
					logTheThing("combat", usr, "throws [src] with g_tk.")
#endif
...
/obj/item/proc/attack(mob/M, mob/user, def_zone, is_special = 0)
	...
	if (src.flags & SUPPRESSATTACK)
		logTheThing("combat", user, "uses [src] ([type], object name: [initial(name)]) on [constructTarget(M,"combat")]")
		return
	...
	if(user.traitHolder && !user.traitHolder.hasTrait("glasscannon"))
		if (!user.process_stamina(src.stamina_cost))
			logTheThing("combat", user, "tries to attack [constructTarget(M,"combat")] with [src] ([type], object name: [initial(name)]) but is out of stamina")
			return
	...
	if (!M.melee_attack_test(user, src, d_zone))
		logTheThing("combat", user, "attacks [constructTarget(M,"combat")] with [src] ([type], object name: [initial(name)]) but the attack is blocked!")
		return
