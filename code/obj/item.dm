...
/obj/item/proc/equipped(var/mob/user, var/slot)
	...
	if (src.hasProperty("meleeprot"))
		APPLY_MOB_PROPERTY(user, PROP_MELEEPROT_BODY, \ref[src], src.getProperty("meleeprot"))
	if (src.hasProperty("meleeprot_head"))
		APPLY_MOB_PROPERTY(user, PROP_MELEEPROT_HEAD, \ref[src], src.getProperty("meleeprot_head"))
	if (src.hasProperty("meleeprot_all"))
		APPLY_MOB_PROPERTY(user, PROP_MELEEPROT_BODY, \ref[src], src.getProperty("meleeprot_all"))
		APPLY_MOB_PROPERTY(user, PROP_MELEEPROT_HEAD, \ref[src], src.getProperty("meleeprot_all"))
	if (src.hasProperty("rangedprot"))
		APPLY_MOB_PROPERTY(user, PROP_RANGEDPROT, \ref[src], src.getProperty("rangedprot"))
	if (src.hasProperty("radprot"))
		APPLY_MOB_PROPERTY(user, PROP_RADPROT, \ref[src], src.getProperty("radprot"))
	if (src.hasProperty("heatprot"))
		APPLY_MOB_PROPERTY(user, PROP_HEATPROT, \ref[src], src.getProperty("heatprot"))
	if (src.hasProperty("coldprot"))
		APPLY_MOB_PROPERTY(user, PROP_COLDPROT, \ref[src], src.getProperty("coldprot"))
...
/obj/item/proc/unequipped(var/mob/user)
	...
	if (!ishuman(user)) //!!currently!! we only want to humans check for these stats, so just abort early if user isnt human, saves us time
		return
	if (src.hasProperty("meleeprot"))
		REMOVE_MOB_PROPERTY(user, PROP_MELEEPROT_BODY, \ref[src])
	if (src.hasProperty("meleeprot_head"))
		REMOVE_MOB_PROPERTY(user, PROP_MELEEPROT_HEAD, \ref[src])
	if (src.hasProperty("meleeprot_all"))
		REMOVE_MOB_PROPERTY(user, PROP_MELEEPROT_BODY, \ref[src])
		REMOVE_MOB_PROPERTY(user, PROP_MELEEPROT_HEAD, \ref[src])
	if (src.hasProperty("rangedprot"))
		REMOVE_MOB_PROPERTY(user, PROP_RANGEDPROT, \ref[src])
	if (src.hasProperty("radprot"))
		REMOVE_MOB_PROPERTY(user, PROP_RADPROT, \ref[src])
	if (src.hasProperty("heatprot"))
		REMOVE_MOB_PROPERTY(user, PROP_HEATPROT, \ref[src])
	if (src.hasProperty("coldprot"))
		REMOVE_MOB_PROPERTY(user, PROP_COLDPROT, \ref[src])
