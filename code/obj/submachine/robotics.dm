	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		switch(action)
			...
			if("Create Patch")
				...
				P.medical = all_safe
				P.on_reagent_change()
				logTheThing("chemistry",user,null,"created a [patchname] patch containing [log_reagents(P)].")
			if("Create Ampoule")
				var/datum/reagents/R = B.reagents
				...
				R.trans_to(A, 5)
				logTheThing("chemistry",user,null,"created a [ampoulename] ampoule containing [log_reagents(A)].")
