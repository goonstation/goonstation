		...
		if (istype(jumper) && istype(S) && istype(apc) )
			...
			if (recipient_cell.charge >= recipient_cell.maxcharge)
				...
				if (jumper.positive)
					...
					user.visible_message("<span class='notice'>[user] transfers some of the power from [apc] to itself!</span>", "<span class='notice'>You transfer [overspill] charge. You are now fully charged.</span>")
