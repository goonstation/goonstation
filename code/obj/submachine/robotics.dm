	...
	attackby(obj/item/W, mob/user)
		...
		switch(action)
			...
			if("Create Patch")
				...
				P.medical = all_safe
				P.on_reagent_change()
				logTheThing("combat", user, "created a [patchname] patch containing [log_reagents(P)].")
			if("Create Ampoule")
				var/datum/reagents/R = B.reagents
				...
				R.trans_to(A, 5)
				logTheThing("combat", user, "created a [ampoulename] ampoule containing [log_reagents(A)].")
		...
		if (src.last_use && world.time < src.last_use + 50)
			...
			switch(src.vend_this)
				...
				if ("Water")
					...
					logTheThing("debug", user, "<b>Convair880</b>: [user]'s food synthesizer was set to an invalid value.")
					src.vend_this = null
					return
