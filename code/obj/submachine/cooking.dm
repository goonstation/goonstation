	...
	attack_hand(var/mob/user)
		...
		for(var/obj/item/P in src.contents)
			if (istype(P,/obj/item/reagent_containers/food/drinks))
				...
				if (milk_amount < MIN_FLUID_INGREDIENT_LEVEL)
					continue

				var/cream_amount = roundfloor(milk_amount / MIN_FLUID_INGREDIENT_LEVEL)
				P.reagents.remove_reagent("milk", cream_amount * MIN_FLUID_INGREDIENT_LEVEL)
