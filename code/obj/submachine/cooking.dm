...
table#cooktime a#start {
	...
	//        more complicated one and pissing off the chef by giving something different than what he wanted!
		...
		if (!src.recipes.len)
			...
			src.recipes += new /datum/cookingrecipe/chilifries(src)
			src.recipes += new /datum/cookingrecipe/chilifries_alt(src)
	...
	attack_hand(var/mob/user)
		...
		for(var/obj/item/P in src.contents)
			if (istype(P,/obj/item/reagent_containers/food/drinks))
				...
				var/miasma_amount = P.reagents.get_reagent_amount("miasma")
				if (miasma_amount >= 5 && milk_amount >= 5)
					var/yoghurt_amount = ((roundfloor(milk_amount / 5) + roundfloor(miasma_amount / 5)) / 2)
					P.reagents.remove_reagent("milk", yoghurt_amount * 5)
					P.reagents.remove_reagent("miasma", yoghurt_amount * 5)
					for (var/i in 1 to yoghurt_amount)
						new/obj/item/reagent_containers/food/snacks/yoghurt(src.loc)

				else if (milk_amount >= 10)
					var/cream_amount = roundfloor(milk_amount / 10)
					P.reagents.remove_reagent("milk", cream_amount * 10)
					for (var/i in 1 to cream_amount)
