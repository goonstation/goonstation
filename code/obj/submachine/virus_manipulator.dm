	...
	Topic(href, href_list)
		if(href_list["ops"])
			...
			if(operation == 4) // Eject Vial
				...
				logTheThing("chemistry", usr, null, "modified <i>(<b>[log_reagents]</b>)</i> to [src.dataspread], cure = [src.datacure], curable = [src.datacurable], regress = [src.dataregress], speed =[src.dataprob], vaccine = [src.datavaccine]")
