...
/obj/shrub/captainshrub
	...
	graze(mob/user)
		...
		if (user.mind && user.mind.assigned_role == "Captain")
			...
			src.destroy()
			logTheThing(LOG_COMBAT, src, "was eaten by [constructTarget(user,"combat")] at [log_loc(target)].")
	...
	attackby(obj/item/W, mob/user)
		...
		if(src.destroyed && !was_destroyed)
			...
			logTheThing(LOG_COMBAT, src, "was destroyed by [constructTarget(user,"combat")] with [W] at [log_loc(target)].")
			src.interesting = "Inexplicably, the genetic code of the bonsai tree has the words 'fuck [user.real_name]' encoded in it over and over again."

	meteorhit(obj/O as obj)
		src.visible_message(SPAN_ALERT("<b>The meteor smashes right through [src]!</b>"))
		src.destroy()
		logTheThing(LOG_COMBAT, src, "was destroyed by a meteor at [log_loc(target)].")
	...
	ex_act(severity)
		...
		src.destroy()
		logTheThing(LOG_COMBAT, src, "was destroyed by an explosion at [log_loc(target)].")
