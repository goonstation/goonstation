...
/obj/item/assembly/receive_signal(datum/signal/signal)
	...
	if(src.force_dud == TRUE)
		message_admins("A [src.name] would have activated at [log_loc(src)] but was forced to dud! Armed by: [key_name(src.last_armer)]; Last touched by: [key_name(last_ckey)]")
		logTheThing(LOG_BOMBING, null, "A [src.name] would have activated at [log_loc(src)] but was forced to dud! Armed by: [key_name(src.last_armer)]; Last touched by: [last_ckey]")
		return
	...
	if (src.secured && (signal && signal.source != src.applier))
		...
		logTheThing(LOG_BOMBING, src.last_armer, "A [src.name] was activated at [log_loc(src)]. Armed by: [key_name(src.last_armer)]; Last touched by: [last_ckey];[src.get_additional_logging_information(src.last_armer)]")
		if(src.requires_admin_messaging())
			message_admins("A [src.name] was activated at [log_loc(src)]. Armed by: [key_name(src.last_armer)]; Last touched by: [last_ckey]")
