...
/datum/action/bar/icon/pin_target
	...
	onEnd()
		..()
		var/mob/ownerMob = owner
		if(owner && ownerMob && target && G && G.state != GRAB_PIN && BOUNDS_DIST(owner, target) == 0 && BOUNDS_DIST(owner, T) == 0 && !GET_ATOM_PROPERTY(target, PROP_MOB_CANT_BE_PINNED) && T.gforce_current > 0)
			G.upgrade_to_pin(T)
		else
			interrupt(INTERRUPT_ALWAYS)
...
/obj/item/grab/block/handle_throw(mob/living/user, atom/target)
	if (isturf(user.loc) && target)
		...
		if(!target_dir)
			target_dir = user.dir
		if (T.gforce_current > 0 && !(user.lying) && can_act(user) && !HAS_ATOM_PROPERTY(user, PROP_MOB_CANTMOVE) && target_dir &&!isghostcritter(user))
