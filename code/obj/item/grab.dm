...
/obj/item/grab //TODO : pool grabs
	...
	disposing()
		...
		if(affecting)
			...
			if (state == GRAB_CHOKE)
				logTheThing("combat", src.assailant, "releases their choke on [constructTarget(src.affecting,"combat")] after [choke_count] cycles at [log_loc(src.affecting)]")
			else if (state == GRAB_PIN)
				logTheThing("combat", src.assailant, "drops their pin on [constructTarget(src.affecting,"combat")] at [log_loc(src.affecting)]")
			else if(!istype(src, /obj/item/grab/block))
				logTheThing("combat", src.assailant, "drops their grab on [constructTarget(src.affecting,"combat")] at [log_loc(src.affecting)]")
		...
		switch (src.state)
			if (GRAB_PASSIVE)
				if (src.affecting.buckled)
					...
					logTheThing("combat", src.assailant, "'s grip upped to aggressive on [constructTarget(src.affecting,"combat")]")
			...
			if (GRAB_CHOKE)
				src.state = GRAB_AGGRESSIVE
				logTheThing("combat", src.assailant, "releases their choke on [constructTarget(src.affecting,"combat")] after [choke_count] cycles")
				for (var/mob/O in AIviewers(src.assailant, null))
	...
	proc/upgrade_to_choke(var/msg_overridden = 0)
		...
		logTheThing("combat", src.assailant, "chokes [constructTarget(src.affecting,"combat")]")
		choke_count = 0
	...
	proc/upgrade_to_pin(var/turf/T)
		...
		icon_state = "pin"
		logTheThing("combat", src.assailant, "pins [constructTarget(src.affecting,"combat")]")
...
/atom/proc/grab_smash(obj/item/grab/G as obj, mob/user as mob)
	...
	logTheThing("combat", user, "smashes [constructTarget(M,"combat")] against [src]")
	...
	if (isturf(user.loc) && target)
		...
		if (!(T.turf_flags & CAN_BE_SPACE_SAMPLE) && !(user.lying) && can_act(user) && !HAS_ATOM_PROPERTY(user, PROP_MOB_CANTMOVE) && target_dir)
			...
			if (dive_attack_hit)
				...
				logTheThing("combat", user, "slides into [dive_attack_hit] at [log_loc(dive_attack_hit)].")
