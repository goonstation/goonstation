...
/*
	process()
		...
		if (!(src in processing_items))
			logTheThing("debug", null, "<b>Convair880</b>: Process() was called for an egun ([src]) that wasn't in the item loop. Last touched by: [src.fingerprintslast]")
		...
		if (findtext(text, "iamthelaw"))
			...
			if (src in M.contents)
				if (M.job != "Head of Security")
					...
					logTheThing("combat", src, "Is not the law. Caused explosion with Lawbringer.")
