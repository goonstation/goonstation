...
/obj/item/sword/proc/handle_parry(mob/target, mob/user)
	if (target != user && ishuman(target))
		...
		if (S && S.active && !(H.lying || isdead(H) || H.hasStatus("stunned", "weakened", "paralysis")))
			var/obj/itemspecialeffect/clash/C = new /obj/itemspecialeffect/clash
			if(target.gender == MALE) playsound(target, pick('sound/weapons/male_cswordattack1.ogg','sound/weapons/male_cswordattack2.ogg'), 70, 0, 5, clamp(1.2, 0.7, 1.0 + (30 - H.bioHolder.age)/60))
			else playsound(target, pick('sound/weapons/female_cswordattack1.ogg','sound/weapons/female_cswordattack2.ogg'), 70, 0, 5, clamp(1.4, 0.7, 1.0 + (30 - H.bioHolder.age)/50))
			C.setup(H.loc)
			var/matrix/m = matrix()
			m.Turn(rand(0,360))
			C.transform = m
...
/obj/item/sword/attack_self(mob/user as mob)
	...
	if (src.active)
		...
		if(ishuman(user) && !ON_COOLDOWN(src, "playsound_on", 2 SECONDS))
			var/mob/living/carbon/human/U = user
			if(U.gender == MALE) playsound(U,"sound/weapons/male_cswordstart.ogg", 70, 0, 5, clamp(1.2, 0.7, 1.0 + (30 - U.bioHolder.age)/60))
			else playsound(U,"sound/weapons/female_cswordturnon.ogg" , 100, 0, 5, clamp(1.4, 0.7, 1.0 + (30 - U.bioHolder.age)/50))
		...
		if(ishuman(user) && !ON_COOLDOWN(src, "playsound_off", 2 SECONDS))
			var/mob/living/carbon/human/U = user
			if(U.gender == MALE) playsound(U,"sound/weapons/male_cswordturnoff.ogg", 70, 0, 5, clamp(1.2, 0.7, 1.0 + (30 - U.bioHolder.age)/60))
			else playsound(U,"sound/weapons/female_cswordturnoff.ogg", 100, 0, 5, clamp(1.4, 0.7, 1.0 + (30 - U.bioHolder.age)/50))
