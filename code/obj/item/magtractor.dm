...
/obj/item/magtractor
	...
	proc/releaseItem()
		...
		if (!src || !src.holding || usr.stat || usr.getStatusDuration("stunned") || usr.getStatusDuration("weakened") || usr.getStatusDuration("unconscious")) return 0
		actions.stopId(/datum/action/magPickerHold, usr)
		return 1
	...
	proc/toggleHighPower()
		...
		if (!src || usr.stat || usr.getStatusDuration("stunned") || usr.getStatusDuration("weakened") || usr.getStatusDuration("unconscious")) return 0
