		...
		if (locate(/obj/grille, user.loc))
			...
			SPAWN(1.5 SECONDS)
				if (T == user.loc && !user.weakened && !user.getStatusDuration("stunned"))
					...
					G.setMaterial(src.material)
					logTheThing("station", user, "builds a Grille in [user.loc.loc] ([log_loc(user)])")
		...
		if (href_list["make"])
			...
			switch(href_list["make"])
				...
				if("closet")
					...
					A.setMaterial(src.material)
					logTheThing("station", usr, "builds a Closet in [usr.loc.loc] ([log_loc(usr)])")
				if("fl_tiles")
				...
				if("computer")
					...
					A.setMaterial(src.material)
					logTheThing("station", usr, "builds a Console Frame in [usr.loc.loc] ([log_loc(usr)])")
				if("hcomputer")
					if(src.amount < 5)
					...
					A.setMaterial(src.material)
					logTheThing("station", usr, "builds a Computer Frame in [usr.loc.loc] ([log_loc(usr)])")
				if("tcomputer")
					if(src.amount < 3)
					...
					A.setMaterial(src.material)
					logTheThing("station", usr, "builds a Terminal Frame in [usr.loc.loc] ([log_loc(usr)])")
				if("construct")
					if (src.amount < 2)
					...
					if ((usr.loc == location))
						...
						A.setMaterial(src.material)
						logTheThing("station", usr, "builds Wall Girders in [usr.loc.loc] ([log_loc(usr)])")
