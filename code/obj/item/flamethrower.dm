...
/obj/item/gun/flamethrower/
	...
	log_shoot(mob/user, turf/T, obj/projectile/P)
		logTheThing("combat", user, "fires \a [src] ([lit ? "lit, " : ""][MODE_TO_STRING(mode)]) from [log_loc(user)], vector: ([T.x - user.x], [T.y - user.y]), dir: <I>[dir2text(get_dir(user, T))]</I>, reagents: [log_reagents(src.fueltank)] with chamber volume [amt_chem]")
