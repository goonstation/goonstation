/obj/item/spellbook
	...
	proc/CheckRequirements(var/mob/living/carbon/human/user,var/datum/uplinkspell/spell)
		if (!user || !spell)
			return FALSE // unknown error
		if (src.vr && !spell.vr_allowed)
			return FALSE // Unavailable in VR
		if (spell.assoc_spell)
			if (user.abilityHolder.getAbility(spell.assoc_spell))
				return FALSE // Already have this spell
		return TRUE

	proc/Spell_Purchased(var/mob/living/carbon/human/user,var/datum/uplinkspell/spell)
		if (!user || !spell)
			return
		logTheThing(LOG_DEBUG, null, "[constructTarget(user)] purchased the spell [src.name] using the [src] uplink.")
		if (spell.assoc_spell)
			user.abilityHolder.addAbility(spell.assoc_spell)
		if (spell.assoc_item)
			new spell.assoc_item(user.loc)
...
/// A list of '/datum/uplinkspell's available to wizards. This is lazily instantiated, use the getter 'get_wizard_spell_list()' to access this, or
/// make sure it's cached first.
var/global/list/wizard_spell_list

// this seems like the most flexible way to do it. Using concrete types doesn't work well with multiple types of spellbooks, especially if
// spells ever want to be shared.
proc/get_wizard_spell_list()
	if (!wizard_spell_list)
		wizard_spell_list = list(
			// Enchantment
			new /datum/uplinkspell/soulguard,
			// Equipment
			new /datum/uplinkspell/staffofcthulhu,
			new /datum/uplinkspell/staffofthunder,
			// Offensive
			new /datum/uplinkspell/bull,
			//new /datum/uplinkspell/shockwave,
			new /datum/uplinkspell/fireball,
			new /datum/uplinkspell/prismatic_spray,
			//new /datum/uplinkspell/shockinggrasp,
			new /datum/uplinkspell/shockingtouch,
			new /datum/uplinkspell/iceburst,
			new /datum/uplinkspell/blind,
			new /datum/uplinkspell/clownsrevenge,
			new /datum/uplinkspell/balefulpolymorph,
			//new /datum/uplinkspell/lightningbolt,
			new /datum/uplinkspell/rathensecret,
			// Defensive
			new /datum/uplinkspell/forcewall,
			new /datum/uplinkspell/blink,
			new /datum/uplinkspell/teleport,
			new /datum/uplinkspell/warp,
			new /datum/uplinkspell/spellshield,
			new /datum/uplinkspell/doppelganger,
			// Utility
			new /datum/uplinkspell/knock,
			new /datum/uplinkspell/empower,
			new /datum/uplinkspell/summongolem,
			new /datum/uplinkspell/stickstosnakes,
			new /datum/uplinkspell/animatedead,
			// Miscellaneous
			new /datum/uplinkspell/pandemonium
		)
	return wizard_spell_list

/obj/item/spellbook/SWF_uplink
	...
	CheckRequirements(var/mob/living/carbon/human/user,var/datum/uplinkspell/spell)
		if (!user || !spell)
			return FALSE // unknown error
		if (src.vr && !spell.vr_allowed)
			return FALSE // Unavailable in VR
		if (spell.assoc_spell)
			if (src.antag_datum.ability_holder.getAbility(spell.assoc_spell))
				return FALSE // Already have this spell
		if (src.uses < spell.cost)
			return FALSE // ran out of points
		return TRUE

	Spell_Purchased(var/mob/living/carbon/human/user,var/datum/uplinkspell/spell)
		if (!user || !spell)
			return
		logTheThing(LOG_DEBUG, null, "[constructTarget(user)] purchased the spell [spell.name] using the [src] uplink.")
		if (spell.assoc_spell)
			src.antag_datum.ability_holder.addAbility(spell.assoc_spell)
		if (spell.assoc_item)
			var/obj/item/I = new spell.assoc_item(user.loc)
			if (istype(I, /obj/item/staff) && user.mind && !isvirtual(user))
				var/obj/item/staff/S = I
				S.wizard_key = user.mind.key
		src.uses -= spell.cost
		src.antag_datum.purchased_spells.Add(src) // Remember spell for crew credits
	...
	New(datum/antagonist/wizard/antag, in_vr = FALSE)
		...
		src.spells = get_wizard_spell_list()
...
ABSTRACT_TYPE(/datum/uplinkspell)
/datum/uplinkspell

	var/name = "Spell"
	var/eqtype = "Spell"
	var/spell_source = null
	...
	proc/CheckRequirements(var/mob/living/carbon/human/user,var/obj/item/spellbook/book)
		return book.CheckRequirements(user, src)

	proc/Spell_Purchased(var/mob/living/carbon/human/user,var/obj/item/spellbook/book)
		return book.Spell_Purchased(user, src)

//------------ ENCHANTMENT SPELLS ------------//
/datum/uplinkspell/soulguard
	name = "Soulguard"
	spell_source = "wizard"
	eqtype = "Enchantment"
	vr_allowed = 0
	...
	CheckRequirements(var/mob/living/carbon/human/user,var/obj/item/spellbook/book)
		. = ..()
...
//------------ EQUIPMENT SPELLS ------------//
/datum/uplinkspell/staffofcthulhu
...
/datum/uplinkspell/staffofthunder
...
/datum/uplinkspell/prismatic_spray
...
*/
/datum/uplinkspell/shockingtouch
	name = "Shocking Touch"
	eqtype = "Offensive"
...
/datum/uplinkspell/balefulpolymorph
...
/datum/uplinkspell/rathensecret
...
/*/datum/uplinkspell/lightningbolt
	name = "Lightning Bolt"
	eqtype = "Offensive"
...
/datum/uplinkspell/doppelganger
	name = "Doppelganger"
	eqtype = "Defensive"
...
/datum/uplinkspell/stickstosnakes
