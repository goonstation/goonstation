...
/obj/deployable_turret
	...
	proc/target_valid(var/mob/living/C)
		...
		if (istype(C,/mob/living/carbon/human))
			...
			if (H.hasStatus(list("resting", "weakened", "stunned", "unconscious"))) // stops it from uselessly firing at people who are already suppressed. It's meant to be a suppression weapon!
				return FALSE
		if (is_friend(C))
			return FALSE
