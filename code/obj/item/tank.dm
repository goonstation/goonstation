	...
	proc/check_status()
		...
		if(pressure > TANK_FRAGMENT_PRESSURE) // 50 atmospheres, or: 5066.25 kpa under current _setup.dm conditions
			...
			if(src.air_contents.radgas > 0)
				...
				// Thank goodness there's a lot of them! (With maxcap values you can get around 5.6 mols fallout in here tops, which is ~40 neutrons)
				var/neutrons_to_emit = ceil( 5 * sqrt( src.air_contents.radgas * range ) )
				...
				// Based off neutrons_to_emit in a way, but to be a multiplier value between 0 and 1
				rad_damage_multiplier = clamp(neutrons_to_emit / 100, 0, 1)
