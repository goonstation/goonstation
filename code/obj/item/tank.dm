		...
		if(pressure > TANK_FRAGMENT_PRESSURE) // 50 atmospheres, or: 5066.25 kpa under current _setup.dm conditions
			...
			if(src in bible_contents)
				...
				for_by_tcl(B, /obj/item/bible)
					...
					if(T)
						logTheThing(LOG_BOMBING, src, "exploded at [log_loc(T)], range: [range], last touched by: [replace_if_null(src.get_last_ckey(), "None")]")
						explosion(src, T, range * 0.25, range * 0.5, range, range * 1.5, flash_radiation_multiplier=rad_damage_multiplier)
				qdel(src)
				return
			var/turf/epicenter = get_turf(loc)
			logTheThing(LOG_BOMBING, src, "exploded at [log_loc(epicenter)], , range: [range], last touched by: [replace_if_null(src.get_last_ckey(), "None")]")
