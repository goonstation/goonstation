...
/obj/item/raw_material/erebite
	...
	ex_act(severity)
		...
		if (src.z != MINING_Z)
			...
			if (bombturf)
				...
				logTheThing(LOG_BOMBING, null, "Erebite detonated by an explosion in [bombarea] ([log_loc(bombturf)]). Last touched by: [last_ckey]")
	...
	temperature_expose(null, temp, volume)
		...
		if (src.z != MINING_Z)
			...
			var/last_ckey = src.get_last_ckey()
			logTheThing(LOG_BOMBING, null, "Erebite detonated by heat in [bombarea]. Last touched by: [last_ckey]")
			if(last_ckey && !istype(get_area(bombturf), /area/mining/magnet))
