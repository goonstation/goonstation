...
/obj/item/raw_material/erebite
	...
	ex_act(severity)
		...
		if (src.z != MINING_Z)
			...
			if (bombturf)
				...
				logTheThing("bombing", null, null, "Erebite detonated by an explosion in [bombarea] ([log_loc(bombturf)]). Last touched by: [src.fingerprintslast]")
	...
	temperature_expose(null, temp, volume)
		...
		if (src.z != MINING_Z)
			...
			logTheThing("bombing", null, null, "Erebite detonated by heat in [bombarea]. Last touched by: [src.fingerprintslast]")
			if(src.fingerprintslast && !istype(get_area(bombturf), /area/mining/magnet))
