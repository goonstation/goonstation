...
/obj/item/clothing/gloves/crafted
	...
	insulating
		onMaterialChanged()
			...
			if(istype(src.material))
				...
				switch(src.material.getProperty(MATERIAL_PROPERTY_ELECTRICAL)/9)
					if(0 to 1)
				...
				var/thermal_insul = max(0, 5 - src.material.getProperty(MATERIAL_PROPERTY_THERMAL)/9)
	...
	armored
		...
		onMaterialChanged()
			...
			if(istype(src.material))
				var/types = list()
				if(src.material.getProperty(MATERIAL_PROPERTY_DENSITY)/9 > 3 || src.material.getProperty(MATERIAL_PROPERTY_HARDNESS)/9 > 3)
					types["blunt"] = 0.5 * (max(src.material.getProperty(MATERIAL_PROPERTY_DENSITY)/9, src.material.getProperty(MATERIAL_PROPERTY_HARDNESS)/9) - 2)
				if(src.material.getProperty(MATERIAL_PROPERTY_DENSITY)/9 > 3)
					types["cut"] = 0.5 * (src.material.getProperty(MATERIAL_PROPERTY_DENSITY)/9 - 2)
				if(src.material.getProperty(MATERIAL_PROPERTY_HARDNESS)/9 > 3)
					types["stab"] = 0.5 * (src.material.getProperty(MATERIAL_PROPERTY_HARDNESS)/9 - 2)

				var/thermal = max(0, 5 - src.material.getProperty(MATERIAL_PROPERTY_THERMAL)/9)
				if(thermal > 0)
					types["burn"] = thermal
