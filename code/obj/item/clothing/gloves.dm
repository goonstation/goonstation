...
/obj/item/clothing/gloves/crafted
	...
	armored
		...
		onMaterialChanged()
			...
			if(istype(src.material))
				var/types = list()
				if(src.material.getProperty("tough") > 3 || src.material.getProperty("hard") > 3)
					types["blunt"] = 0.5 * (max(src.material.getProperty("tough"), src.material.getProperty("hard")) - 2)
				if(src.material.getProperty("tough") > 3)
					types["cut"] = 0.5 * (src.material.getProperty("tough") - 2)
