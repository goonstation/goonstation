...
/obj/item/clothing/gloves/crafted
		...
		onMaterialChanged()
			...
			if(istype(src.material))
				...
				switch(src.material.getProperty("electrical"))
					if(0 MAT to 1 MAT)
						src.setProperty("conductivity", 0.15 MAT)
					if(1 MAT to 2 MAT)
						src.setProperty("conductivity", 0.3 MAT)
					if(3 MAT to 4 MAT)
						src.setProperty("conductivity", 0.45 MAT)
					else
						src.setProperty("conductivity", 1 MAT)

				var/thermal_insul = max(0, 5 MAT - src.material.getProperty("thermal"))
	...
	armored
		...
		onMaterialChanged()
			...
			if(istype(src.material))
				var/types = list()
				if(src.material.getProperty("density") > 3 MAT || src.material.getProperty("hard") > 3 MAT)
					types["blunt"] = 0.5 * (max(src.material.getProperty("density"), src.material.getProperty("hard")) - 2 MAT) / (1 MAT)
				if(src.material.getProperty("density") > 3 MAT)
					types["cut"] = 0.5 * (src.material.getProperty("density") - 2 MAT) / (1 MAT)
				if(src.material.getProperty("hard") > 3 MAT)
					types["stab"] = 0.5 * (src.material.getProperty("hard") - 2 MAT) / (1 MAT)

				var/thermal = max(0, 5 MAT - src.material.getProperty("thermal"))
				if(thermal > 0 MAT)
					types["burn"] = thermal
