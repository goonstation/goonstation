...
/obj/item/clothing/glasses/flock
	name = "gnesis glasses"
	desc = "A set of empathy eyewear designed to allow humanoids to see what the universe looks like from the perspective of a weird teal "
	icon_state = "flock"
	item_state = "flock"
	mats = 4
	var/current_matrix = COLOR_MATRIX_FLOCKMIND
	var/current_matrix_label = COLOR_MATRIX_FLOCKMIND_LABEL

	New()
		. = ..()
		src.desc += "[lowertext(pick_string("agent_callsigns.txt", "birds"))]." // caw
		src.setMaterial(getMaterial("gnesis"), 0, 0)

	equipped(var/mob/user, var/slot)
		..()
		if(!isflock(user))
			user?.apply_color_matrix(current_matrix, current_matrix_label)

	unequipped(var/mob/user, var/slot)
		..()
		if(!isflock(user))
			user.remove_color_matrix(current_matrix_label)

	emag_act(mob/user, obj/item/card/emag/E)
		. = ..()
		src.alter_flockvision()
		if(src.current_matrix_label == COLOR_MATRIX_FLOCKMANGLED_LABEL)
			boutput(user, "You swipe \the [E] across \the [src], jazzing up the bird molecules and making it look a bit antsy.")
		else
			boutput(user, "You swipe \the [E] across \the [src], calming the bird molecules.")

	emp_act()
		. = ..()
		src.alter_flockvision(fuck_only = 1)

	proc/alter_flockvision(var/fuck_only)
		/// Now if we're just supposed to fuck up the vision, do just that
		if(fuck_only)
			src.current_matrix = COLOR_MATRIX_FLOCKMANGLED
			src.current_matrix_label = COLOR_MATRIX_FLOCKMANGLED_LABEL
			src.icon_state = "flock_busted"
			src.item_state = "flock_busted"
		/// Otherwise, just swap between modes
		else
			switch(current_matrix_label)
				if(COLOR_MATRIX_FLOCKMIND_LABEL)
					src.current_matrix = COLOR_MATRIX_FLOCKMANGLED
					src.current_matrix_label = COLOR_MATRIX_FLOCKMANGLED_LABEL
					src.icon_state = "flock_busted"
					src.item_state = "flock_busted"
				if(COLOR_MATRIX_FLOCKMANGLED_LABEL)
					src.current_matrix = COLOR_MATRIX_FLOCKMIND
					src.current_matrix_label = COLOR_MATRIX_FLOCKMIND_LABEL
					src.icon_state = "flock"
					src.item_state = "flock"

		if(isflock(src.loc))
			return // let's not mess with actual flockies, they have a weird enough time as it is

		/// Is it being worn? Let's update the vision then!
		if(ishuman(src.loc))
			var/mob/living/carbon/human/H = src.loc
			if (istype(H.glasses, /obj/item/clothing/glasses/flock))
				/// first remove any flock matrices
				H.remove_color_matrix(COLOR_MATRIX_FLOCKMIND_LABEL)
				H.remove_color_matrix(COLOR_MATRIX_FLOCKMANGLED_LABEL)
				/// then apply the matrix
				H.apply_color_matrix(current_matrix, current_matrix_label)
				/// Let's also play a sound too
				playsound(H, "sound/machines/glitch[rand(1,3)].ogg", 20, 1)
				if(fuck_only)
					boutput(H, "\the [src] flaps the flock out!")
