	...
	proc/do_stun(var/mob/user, var/mob/victim, var/type = "", var/stun_who = 2)
		...
		switch (type)
			...
			if ("stun")
				...
				logTheThing(LOG_DEBUG, user, "<b>Convair880</b>: stun baton ([src.type]) do_stun() was called with an invalid argument ([type]), aborting. Last touched by: [src.get_last_ckey()]")
				return
	...
	proc/set_state(var/state, var/mob/user)
		...
		switch(state)
			...
			if(EXTENDO_BATON_CLOSED_AND_OFF)
				...
				logTheThing(LOG_DEBUG, user, "Extendable stun baton ([src.type]) set_state() was called with an invalid argument ([state]), aborting. Last touched by: [src.get_last_ckey()]")
