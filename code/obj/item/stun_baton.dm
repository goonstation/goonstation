...
/obj/item/baton
	...
	proc/do_stun(var/mob/user, var/mob/victim, var/type = "", var/stun_who = 2)
		...
		switch (type)
			if ("failed")
				logTheThing("combat", user, "accidentally stuns [himself_or_herself(user)] with the [src.name] at [log_loc(user)].")
			...
			if ("failed_stun")
				...
				logTheThing("combat", user, "unsuccessfully tries to stun [constructTarget(victim,"combat")] with the [src.name] at [log_loc(victim)].")
			...
			if ("failed_harm")
				...
				logTheThing("combat", user, "unsuccessfully tries to beat [constructTarget(victim,"combat")] with the [src.name] at [log_loc(victim)].")

			if ("stun")
				user.visible_message("<span class='alert'><B>[victim] has been stunned with the [src.name] by [user]!</B></span>")
				logTheThing("combat", user, "stuns [constructTarget(victim,"combat")] with the [src.name] at [log_loc(victim)].")
				playsound(src, "sound/impact_sounds/Energy_Hit_3.ogg", 50, 1, -1)
				flick(flick_baton_active, src)
				JOB_XP(victim, "Clown", 3)

			else
				logTheThing("debug", user, "<b>Convair880</b>: stun baton ([src.type]) do_stun() was called with an invalid argument ([type]), aborting. Last touched by: [src.fingerprintslast ? "[src.fingerprintslast]" : "*null*"]")
				return
