...
/obj/item/mine
	...
	proc/log_me(var/atom/M, var/mob/T)
		...
		logTheThing(LOG_BOMBING, M && ismob(M) ? M : null, "The [src.name] was triggered at [log_loc(src)][T && ismob(T) ? ", affecting [constructTarget(logtarget,"bombing")]." : "."] Last touched by: [src.get_last_ckey()]")
