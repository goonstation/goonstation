		...
		switch (organ_name)
			...
			if ("right_kidney")
				if (!get_working_kidney_amt())
					donor.take_toxin_damage(2, 1)
			if ("tail")
				if (donor.mob_flags & SHOULD_HAVE_A_TAIL)
					if (ishuman(donor))
						var/mob/living/carbon/human/H = donor
						if(ischangeling(H) || H.reagents?.get_reagent_amount("ethanol") > 50 || H.belt?.flags & ISTAIL) // drunkenness prevents tail-clumsiness
							H.bioHolder?.RemoveEffect("clumsy")
						else // Only become clumsy if you should have a tail and are not a shapeshifting alien. And you arent hammered or wearing some kind of tail on your belt
							H.bioHolder?.AddEffect("clumsy", 0, 0, 0, 1)
