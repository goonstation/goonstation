	...
	attackby(obj/item/item, mob/user)
		...
		if(istype(item, /obj/item/tank) || istype(item, /obj/item/clothing/head/butt))
			...
			if(tank_one && tank_two)
				...
				logTheThing("bombing", user, "made a transfer valve [butt ? "butt" : "bomb"] at [log_loc(T)].")
			...
			logTheThing("bombing", user, "made a bomb using a[extra] [item.name] and a transfer valve.")
		...
		if (src.loc == usr)
			...
			if(href_list["open"])
				if (valve_open)
					...
					logTheThing("bombing", usr, "closed the valve on a tank transfer valve at [log_loc(bombturf)].")
					message_admins("[key_name(usr)] closed the valve on a tank transfer valve at [log_loc(bombturf)].")
				else
					var/turf/bombturf = get_turf(src)
					logTheThing("bombing", usr, "opened the valve on a tank transfer valve at [log_loc(bombturf)].")
	...
	proc
		toggle_valve()
			...
			if(valve_open && force_dud)
				...
				logTheThing("bombing", null, "A bomb valve would have opened at [log_loc(src)] but was forced to dud! Last touched by: [src.fingerprintslast ? "[src.fingerprintslast]" : "*null*"]")
				return
			...
			if(valve_open && (tank_one && tank_two) && tank_one.air_contents && tank_two.air_contents)
				...
				if(!A.dont_log_combat)
					logTheThing("bombing", null, "Bomb valve opened in [log_loc(bombturf)]. Last touched by [src.fingerprintslast]")
