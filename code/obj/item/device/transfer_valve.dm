	...
	proc
		toggle_valve()
			...
			if(valve_open && force_dud)
				...
				logTheThing(LOG_BOMBING, null, "A bomb valve would have opened at [log_loc(src)] but was forced to dud! Last touched by: [src.get_last_ckey()]")
				return
			...
			if(valve_open && (tank_one && tank_two) && tank_one.air_contents && tank_two.air_contents)
				...
				if(!A.dont_log_combat)
					...
					logTheThing(LOG_BOMBING, null, "TTV tank transfer valve bomb opened in [log_loc(bombturf)]. Last touched by [last_ckey]")
					message_admins("TTV tank transfer valve bomb valve opened in [log_loc(bombturf)]. Last touched by [last_ckey]")
