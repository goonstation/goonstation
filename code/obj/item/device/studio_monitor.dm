...
/datum/statusEffect/focus/music
	...
	onAdd(optional=null)
		...
		if(ismob(owner))
			var/mob/M = owner
			APPLY_ATOM_PROPERTY(M, PROP_DISARM_RESIST, "focus_music", 10)
			APPLY_ATOM_PROPERTY(M, PROP_DISORIENT_RESIST_BODY, "focus_music", 10)

	onRemove()
		. = ..()
		if(ismob(owner))
			var/mob/M = owner
			REMOVE_ATOM_PROPERTY(M, PROP_DISARM_RESIST, "focus_music")
			REMOVE_ATOM_PROPERTY(M, PROP_DISORIENT_RESIST_BODY, "focus_music")
...
/datum/statusEffect/simplehot/epic_climax // totally not mild stimulants...
	...
	onAdd(optional)
		...
		if(ismob(owner))
			var/mob/M = owner
			APPLY_ATOM_PROPERTY(M, PROP_STAMINA_REGEN_BONUS, "stims", 100)
			M.add_stam_mod_max("stims", 100)
			APPLY_ATOM_PROPERTY(M, PROP_STUN_RESIST, "stims", 100)
			APPLY_ATOM_PROPERTY(M, PROP_STUN_RESIST_MAX, "stims", 100)
