...
/obj/item/device/chameleon/bomb
	...
	attackby(obj/item/W, mob/user)
		if (src.active)
			if (user)
				...
				logTheThing("bombing", user, "triggers a chameleon bomb ([src]) by hitting it with [W] at [log_loc(user)].")
			src.disrupt()
		else
			return ..()
	...
	attack_hand(var/mob/user)
		if (src.active && isturf(loc))
			...
			logTheThing("bombing", user, "picks up and triggers a chameleon bomb ([src]) at [log_loc(user)].")
			src.disrupt()
		else
			return ..()
	...
	toggle()
		...
		if (active)
			...
			logTheThing("bombing", usr, "disarms a chameleon bomb ([src]) at [log_loc(usr)].")
			...
			logTheThing("bombing", usr, "arms a chameleon bomb ([src]) at [log_loc(usr)].")
