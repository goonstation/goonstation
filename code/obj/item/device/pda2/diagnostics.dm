		...
		if (href_list["adj_freq"])
			...
			SPAWN( 0 )
				...
				if ((send_freq == FREQ_PDA) && (!isnull(signal.data["message"])) && (signal.data["command"] == "text_message"))
					logTheThing(LOG_PDAMSG, null, "<i><b>[src.master.owner]'s PDA used by [src.master.loc.name] ([src.master.get_last_ckey()]) (as [isnull(signal.data["sender_name"]) ? "Nobody" : signal.data["sender_name"]]) &rarr; [isnull(signal.data["address_1"]) ? "Everybody" : "[signal.data["address_1"]]"]:</b></i> [signal.data["message"]]")
