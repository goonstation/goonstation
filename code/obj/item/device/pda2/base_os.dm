...
//Main os program: Provides old pda interface and four programs including file browser, notes, messenger, and atmos scan
	main_os
				...
				switch(href_list["input"])
					if("tone")
						...
						if (t == src.master?.uplink?.lock_code)
							...
							src.message_tone = t
							logTheThing("pdamsg", usr, "sets ring message of <b>[src.master]</b> to: [src.message_tone]")
		...
		pda_message(var/target_id, var/target_name, var/message, var/is_department_message)
			...
			logTheThing("pdamsg", null, "<i><b>[src.master.owner]'s PDA used by [key_name(src.master.loc)] &rarr; [target_name]:</b></i> [message]")
			return 0
