...
//		src.master:r_signal(signal)
	if(src.master && (src.wires & WIRE_SIGNAL))
		...
		if (src.master && istype(src.master, /obj/item/device/transfer_valve))
			logTheThing(LOG_BOMBING, usr, "signalled a radio on a transfer valve at [T ? "[log_loc(T)]" : "horrible no-loc nowhere void"] with code [src.code] on freq [src.frequency].")
			message_admins("[key_name(usr)] signalled a radio on a transfer valve at [T ? "[log_loc(T)]" : "horrible no-loc nowhere void"] with code [src.code] on freq [src.frequency].")
