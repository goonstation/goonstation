...
/obj/item/device/flash/proc/flash_mob(mob/living/M as mob, mob/user as mob)
	...
	if (src.turboflash)
		...
		if (src.cell && istype(src.cell,/obj/item/cell/erebite))
			...
			logTheThing("combat", user, "tries to blind [constructTarget(M,"combat")] with [src] (erebite power cell) at [log_loc(user)].")
	...
	logTheThing("combat", user, "blinds [constructTarget(M,"combat")] with [src] at [log_loc(user)].")
	if (src.emagged)
		logTheThing("combat", user, "blinds themself with [src] at [log_loc(user)].")
...
/obj/item/device/flash/attack_self(mob/user as mob)
	...
	if (src.turboflash)
		...
		if (src.cell && istype(src.cell,/obj/item/cell/erebite))
			user.visible_message("<span class='alert'>[user]'s flash/cell assembly violently explodes!</span>")
			logTheThing("combat", user, "tries to area-flash with [src] (erebite power cell) at [log_loc(user)].")
			var/turf/T = get_turf(src.loc)
			explosion(src, T, 0, 1, 2, 2)
			SPAWN(0.1 SECONDS)
				if (src) qdel(src)
