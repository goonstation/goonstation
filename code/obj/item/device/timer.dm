	...
	switch (action)
		...
		if ("toggle-timing")
			...
			if (istype(master, /obj/item/device/transfer_valve))
				logTheThing("bombing", usr, "[timing ? "initiated" : "defused"] a timer on a transfer valve at [log_loc(src.master)].")
				...
				logTheThing("bombing", usr, "[timing ? "initiated" : "defused"] a timer on a timer-igniter assembly at [log_loc(src.master)]. Contents: [log_reagents(RI.part3)]")
				SEND_SIGNAL(src.master, "[timing ? COMSIG_ITEM_BOMB_SIGNAL_START : COMSIG_ITEM_BOMB_SIGNAL_CANCEL]")
			else if(istype(src.master, /obj/item/assembly/time_bomb))	//Timer-detonated single-tank bombs
				logTheThing("bombing", usr, "[timing ? "initiated" : "defused"] a timer on a single-tank bomb at [log_loc(src.master)].")
				...
				logTheThing("bombing", usr, "[timing ? "initiated" : "defused"] a timer on a [src.master.name] at [log_loc(src.master)].")
				SEND_SIGNAL(src.master, "[timing ? COMSIG_ITEM_BOMB_SIGNAL_START : COMSIG_ITEM_BOMB_SIGNAL_CANCEL]")
			. = TRUE
