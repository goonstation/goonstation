...
/obj/item/bible
	...
	proc/smite(mob/M)
		...
		logTheThing(LOG_COMBAT, M, "farted on [src] at [log_loc(src)] last touched by <b>[src.get_last_ckey()]</b>.")
		M.smite_gib()
...
/obj/item/bible/mini
	...
	farty_heresy(mob/user) //fuk u always die
		...
		logTheThing(LOG_COMBAT, user, "farted on [src] at [log_loc(src)] last touched by <b>[src.get_last_ckey()]</b>.")
		smite(user)
		return TRUE
