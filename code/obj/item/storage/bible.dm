...
/obj/item/sacred_tome
	...
	///Called when someone farts on a sacred texts. Return TRUE if we killed them, FALSE otherwise.
	proc/farty_heresy(mob/user)
		...
		if (user.traitHolder?.hasTrait("atheist"))
			user.visible_message(SPAN_ALERT("[user] farts on the sacred texts with particular vindication.<br><b>Against all odds, [user] remains unharmed!</b>"))
			return FALSE
		else if (ishuman(user) && user:unkillable)
			user.visible_message(SPAN_ALERT("[user] farts on the sacred texts."))
			user:unkillable = 0
	...
	proc/smite(mob/M)
		M.visible_message(SPAN_ALERT("[M] farts on the sacred texts.<br><b>A mysterious force smites [M]!</b>"))
