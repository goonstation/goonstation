...
/obj/item/storage/bible
	...
	//		return
		...
		if (iswraith(M) || (M.bioHolder && M.bioHolder.HasEffect("revenant")))
			...
			logTheThing("combat", user, "biblically smote [constructTarget(M,"combat")]")
			...
			if ((ishuman(H) && prob(60) && !(M.traitHolder?.hasTrait("atheist"))))
				...
				if (narrator_mode)
					...
					playsound(src.loc, "punch", 25, 1, -1)
				logTheThing("combat", user, "biblically healed [constructTarget(M,"combat")]")
			else
				...
				logTheThing("combat", user, "biblically injured [constructTarget(M,"combat")]")
	...
	proc/smite(mob/M)
		...
		logTheThing("combat", M, "farted on [src] at [log_loc(src)] last touched by <b>[src.fingerprintslast ? src.fingerprintslast : "unknown"]</b>.")
...
/obj/item/storage/bible/mini
	...
	farty_heresy(mob/user) //fuk u always die
		...
		logTheThing("combat", user, "farted on [src] at [log_loc(src)] last touched by <b>[src.fingerprintslast ? src.fingerprintslast : "unknown"]</b>.")
		smite(user)
		return TRUE
