	...
	proc/effect(var/cardname, mob/living/carbon/user as mob, var/obj/item/zoldorfdeck/deck)
		...
		switch(cardname)
			...
			if("Crusher")
				...
				user.u_equip(deck)
				deck.set_loc(get_turf(user))
				logTheThing("combat", user, "was gibbed by Zoldorf's crusher card at [log_loc(user)].")
				user.gib(1)
			if("Geneticist")
		...
		switch(src.icon_state)
			...
			if("presto")
				...
				if(isrestrictedz(user.z))
					...
					logTheThing("user", user, "was gibbed for trying to use Zoldorf's presto scroll at [log_loc(user)].")
					user.gib()
