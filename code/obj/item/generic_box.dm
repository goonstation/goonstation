	...
	assorted
		...
		take_from()
			if( !contained_items.len )
				...
				logTheThing("debug", src, "has no items in it!")
				return
		...
		if (src.contained_items_proc)
			...
			SPAWN(1 SECOND)
				...
				if (!ispath(src.contained_item))
					logTheThing("debug", src, "has a non-path contained_item, \"[src.contained_item]\", and is being disposed of to prevent errors")
					qdel(src)
					return
