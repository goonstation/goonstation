...
/obj/item/canvas
	...
	attackby(obj/item/W, mob/user)
		...
		if (istype(W, /obj/item/paint_can))
			...
			logTheThing("station", user, "coated [src] in paint: [log_loc(src)]: canvas{\ref[src], -1, -1, [P.paint_color]}")
	...
	Topic(href, href_list)
		...
		logTheThing("station", usr, "draws on [src]: [log_loc(src)]: canvas{\ref[src], [x], [y], [dot_color]}")
