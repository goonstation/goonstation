...
/obj/item/cable_coil/attackby(obj/item/W, mob/user)
	if (issnippingtool(W) && src.amount > 1)
		var/amt = tgui_input_number(user, "How long of a wire do you wish to cut?", "Length of [src.amount]", 1, src.max_stack - 1, 1)
		amt = min(amt, src?.amount - 1)
		var/obj/item/cable_coil/A = split_stack(amt)
		if (istype(A))
