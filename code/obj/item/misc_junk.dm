...
/obj/item/brick
	...
	suicide(var/mob/user as mob)
		...
		user.visible_message(SPAN_ALERT("<b>[user] throws the [src] into the air, and it lands square on [his_or_her(user)] face!.</b>"))

		src.set_loc(get_turf(src))
		src.anchored = ANCHORED_ALWAYS
		src.layer += 4
		animate(src, pixel_y = 40, easing = EASE_OUT | QUAD_EASING, time = 1 SECOND)
		SPAWN(1 SECOND)
			animate(src, pixel_y = 15, easing = EASE_IN | QUAD_EASING, time = 1 SECOND)
			SPAWN(1 SECOND)
