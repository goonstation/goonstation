...
/obj/item/clothing/mask/cigarette
	...
	proc/trick_explode()
		...
		// Added (Convair880).
		if (ismob(src.loc))
			logTheThing("bombing", null, "A trick cigarette (held/equipped by [constructTarget(src.loc,"bombing")]) explodes at [log_loc(src)].")
		else
			logTheThing("bombing", src.fingerprintslast, "A trick cigarette explodes at [log_loc(src)]. Last touched by [src.fingerprintslast ? "[src.fingerprintslast]" : "*null*"].")
