		...
		if (ishuman(M))
			if (src.on > 0)
				...
				if (fella.wear_mask && istype(fella.wear_mask, /obj/item/clothing/mask/cigarette))
					...
					return
				else if (fella.bleeding || (fella.organHolder?.back_op_stage == 4 && user.zone_sel.selecting == "chest"))
...
/obj/item/device/light/zippo
	...
	attack(mob/target, mob/user)
		if (ishuman(target))
			...
			if (fella.bleeding || (fella.organHolder?.back_op_stage == 4 && user.zone_sel.selecting == "chest"))
