...
/obj/item/rods
	...
	attack_self(mob/user as mob)
		...
		if (istype(user.loc, /obj/vehicle/segway))
			...
			SETUP_GENERIC_ACTIONBAR(user, src, 1.5 SECONDS, TYPE_PROC_REF(/obj/item/rods, build_grille), list(user), src.icon, src.icon_state, null, INTERRUPT_MOVE | INTERRUPT_ACT | INTERRUPT_ATTACKED | INTERRUPT_STUNNED | INTERRUPT_ACTION)
