...
/obj/item/sheet
	name = "sheet"
	inhand_image_icon = 'icons/mob/inhand/hand_materials.dmi'
	...
	proc/update_appearance()
		...
		if(src.material)
			...
			if(src.material.getID() == "rock")
				src.icon_state_base = "rock_"
				src.item_state = "sheet-rock"
				base_name = "slab"
			else if(material_flags & MATERIAL_METAL)
				src.icon_state_base = "metal_"
				src.item_state = "sheet-metal"
				if(src.reinforcement)
					src.icon_state_base += "r"
					src.item_state += "-r"
			else if(material_flags & MATERIAL_CRYSTAL)
				src.icon_state_base = "glass_"
				src.item_state = "sheet-glass"
				if(src.reinforcement)
					src.icon_state_base += "r"
					src.item_state += "-r"
			else if(material_flags & MATERIAL_CLOTH)
				src.icon_state_base = "fabric_"
				src.item_state = "sheet-fabric"
			else if(material_flags & MATERIAL_WOOD)
				src.icon_state_base = "wood_"
				src.item_state = "sheet-wood"
			else if(material_flags & MATERIAL_RUBBER)
				src.icon_state_base = "rubber_"
				src.item_state = "sheet-rubber"
			else if(material_flags & MATERIAL_ORGANIC)
				src.icon_state_base = "organic_"
				src.item_state = "sheet-organic"

		if(src.amount > 1)
			base_name += "s"
		src.name = "[material.getName()] " + base_name
	...
	_update_stack_appearance()
		switch(src.amount)
			...
			if(10 to 19)
				icon_state = "[icon_state_base]2"
			if(20 to 29)
				icon_state = "[icon_state_base]3"
			if(30 to 39)
				icon_state = "[icon_state_base]4"
			if(40 to 49)
				icon_state = "[icon_state_base]5"
			if(50 to INFINITY)
				icon_state = "[icon_state_base]6"
...
/obj/item/sheet/steel
	...
	reinforced
		icon_state = "metal_r1"
		item_state = "sheet-metal-r"
		default_reinforcement = "steel"

/obj/item/sheet/glass
	icon_state = "glass_1" //overriden in-game but shows up in map editors
	item_state = "sheet-glass"
	default_material = "glass"

	reinforced
		icon_state = "glass_r1"
		item_state = "sheet-glass-r"
		default_reinforcement = "steel"
	...
	_update_stack_appearance()
		switch(amount)
			...
			if(10 to 14)
				icon_state = "[icon_state_base]6"
			if(15 to 19)
				icon_state = "[icon_state_base]7"
			if(20 to 24)
				icon_state = "[icon_state_base]8"
			if(25 to 29)
				icon_state = "[icon_state_base]9"
			if(30 to 34)
				icon_state = "[icon_state_base]10"
			if(35 to 39)
				icon_state = "[icon_state_base]11"
			if(40 to 44)
				icon_state = "[icon_state_base]12"
			if(45 to 49)
				icon_state = "[icon_state_base]13"
			else
				icon_state = "[icon_state_base]14"
...
/obj/item/sheet/bamboo
	item_state = "sheet-wood"
	icon_state = "wood_1$$bamboo"
	default_material = "bamboo"
	amount = 10

/obj/item/sheet/mauxite
	item_state = "sheet-metal"
	icon_state = "wood_1$$mauxite"
