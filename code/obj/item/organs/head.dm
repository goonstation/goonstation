...
/obj/item/organ/head // vOv
	...
	update_icon(var/makeshitup, var/ignore_transplant) // should only happen once, maybe again if they change mutant race
		...
		if (AHead.customizations["hair_top"].style.id == "hetcroL")
		...
		else if (AHead.customizations["hair_bottom"].style.id == "hetcroR")
		...
		src.head_image_cust_one = image(icon = AHead.customizations["hair_top"].style.icon, icon_state = AHead.customizations["hair_top"].style.id, layer = AHead.customizations["hair_top"].style.default_layer)
		...
		src.head_image_cust_three = image(icon = AHead.customizations["hair_bottom"].style.icon, icon_state = AHead.customizations["hair_bottom"].style.id, layer = AHead.customizations["hair_bottom"].style.default_layer)

		src.head_image_cust_one.color = AHead.customizations["hair_top"].color
		src.head_image_cust_two.color = AHead.customizations["hair_middle"].color
		src.head_image_cust_three.color = AHead.customizations["hair_bottom"].color
