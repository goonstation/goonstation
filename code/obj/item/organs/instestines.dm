/obj/item/organ/intestines
	...
	on_transplant(mob/M)
		. = ..()
		if(!broken)
			APPLY_ATOM_PROPERTY(M, PROP_DIGESTION_EFFICIENCY, src, digestion_efficiency)

	on_removal()
		REMOVE_ATOM_PROPERTY(src.donor, PROP_DIGESTION_EFFICIENCY, src)
		. = ..()

	unbreakme()
		..()
		if(..() && donor)
			APPLY_ATOM_PROPERTY(src.donor, PROP_DIGESTION_EFFICIENCY, src, digestion_efficiency)
