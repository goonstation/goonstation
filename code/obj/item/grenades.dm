...
/obj/item/pipebomb/bomb
	...
	proc/do_explode()
		if (src.strength)
			...
			if (butt)
				if (butt > 1)
					playsound(src.loc, "sound/voice/farts/superfart.ogg", 90, 1, channel=VOLUME_CHANNEL_SCREAMFART)
					for (var/mob/M in view(3+butt,src.loc))
						ass_explosion(M, 0, 5)
				else
					playsound(src.loc, "sound/voice/farts/poo2.ogg", 90, 1, channel=VOLUME_CHANNEL_SCREAMFART)
					for (var/mob/M in view(3,src.loc))
						ass_explosion(M, 0, 5)
			if (confetti)
				if (confetti > 1)
