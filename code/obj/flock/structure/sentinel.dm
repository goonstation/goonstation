	...
	if(powered == 1)
		switch(charge_status)
			...
			if(CHARGED)
				...
				logTheThing(LOG_COMBAT, src, "Flock sentinel belonging to [key_name(src.flock?.flockmind)] fires an arcflash at [constructTarget(mobtohit)].")
				hit += mobtohit
				for(var/i in 1 to rand(5,6))//this facilitates chaining. legally distinct from the loop above
					for(var/mob/nearbymob in view(2, mobtohit.loc))
						if(nearbymob != mobtohit && !isflockmob(nearbymob) && !(nearbymob in hit) && isturf(nearbymob.loc) && src.flock?.isEnemy(nearbymob) && isalive(loopmob) && !isintangible(loopmob))
							...
							logTheThing(LOG_COMBAT, src, "Flock sentinel belonging to [key_name(src.flock?.flockmind)] hits [constructTarget(nearbymob)] with a chained arcflash.")
							hit += nearbymob
							mobtohit = nearbymob
