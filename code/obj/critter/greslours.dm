/////////// cogwerks - hideous greslours beast

/obj/critter/greslours
	name = "greslours"
	desc = "Oh god."
	...
	var/obj/item/parts/human_parts/arm/left/greslours/left_arm
	var/obj/item/parts/human_parts/arm/right/greslours/right_arm

	skinresult = /obj/item/material_piece/cloth/greslourshide

	New()
		src.left_arm = new /obj/item/parts/human_parts/arm/left/greslours(src)
		src.right_arm = new /obj/item/parts/human_parts/arm/right/greslours(src)
		..()
	...
	CritterDeath()
		...
		playsound(src.loc, "sound/voice/animal/greslours_cry.ogg", 60, 1)
	...
	seek_target()
		...
		for (var/mob/living/C in hearers(src.seekrange,src))
			...
			if(king)
				boredom_countdown = rand(0,1) // king greslourss are pretty much grump elementals
			src.target = C
			src.oldtarget_name = C.name
			src.task = "chasing"
			src.appear()
			if(king)
				playsound(src.loc, "sound/voice/animal/greslours_roar.ogg", 75, 1)
	...
	attackby(obj/item/W as obj, mob/living/user as mob) //ARRRRGH WHY
		...
		if(prob(25) && alive) // crowds shouldn't be able to beat the fuck out of a confused greslours with impunity, fuck that
			src.target = user
	...
	attack_hand(var/mob/user as mob)
		...
		if (user.a_intent == INTENT_HARM)
			...
			playsound(src.loc, "sound/voice/animal/greslours_laugh.ogg", 60, 1)
	...
	ChaseAttack(mob/M)
		if(prob(10))
			playsound(src.loc, "sound/voice/animal/greslours_scream.ogg", 75, 1)
		...
		if (isrobot(M))
			...
			if (!BORG.part_head)
				...
				if (BORG.part_head.ropart_get_damage_percentage() >= 85)
					...
					playsound(src.loc, "sound/voice/animal/greslours_laugh.ogg", 70, 1)
					playsound(src.loc, 'sound/impact_sounds/Metal_Hit_Lowfi_1.ogg', 70, 1)
					...
					if (BORG.part_head.ropart_take_damage(rand(20,40),0) == 1)
						BORG.compborg_lose_limb(BORG.part_head)
					if (prob(33)) playsound(src.loc, "sound/voice/animal/greslours_scream.ogg", 75, 1)
					attack_delay = 5
		else
	...
	proc/appear()
		if (!invisibility)
			return
		src.icon_state = "greslours_appear"
		src.invisibility = 0
		set_density(1)
		SPAWN_DBG(1.2 SECONDS)
			if(king)
				src.icon_state = "gresloursking"
			else
				src.icon_state = "greslours"
			playsound(src.loc, "sound/voice/animal/greslours_scream.ogg", 85, 1)
	...
	proc/frenzy(mob/M)
		...
		SPAWN_DBG(0)
			...
			playsound(src.loc, "sound/voice/animal/greslours_maul.ogg", 80, 1)
			if(king)
				playsound(src.loc, "sound/voice/animal/greslours_roar.ogg", 80, 1)
...
////////////////
//////king greslours, why not
///////////////

/obj/critter/greslours/king
	name = "greslours king"
	desc = "You should run."
	death_text = "%src% collapses in a heap!"
	health = 500
	icon_state = "gresloursking"
	king = 1

	skinresult = /obj/item/material_piece/cloth/kinggreslourshide
	...
	CritterDeath()
		..()
		playsound(src.loc, "sound/voice/animal/greslours_roar.ogg", 75, 1)
		playsound(src.loc, "sound/voice/animal/greslours_cry.ogg", 75, 1)

////////////////
////// e-egg?
///////////////

/obj/item/reagent_containers/food/snacks/ingredient/egg/critter/greslours
	name = "greslours egg"
	desc = "They lay eggs?!"
	critter_type = /obj/critter/greslours
	warm_count = 100
	critter_reagent = "ice"
