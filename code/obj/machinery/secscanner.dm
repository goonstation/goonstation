	...
	proc/do_scan_item (var/obj/item/I)
		...
		if (contraband >= 2)
			...
			if (src.report_scans && (I.name != last_perp || contraband != last_contraband))
				...
				pdaSignal.data = list("address_1"="00000000", "command"="text_message", "sender_name"="SEC-MAILBOT",  "group"=MGD_SECURITY, "alert"=MGA_CHECKPOINT, "sender"="00000000", "message"="An item [I.name] failed checkpoint scan at [scan_location]! Threat Level : [contraband]")
				...
				if (src.report_scans)
					...
					if (ishuman(target))
						...
						if (perpname != last_perp || contraband != last_contraband)
							var/datum/signal/pdaSignal = get_free_signal()
							pdaSignal.data = list("address_1"="00000000", "command"="text_message", "sender_name"="SEC-MAILBOT",  "group"=MGD_SECURITY, "alert"=MGA_CHECKPOINT, "sender"="00000000", "message"="[uppertext(perpname)] FAILED A VIBE CHECK AT [uppertext(scan_location)]! BAD VIBES LEVEL : [contraband]")
							SEND_SIGNAL(src, COMSIG_MOVABLE_POST_RADIO_PACKET, pdaSignal)
		...
		if (contraband >= 4)
			...
			if (src.report_scans)
				...
				if (ishuman(target))
					...
					if (perpname != last_perp || contraband != last_contraband)
						var/datum/signal/pdaSignal = get_free_signal()
						pdaSignal.data = list("address_1"="00000000", "command"="text_message", "sender_name"="SEC-MAILBOT",  "group"=MGD_SECURITY, "alert"=MGA_CHECKPOINT, "sender"="00000000", "message"="[perpname] failed checkpoint scan at [scan_location]! Threat Level : [contraband]")
						SEND_SIGNAL(src, COMSIG_MOVABLE_POST_RADIO_PACKET, pdaSignal)
