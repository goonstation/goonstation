		...
		switch (src.teleport_sanity_check(P, user, machinery_loc != home_loc ? home_loc : null, zlevel_check_bypass))
			...
			if (5)
				...
				if (machinery_loc == home_loc)
					...
					logTheThing("station", usr, "teleports [P] to [log_loc(our_loc)].")
				else
					P.set_loc(home_loc) // Send back to home location.
					flick("[P.icon_state]-tele", P)
					user.show_text("[src.machinery_name] sent to home turf.", "blue")
					logTheThing("station", usr, "teleports [P] to its home turf [log_loc(home_loc)].")
	...
	attackby(obj/item/W, mob/user as mob)
		...
		if (istype(W, /obj/item/card/id))
			if (src.allowed(user))
				...
				if (src.occupant)
					logTheThing("station", user, "[src.locked ? "locks" : "unlocks"] [src.name] with [constructTarget(src.occupant,"station")] inside at [log_loc(src)].")
			else
			...
			if((force_body_swap || prob(1)) && has_mob)
				...
				if(body_list.len > 1)
					...
					for(var/I = 1, I <= body_list.len , I++)
						...
						if(M.mind)
							logTheThing("combat", src, "swapped [key_name(M)] and [constructTarget(body_list[next_in_line],"combat")]'s bodies!")
