		...
		if(length(contents) > 0)
			...
			if(M)
				...
				if (ishuman(M))
					...
					if(!isnull(R) && ((R["criminal"] == SECURITY::ARREST_STATE::INCARCERATED) || (R["criminal"] == SECURITY::ARREST_STATE::ARREST) || (R["criminal"] == SECURITY::ARREST_STATE::DETAIN)))
						R["criminal"] = SECURITY::ARREST_STATE::RELEASED
