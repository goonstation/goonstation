...
/obj/machinery/door_timer/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if ("set-time")
			...
			if (params["finish"])
				logTheThing("station", usr, "set timer to [src.time]sec (previously: [previous_time]sec) on a door timer: [src] [log_loc(src)].")
		...
		if ("toggle-timing")
			...
			logTheThing("station", usr, "[src.timing ? "starts" : "stops"] a door timer: [src] [log_loc(src)].")
			...
			for_by_tcl(F, /obj/machinery/flasher)
				...
				if (F.id == src.id)
					...
					if (GET_COOLDOWN(F, "flash"))
						return
					F.flash()
					logTheThing("station", usr, "sets off flashers from a door timer: [src] [log_loc(src)].")
					return TRUE
		...
		if ("toggle-flusher")
			for_by_tcl(FF,/obj/machinery/floorflusher)
				...
				if (FF.id == src.id)
					...
					if (FF.open != 1)
						FF.openup()
						logTheThing("station", usr, "opens a floor flusher from a door timer: [src] [log_loc(src)].")
					else
						FF.closeup()
						logTheThing("station", usr, "closes a floor flusher from a door timer: [src] [log_loc(src)].")
					return TRUE
