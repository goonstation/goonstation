...
/obj/machinery/door/airlock/proc/shock_temp(mob/user)
	...
	if (src.isWireCut(AIRLOCK_WIRE_ELECTRIFY))
		...
		logTheThing("combat", user, "electrified airlock ([src]) at [log_loc(src)] for 30 seconds.")
...
/obj/machinery/door/airlock/proc/toggle_bolt(mob/user)
	...
	if(src.locked)
		...
		logTheThing("station", user, "[user] has bolted a door at [log_loc(src)].")
		src.locked = 1
		UpdateIcon()
...
/obj/machinery/door/airlock/proc/shock_perm(mob/user)
	...
	if (src.isWireCut(AIRLOCK_WIRE_ELECTRIFY))
		...
		logTheThing("combat", user, "electrified airlock ([src]) at [log_loc(src)] indefinitely.")
...
/obj/machinery/door/airlock/proc/shock_restore(mob/user)
	...
	if (src.isWireCut(AIRLOCK_WIRE_ELECTRIFY))
		...
		src.secondsElectrified = 0
		logTheThing("combat", user, "de-electrified airlock ([src]) at [log_loc(src)].")
...
/obj/machinery/door/airlock/proc/pulse(var/wireColor)
	...
	switch(wireIndex)
		...
		if (AIRLOCK_WIRE_DOOR_BOLTS)
			...
			if (!src.locked)
				src.locked = 1
				logTheThing("station", usr, "[usr] has bolted a door at [log_loc(src)].")
		...
		if (AIRLOCK_WIRE_ELECTRIFY)
			...
			if (src.secondsElectrified==0)
				src.secondsElectrified = 30
				logTheThing("station", usr, "temporarily electrified an airlock at [log_loc(src)] with a pulse.")
				SPAWN(1 SECOND)
		...
		if(AIRLOCK_WIRE_SAFETY)
			logTheThing("station", usr, "caused an airlock to close and crush at [log_loc(src)] with a pulse.")
			src.safety = 0
			src.close(1)
			src.safety = 1
			SPAWN(1 DECI SECOND)
...
/obj/machinery/door/airlock/proc/attach_signaler(var/wire_color, mob/user)
	...
	tgui_process.update_uis(src)
	logTheThing("station", user, "attaches a remote signaller on frequency [R.frequency] to [src] at [log_loc(src)].")
	return TRUE
	...
	switch(wireIndex)
		...
		if (AIRLOCK_WIRE_ELECTRIFY)
			...
			if (src.secondsElectrified != -1 && can_shock)
				logTheThing("station", usr, "permanently electrified an airlock at [log_loc(src)] by cutting the shock wire.")
				src.secondsElectrified = -1

		if(AIRLOCK_WIRE_SAFETY)
			logTheThing("station", usr, "permanently disabled the safety of an airlock at [log_loc(src)] by cutting the safety wire.")
			src.safety = 0
...
/obj/machinery/door/airlock/proc/mend(var/wireColor)
	...
	switch(wireIndex)
		...
		if(AIRLOCK_WIRE_SAFETY)
			logTheThing("station", usr, "re-enabled the safety of an airlock at [log_loc(src)] by mending the safety wire.")
			src.safety = 1
