...
/obj/machinery/computer/door_control/proc/alarm()
	if(status & (NOPOWER|BROKEN))
		return
	for(var/obj/machinery/door/window/brigdoor/M as anything in by_type[/obj/machinery/door])
		if (M.id == src.id)
			if(M.density)
				SPAWN_DBG( 0 )
					M.open()
...
/obj/machinery/computer/door_control/attack_hand(var/mob/user as mob)
	...
	src.add_dialog(user)
	for(var/obj/machinery/door/window/brigdoor/M as anything in by_type[/obj/machinery/door])  //DOORS OH MY GOD (was previously looping through WORLD)
		if(M.id == 1)
