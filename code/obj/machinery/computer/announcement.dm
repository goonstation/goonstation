...
/obj/machinery/computer/announcement
	...
	proc/send_message(var/mob/user)
		...
		logTheThing("say", user, "as [ID.registered] ([ID.assignment]) created a command report: [message]")
		logTheThing("diary", user, "as [ID.registered] ([ID.assignment]) created a command report: [message]", "say")
	...
	proc/set_arrival_alert(var/mob/user)
		...
		logTheThing("station", user, "sets the arrival announcement on [constructTarget(src,"station")] to \"[src.arrivalalert]\"")
	...
	proc/announce_arrival(var/mob/living/person)
		...
		logTheThing("station", src, "ANNOUNCES: [message]")
		return 1
