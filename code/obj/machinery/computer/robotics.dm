		...
		switch(href_list["gib"])
			if("drone")
				...
				if (istype(drone))
					if(src.check_access(I))
						...
						logTheThing("combat", usr, "killswitched drone [constructTarget(drone,"combat")]")
						if(drone.client)
				...
				if (istype(I))
					if(src.check_access(I))
						if(istype(R))
							...
							logTheThing("combat", usr, "has activated the robot killswitch process on [constructTarget(R,"combat")]")
							if(R.client)
							...
							logTheThing("combat", usr, "has activated the AI killswitch process on [constructTarget(message,"combat")]")
							if(message.client)
			...
			if("2")
				if(istype(R))
					...
					logTheThing("combat", usr, "has stopped the robot killswitch process on [constructTarget(R,"combat")].")
					if(R.client)
					...
					logTheThing("combat", usr, "has stopped the AI killswitch process on [constructTarget(message,"combat")].")
					if(message.client)
		...
		switch(href_list["lock"])
			if("1")
				if(istype(R))
					...
					R.uneq_active()
					logTheThing("combat", usr, "has activated [constructTarget(R,"combat")]'s weapon lock (120 seconds).")
					...
					if (istype(I))
						if(src.check_access(I))
							...
							if(message.client)
								...
								logTheThing("combat", usr, "has activated [constructTarget(message,"combat")]'s weapon lock (120 seconds).")
					else
						boutput(usr, "<span class='alert'>Access Denied.</span>")
			...
			if("2")
				if(istype(R))
					...
					R.weapon_lock = 0
					R.weaponlock_time = 120
					logTheThing("combat", usr, "has deactivated [constructTarget(R,"combat")]'s weapon lock.")
					...
					A.weapon_lock = 0
					A.weaponlock_time = 120
					logTheThing("combat", usr, "has deactivated [constructTarget(message,"combat")]'s weapon lock.")
