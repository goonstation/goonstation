	...
	proc/print_auth_needed(var/mob/author)
		if (author)
			for (var/mob/O in hearers(src, null))
				O.show_message(SPAN_SUBTLE("<span class='say'>[SPAN_NAME("[src]")] beeps, \"[author] request accepted. [src.auth_need - src.authorized.len] authorizations needed until Armory is opened.\"</span>"), 2)
		else
			for (var/mob/O in hearers(src, null))
				O.show_message(SPAN_SUBTLE("<span class='say'>[SPAN_NAME("[src]")] beeps, \"[src.auth_need - src.authorized.len] authorizations needed until Armory is opened.\"</span>"), 2)
