...
/obj/machinery/computer/cloning/attackby(obj/item/W, mob/user)
	if (wagesystem.clones_for_cash && istype(W, /obj/item/spacecash))
		...
		user.drop_item()
		logTheThing("combat", src, "[user] has added clone module ([W]) to ([src]) at [log_loc(user)].")
		qdel(W)
		...
		logTheThing("combat", src, "[user] has added clone module ([W] - [module.BE]) to ([src]) at [log_loc(user)].")
		qdel(module)
...
/obj/machinery/computer/cloning/ui_act(action, params)
	...
	switch(action)
		if("delete")
			...
			if(selected_record)
				logTheThing("station", usr, "deletes the cloning record [selected_record["name"]] for player [selected_record["ckey"]] at [log_loc(src)].")
