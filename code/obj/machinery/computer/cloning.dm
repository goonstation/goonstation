...
/obj/machinery/computer/cloning/attackby(obj/item/W as obj, mob/user as mob)
	if (wagesystem.clones_for_cash && istype(W, /obj/item/spacecash))
		...
		user.drop_item()
		logTheThing("station", src, user, "[user] has added clone module ([W]) to ([src]) at [log_loc(user)].")
		qdel(W)
		...
		logTheThing("station", src, user, "[user] has added clone module ([W] - [module.BE]) to ([src]) at [log_loc(user)].")
		qdel(module)
