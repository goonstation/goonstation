...
//				dat += "<BR>\[ <A HREF='byond://?src=\ref[src];operation=call-prison'>Send Prison Shutle</A> \]"
	...
	dat += "<BR>\[ [(src.state != STATE_DEFAULT) ? "<A HREF='byond://?src=\ref[src];operation=main'>Main Menu</A> | " : ""]<A HREF='?action=mach_close&window=communications'>Close</A> \]"
	user.Browse(dat.Join(), "window=communications;size=400x500")
	onclose(user, "communications")
...
//			dat += "<BR>\[ <A HREF='byond://?src=\ref[src];operation=call-prison'>Send Prison Shutle</A> \]"
	...
	dat += "<BR>\[ [(src.aistate != STATE_DEFAULT) ? "<A HREF='byond://?src=\ref[src];operation=ai-main'>Main Menu</A> | " : ""]<A HREF='?action=mach_close&window=communications'>Close</A> \]"
	return dat
