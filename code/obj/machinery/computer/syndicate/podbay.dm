	...
	if (choice == "Yes")
		...
		if (length(src.authorized) < src.auth_need)
			...
			for (var/mob/M in hearers(src, null))
				M.show_message(SPAN_SUBTLE("<span class='say'>[SPAN_NAME("[src]")] beeps, \"[user.real_name]'s request accepted. [auths_required] authorization[s_es(auths_required)] needed until Podbay is opened.\"</span>"), 2)
		else
			src.authorize()
