...
/obj/machinery/computer/supplycomp/attack_hand(var/mob/user)
	...
	<div id="topBar">
		...
		Budget: <strong>[wagesystem.budgets[BUDGET_CAT_DEPT_SUPPLY]]</strong> Credits
		<div style='clear: both; text-align: center; font-weight: bold; padding: 0.2em;'>
		...
		switch (subaction)
			...
			if ("buy")
				...
				if(istype(locate(href_list["what"]), /datum/supply_order))
					...
					if(wagesystem.budgets[BUDGET_CAT_DEPT_SUPPLY] >= P.cost)
						O.object = P
						...
						if (isnull(O.comment) || P.cost > wagesystem.budgets[BUDGET_CAT_DEPT_SUPPLY])
							shippingmarket.supply_requests += O
						...
						. = {"<strong>Insufficient funds in supply budget.</strong>"}
						shippingmarket.supply_requests += O
				else
	...
	rockbox_controls(subaction, href_list)
		switch (subaction)
			...
			if ("premium_service")
				...
				if(response == "Yes")
					if(wagesystem.budgets[BUDGET_CAT_DEPT_SUPPLY] >= 10000)
						wagesystem.budgets[BUDGET_CAT_DEPT_SUPPLY] -= 10000
	...
	switch (href_list["action"])
		...
		if ("trader_buy_cart")
			...
			if (total_cart_amount > buy_cap)
				...
				if (wagesystem.budgets[BUDGET_CAT_DEPT_SUPPLY] < cart_cost)
