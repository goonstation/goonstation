		...
		if(istype(P))
			...
			if(account) //buy it with their money
				if(account["current_money"] < P.cost)
					...
					pdaSignal.data = list("address_1"="00000000", "command"="text_message", "sender_name"="SUPPLY-MAILBOT",  "group"=MGD_CARGO, "alert"=MGA_SHIPPING, "sender"="00000000", "message"="[O.object] ordered by [O.orderedby] using personal account at [O.console_location].")
