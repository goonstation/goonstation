	...
	if (href_list["order"])
		...
		if(istype(P))
			...
			if(account) //buy it with their money
				if(account.fields["current_money"] < P.cost)
					...
					pdaSignal.data = list("address_1"="00000000", "command"="text_message", "sender_name"="CARGO-MAILBOT", "tag" = PDA_CARGO_ALERT_TAG, "group"=MGD_CARGO, "sender"="00000000", "message"="Notification: [O.object] ordered by [O.orderedby] using personal account at [O.console_location].")
		...
		if (src.scan)
			...
			if (account.fields["current_money"] >= transaction && (transaction > 0))
				...
				var/datum/radio_frequency/transmit_connection = radio_controller.return_frequency("1149")
				var/datum/signal/pdaSignal = get_free_signal()
				pdaSignal.data = list("address_1"="00000000", "command"="text_message", "sender_name"="CARGO-MAILBOT", "tag" = PDA_CARGO_ALERT_TAG, "group"=MGD_CARGO, "sender"="00000000", "message"="Notification: [transaction] credits transfered to shipping budget from [src.scan.registered].")
				pdaSignal.transmission_method = TRANSMISSION_RADIO
				if(transmit_connection != null)
					transmit_connection.post_signal(src, pdaSignal)
				//////////
