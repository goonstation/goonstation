...
/obj/machinery/computer/card/Topic(href, href_list)
		...
		if(src.authenticated)
			...
			if(access_type in get_all_accesses())
				if(!access_allowed)
					...
					src.modify.access += access_type
				logTheThing("station", usr, "[access_allowed ? "adds" : "removes"] [get_access_desc(access_type)] access to the ID card (<b>[src.modify.registered]</b>).")
	...
	if (href_list["assign"])
		if (src.authenticated && src.modify)
			...
			if (t1 == "Custom Assignment")
				...
				logTheThing("station", usr, "changes the assignment on the ID card (<b>[src.modify.registered]</b>) from <b>[src.modify.assignment]</b> to <b>[t1]</b>.")
				...
				logTheThing("station", usr, "changes the access and assignment on the ID card (<b>[src.modify.registered]</b>) to <b>[t1]</b>.")
	...
	if (href_list["reg"])
		if (src.authenticated)
			...
			if ((src.authenticated && src.modify == t2 && (in_interact_range(src, usr) || (issilicon(usr) || isAI(usr))) && istype(src.loc, /turf)))
				logTheThing("station", usr, "changes the registered name on the ID card from <b>[src.modify.registered]</b> to <b>[t1]</b>.")
	...
	if (href_list["pin"])
		if (src.authenticated)
			...
			if ((src.authenticated && src.modify == currentcard && (in_interact_range(src, usr) || (istype(usr, /mob/living/silicon))) && istype(src.loc, /turf)))
				...
				logTheThing("station", usr, "changes the pin on the ID card (<b>[src.modify.registered]</b>) to [src.modify.pin].")
				playsound(src.loc, "keyboard", 50, 1, -15)
	...
	if (href_list["save"])
		...
		logTheThing("station", usr, "saves custom assignment <b>[src.custom_names[slot]]</b>.")
	if (href_list["apply"])
		...
		logTheThing("station", usr, "changes the access and assignment on the ID card (<b>[src.modify.registered]</b>) to custom assignment <b>[src.modify.assignment]</b>.")
	if (src.modify)
