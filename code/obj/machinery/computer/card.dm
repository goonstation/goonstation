...
/obj/machinery/computer/card/Topic(href, href_list)
	...
	if (href_list["pin"])
		if (src.authenticated)
			...
			var/newpin = tgui_input_number(usr, "Enter a new PIN.", "ID computer", PIN_MIN, PIN_MAX, PIN_MIN)

			if (src.authenticated && src.modify == currentcard && (in_interact_range(src, usr) || (istype(usr, /mob/living/silicon))) && istype(src.loc, /turf) && newpin)
