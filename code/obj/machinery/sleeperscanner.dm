...
/obj/machinery/sleeperscanner
	...
	if (ishuman(user))
		...
		if (user.mind?.get_antagonist(ROLE_SLEEPER_AGENT || ROLE_TRAITOR || ROLE_SPY_THIEF || ROLE_NUKEOP || ROLE_NUKEOP_COMMANDER)) //Probably adjusting to only sleepers during map PR
