	...
	proc/spawn_items()
		if (islist(src.guaranteed) && length(src.guaranteed))
			for (var/obj/new_item in src.guaranteed)
				if (!ispath(new_item))
					logTheThing("debug", src, "has a non-path item in its guaranteed list, [new_item]")
		...
		if (!islist(src.items2spawn) || !length(src.items2spawn))
			logTheThing("debug", src, "has an invalid items2spawn list")
			return
		if (rare_chance)
			if (!islist(src.rare_items2spawn) || !length(src.rare_items2spawn))
				logTheThing("debug", src, "has an invalid rare_items2spawn list")
				return
		if (amt2spawn == 0)
		...
		for (amt2spawn, amt2spawn>0, amt2spawn--)
			...
			if (prob(rare_chance))
				if (rare_items2spawn)
					...
					logTheThing("debug", src, "has an invalid rare spawn list, [rare_items2spawn]")
			...
			if (!ispath(new_item))
				logTheThing("debug", src, "has a non-path item in its spawn list, [new_item]")
