/obj/table
	...
	attackby(obj/item/W, mob/user, params)
		if (istype(W, /obj/item/grab))
			...
			if (user.a_intent == "harm")
				...
				logTheThing("combat", user, "slams [constructTarget(grabbed,"combat")] onto a table at [log_loc(grabbed)]")
			else
				src.gentle_slam(user, grabbed)
				logTheThing("station", user, "puts [constructTarget(grabbed,"combat")] onto a table at [log_loc(grabbed)]")
			qdel(W)
			return
	...
	attack_hand(mob/user)
		if (user.is_hulk() && !hulk_immune)
			...
			logTheThing("combat", user, "uses hulk to smash a table at [log_loc(src)].")
			deconstruct()
			return
...
/obj/table/glass
	...
	attackby(obj/item/W, mob/user, params)
		...
		if (istype(W, /obj/item/grab))
			...
			if (user.a_intent == "harm")
				logTheThing("combat", user, "slams [constructTarget(grabbed,"combat")] onto a glass table")
				src.harm_slam(user, grabbed)
			else
				logTheThing("station", user, "puts [constructTarget(grabbed,"combat")] onto a glass table")
				src.gentle_slam(user, grabbed)
	...
	hitby(atom/movable/AM, datum/thrown_thing/thr)
		...
		if (ismob(AM))
			...
			if ((prob(src.reinforced ? 60 : 80)))
				logTheThing("combat", thr.user, "throws [constructTarget(M,"combat")] into a glass table, breaking it")
