#define TURBINE_OVERTEMP_WARNING 2500
#define TURBINE_OVERTEMP_LIMIT 3000
#define TURBINE_STATOR_LOAD_MIN 1
#define TURBINE_STATOR_LOAD_MAX 10e30
#define TURBINE_FLOW_RATE_MIN 1
#define TURBINE_OVERSPEED_MULTIPLIER 1.2
...
/obj/machinery/reactor_turbine
	...
	/// Most efficient power generation at this value; overspeed is defined by TURBINE_OVERSPEED_MULTIPLIER.
	var/best_RPM = 600
	...
	var/stalling = FALSE
	/// Flag for RPM being above the overspeed threshold.
	var/overspeed = FALSE
	...
	/// Last inlet and outlet gas conditions processed by the turbine
	var/last_input_temperature = 0
	var/last_input_pressure = 0
	var/last_output_temperature = 0
	var/last_output_pressure = 0
	...
	New()
		...
		src.history = list()
		src.input = new /obj/machinery/atmospherics/unary/node{dir = WEST}(get_step(src, WEST))
		src.output = new /obj/machinery/atmospherics/unary/node{dir = EAST}(get_step(src, EAST))
	...
	process()
		. = ..()
		src.last_output_temperature = 0
		src.last_output_pressure = 0
		...
		if(!src.ruined && air_contents)
			...
			src.overspeed = (src.RPM > src.best_RPM * TURBINE_OVERSPEED_MULTIPLIER)
		...
		src.history += list(
			list(
				...
				"power" = src.lastgen,
				"temperature" = src.last_input_temperature,
				"pressure" = src.last_input_pressure,
				"outletTemperature" = src.last_output_temperature,
				"outletPressure" = src.last_output_pressure,
			)
		)
	...
	proc/get_tgui_data()
		...
		data["connected"] = TRUE
		data["rpm"] = src.RPM
		data["load"] = src.stator_load
		data["power"] = src.lastgen
		data["volume"] = src.flow_rate
		data["volumeMax"] = src.flow_rate_max
		data["statorLoadMin"] = TURBINE_STATOR_LOAD_MIN
		data["statorLoadMax"] = TURBINE_STATOR_LOAD_MAX
		data["flowRateMin"] = TURBINE_FLOW_RATE_MIN
		data["optimalRPM"] = src.best_RPM
		data["overspeedRPM"] = src.best_RPM * TURBINE_OVERSPEED_MULTIPLIER
		data["minimumTemperature"] = T20C
		data["overtempWarning"] = TURBINE_OVERTEMP_WARNING
		data["overtempLimit"] = TURBINE_OVERTEMP_LIMIT
		data["history"] = src.history
		data["overspeed"] = src.overspeed
		data["overtemp"] = src.overtemp
		data["undertemp"] = src.undertemp
		data["stalling"] = src.stalling
		data["ruined"] = src.ruined
		data["inletTemperature"] = src.last_input_temperature
		data["inletPressure"] = src.last_input_pressure
		data["outletTemperature"] = src.last_output_temperature
		data["outletPressure"] = src.last_output_pressure
		data["blade"] = list(
			"installed" = !isnull(src.current_blade),
			"healthPercent" = src.max_blade_health ? clamp(src.blade_health / src.max_blade_health * 100, 0, 100) : 0,
		)
		data["stator"] = list(
			"installed" = !isnull(src.current_stator),
		)
		return data
	...
	ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
		...
		switch(action)
			if("loadChange")
				var/x = params["newVal"]
				src.stator_load = min(max(x, TURBINE_STATOR_LOAD_MIN), TURBINE_STATOR_LOAD_MAX)
				logTheThing(LOG_STATION, src, "[src] stator load configured to [x] by [ui.user]")
			if("volChange")
				var/x = params["newVal"]
				src.flow_rate = min(max(x, TURBINE_FLOW_RATE_MIN), src.flow_rate_max)
...
#undef TURBINE_OVERTEMP_WARNING
#undef TURBINE_OVERTEMP_LIMIT
#undef TURBINE_STATOR_LOAD_MIN
#undef TURBINE_STATOR_LOAD_MAX
#undef TURBINE_FLOW_RATE_MIN
#undef TURBINE_OVERSPEED_MULTIPLIER
