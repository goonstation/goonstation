...
/obj/machinery/atmospherics/binary/reactor_turbine
	...
	process()
		...
		if(air_contents)
			...
			src.current_blade.temperature_expose(src.air_contents, src.air_contents.temperature, src.air_contents.volume)
	...
	attackby(obj/item/I, mob/user)
		if(isweldingtool(I))
			...
			if(I:try_weld(user,1))
				if(src.ruined)
					...
					boutput(user,"You repair some of the damage to \the [src].")
				else
		...
		if(istype(I, /obj/item/turbine_component))
			if(src.RPM > 1)
				...
				return
			if(istype(I, /obj/item/turbine_component/blade))
				if(isnull(src.current_blade))
					boutput(user, "You add the [I] to the turbine's drive shaft.")
				else
					boutput(user, "You replace the turbine's [src.current_blade] with the [I]")
				user.u_equip(I)
				I.set_loc(src)
				user.put_in_hand_or_drop(src.current_blade) //no-op if item is null
				SetTurbineBlade(I)
			else if(istype(I, /obj/item/turbine_component/stator))
				if(isnull(src.current_stator))
					boutput(user, "You add the [I] to the turbine's generator.")
				else
					boutput(user, "You replace the turbine's [src.current_stator] with the [I]")
				user.u_equip(I)
				I.set_loc(src)
	...
	proc/SetTurbineBlade(var/obj/item/turbine_component/blade/NewBlade)
		...
		src.blade_health = max(src.blade_health, 0.25*src.max_blade_health)
