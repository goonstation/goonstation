		...
		for(var/b=0, b<5, b++)
			...
			for(var/atom/atm in T)
				if(isliving(atm) && src.owner && (R.total_temperature != T20C || R.get_reagent_amount("ff-foam") != R.total_volume))
					logTheThing("combat", atm, "is hit by water spray [log_reagents(R)] from [owner] at [log_loc(atm)].")
					logTheThing("combat", owner, "hits [constructTarget(atm,"combat")], with extinguisher spray [log_reagents(R)] at [log_loc(atm)]")
