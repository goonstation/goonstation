...
/obj/effects/foam/proc/die()
	...
	if(!metal && reagents) //We don't want a foam that's done the transfer to do it's own thing
		...
		for(var/atom/A in src.loc)
			...
			if(isliving(A))
				var/mob/living/L = A
				logTheThing("combat", L, "is hit by chemical foam [log_reagents(src)] at [log_loc(src)].")
			if (reagents)
...
/obj/effects/foam/Crossed(atom/movable/AM)
	...
	if (ishuman(AM))
		...
		if (M.slip())
			logTheThing("combat", M, "is hit by chemical foam [log_reagents(src)] at [log_loc(src)].")
