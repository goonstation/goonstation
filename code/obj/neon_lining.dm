/obj/neon_lining
	...
	var/lining_color = 0          												//Colors: 0 = blue, 1 = pink, 2 = yellow.
	var/lining_color_string = "blue"											//Converts the current color to a string in lining_update_icon.
	...
	proc/lining_update_icon()
		if (lining_color == 1)
			lining_color_string = "pink"
			light.set_color(108, 7, 67)
		else if (lining_color == 2)
			lining_color_string = "yellow"
			light.set_color(126, 114, 54)
		else
			lining_color_string = "blue"
			light.set_color(17, 74, 124)
		...
		if (lining_shape < 1 || lining_shape > 6)
			...
			if (lining_pattern < 2)
				...
				glow.icon_state = "[lining_color_string][lining_shape]_1"
				src.UpdateOverlays(glow, "glow")
			else if (lining_pattern > 1 && lining_pattern < 4)
				set_icon_state("base3_1")
				glow.icon_state = "[lining_color_string][lining_shape]_2"
				src.UpdateOverlays(glow, "glow")
			else if (lining_pattern > 3 && lining_pattern < 6)
				set_icon_state("base3_2")
				glow.icon_state = "[lining_color_string][lining_shape]_3"
				src.UpdateOverlays(glow, "glow")
			else if (lining_pattern > 5 && lining_pattern < 8)
				set_icon_state("base3_2")
				glow.icon_state = "[lining_color_string][lining_shape]_4"
				src.UpdateOverlays(glow, "glow")
		else
			set_icon_state("base[lining_shape]")
			if (lining_shape == 1)
				glow.icon_state = "[lining_color_string]1"
			else
				glow.icon_state = "[lining_color_string][lining_shape]_[lining_icon_state]"
			src.UpdateOverlays(glow, "glow")
		return
