		...
		if (!isturf(target))
			...
			if (destination)
				if (prob(failchance)) //oh dear a problem, put em in deep space
					...
					return
				if(ismob(M))
					logTheThing("combat", M, "entered [src] at [log_loc(src)] and teleported to [src.target] at [log_loc(destination)]")
				do_teleport(M, destination, 1)
			else return
		else
			if(ismob(M))
				logTheThing("combat", M, "entered [src] at [log_loc(src)] and teleported to [log_loc(src.target)]")
