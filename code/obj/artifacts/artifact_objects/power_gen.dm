		...
		if(mode == 0)
			...
			if(isturf(T) && !T.intact)
				...
				if(!attached)
					...
					logTheThing("station", user, "connected power generator artifact [O] at [log_loc(O)].")
			...
			logTheThing("station", user, "discconnected power generator artifact [O] at [log_loc(O)].")
			var/obj/machinery/artifact/power_gen/L = O
			if (L.light)
				L.light.disable()
