...
/datum/artifact/combiner
	...
	proc/clear_first_art(play_fx = TRUE)
		...
		if (play_fx)
			...
			playsound(T, 'sound/items/lattice_combiner_transform_art.ogg', 50, TRUE)
			T.visible_message(SPAN_NOTICE("[src.holder] stops glowing green."))
		src.holder.remove_filter("first_art_picked")

	proc/try_combine_arts(obj/receiver, obj/to_merge)
		if (BOUNDS_DIST(receiver, to_merge) > 0)
			var/turf/T = get_turf(src.holder)
			T.visible_message(SPAN_NOTICE("[src.holder] glows yellow momentarily."))
			playsound(T, 'sound/items/lattice_combiner_error.ogg', 50, TRUE)
			src.clear_first_art(FALSE)
			src.holder.add_filter("yellow_error", 1, outline_filter(size = 1, color = "#fbff00"))
			SPAWN(0.5 SECONDS)
				src.holder.remove_filter("yellow_error")
			return FALSE
		if (!receiver.can_combine_artifact(to_merge))
			var/turf/T = get_turf(src.holder)
			T.visible_message(SPAN_NOTICE("[src.holder] glows red momentarily."))
			playsound(T, 'sound/items/lattice_combiner_error.ogg', 50, TRUE)
			src.clear_first_art(FALSE)
			src.holder.add_filter("red_error", 1, outline_filter(size = 1, color = "#ff0000"))
			SPAWN(0.5 SECONDS)
				src.holder.remove_filter("red_error")
			return FALSE
		return TRUE
