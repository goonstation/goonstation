...
///////////////////////////////////////
		...
		medal
			...
			proc/get_award_text(var/datum/mind/M)
				...
				if(M?.current && M.current.client && M.current.client.preferences && M.current.client.preferences.name_last)
					hosname = M.current.client.preferences.name_last
				var/hosage = 50
				if(M?.current && M.current.bioHolder && M.current.bioHolder.age)
