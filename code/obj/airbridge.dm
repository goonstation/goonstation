	...
	Topic(href, href_list)
		...
		if (href_list["create"])
			...
			if (src.establish_bridge())
				logTheThing("station", usr, "extended the airbridge at [usr.loc.loc] ([log_loc(usr)])")
			...
			if (src.remove_bridge())
				logTheThing("station", usr, "retracted the airbridge at [usr.loc.loc] ([log_loc(usr)])")
			...
			if (!(src.allowed(usr)))
				...
				return
			if (src.pressurize())
				logTheThing("station", usr, "pressurized the airbridge at [usr.loc.loc] ([log_loc(usr)])")
