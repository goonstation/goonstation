...
/client/proc/toggle_numbers_station_messages()
	...
	logTheThing("admin", usr, "toggled numbers station alerts [shut_up_about_the_fucking_numbers_station ? "off" : "on"].")
	logTheThing("diary", usr, "toggled numbers station alerts [shut_up_about_the_fucking_numbers_station ? "off" : "on"].", "admin")
...
/datum/numbers_station // This is not a physical entity!
	...
	proc/process()
		...
		if (ticker.round_elapsed_ticks >= next_play)
			...
			logTheThing("debug", null, "<b>Numbers station</b>: [name] is broadcasting on frequency [frequency / 10].")
			gather_listeners()
			if (!listeners.len)
				logTheThing("debug", null, "<b>Numbers station:</b> [name] broadcast aborted: no listeners.")
				return
			SPAWN(1 SECOND)
...
/proc/lincolnshire_numbers(data)
	if (islist(data))
		logTheThing("debug", null, "<b>Numbers station</b>: numbers: [data["numbers"]]")
		var/TP = data["numbers"]
		if (TP == null)
			return 1
		var/list/nums = splittext(TP, " ")
		if (nums.len < 21)
			logTheThing("debug", null, "<b>Numbers station</b> got too few numbers.")
			return 2
		for (var/i = 1, i <= 21, i++)
			lincolnshire.numbers[i] = text2num(nums[i])
		logTheThing("debug", null, "<b>Numbers station</b> woo success")
		return 0
	return 3
