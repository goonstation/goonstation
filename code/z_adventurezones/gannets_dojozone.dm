...
/datum/job/special/samurai
	...
	limit = 0
	wages = 0
//	slot_belt = /obj/item/melee/swords_sheaths/katana
	...
	slot_belt = list(/obj/item/melee/swords_sheaths/katana/reverse)
		...
		if(!QDELETED(src) && can_quench && (src.strikes > (src.strikes_to_complete * 0.25)) && (src.temperature > T0C+1000))
			if( src.strikes > src.strikes_to_complete )
				if(RC.reagents.has_reagent("reversium",1))
					new /obj/item/melee/swords/katana/reverse(get_turf(target))
				else
					new /obj/item/melee/swords/katana/crafted(get_turf(target))
				RC.reagents.smoke_start(RC.reagents.total_volume)
				user.u_equip(src)
				qdel(src)
			else
				RC.reagents.smoke_start(RC.reagents.total_volume)
				var/obj/item/melee/swords/fragile_sword/sword = new(get_turf(target))
