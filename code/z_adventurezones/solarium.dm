			...
			if (!O:on)
				...
				if (!user.is_heat_resistant())
					SPAWN(1 SECOND)
						...
						logTheThing("combat", user, "was firegibbed by [src] ([src.type]) at [log_loc(user)].")
						user.firegib()
				else
...
/obj/the_server_ingame_whoa
	...
	proc/breakdown()
		...
		SPAWN(0)
			...
			logTheThing("diary", null, "The server would have restarted, if I hadn't removed the line of code that does that. Instead, we play through.", "game")
