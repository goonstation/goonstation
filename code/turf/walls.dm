	...
	if (user.is_hulk())
		if(isrwall(src))
			...
			if (prob(40))
				...
				logTheThing("combat", user, "uses hulk to smash a wall at [log_loc(src)].")
				dismantle_wall(1)
				return
			else
...
/turf/simulated/wall/proc/weld_action(obj/item/W, mob/user)
	logTheThing("station", user, "deconstructed a wall ([src.name]) using \a [W] at [get_area(user)] ([log_loc(user)])")
	dismantle_wall()
	...
	if (istype(W, /obj/item/pen))
		...
		if (src.d_state == 3)
			...
			if ((user.loc == T && user.equipped() == W))
				...
				dismantle_wall()
				logTheThing("station", user, "dismantles a reinforced wall at [log_loc(user)].")
				return
			else if((isrobot(user) && (user.loc == T)))
				boutput(user, "<span class='notice'>You removed the outer sheath.</span>")
				dismantle_wall()
