...
/turf/unsimulated/floor/auto
	...
	New()
		...
		SPAWN(0.5 SECONDS) //give neighbors a chance to spawn in
			edge_overlays()
	Del()
		edge_overlays(TRUE) // remove from neighbors
		..()
	proc/edge_overlays(var/remove=FALSE)
		for (var/turf/T in orange(src,1))
			if (istype(T, /turf/simulated/shuttle/wall/cockpit))
				continue
			...
			if (!remove)
				edge_overlay = image(src.icon, "[icon_state_edge][direction]")
