//DISABLE THIS FILE ONCE WE UPGRADE TO A NEWER AND MORE STABLE BYOND VERSION
	...
	proc
		...
		ReadObject(list/tokens)
			...
			while(i <= tokens.len)
				...
				if(K.value == "}")
					next_token()
					return//emply object you fucking shithead
...
#define json_decode json2list
#define json_encode list2json
