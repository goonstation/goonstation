...
/datum/mapSwitchHandler
	...
	proc/setupPickableList()
		...
		if (!src.active)
			logTheThing("debug", null, "<b>Map Switcher:</b> Failed to find an entry in mapNames. map_setting: [map_setting]")
			return
	...
	proc/endMapVote()
		...
		logTheThing("debug", null, "<b>Map Vote Debug:</b> Vote data: [json_encode(votes)]. Report data: [json_encode(reportData)]")
		...
		//handle ties
		if (tiedMaps.len)
			logTheThing("debug", null, "Map tie detected. Choices: [json_encode(tiedMaps)]")
		...
		if (src.voteChosenMap == src.current)
			...
			try
				...
				logTheThing("admin", null, "Failed to set map <b>[src.voteChosenMap]</b> from map vote: [e]")
				logTheThing("diary", null, "Failed to set map <b>[src.voteChosenMap]</b> from map vote: [e]", "debug")
				return
		...
		//log this
		logTheThing("admin", null, "The players voted for <b>[src.voteChosenMap]</b> as the next map.")
		logTheThing("diary", null, "The players voted for [src.voteChosenMap] as the next map.", "admin")
