...
/atom/proc/UpdateOverlays(var/image/I, var/key, var/force=0, var/retain_cache = 0)
	...
	if(index > 0) //There is an existing overlay in place in this slot, remove it
		src.overlays.Cut(index, min(index+1,src.overlays.len)) //Fuck yoooou byond (this gotta be by index or it'll fail if the same thing's in overlays several times)
