...
/mob/proc/buttgib(give_medal)
	...
	for (var/i = 0, i < 16, i++)
		...
		if (organHolder)
			...
			the_butt.setMaterial(getMaterial(/datum/material/organic/ectoplasm), appearance = TRUE, setname = TRUE)
		else if (istype(src, /mob/living/intangible/blob_overmind))
			the_butt = new /obj/item/clothing/head/butt
			the_butt.setMaterial(getMaterial(/datum/material/organic/blob), appearance = TRUE, setname = TRUE)
		else
