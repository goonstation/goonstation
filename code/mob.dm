		...
		if (result == 1)
			...
			for(var/A in rewardDB)
				...
				if (D.required_medal == title)
					unlocks.Add(D)
			if (announce)
				boutput(world, SPAN_MEDAL("[displayed_key] earned the [title] medal."))//src.client.stealth ? src.client.fakekey : << seems to be causing trouble
			else if (ismob(src) && src.client)
				boutput(src, SPAN_MEDAL("You earned the [title] medal."))

			if (length(unlocks))
				for(var/datum/achievementReward/B in unlocks)
					boutput(src, SPAN_MEDAL("<FONT FACE=Arial SIZE=+1>You've unlocked a Reward : [B.title]!</FONT>"))

		else if (isnull(result) && ismob(src) && src.client)
			return
//			boutput(src, SPAN_ALERT("You would have earned the [title] medal, but there was an error communicating with the BYOND hub."))
