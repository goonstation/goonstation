...
/mob/bump(atom/A)
	...
	if (ismob(AM))
		...
		if (!issilicon(AM) && !issilicon(src))
			if (tmob.a_intent == "help" && src.a_intent == "help" && tmob.canmove && src.canmove && !tmob.buckled && !src.buckled &&!src.throwing && !tmob.throwing) // mutual brohugs all around!
				...
				else if ((locate(/obj/hotspot) in tmob.loc) || (locate(/obj/chem_fire) in tmob.loc))
	...
	if (istype(AM, /atom/movable) && !AM.anchored)
		...
		if (isliving(AM))
			...
			var/was_in_fire = (locate(/obj/hotspot) in victim.loc) || (locate(/obj/chem_fire) in victim.loc)
			...
			else if (!was_in_fire && ((locate(/obj/hotspot) in victim.loc) || (locate(/obj/chem_fire) in victim.loc)))
