...
/mob/Logout()
	...
	//logTheThing("diary", src, "logged out", "access") <- sometimes shits itself and has been known to out traitors. Disabling for now.
		...
		if (!issilicon(AM))
			if (tmob.a_intent == "help" && src.a_intent == "help" && tmob.canmove && src.canmove && !tmob.buckled && !src.buckled && !src.throwing && !tmob.throwing) // mutual brohugs all around!
				...
				if (istype(tmob.loc, /turf/space))
					logTheThing("combat", src, "trades places with (Help Intent) [constructTarget(tmob,"combat")], pushing them into space.")
				else if (locate(/obj/hotspot) in tmob.loc)
					logTheThing("combat", src, "trades places with (Help Intent) [constructTarget(tmob,"combat")], pushing them into a fire.")
	...
	if (istype(AM, /atom/movable) && !AM.anchored)
		...
		if (isliving(AM))
			...
			if (!was_in_space && istype(victim.loc, /turf/space))
				logTheThing("combat", src, "pushes [constructTarget(victim,"combat")] into space.")
			else if (!was_in_fire && (locate(/obj/hotspot) in victim.loc))
				logTheThing("combat", src, "pushes [constructTarget(victim,"combat")] into a fire.")
...
/mob/ex_act(severity, last_touched)
	logTheThing("combat", src, "is hit by an explosion (Severity: [severity]) at [log_loc(src)]. Explosion source last touched by [last_touched]")
	return
...
/mob/proc/add_miranda()
	...
	if (src.mind.last_memory_time + 10 <= world.time) // leaving it using this var cause vOv
		...
		logTheThing("telepathy", src, "has set their miranda rights quote to: [src.mind.miranda]")
...
/mob/verb/abandon_mob()
	...
	logTheThing("diary", usr, "used abandon mob.", "game")
...
/mob/verb/succumb()
	...
	if (src.health < 0)
		...
		logTheThing("combat", src, "succumbs")
...
/mob/proc/updatehealth()
	if (src.nodamage == 0)
		...
		if (src.health < 0 && !src.incrit)
			src.incrit = 1
			logTheThing("combat", src, "goes into crit [log_health(src)] at [log_loc(src)].")
...
#endif
	logTheThing("combat", src, "is gibbed at [log_loc(src)].")
	src.death(TRUE)
...
/mob/proc/elecgib()
	...
	game_stats.Increment("violence")
#endif
	logTheThing("combat", src, "is electric-gibbed at [log_loc(src)].")
	src.death(TRUE)
	var/atom/movable/overlay/gibs/animation = null
	src.transforming = 1
	src.canmove = 0
...
/mob/proc/firegib(var/drop_clothes = TRUE)
	...
	game_stats.Increment("violence")
#endif
	logTheThing("combat", src, "is fire-gibbed at [log_loc(src)].")
	src.death(TRUE)
	var/atom/movable/overlay/gibs/animation = null
	src.transforming = 1
	src.canmove = 0
...
/mob/proc/partygib(give_medal)
	...
	game_stats.Increment("violence")
#endif
	logTheThing("combat", src, "is party-gibbed at [log_loc(src)].")
	src.death(TRUE)
	var/atom/movable/overlay/gibs/animation = null
	src.transforming = 1
	src.canmove = 0
...
/mob/proc/owlgib(give_medal, control_chance = 1)
	...
	logTheThing("combat", src, "is owl-gibbed at [log_loc(src)].")
	src.death(TRUE)
	var/atom/movable/overlay/gibs/animation = null
	src.transforming = 1
	src.canmove = 0
...
/mob/proc/vaporize(give_medal, forbid_abberation)
	...
	src.icon = null
	APPLY_ATOM_PROPERTY(src, PROP_MOB_INVISIBILITY, "transform", INVIS_ALWAYS)
	logTheThing("combat", src, "is vaporized at [log_loc(src)].")
...
/mob/proc/implode(give_medal)
	...
	game_stats.Increment("violence")
#endif
	logTheThing("combat", src, "imploded at [log_loc(src)].")
	src.death(TRUE)
	var/atom/movable/overlay/gibs/animation = null
	src.transforming = 1
	src.canmove = 0
...
/mob/proc/cluwnegib(var/duration = 30, var/anticheat = 0)
	...
	SPAWN(0) //multicluwne
		...
		game_stats.Increment("violence")
	#endif
		logTheThing("combat", src, "is taken by the floor cluwne at [log_loc(src)].")
		src.transforming = 1
		src.canmove = 0
		src.anchored = 1
		src.mouse_opacity = 0
...
/mob/proc/buttgib(give_medal)
	...
	game_stats.Increment("violence")
#endif
	logTheThing("combat", src, "is butt-gibbed at [log_loc(src)].")
	var/atom/movable/overlay/gibs/animation = null
	src.transforming = 1
	src.canmove = 0
	src.icon = null
...
/mob/proc/damn()
	...
	SPAWN(0) //multisatan
		logTheThing("combat", src, "is damned to hell from [log_loc(src)].")
		src.transforming = 1
		src.canmove = 0
		src.anchored = 1
		src.mouse_opacity = 0
