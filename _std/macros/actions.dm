...
*/
#define SETUP_GENERIC_ACTIONBAR(owner, target, duration, owner_proc_path, target_proc_path, action_icon, action_icon_state, end_message) \
	actions.start(new /datum/action/bar/icon/callback(owner, target, duration, owner_proc_path, target_proc_path, action_icon, \
	action_icon_state, end_message), owner)
...
/obj/item/foo
	...
	attack_self(var/mob/M)
		...
		SETUP_GENERIC_ACTIONBAR(M, src, 5 SECONDS, null, /obj/item/foo/proc/cool_proc, src.icon, src.icon_state,\
...
/obj/item/foo
	...
	attack_self(var/mob/M)
		M.visible_message("[M] starts fiddling with \the [src].")
		var/datum/action/bar/icon/callback/action_bar = new /datum/action/bar/icon/callback(M, src, 5 SECONDS, null, /obj/item/foo/proc/cool_proc,\
