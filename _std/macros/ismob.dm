...
#define isadminghost(x) (x.client && x.client.holder && rank_to_level(x.client.holder.rank) >= LEVEL_MOD && !x.client.player_mode && (istype(x, /mob/dead/observer) || istype(x, /mob/dead/target_observer))) // For antag overlays.
