...
#define _LOGGING_CONCENTRATION_REPORT(GAS, _, NAME, MIXTURE, SEP) "[(round(MIXTURE.GAS / total_moles * 100)) ? "[NAME]: [round(MIXTURE.GAS / total_moles * 100)]%[SEP]" : ""]" +
#define _LOGGING_UNKNOWN_CONCENTRATION_REPORT(MIXTURE, SEP) "[(length((MIXTURE).trace_gases) ? "Unknown: [round((total_moles - BASE_GASES_TOTAL_MOLES(MIXTURE)) / total_moles * 100)]%[SEP]": "")]"
#define LOGGING_CONCENTRATION_REPORT(MIXTURE, SEP) (APPLY_TO_GASES(_LOGGING_CONCENTRATION_REPORT, MIXTURE, SEP) _LOGGING_UNKNOWN_CONCENTRATION_REPORT(MIXTURE, SEP))
