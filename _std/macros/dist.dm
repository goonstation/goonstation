...
/// Returns 0 if A and B are on on the same turf, through any amount of nested objs/mobs. Otherwise returns bounds_dist.
#define BOUNDS_DIST(A, B) ((GET_DIST(A, B) <= 1 ? 0 : bounds_dist(A, B)))
