...
#define ANALYSER_BLACKLIST 0 //!Set to this to unset all flags so scanning always fails silently.
//Scan options
#define ANALYSER_ALLOWED 1<<0 //!Allows this item to be scanned.
#define ANALYSER_ALL 1<<0 //!Syntaxic sugar, Allows a scanner to scan anything that is allowed.
#define ANALYSER_FAILFEEDBACK 1<<1 //!If scanning would fail, gives feedback instead of silently failing.
#define ANALYSER_SKIP_IF_FAIL 1<<2 //!If scanning would fail, it does the normal attackby logic. Used for putting things on tables, etc. This behaviour overrides ANALYSER_FAILFEEDBACK.
