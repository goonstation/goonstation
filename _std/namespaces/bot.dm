...
CREATE_NAMESPACE(BOT, MODE, SECBOT)
ADD_TO_NAMESPACE(BOT, MODE, SECBOT)(var/const/IDLE = 0) //! Idle, handles routing to basic patrol-or-dont secbotting
ADD_TO_NAMESPACE(BOT, MODE, SECBOT)(var/const/AGGRO = 1) //! Bot is angry, chasing someone or arresting them
ADD_TO_NAMESPACE(BOT, MODE, SECBOT)(var/const/START_PATROL = 2) //! Starting patrol, looking for a patrol node
ADD_TO_NAMESPACE(BOT, MODE, SECBOT)(var/const/PATROL = 3) //! On patrol!
ADD_TO_NAMESPACE(BOT, MODE, SECBOT)(var/const/SUMMON = 4) //! Summoned by PDA
ADD_TO_NAMESPACE(BOT, MODE, SECBOT)(var/const/GUARD_IDLE = 5) //! Idle again, but handles routing for guard-related stuff
ADD_TO_NAMESPACE(BOT, MODE, SECBOT)(var/const/GUARD_START = 6) //! Was ordered to guard an area. Checking to see if that's something it can do
ADD_TO_NAMESPACE(BOT, MODE, SECBOT)(var/const/GUARD = 7) //! Currently guarding an area and milling about like an asshole
ADD_TO_NAMESPACE(BOT, MODE, SECBOT)(var/const/GUARD_AGGRO = 8) //! Bot is angry, but was guarding an area and should go back to guarding after this
