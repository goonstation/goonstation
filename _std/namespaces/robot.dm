...
ADD_TO_NAMESPACE(ROBOT, MODE)(var/const/SECBOT_IDLE = 0) //! Idle, handles routing to basic patrol-or-dont secbotting
ADD_TO_NAMESPACE(ROBOT, MODE)(var/const/SECBOT_AGGRO = 1) //! Bot is angry, chasing someone or arresting them
ADD_TO_NAMESPACE(ROBOT, MODE)(var/const/SECBOT_START_PATROL = 2) //! Starting patrol, looking for a patrol node
ADD_TO_NAMESPACE(ROBOT, MODE)(var/const/SECBOT_PATROL = 3) //! On patrol!
ADD_TO_NAMESPACE(ROBOT, MODE)(var/const/SECBOT_SUMMON = 4) //! Summoned by PDA
ADD_TO_NAMESPACE(ROBOT, MODE)(var/const/SECBOT_GUARD_IDLE = 5) //! Idle again, but handles routing for guard-related stuff
ADD_TO_NAMESPACE(ROBOT, MODE)(var/const/SECBOT_GUARD_START = 6) //! Was ordered to guard an area. Checking to see if that's something it can do
ADD_TO_NAMESPACE(ROBOT, MODE)(var/const/SECBOT_GUARD = 7) //! Currently guarding an area and milling about like an asshole
ADD_TO_NAMESPACE(ROBOT, MODE)(var/const/SECBOT_GUARD_AGGRO = 8) //! Bot is angry, but was guarding an area and should go back to guarding after this
