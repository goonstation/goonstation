...
export const getIsAccessPanelVisible = ({
  is_ai,
  wiresexposed,
}: ApcAccessPanelData) => !!wiresexposed && !is_ai;

const localInterfaceTypes = [
  InterfaceType.LocalOnly,
  InterfaceType.LocalAndNetwork,
];
export const getIsLocalAccess = ({
  can_access_remotely,
  setup_networkapc,
}: ApcInterfaceData) =>
...
const calculateMainSectionHeight = (
  area_requires_power: BooleanLike,
  isLocalAccess: boolean,
) =>
  (area_requires_power ? (isLocalAccess ? 181 : 132) : 62) +
...
export const calculateWindowHeight = (
  ...
  const mainSectionHeight = calculateMainSectionHeight(
    area_requires_power,
    isLocalAccess,
  );
