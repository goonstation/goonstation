...
export const buildColumnConfigs = (callbacks: ColumnConfigsCallbacks): ColumnConfig<BanResource>[] => [
  ...
  },
  {
    ...
    renderContents: ({ value }) => <a href={`https://tomato.goonhub.com/admin/bans/${value}`}>{value}</a>,
    basis: 4,
  },
  {
    header: 'ckey',
    ...
    renderContents: ({ value }) => <a href={`https://tomato.goonhub.com/admin/players/${value}`}>{value}</a>,
  ...
  },
  {
    ...
    renderContents: ({ value }) => (
      <a href={`https://tomato.goonhub.com/admin/game-admins/${value.id}`} class="NoColor">
        {value.name}
      </a>
    ),
    getValueTooltip: (data) => {
