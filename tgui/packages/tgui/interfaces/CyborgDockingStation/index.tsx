    ...
    return (
      <>
        <Box backgroundColor="#773333" p="5px" mb="5px" bold textAlign="center">
          {(data.viewer_is_robot
            && !data.viewer_is_occupant
      ...
      <>
        <LabeledList>
          ...
          <LabeledList.Item label="Type">
            <OccupantType kind={occupant.kind} user={occupant.user} />
          </LabeledList.Item>
        </LabeledList>
        <Section title="Status">
          {occupant.kind === 'robot' && (
            <OccupantStatusRobot occupant={occupant} fuel={fuel} cabling={cabling} act={act} />
          )}
...
const OccupantStatusRobot = (props) => {
  ...
  return (
    <>
      ...
      <OccupantUpgradeDisplay upgrades={occupant.upgrades} upgrades_max={occupant.upgrades_max} act={act} />
  ...
  return (
    <LabeledList>
      <LabeledList.Item label={occupant.cell.name}>
        <CellChargeBar cellData={occupant.cell} />
      </LabeledList.Item>
    </LabeledList>
  );
};
        ...
        clothes.map((cloth) => {
          return (
            <Box key={cloth.ref}>
              {cloth.name}
              <DockingAllowedButton
                onClick={() => act('clothing-remove', { clothingRef: cloth.ref })}
                icon="minus-circle"
                color="transparent"
      ...
      <LabeledList>
        <LabeledList.Item
          ...
          buttons={
            <DockingAllowedButton
              ...
              onClick={() => act('cosmetic-change-head')}
            />
          }>
          {cosmetics.head || 'None'}
        </LabeledList.Item>
        <LabeledList.Item
          ...
          buttons={
            <DockingAllowedButton
              ...
              onClick={() => act('cosmetic-change-chest')}
            />
          }>
        ...
        <LabeledList.Item
          ...
          buttons={
            <DockingAllowedButton
              ...
              onClick={() => act('cosmetic-change-arms')}
            />
          }>
          {cosmetics.arms || 'None'}
        </LabeledList.Item>
        <LabeledList.Item
          ...
          buttons={
            <DockingAllowedButton
              ...
              onClick={() => act('cosmetic-change-legs')}
            />
          }>
          {cosmetics.legs || 'None'}
        </LabeledList.Item>
        <LabeledList.Item
          ...
          buttons={
            <>
              {!cosmetics.paint && (
                <DockingAllowedButton icon="plus" tooltip="Add paint" onClick={() => act('occupant-paint-add')} />
              )}
              {cosmetics.paint && (
                <DockingAllowedButton
                  icon="tint"
                  tooltip="Change colour"
                  onClick={() => act('occupant-paint-change')}
                />
              )}
              {cosmetics.paint && (
                <DockingAllowedButton
                  ...
                  onClick={() => act('occupant-paint-remove')}
                />
              )}
            </>
          }>
        ...
        <LabeledList.Item
          label="Eyes"
          buttons={<DockingAllowedButton icon="tint" tooltip="Change eye glow" onClick={() => act('occupant-fx')} />}>
              ...
              <div key={cell.ref}>
                <LabeledList.Item
                  ...
                  buttons={
                    <>
                      <DockingAllowedButton
                        onClick={() => act('cell-install', { cellRef: cell.ref })}
                        icon="plus"
                        tooltip="Add to occupant"
                      />
                      <DockingAllowedButton
                        onClick={() => act('cell-eject', { cellRef: cell.ref })}
                        icon="eject"
        ...
        modules.map((modu) => {
          return (
            <div key={modu.ref}>
              {modu.name}
              <DockingAllowedButton
                onClick={() => act('module-install', { moduleRef: modu.ref })}
                icon="plus-circle"
                color="transparent"
                tooltip="Add to occupant"
              />
              <DockingAllowedButton
                onClick={() => act('module-eject', { moduleRef: modu.ref })}
                icon="eject"
                color="transparent"
                tooltip="Eject from station"
              />
        ...
        upgrades.map((upgrade) => {
          return (
            <div key={upgrade.ref}>
              {upgrade.name}
              <DockingAllowedButton
                onClick={() => act('upgrade-install', { upgradeRef: upgrade.ref })}
                icon="plus-circle"
                color="transparent"
                tooltip="Add to occupant"
              />
              <DockingAllowedButton
                onClick={() => act('upgrade-eject', { upgradeRef: upgrade.ref })}
                icon="eject"
                color="transparent"
                tooltip="Eject from station"
              />
        ...
        clothes.map((cloth) => {
          return (
            <Box key={cloth.ref}>
              {cloth.name}
              <DockingAllowedButton
                onClick={() => act('clothing-install', { clothingRef: cloth.ref })}
                icon="plus-circle"
                color="transparent"
                tooltip="Add to occupant"
              />
              <DockingAllowedButton
                onClick={() => act('clothing-eject', { clothingRef: cloth.ref })}
                icon="eject"
                color="transparent"
                tooltip="Eject from station"
              />
...
const CellChargeBar = (props) => {
  ...
  return (
    <Tooltip position="bottom" content={Math.floor(cellData.current) + '/' + cellData.max}>
      <ProgressBar
        position="relative"
        value={charge}
...
const DockingTabView = (props) => {
  ...
  switch (tabIndex) {
    case 1:
      return <OccupantTab occupant={data.occupant} cabling={data.cabling} fuel={data.fuel} act={act} />;
    case 2:
      return (
        <Section title="Supplies">
          <LabeledList>
            <LabeledList.Item label="Welding Fuel">{data.fuel}</LabeledList.Item>
            <LabeledList.Item label="Wire Cabling">{data.cabling}</LabeledList.Item>
            <LabeledList.Item label="Self Service">
              <Button.Checkbox
                tooltip="Toggle self-service."
                checked={data.allow_self_service}
                disabled={data.viewer_is_robot}
                onClick={() => act('self-service')}
              />
              {data.allow_self_service ? 'Enabled' : 'Disabled'}
            </LabeledList.Item>
          </LabeledList>
