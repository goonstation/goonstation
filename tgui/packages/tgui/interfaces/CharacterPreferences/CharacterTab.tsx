  ...
  return (
    <Fragment>
      <Box mb="5px">
        <Button
          icon="chevron-left"
          onClick={() => act("update", { detail: 1, id, previousStyle: 1 })}
        />
        <Button
          onClick={() => act("update", { detail: 1, id, nextStyle: 1 })}
          icon="chevron-right"
        />
        <Button onClick={() => act("update", { detail: 1, id, style: 1 })}>
          {style}
        </Button>
      </Box>
      <Box>
          ...
          <Flex.Item shrink="1" pr="5px">
            <LabeledList>
              ...
              <LabeledList.Item label="Randomization">
                <Button.Checkbox checked={data.randomAppearance}>
                  Always use a randomized appearance
                </Button.Checkbox>
              </LabeledList.Item>
            </LabeledList>
          </Flex.Item>
          <Flex.Item basis="80px" shrink="0" ml="10px">
            <ByondUi
              params={{
                id: data.preview,
                type: "map",
      ...
      <Section title="Sounds">
        <LabeledList>
          <LabeledList.Item label="Fart">
            <Button onClick={() => act("update", { fartsound: 1 })}>
              {data.fartsound}
            </Button>
            <Button
              icon="volume-up"
          ...
          <LabeledList.Item label="Scream">
            <Button onClick={() => act("update", { screamsound: 1 })}>
              {data.screamsound}
            </Button>
            <Button
              icon="volume-up"
          ...
          <LabeledList.Item label="Chat">
            <Button onClick={() => act("update", { chatsound: 1 })}>
              {data.chatsound}
            </Button>
            <Button
              icon="volume-up"
