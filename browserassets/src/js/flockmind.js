...
$(document).ready(function () {
  ...
  function getGradientFromRange(gradient, value, min, max) {
    // probably not the right name but fuck it
    ...
    return rgbToHex(
      Math.floor(lerpCol.r),
      Math.floor(lerpCol.g),
      Math.floor(lerpCol.b)
    );
  }
  ...
  function updateVitalsTable() {
    ...
    table.find('td.value').each(function (index, element) {
      ...
      element.innerHTML =
        typeof vitals[field] !== 'undefined' ? vitals[field] : '???';
    });
  }
      ...
      if (entity.ref) {
        ...
        $.each(properties, function (index, value) {
          ...
          if (value.action) {
            ...
            button = td.find(
              '#' + entitiesKey + '-' + value.name + '-' + htmlSafeRef
            );
            if (button.length === 0) {
              button = $('<a>')
                .addClass('button')
                .attr('id', entitiesKey + '-' + value.name + '-' + htmlSafeRef);
              td.append(button);
  ...
  function onAction(ref, action) {
    window.location =
      '?src=' + flockPanelRef + '&action=' + action + '&origin=' + ref;
  }
