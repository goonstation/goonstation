...
var tooltip = {
	...
	$body: null,
	mapOffsets: {
		//Where the map is located on the screen
		x: 0,
		y: 0,
		w: 0,
		h: 0,
	...
	screenProperties: {
		//What the user's screen looks like (not counting offsets like OS toolbars)
		x: 0,
		y: 0,
	},
	showDelay: 100,
	...
	log: function (text) {
		window.location =
			"?src=" + window.tooltipRef + ";action=log&msg=" + escaper(text);
	},
	...
	show: function (docWidth, docHeight, posX, posY) {
		...
		//Animate in if set
		if (
			tooltip.options.hasOwnProperty("transition") &&
			animatePopup.isValidAnimation(tooltip.options.transition)
		) {
		...
		$(window)
			...
			.one("keypress", function (e) {
				if (!$(e.target).is("input, textarea, select, option, button")) {
					window.location =
						"byond://winset?" + tooltip.getMapControlFor("map") + ".focus=true";
				}
			});
	...
	position: function (params) {
		...
		if (
			tooltip.options.hasOwnProperty("size") &&
			typeof tooltip.options.size === "string"
		) {
			var size = tooltip.options.size.split("x");
			var widthString = size[0].toLowerCase();
			var heightString = size[1].toLowerCase();
			docWidth =
				widthString === "auto" ? tooltip.$wrap.outerWidth() : parseInt(size[0]);
			docHeight =
				heightString === "auto"
					? tooltip.$wrap.outerHeight()
					: parseInt(size[1]);
		...
		if (
			tooltip.params.hasOwnProperty("flags") &&
			tooltip.params.flags.length > 0
		) {
			var alignment = "bottom";
			if ($.inArray("top", tooltip.params.flags) !== -1) {
				//TOOLTIP_TOP
				alignment = "top";
				posY = posY - docHeight - realIconSizeY - tooltip.padding * 2;
			}
			if ($.inArray("top2", tooltip.params.flags) !== -1) {
				//TOOLTIP_TOP_2 (give 1 tile of margin if tooltipping something at the bottom of view  (hud))
				alignment = "top";
				posY = posY - docHeight - realIconSizeY - tooltip.padding * 2;
				if (yloc <= 1) {
			...
			if ($.inArray("right", tooltip.params.flags) !== -1) {
				//TOOLTIP_RIGHT
				alignment = "right";
				posX = posX + realIconSizeX;
				posY = posY - realIconSizeY;
			}
			if ($.inArray("left", tooltip.params.flags) !== -1) {
				//TOOLTIP_LEFT
				alignment = "left";
				posX = posX - docWidth - tooltip.padding * 2;
				posY = posY - realIconSizeY;
			}
			if ($.inArray("center", tooltip.params.flags) !== -1) {
				//TOOLTIP_CENTER
		...
		var boundaryY =
			tooltip.map.size.y + (tooltip.mapOffsets.y - tooltip.screenProperties.y);
		if (posY + docHeight > boundaryY) {
			//Is the bottom edge below the window? Snap it up if so
			posY = posY - docHeight - realIconSizeY - tooltip.padding;
		}

		var boundaryX =
			tooltip.map.size.x + (tooltip.mapOffsets.x - tooltip.screenProperties.x);
		if (posX + docWidth > boundaryX) {
			//Is the right edge outside the map area? Snap it back left if so
			posX = posX - (posX + docWidth - boundaryX) - tooltip.padding * 2;
		}

		tooltip.debugLog(
			"Position called. Width: " +
				docWidth +
				". Height: " +
				docHeight +
				". PosX: " +
	...
	updateCallback: function (map) {
		...
		tooltip.debugLog(
			"updateCallback called. map: " +
				JSON.stringify(map) +
				". params: " +
				JSON.stringify(tooltip.params) +
				". clientViewX: " +
				tooltip.clientViewX +
				". clientViewY: " +
				tooltip.clientViewY +
				". title: " +
				htmlEntities(tooltip.options.title) +
				". theme: " +
		...
		window.location =
			...
			tooltip.interface +
			";pos=" +
			tooltip.map.viewSize.x +
			",0;size=999x999;alpha=0";
	...
	update: function (params, options, clientViewX, clientViewY, stuck) {
		...
		window.location =
			...
			tooltip.interface +
			":tooltip.updateCallback;id=" +
			tooltip.getMapControlFor("map") +
			"," +
			tooltip.getMapControlFor("helper") +
			";property=pos,size,view-size,saved-params";
	},
};
