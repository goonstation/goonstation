...
function callByond(action, data) {
	var newLoc =
		"byond://?src=" + ref + "&jscall=" + action + "&" + data.join("&");
...
function rebuildEventList() {
	for (var x = events.length - 1; x >= 0; x--) {
		...
		var newHtml = "";
		newHtml +=
			"<a class='telescopeButton " +
			(curr.discovered ? "discovered" : "undiscovered") +
			" " +
			(curr.tracking ? "underline" : "") +
			"' href='#' onclick='buttonClick(\"" +
			curr.reference +
			"\");return false;'>" +
			curr.name +
			"</a>";
		// document.getElementById("footer").innerText = Date.now() + " - " + curr.tracking + " - " + newHtml;
		if (curr.type & tags.TAG_ORE)
			newHtml +=
				"<div class='tooltip'><i class='icon-star-empty'></i><span class='tooltiptext'>Ore</span></div>";
		if (curr.type & tags.TAG_WRECKAGE)
			newHtml +=
				"<div class='tooltip'><i class='icon-gear'></i><span class='tooltiptext'>Wreckage</span></div>";
		if (curr.type & tags.TAG_PLANET)
			newHtml +=
				"<div class='tooltip'><i class='icon-globe'></i><span class='tooltiptext'>Planet</span></div>";
		if (curr.type & tags.TAG_ANOMALY)
			newHtml +=
				"<div class='tooltip'><i class='icon-question'></i><span class='tooltiptext'>Anomaly</span></div>";
		if (curr.type & tags.TAG_SPACE)
			newHtml +=
				"<div class='tooltip'><i class='icon-circle-blank'></i><span class='tooltiptext'>Space</span></div>";
		if (curr.type & tags.TAG_NPC)
			newHtml +=
				"<div class='tooltip'><i class='icon-comment-alt'></i><span class='tooltiptext'>Person</span></div>";
		if (curr.type & tags.TAG_TELEPORT_LOC)
			newHtml +=
				"<div class='tooltip'><i class='icon-anchor'></i><span class='tooltiptext'>Teleporter</span></div>";
		if (curr.type & tags.TAG_CRUISER_LOC)
			newHtml +=
				"<div class='tooltip'><i class='icon-rocket'></i><span class='tooltiptext'>Cruiser</span></div>";
		if (curr.type & tags.TAG_MAGNET_LOC)
			newHtml +=
				"<div class='tooltip'><i class='icon-magnet'></i><span class='tooltiptext'>Magnet</span></div>";
		newHtml += "<br>";
...
function addEvent(name, type, discovered, tracking, reference) {
	...
	events.push({
		name: name,
		type: parseInt(type),
		discovered: parseInt(discovered),
...
function update() {
	...
	if (
		Date.now() - glitchProps.lastGlitch >= glitchProps.glitchCooldown &&
