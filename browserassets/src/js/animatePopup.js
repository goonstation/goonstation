(function (window, $) {
  var AnimatePopup = function () {
    ...
    var byond = function (type, properties) {
      ...
      window.location =
        'byond://win' + type + '?id=' + _settings.interface + params;
    };

    var animate = function (step) {
      _running = $({ boop: 0 }).animate(
        { boop: 1 },
        {
    ...
    var animations = {
      ...
      bounce: function () {
        ...
        sequence([
          function (now) {
            byond('set', {
              pos: _details.pos.x + ',' + (_details.pos.y - bounceAmount * now),
            });
          },
          function (now) {
            byond('set', {
              pos:
                _details.pos.x +
                ',' +
                (_details.pos.y - bounceAmount + bounceAmount * now),
            });
          },
          function (now) {
            byond('set', {
              pos:
                _details.pos.x +
                ',' +
                (_details.pos.y - (bounceAmount / 2) * now),
            });
          },
          function (now) {
            byond('set', {
              pos:
                _details.pos.x +
                ',' +
                (_details.pos.y - bounceAmount / 2 + (bounceAmount / 2) * now),
            });
          },
        ]);
      },
      ...
      shake: function () {
        ...
        sequence([
          function (now) {
            byond('set', {
              pos: _details.pos.x - shakeAmount * now + ',' + _details.pos.y,
            });
          },
          function (now) {
            byond('set', {
    ...
    this.run = function (animation, options) {
      ...
      if (typeof animations[animation] === 'undefined') {
        throw new AnimatePopupException(
