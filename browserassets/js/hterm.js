...
lib.registerInit('hterm', function (onInit) {
  ...
  function initMessageManager() {
    lib.f.getAcceptLanguages(function (languages) {
      if (!hterm.messageManager) hterm.messageManager = new lib.MessageManager(languages);
...
//  }
  ...
  [38, '[UP]', c('onKeyArrowUp_'), DEFAULT, DEFAULT, DEFAULT], [40, '[DOWN]', c('onKeyArrowDown_'), DEFAULT, DEFAULT, DEFAULT], [39, '[RIGHT]', ac(CSI + 'C', SS3 + 'C'), DEFAULT, DEFAULT, DEFAULT], [37, '[LEFT]', ac(CSI + 'D', SS3 + 'D'), DEFAULT, DEFAULT, DEFAULT], [144, '[NUMLOCK]', PASS, PASS, PASS, PASS],
...
hterm.Keyboard.KeyMap.prototype.onF11_ = function (e, keyDef) {
  if (hterm.windowType != 'popup') return hterm.Keyboard.KeyActions.PASS;else return '\x1b[23~';
};
...
hterm.PreferenceManager.categoryDefinitions = [{ id: hterm.PreferenceManager.categories.Appearance,
  text: 'Appearance (fonts, colors, images)' }, { id: hterm.PreferenceManager.categories.CopyPaste,
  text: 'Copy & Paste' }, { id: hterm.PreferenceManager.categories.Encoding,
  text: 'Encoding' }, { id: hterm.PreferenceManager.categories.Keyboard,
  text: 'Keyboard' }, { id: hterm.PreferenceManager.categories.Scrolling,
  text: 'Scrolling' }, { id: hterm.PreferenceManager.categories.Sounds,
  text: 'Sounds' }, { id: hterm.PreferenceManager.categories.Extensions,
  text: 'Extensions' }, { id: hterm.PreferenceManager.categories.Miscellaneous,
  text: 'Misc.' }];
...
hterm.PreferenceManager.defaultPreferences = {
  ...
  'alt-backspace-is-meta-backspace': [hterm.PreferenceManager.categories.Keyboard, false, 'bool', 'If set, undoes the Chrome OS Alt-Backspace->DEL remap, so that ' + 'alt-backspace indeed is alt-backspace.'],
  ...
  'close-on-exit': [hterm.PreferenceManager.categories.Miscellaneous, true, 'bool', 'Whether or not to close the window when the command exits.'],

  'cursor-blink': [hterm.PreferenceManager.categories.Appearance, false, 'bool', 'Whether or not to blink the cursor by default.'],

  'cursor-blink-cycle': [hterm.PreferenceManager.categories.Appearance, [1000, 500], 'value', 'The cursor blink rate in milliseconds.\n' + '\n' + 'A two element array, the first of which is how long the cursor should be ' + 'on, second is how long it should be off.'],
  ...
  'use-default-window-copy': [hterm.PreferenceManager.categories.CopyPaste, false, 'bool', 'Whether to use the default window copy behavior'],
  ...
  'ctrl-plus-minus-zero-zoom': [hterm.PreferenceManager.categories.Keyboard, true, 'bool', 'If true, Ctrl-Plus/Minus/Zero controls zoom.\n' + 'If false, Ctrl-Shift-Plus/Minus/Zero controls zoom, Ctrl-Minus sends ^_, ' + 'Ctrl-Plus/Zero do nothing.'],
  ...
  'enable-blink': [hterm.PreferenceManager.categories.Appearance, true, 'bool', 'True if we should respect the blink attribute.  False to ignore it.  '],
  ...
  'enable-clipboard-write': [hterm.PreferenceManager.categories.CopyPaste, true, 'bool', 'Allow the remote host to write directly to the local system clipboard.\n' + 'Read access is never granted regardless of this setting.\n' + '\n' + 'This is used to control access to features like OSC-52.'],
  ...
  'home-keys-scroll': [hterm.PreferenceManager.categories.Keyboard, false, 'bool', 'If true, home/end will control the terminal scrollbar and shift home/end ' + 'will send the VT keycodes.  If false then home/end sends VT codes and ' + 'shift home/end scrolls.'],
  ...
  'media-keys-are-fkeys': [hterm.PreferenceManager.categories.Keyboard, false, 'bool', 'If true, convert media keys to their Fkey equivalent. If false, let ' + 'the browser handle the keys.'],
  ...
  'word-break-match-right': [hterm.PreferenceManager.categories.CopyPaste, '[^\\s\\[\\](){}<>"\'\\^!@#$%&*,;:~.`]', 'string', 'Regular expression to halt matching to the right (end) of a selection.\n' + '\n' + 'Normally this is a character class to reject specific characters.'],
  ...
  'page-keys-scroll': [hterm.PreferenceManager.categories.Keyboard, false, 'bool', 'If true, page up/down will control the terminal scrollbar and shift ' + 'page up/down will send the VT keycodes.  If false then page up/down ' + 'sends VT codes and shift page up/down scrolls.'],
  ...
  'scroll-wheel-may-send-arrow-keys': [hterm.PreferenceManager.categories.Scrolling, false, 'bool', 'When using the alternative screen buffer, and DECCKM (Application Cursor ' + 'Keys) is active, mouse wheel scroll events will emulate arrow keys.\n' + '\n' + 'It can be temporarily disabled by holding the shift key.\n' + '\n' + 'This frequently comes up when using pagers (less) or reading man pages ' + 'or text editors (vi/nano) or using screen/tmux.'],

  'scroll-wheel-move-multiplier': [hterm.PreferenceManager.categories.Scrolling, 1, 'int', 'The multiplier for the pixel delta in wheel events caused by the ' + 'scroll wheel. Alters how fast the page scrolls.'],

  'send-encoding': [hterm.PreferenceManager.categories.Encoding, 'utf-8', ['utf-8', 'raw'], 'Set the encoding for data sent to host.'],

  'terminal-encoding': [hterm.PreferenceManager.categories.Encoding, 'utf-8', ['iso-2022', 'utf-8', 'utf-8-locked'], 'The default terminal encoding (DOCS).\n' + '\n' + 'ISO-2022 enables character map translations (like graphics maps).\n' + 'UTF-8 disables support for those.\n' + '\n' + 'The locked variant means the encoding cannot be changed at runtime ' + 'via terminal escape sequences.\n' + '\n' + 'You should stick with UTF-8 unless you notice broken rendering with ' + 'legacy applications.'],
  ...
  'user-css-text': [hterm.PreferenceManager.categories.Appearance, '', 'multiline-string', 'Custom CSS text for styling the terminal.'],
...
hterm.Screen.prototype.insertString = function (str) {
  ...
  if (reverseOffset < 0) {
    ...
    if (!(this.textAttributes.underline || this.textAttributes.strikethrough || this.textAttributes.background || this.textAttributes.wcNode || !this.textAttributes.asciiNode || this.textAttributes.tileData != null)) {
      // Best case scenario, we can just pretend the spaces were part of the
      // original string.
      str = ws + str;
    } else if (cursorNode.nodeType == Node.TEXT_NODE || !(cursorNode.wcNode || !cursorNode.asciiNode || cursorNode.tileNode || cursorNode.style.textDecoration || cursorNode.style.textDecorationStyle || cursorNode.style.textDecorationLine || cursorNode.style.backgroundColor)) {
...
hterm.Screen.prototype.deleteChars = function (count) {
  ...
  while (node && count) {
    ...
    // up the slack.
    if (node.wcNode && offset < startLength && (endLength && startLength == endLength || !endLength && offset == 1)) {
...
hterm.Terminal.prototype.setProfile = function (profileId, opt_callback) {
  ...
  this.prefs_.addObservers(null, {
    ...
    'send-encoding': function sendEncoding(v) {
      if (!/^(utf-8|raw)$/.test(v)) {
...
hterm.Terminal.prototype.runCommandClass = function (commandClass, argString) {
  ...
  if ((typeof environment === 'undefined' ? 'undefined' : _typeof(environment)) != 'object' || environment == null) environment = {};
...
hterm.Terminal.prototype.setCssVar = function (name, value) {
  ...
  this.document_.documentElement.style.setProperty('' + opt_prefix + name, value);
};
...
hterm.Terminal.prototype.getCssVar = function (name) {
  ...
  return this.document_.documentElement.style.getPropertyValue('' + opt_prefix + name);
};
...
hterm.Terminal.prototype.reset = function () {
  ...
  var resetScreen = function resetScreen(screen) {
    ...
    _this.clearHome(screen);
    screen.saveCursorAndState(_this.vt);
  };
...
hterm.Terminal.prototype.softReset = function () {
  ...
  var resetScreen = function resetScreen(screen) {
    ...
    screen.textAttributes.reset();
    screen.textAttributes.resetColorPalette();
    screen.saveCursorAndState(_this2.vt);
  };
  resetScreen(this.primaryScreen_);
  resetScreen(this.alternateScreen_);
...
hterm.Terminal.prototype.decorate = function (div) {
  ...
  if (charset != 'utf-8') {
    console.warn('Document encoding should be set to utf-8, not "' + charset + '";' + ' Add <meta charset=\'utf-8\'/> to your HTML <head> to fix.');
  }
...
hterm.Terminal.prototype.showOverlay = function (msg, opt_timeout) {
  ...
  this.overlayTimeout_ = setTimeout(function () {
    _this3.overlayNode_.style.opacity = '0';
    _this3.overlayTimeout_ = setTimeout(function () {
...
hterm.Terminal.prototype.displayImage = function (options) {
  ...
  if (this.allowImagesInline !== true) {
    ...
    button.addEventListener('click', function () {
      _this4.prefs_.set('allow-images-inline', false);
    });
    ...
    button.style.borderStyle = 'solid';
    button.addEventListener('click', function () {
      _this4.prefs_.set('allow-images-inline', true);
    });
    span.appendChild(button);
  ...
  if (options.inline) {
    ...
    img.onload = function () {
      ...
      img.style.maxWidth = _this4.document_.body.clientWidth + 'px';
      img.style.maxHeight = _this4.document_.body.clientHeight + 'px';
      ...
      var parseDim = function parseDim(dim, maxDim, cssVar) {
        ...
        if (ary) {
          if (ary[2] == '%') return maxDim * parseInt(ary[1]) / 100 + 'px';else if (ary[2] == 'px') return dim;else return 'calc(' + dim + ' * var(' + cssVar + '))';
        }

        return '';
      };
      img.style.width = parseDim(options.width, _this4.document_.body.clientWidth, '--hterm-charsize-width');
      img.style.height = parseDim(options.height, _this4.document_.body.clientHeight, '--hterm-charsize-height');
      ...
      var padRows = Math.ceil(img.clientHeight / _this4.scrollPort_.characterSize.height);
      ...
      _this4.document_.body.removeChild(img);
      ...
      var div = _this4.document_.createElement('div');
      div.style.position = 'relative';
      div.style.textAlign = options.align;
      img.style.position = 'absolute';
      img.style.bottom = 'calc(0px - var(--hterm-charsize-height))';
      div.appendChild(img);
      var row = _this4.getRowNode(_this4.scrollbackRows_.length + _this4.getCursorRow() - 1);
      row.appendChild(div);
...
hterm.Terminal.prototype.onMouse_ = function (e) {
  ...
  if (!reportMouseEvents) {
    ...
    if (e.type == 'mousedown') {
      if (this.mouseRightClickPaste && e.button == 2 /* right button */ || e.button == this.mousePasteButton) {
    ...
    if (this.scrollWheelArrowKeys_ && !e.shiftKey && this.keyboard.applicationCursor && !this.isPrimaryScreen()) {
      if (e.type == 'wheel') {
        ...
        var lines = lib.f.smartFloorDivide(Math.abs(delta), this.scrollPort_.characterSize.height);
...
hterm.TextAttributes.prototype.createContainer = function (opt_textContent) {
  ...
  if (this.underlineSource != this.SRC_DEFAULT) style.textDecorationColor = this.underlineColor;
...
hterm.TextAttributes.prototype.syncColors = function () {
  ...
  if (foregroundSource == this.SRC_DEFAULT) this.foreground = defaultForeground;else if (Number.isInteger(foregroundSource)) this.foreground = this.colorPalette[foregroundSource];else this.foreground = foregroundSource;
  ...
  if (backgroundSource == this.SRC_DEFAULT) this.background = defaultBackground;else if (Number.isInteger(backgroundSource)) this.background = this.colorPalette[backgroundSource];else this.background = backgroundSource;

  // Process invisible settings last to keep it simple.
  if (this.invisible) this.foreground = this.background;

  if (this.underlineSource == this.SRC_DEFAULT) this.underlineColor = '';else if (Number.isInteger(this.underlineSource)) this.underlineColor = this.colorPalette[this.underlineSource];else this.underlineColor = this.underlineSource;
};
...
hterm.VT.prototype.onTerminalMouse_ = function (e) {
  ...
  if (this.mouseReport == this.MOUSE_REPORT_DISABLED) return;else if (this.mouseReport != this.MOUSE_REPORT_DRAG && e.type == 'mousemove') return;
  ...
  switch (e.type) {
    case 'wheel':
      ...
      b |= mod;
      if (this.mouseCoordinates == this.MOUSE_COORDINATES_SGR) response = '\x1B[<' + b + ';' + x + ';' + y + 'M';else response = '\x1b[M' + String.fromCharCode(b) + x + y;
...
hterm.VT.prototype.updateEncodingState_ = function () {
  ...
  var cc1 = Object.keys(hterm.VT.CC1).filter(function (e) {
    return !_this5.codingSystemUtf8_ || e.charCodeAt() < 0x80;
  }).map(function (e) {
...
hterm.VT.prototype.parseCSI_ = function (parseState) {
  ...
  var finishParsing = function finishParsing() {
...
hterm.VT.prototype.parseUntilStringTerminator_ = function (parseState) {
  ...
  if (terminator == '\x1b' && buf[nextTerminator + 1] != '\\') foundTerminator = false;else foundTerminator = nextTerminator != -1;
...
hterm.VT.prototype.setDECMode = function (code, state) {
  switch (parseInt(code, 10)) {
    ...
    case 9:
      // Report on mouse down events only (X10).
    ...
    case 1000:
      // Report on mouse clicks only (X11).
    ...
    case 1048:
      ...
      if (state) this.terminal.saveCursorAndState();else this.terminal.restoreCursorAndState();
      break;
...
hterm.VT.OSC['1337'] = function (parseState) {
  ...
  if (!args) {
    if (this.warnUnimplemented) console.log('iTerm2 1337: unsupported sequence: ' + args[1]);
    return;
  }
    ...
    if (arg < 30) {
      if (arg == 0) {
        ...
        if (parseState.argHasSubargs(i)) {
          ...
          if (uarg == 0) attrs.underline = false;else if (uarg == 1) attrs.underline = 'solid';else if (uarg == 2) attrs.underline = 'double';else if (uarg == 3) attrs.underline = 'wavy';else if (uarg == 4) attrs.underline = 'dotted';else if (uarg == 5) attrs.underline = 'dashed';
        } else {
...
hterm.VT.CharacterMap.prototype.sync_ = function (opt_glmap) {
  ...
  if (opt_glmap) this.glmap_ = Object.assign({}, this.glmapBase_, opt_glmap);else this.glmap_ = this.glmapBase_;
  ...
  this.GL = function (str) {
    return str.replace(_this6.glre_, function (ch) {
      return _this6.glmap_[ch];
    });
  };
};
...
hterm.VT.CharacterMaps.prototype.getMap = function (name) {
  if (this.maps_.hasOwnProperty(name)) return this.maps_[name];else return undefined;
};
...
lib.resource.add('hterm/audio/bell', 'audio/ogg;base64', 'T2dnUwACAAAAAAAAAADhqW5KAAAAAMFvEjYBHgF2b3JiaXMAAAAAAYC7AAAAAAAAAHcBAAAAAAC' + '4AU9nZ1MAAAAAAAAAAAAA4aluSgEAAAAAesI3EC3//////////////////8kDdm9yYmlzHQAAAF' + 'hpcGguT3JnIGxpYlZvcmJpcyBJIDIwMDkwNzA5AAAAAAEFdm9yYmlzKUJDVgEACAAAADFMIMWA0' + 'JBVAAAQAABgJCkOk2ZJKaWUoSh5mJRISSmllMUwiZiUicUYY4wxxhhjjDHGGGOMIDRkFQAABACA' + 'KAmOo+ZJas45ZxgnjnKgOWlOOKcgB4pR4DkJwvUmY26mtKZrbs4pJQgNWQUAAAIAQEghhRRSSCG' + 'FFGKIIYYYYoghhxxyyCGnnHIKKqigggoyyCCDTDLppJNOOumoo4466ii00EILLbTSSkwx1VZjrr' + '0GXXxzzjnnnHPOOeecc84JQkNWAQAgAAAEQgYZZBBCCCGFFFKIKaaYcgoyyIDQkFUAACAAgAAAA' + 'ABHkRRJsRTLsRzN0SRP8ixREzXRM0VTVE1VVVVVdV1XdmXXdnXXdn1ZmIVbuH1ZuIVb2IVd94Vh' + 'GIZhGIZhGIZh+H3f933f930gNGQVACABAKAjOZbjKaIiGqLiOaIDhIasAgBkAAAEACAJkiIpkqN' + 'JpmZqrmmbtmirtm3LsizLsgyEhqwCAAABAAQAAAAAAKBpmqZpmqZpmqZpmqZpmqZpmqZpmmZZlm' + 'VZlmVZlmVZlmVZlmVZlmVZlmVZlmVZlmVZlmVZlmVZlmVZQGjIKgBAAgBAx3Ecx3EkRVIkx3IsB' + 'wgNWQUAyAAACABAUizFcjRHczTHczzHczxHdETJlEzN9EwPCA1ZBQAAAgAIAAAAAABAMRzFcRzJ' + '0SRPUi3TcjVXcz3Xc03XdV1XVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVYHQkFU' + 'AAAQAACGdZpZqgAgzkGEgNGQVAIAAAAAYoQhDDAgNWQUAAAQAAIih5CCa0JrzzTkOmuWgqRSb08' + 'GJVJsnuamYm3POOeecbM4Z45xzzinKmcWgmdCac85JDJqloJnQmnPOeRKbB62p0ppzzhnnnA7GG' + 'WGcc85p0poHqdlYm3POWdCa5qi5FJtzzomUmye1uVSbc84555xzzjnnnHPOqV6czsE54Zxzzona' + 'm2u5CV2cc875ZJzuzQnhnHPOOeecc84555xzzglCQ1YBAEAAAARh2BjGnYIgfY4GYhQhpiGTHnS' + 'PDpOgMcgppB6NjkZKqYNQUhknpXSC0JBVAAAgAACEEFJIIYUUUkghhRRSSCGGGGKIIaeccgoqqK' + 'SSiirKKLPMMssss8wyy6zDzjrrsMMQQwwxtNJKLDXVVmONteaec645SGultdZaK6WUUkoppSA0Z' + 'BUAAAIAQCBkkEEGGYUUUkghhphyyimnoII
GUuuUUcCvFkcPA/PwCRnYODAJoA3AdNRy1anGABCA7O9vHRnYOdrx8' + '4sdgBubm5rY5ICa4m/8Sk1enMTQ00A2gG8BbKOcCBmpzgASgj44M7+/oaJfXpmfR3t5xy07AWsU' + 'FhUIRlyemcOcde9OpVHJgaWn5FawhgqLfhkmOje26nZmRyxXQtePmfU3xVoFpmbg2PYtMW1rr6+' + '3eeX5pOaqEgyWJShHkJ9px297RXddnsiiWbCwuLv5UiJ9aX/bYSBlE7nV5OYe2dAqDA727zl94s' + '5IAZSIoKv9FImHt2rN7pDs7N4/l5WVIOesRwH8Tbs2qgwvXi6uKr9PB+u8ujomSeKlonZG0RmRl' + '6AcPHcTAQC8GB/uGEb5RPToh46j3bhCxc3hg39Bgn9nbswPpVBK53ErZR2tqOV358eVx4X2wzRR' + 'x2K103q12yEXo5Bvcry99I4ewuI5kYdsj6SIOxV5omXOwphS6ujoghMDw0EAvXEvoSgTfAKrfaU' + 'MA9F0jQ7d3d3chk0njoQ+9b83NiK0VTnHendOqdnLdIIY7K3YJ0N8ppeixbecMYixFpHaNDI+mU' + '0n3pdl8a9n+NxJ87ujv7030dO8YvHL1mr8zWsYBlZrZymTSKaUlQNLAVo/vmxsIxCV0tLeJzs72' + 'bo8AboSH71qroStLS8u567PzyK86G9ox32yjW1lU6/sTrYFhmQqWZSGdSmZqpVZlqV3IzcxkZ6e' + 'vTWFpebWmT2+tj6MF76OtdbSL61gyzDXTlZ0hKE9Q9rEGrrK8uELec1Vc+bcJIvfRwyM1wpiry2' + 'sU5opvRqYtCcuUKBSKJYQf/QzcFX0CRN0Rc8dPnD5qJZ7okVKCHYd8V27/RRcM9gAAewc/2bsLH' + '+GnCf+Xp/PmFsFtEBumLqss8oTIX9lzUFCQJ9rAijRV92VtjTxHyquqpKzLjn+Fu+xsKyULzLzy' + 'xhuXnkSNL66WnYRB+KnCDNydHP/dZzpCU7WWUuAGzxwjvlYZ9cLWm4cbxMUpD2vkqQzzkVwEUIC' + '7Gb/iXQvez3fSYlWR0YZLuUUvkYHw453+JGK9EKdTrdT0Db2TW9CO6DeGSyhHetWXVqOfvXAq7m' + '0vY9xvBW+28RvJ3ygP4ca3KcpJUU7wER/VAQBqK2H/DRZ+hspDe81EYKsQsZV1Vg7oKNKjyGegs' + 'XNuFOE302Ywr/G8Fe2pq4fqIfZmQvjbHbZ6AGzDNmzDNmzD2xT+H+5UT7Tyxc2HAAAAJXRFWHRk' + 'YXRlOmNyZWF0ZQAyMDE2LTA2LTMwVDExOjUwOjAyLTA0OjAwOaSkCgAAACV0RVh0ZGF0ZTptb2' + 'RpZnkAMjAxMy0xMS0wMVQxMDozODoyNC0wNDowMNba8BsAAAAASUVORK5CYII=');

lib.resource.add('hterm/concat/date', 'text/plain', 'Sun, 15 Apr 2018 22:42:12 +0000');

lib.resource.add('hterm/changelog/version', 'text/plain', '1.78');

lib.resource.add('hterm/changelog/date', 'text/plain', '2018-01-29');

lib.resource.add('hterm/git/HEAD', 'text/plain', '1b931fe23b54127de6bceb6ee95b9e1596799887');
