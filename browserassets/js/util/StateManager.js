...
var StateManager = (function() {
	...
	var StateListener = (function () {
		...
		function StateListener(callback, fields) {
			if (typeof callback === 'function') {
				...
				throw new TypeError('Unexpected "callback" argument of type "' + (typeof callback) + '"; expected function.');
			}
			if (typeof fields === 'undefined' || fields === null) {
				...
				for (fieldIndex = 0; fieldIndex < fields.length; fieldIndex++) {
					...
					if (typeof field !== 'string') {
						throw new TypeError('Unexpected member of "fields" argument at index ' + fieldIndex + ' of type "' + (typeof field) + '"; expected string.');
					}
				}
				this._fields = fields;
			} else {
				throw new TypeError('Unexpected "fields" argument of type "' + (typeof fields) + '"; expected undefined, null, string, or array of strings.')
			}
		}
		...
		function trigger(diff, newState) {
			...
			if (this._fields === null) {
				...
				for (diffFieldIndex = 0; diffFieldIndex < diffFields.length; diffFieldIndex++) {
	...
	function getState(fields) {
		...
		if (typeof fields === 'undefined') {
			...
			throw new TypeError('Unexpected "fields" argument of type "' + (typeof fields) + '"; expected undefined, string, or array of strings.');
		}
	...
	function removeStateListener(stateListener) {
		...
		for (stateListenerIndex = 0; stateListenerIndex < this._stateListeners.length; stateListenerIndex++) {
	...
	function setState(updater) {
		...
		diff = {};
		resolvedUpdater = typeof updater === 'function' ? updater(this._state) : updater;
		if (typeof resolvedUpdater !== 'object') {
			throw new TypeError('Unexpected argument of type "' + (typeof updater) + '" passed to setState; expected function or object.');
		}
		// generate state diff
