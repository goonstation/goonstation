...
if (!String.prototype.padStart) {
  String.prototype.padStart = function (targetLength, padString) {
    ...
    if (targetLength <= 0) return String(this);

    if (padString === undefined) padString = ' ';

    // In case the pad is multiple chars long.
    if (targetLength > padString.length) padString = padString.repeat(targetLength / padString.length + 1);
...
lib.array.arrayBigEndianToUint32 = function (array) {
  var maybeSigned = array[0] << 24 | array[1] << 16 | array[2] << 8 | array[3] << 0;
...
lib.array.uint32ToArrayBigEndian = function (uint32) {
  return [uint32 >>> 24 & 0xFF, uint32 >>> 16 & 0xFF, uint32 >>> 8 & 0xFF, uint32 >>> 0 & 0xFF];
};
 ...
 * @returns {!TYPED_ARRAY}
 */
lib.array.concatTyped = function () {
  var resultLength = 0;
  ...
  var result = new arrays[0].constructor(resultLength);
  var pos = 0;
...
lib.colors.normalizeCSS = function (def) {
  if (def.startsWith('#')) return lib.colors.hexToRGB(def);
...
'#000000', '#CC0000', '#4E9A06', '#C4A000', '#3465A4', '#75507B', '#06989A', '#D3D7CF', '#555753', '#EF2929', '#00BA13', '#FCE94F', '#729FCF', '#F200CB', '#00B5BD', '#EEEEEC',
...
lib.f.parseQuery = function (queryString) {
  ...
  for (var i = 0; i < pairs.length; i++) {
    ...
    if (key.endsWith('[]')) {
      ...
      if (!(rv[key] instanceof Array)) rv[key] = [];
      rv[key].push(val);
    } else {
      // It's a plain string.
      rv[key] = val;
...
lib.f.alarm = function (callback, opt_ms) {
  ...
  return function () {
    ...
    var wrapperGenerator = function wrapperGenerator(callback) {
      return function () {
        if (timeout) {
...
lib.f.getOs = function () {
  ...
  if (window.browser && browser.runtime && browser.runtime.getPlatformInfo) return browser.runtime.getPlatformInfo().then(function (info) {
    return info.os;
  });
 ...
 */
lib.f.lastError = function () {
  var defaultMsg = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;

  var lastError = void 0;
  if (window.browser && browser.runtime) lastError = browser.runtime.lastError;else if (window.chrome && chrome.runtime) lastError = chrome.runtime.lastError;

  if (lastError && lastError.message) return lastError.message;else return defaultMsg;
};
...
lib.MessageManager.prototype.addMessages = function (defs) {
  for (var key in defs) {
    ...
    if (!def.placeholders) {
      ...
      this.messages[key] = def.message.replace(/\$([a-z][^\s\$]+)\$/ig, function (m, name) {
...
lib.MessageManager.prototype.loadMessages = function (url, onSuccess, opt_onError) {
  ...
  xhr.onload = function () {
    _this.addMessages(JSON.parse(xhr.responseText));
    onSuccess();
  };
  if (opt_onError) xhr.onerror = function () {
...
lib.MessageManager.prototype.get = function (msgname, opt_args, opt_default) {
  ...
  if (msgname in this.messages) {
    ...
    if (window.chrome && window.chrome.i18n) message = chrome.i18n.getMessage(msgname);
...
lib.MessageManager.prototype.processI18nAttribute = function (node) {
  ...
  for (var key in i18n) {
    ...
    if (msgname.startsWith('$')) msgname = thunk(node.getAttribute(msgname.substr(1)) + '_' + key);

    // Finally load the message.
    var msg = this.get(msgname);
    if (attr == '_') node.textContent = msg;else node.setAttribute(attr, msg);
  }
};
...
lib.PreferenceManager.Record.prototype.get = function () {
  if (this.currentValue === this.DEFAULT_VALUE) {
    if (/^(string|number)$/.test(_typeof(this.defaultValue))) return this.defaultValue;

    if (_typeof(this.defaultValue) == 'object') {
...
lib.PreferenceManager.prototype.readStorage = function (opt_callback) {
  ...
  this.storage.getItems(keys, function (items) {
    ...
    for (var key in items) {
      ...
      var needSync = name in this.childLists_ && JSON.stringify(value) != JSON.stringify(this.prefRecords_[name].currentValue);
...
lib.PreferenceManager.prototype.definePreference = function (name, value, opt_onChange) {
  ...
  if (record) {
    ...
    record = this.prefRecords_[name] = new lib.PreferenceManager.Record(name, value);
  }
...
lib.PreferenceManager.prototype.diff = function (a, b) {
  ...
  if ((typeof a === 'undefined' ? 'undefined' : _typeof(a)) !== (typeof b === 'undefined' ? 'undefined' : _typeof(b)) || !/^(undefined|boolean|number|string)$/.test(typeof a === 'undefined' ? 'undefined' : _typeof(a))) {
    return true;
  }
...
lib.PreferenceManager.prototype.importFromJson = function (json, opt_onComplete) {
  var pendingWrites = 0;
  var onWriteStorage = function onWriteStorage() {
    if (--pendingWrites < 1 && opt_onComplete) opt_onComplete();
  };
...
/**
 * Namespace for implementations of persistent, possibly butt-backed
 * storage.
 */
  ...
  var onComplete = function onComplete() {
    ...
    if (err) {
      ...
      if (err.indexOf('MAX_WRITE_OPERATIONS')) {
        ...
        setTimeout(function () {
          return _this3.setItem(key, value, onComplete);
        }, 1000);
        return;
      } else {
...
lib.TestManager.Log = function () {
  ...
  var opt_console = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : console;
  ...
  ['log', 'debug', 'info', 'warn', 'error'].forEach(function (level) {
    ...
    var oLog = _this4.console_[level];
    _this4[level] = _this4.console_[level] = function () {
      for (var _len2 = arguments.length, args = Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
        args[_key2] = arguments[_key2];
      }

      if (_this4.save) _this4.data += _this4.prefix_ + msgPrefix + args.join(' ') + '\n';
      oLog.apply(_this4.console_, args);
    };
  });

  // Wrap/bind the group functions.
  ['group', 'groupCollapsed'].forEach(function (group) {
    var oGroup = _this4.console_[group];
    _this4[group] = _this4.console_[group] = function () {
      var label = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';

      oGroup(label);
      if (_this4.save) _this4.data += _this4.prefix_ + label + '\n';
      _this4.prefix_ = '  '.repeat(++_this4.prefixStack_);
    };
  });

  var oGroupEnd = this.console_.groupEnd;
  this.groupEnd = this.console_.groupEnd = function () {
    oGroupEnd();
    if (_this4.prefixStack_) _this4.prefix_ = '  '.repeat(--_this4.prefixStack_);
  };
};
...
lib.TestManager.TestRun.prototype.selectPattern = function (pattern) {
  ...
  for (var i = 0; i < lib.TestManager.Suite.subclasses.length; i++) {
    selectCount += this.selectSuite(lib.TestManager.Suite.subclasses[i], pattern);
  }
...
lib.TestManager.TestRun.prototype.onUncaughtException_ = function (message, file, line) {
  ...
  if (message.indexOf('Uncaught lib.TestManager.Result.TestComplete') == 0 || message.indexOf('status: passed') != -1) {
  ...
  this.log.error('Uncaught exception ' + when + ' test case: ' + this.currentResult.test.fullName);
...
lib.TestManager.TestRun.prototype.onResultComplete = function (result) {
  try {
    ...
    this.log.error('Unexpected exception in postamble: ' + (ex.stack ? ex.stack : ex));
    this.panic = true;
  }

  if (result.status != result.PASSED) this.log.error(result.status);else if (result.duration > 500) this.log.warn('Slow test took ' + this.msToSeconds_(result.duration));
  this.log.groupEnd();
  ...
  if (result.status == result.FAILED) {
    ...
    this.log.error('Unknown result status: ' + result.test.fullName + ': ' + result.status);
    this.panic = true;
    return;
  }
...
lib.TestManager.TestRun.prototype.onResultReComplete = function (result, lateStatus) {
  this.log.error('Late complete for test: ' + result.test.fullName + ': ' + lateStatus);
...
lib.TestManager.TestRun.prototype.runNextTest_ = function () {
  ...
  try {
    ...
    this.log.error('Unexpected exception during test preamble: ' + (ex.stack ? ex.stack : ex));
    this.log.groupEnd();
  ...
  try {
    ...
    this.log.error('Unexpected exception during test run: ' + (ex.stack ? ex.stack : ex));
    this.panic = true;
  }
};
...
lib.TestManager.TestRun.prototype.summarize = function () {
  ...
  if (this.testQueue_.length) {
    this.log.warn('Test run incomplete: ' + this.testQueue_.length + ' test(s) were not run.');
  }
};
...
lib.TestManager.Result.prototype.assertEQ = function (actual, expected, opt_name) {
  ...
  if (expected instanceof Array) if (lib.array.compare(actual, expected)) return;
...
lib.wc.binaryTableSearch_ = function (ucs, table) {
  ...
  if (ucs < table[min][0] || ucs > table[max][1]) return false;
  while (max >= min) {
...
lib.wc.charWidthDisregardAmbiguous = function (ucs) {
  ...
  if (ucs < 0x7f) {
    if (ucs >= 0x20) return 1;else if (ucs == 0) return lib.wc.nulWidth;else /* if (ucs < 0x20) */
...
lib.wc.substr = function (str, start, opt_width) {
  ...
  if (opt_width != undefined) {
    for (endIndex = startIndex, width = 0; endIndex < str.length;) {
      var _codePoint = str.codePointAt(endIndex);
      width += lib.wc.charWidth(_codePoint);
      if (width > opt_width) break;
      endIndex += _codePoint <= 0xffff ? 1 : 2;
    }
...
lib.resource.add('libdot/changelog/version', 'text/plain', '1.21');

lib.resource.add('libdot/changelog/date', 'text/plain', '2018-01-05');
