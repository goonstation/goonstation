...
- **Door button mapping helpers** are a tool to quickly set up sets of doors and buttons that use the "id" variable to communicate with each other. **Area** door button helpers generate an ID by applying ckey() (all lower case, no spaces) to the name of the area they're in, and are useful for hangar bays or shutterable rooms where you have a large set of doors and buttons throughout one area. **Pair** door button helpers are **placed on a button and pointed in a direction** (all 8 directions work), and will link that button to an adjacent door in the facing direction; they're useful for situations where you need many simple sets of paired bolt buttons, such as lockable bathrooms or crew quarters. The base path of the door button mapping helper simply accepts a var-edited ID and applies it to doors and buttons underneath it. Please note that **these will not work in cases where multiple doors occupy one tile** (most notably, shutters placed over other doors, such as kitchen shutters over thin doors).
