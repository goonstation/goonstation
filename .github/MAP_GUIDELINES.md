...
- **Mapping Helpers** under `code/modules/mapping` there is a set of objects that are made specially for helping mappers, these range from window grille spawners to airlock bolters. These can be used instead of creating subtypes where such doesn't scale well (airlocks are a good example because there are many of them) or varediting which makes things less obvious for someone later. If you want to add one of these, check the current files and put it in where it fits. If no files are suitable make a new one.
