...
#define START_TRACKING do { var/_type = text2path(replacetext("[/datum.proc/disposing]", "/proc/disposing", "")); if(!by_type[_type]) { by_type[_type] = list(src) } else { by_type[_type].Add(src) } } while (FALSE)
#define STOP_TRACKING do { var/_type = text2path(replacetext("[/datum.proc/disposing]", "/proc/disposing", "")); by_type[_type].Remove(src) } while (FALSE)
